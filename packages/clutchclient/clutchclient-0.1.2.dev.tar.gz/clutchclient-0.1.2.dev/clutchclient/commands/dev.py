...
from clutchclient.utils import json
from clutchclient.utils import get_config, remote_call
...
def handle(namespace, extra):
    ...
    def _ping_dev():
        url = '%sview/%s/%s/' % (TUNNEL_URL, config['username'], namespace.app)
        while 1:
            remote_call(config, 'start_dev',
                app_slug=namespace.app,
                url=url,
