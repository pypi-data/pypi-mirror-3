...
class Test_facebook_canvas(unittest.TestCase):
    ...
    def test__init(self):
        ...
        config = {'permission': ViewCanvas, 'route_name': 'facebook_canvas'}
        for k, v in config.iteritems():
            self.assertEqual(v, getattr(dec, k))



class Test_facebook_payments_get_items(unittest.TestCase):

    @mock.patch('pyramid_facebook.venusian')
    def test__call(self, m_venusian):
    ...
    @mock.patch('pyramid_facebook.json')
    def test_decorate(self, m_json):
        from pyramid_facebook import facebook_payments_get_items
        ...
        wrapped.assert_called_once_with(
            context,
            request,
            order_info=context.facebook_data["credits"]["order_info"]
            )
        m_json.dumps.assert_called_once_with({
