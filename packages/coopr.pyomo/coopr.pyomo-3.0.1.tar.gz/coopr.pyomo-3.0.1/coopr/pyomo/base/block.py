...
class BlockComponents(object):
    ...
    def _clear_attribute(self, name):
        ...
        if (type(val) == weakref and isinstance(val(), Block)):
            val().__dict__['parent'] = None
            val().model = weakref.ref(val)
...
class Block(IndexedComponent):
    ...
    #       weakrefs via the 'model' and 'parent' attributes.
    #
    ...
    def __getitem__(self, ndx):
        """This method returns a Block object.
        """
        if ndx in self._data:
            return self._data[ndx]
        msg = "Unknown index in %s '%s': %s"
        raise KeyError, msg % ( self.__class__.__name__, self.name, str(ndx) )
    ...
    def block_namespace(self):
        fullName = ''
        b = self
        while b.parent is not None and b.parent() is not None:
...
#                        "%s '%s'", self.__class__.__name__, self.name)
        ...
        for val in self._index:
            ...
            if val is None:
                apply_parameterized_indexed_rule(
                    self, self._rule, self.model(), self, () )
            else:
                block = self.__class__()
                self._add_component(name, block)
                apply_parameterized_indexed_rule(
                    self, self._rule, self.model(), block, val )
    ...
    def _add_component(self, name, val):
        ...
        # eventually be solved, whereas 'parent' refers to the
        # immediate parent.

        # Update the (new) child block's parent.
        if isinstance(val, Block):
            if val.parent is not None and val.parent() is not None:
            ...
            val.__dict__['parent'] = weakref.ref(self)
        else:
            val.parent = weakref.ref(self)
    ...
    def __setattr__(self,name,val):
        ...
        if name == "model":
            ...
            # "self.model = None" before Block.__init__ defines the
            # _declarations map.
            #
            ...
            if val is not None and not isinstance(val, Block) and \
                    not (type(val) == weakref.ref and isinstance(val(), Block)):
                raise ValueError, "Cannot set Block.parent to a non-Block " \
                      "object (%s)" % ( val )
            self.__dict__["parent"] = val
