...
plugin_packages = ['jsb3.plugs', 'jsb3.plugs.core', 'myplugs']
...
def boot(ddir=None, force=False, encoding="utf-8", umask=None, saveperms=True, fast=False, clear=False, loadall=False):
    ...
    if os.path.isdir('jsb3'):
        ...
        if changed:
            logging.debug("jsb3-myplugs has changed -=- %s" % str(changed))
            for plugfile in changed:
    ...
    if os.path.isdir('jsb'):
        ...
        if corechanges:
            ...
            dosave = True
    if maincfg.extraplugs: plugin_packages.append("jsb3.plugs.extra")
    if True and maincfg.dbenable:
        plugin_packages.append("jsb3.plugs.db") 
        try:
...
def checktimestamps(d=None, dbenable=False):
    ...
    for f in os.listdir(d):
        if os.path.isdir(d + os.sep + f):
            ...
            if f.startswith("__"): logging.warn("skipping %s" % f) ; continue
            dname = d + os.sep + f
