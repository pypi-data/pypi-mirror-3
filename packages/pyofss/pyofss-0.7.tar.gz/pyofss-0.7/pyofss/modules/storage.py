...
def reduce_to_range( x, ys, first_value, last_value ):
   ...
   if( last_value > first_value ):
      ...
      # inclued the element corresponding to last_index, the second index 
...
class Storage():
   ...
   def find_nearest( self, array, value ):
      ...
      return index, array[index]
   # ===========================================================================
   def reduce_to_range( self, first_t, last_t ):
      """
      :param double first_t: Initial value of temporal range
      :param double last_t: Final value of temporal range

      Reduce the size of stored arrays, As and t.
      Start by finding left and right index corresponding to first_t and last_t 
      respectively. Use these indices to slice both t array and As array.
      """
      # ========================================================================
      print "Attempting to reduce storage arrays to specified range..."
      if( last_t > first_t ):

         first_index, t_first = self.find_nearest( self.t, first_t )
         last_index, t_last = self.find_nearest( self.t, last_t )
         # =====================================================================
         print "Required range: [{0}, {1}]\nActual range: [{2}, {3}]".format( \
               first_t, last_t, t_first, t_last ) 
         # =====================================================================
         # The returned slice does NOT include the second index parameter. To 
         # inclued the element corresponding to last_index, the second index 
         # parameter should be last_index + 1:         
         self.t = self.t[first_index : last_index + 1]

         # As is a list of arrays. Does each array contain additional arrays:
         import collections
         if( isinstance(self.As[0][0], collections.Iterable) ):
            self.As = [ [A_c0[first_index : last_index + 1],
                         A_c1[first_index : last_index + 1]]
                        for (A_c0, A_c1) in self.As ]
         else:
            self.As = [ A[first_index : last_index + 1] for A in self.As ]
      else:
         print "Cannot reduce storage arrays unless last_t > first_t"
   # ===========================================================================
