...
"""
Simple Disambiguator using nltk
--KATHURIA Pulkit
"""
...
def classify(text, synsets_scores):
    ...
    for line in text:
        ...
        for sentence in line.split('.'):
            ...
            if sent_score_pos: pos += sent_score_pos
            if sent_score_neg: neg += sent_score_neg
    return pos, neg
...
if __name__ == "__main__":
    ...
    if os.path.exists(pickles[0]) and myarguments.files:
        ...
        for file in myarguments.files:
            pos, neg = classify(file.readlines(), synsets_scores)
            if not scorer.has_key(file.name):
                scorer[file.name] = defaultdict(float)
