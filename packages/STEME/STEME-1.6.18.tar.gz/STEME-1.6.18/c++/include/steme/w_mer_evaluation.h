...
: boost::less_than_comparable< w_mer_evaluation >
, boost::equality_comparable< w_mer_evaluation >
{
    typedef std::vector< w_mer_evaluation >      vec;        ///< A vector of W-mer evaluations.
    typedef boost::shared_ptr< vec >             vec_ptr;    ///< A pointer to a vector of W-mer evaluations.

    double          Z;                      ///< Z for this W-mer.
    int             global_pos;             ///< Global position.
    ...
    /** Constructor. */
    w_mer_evaluation( double Z, int global_pos, bool rev_comp )
    : Z( Z )
...
sort_instances_by_position( Instances & instances ) {
    ...
    boost::sort( instances, bind( &w_mer_evaluation::global_pos, boost::lambda::_1 ) < bind( &w_mer_evaluation::global_pos, boost::lambda::_2 ) );
}
...
sort_instances_by_score( Instances & instances ) {
    ...
    boost::sort( instances, bind( &w_mer_evaluation::Z, boost::lambda::_1 ) > bind( &w_mer_evaluation::Z, boost::lambda::_2 ) );
}
...
output_non_overlapping_instances_from_sorted( Instances & instances, int W, OutIt out ) {
    boost::optional< w_mer_evaluation > last;
    BOOST_FOREACH( const w_mer_evaluation & instance, instances ) {
        ...
        else if( instance.global_pos - last->global_pos < W ) {
...
inline
void
erase_overlapping_from_score_sorted( w_mer_evaluation::vec & instances, int W, int max_size ) {
    w_mer_evaluation::vec::iterator i = instances.begin();
    while( instances.end() != i && i - instances.begin() != max_size ) {
        w_mer_evaluation::vec::iterator j = instances.begin();
        for( ; j != i; ++j ) {
...
calculate_overlap( InIt1 b1, InIt1 e1, int W1, InIt2 b2, InIt2 e2, int W2 ) {
    ...
    while( b1 != e1 && b2 != e2 ) {
        const right_open_interval< int > i1( b1->global_pos, b1->global_pos + W1 );
        const right_open_interval< int > i2( b2->global_pos, b2->global_pos + W2 );

        result += length( i1 & i2 );

        // advance the the iterator that is most behind
        if( first( i1 ) < first( i2 ) ) {
            const int previous_pos = b1->global_pos;
            ++b1;
            if( b1 != e1 && previous_pos + W1 > b1->global_pos ) {
            ...
            const int previous_pos = b2->global_pos;
            ++b2;
            if( b2 != e2 && previous_pos + W2 > b2->global_pos ) {
                throw std::logic_error(
                    MS_MAKE_STRING(
...
do_instances_overlap( Instances & instances, int W, bool already_sorted_by_position = false ) {
    ...
    BOOST_FOREACH( const w_mer_evaluation & instance, instances ) {
        ...
        if( last && instance.global_pos - last->global_pos < W ) {
            return true;
        }
        last = instance;
    }
...
OS &
operator<<( OS & os, const steme::w_mer_evaluation & e ) {
