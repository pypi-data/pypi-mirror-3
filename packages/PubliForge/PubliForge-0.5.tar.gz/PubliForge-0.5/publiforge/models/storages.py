# $Id: storages.py b59b421b4fa7 2012/02/21 10:24:51 yvon $
# -*- coding: utf-8 -*-
...
VCS_ENGINES = {'none': _(u'none – None'), 'local': _(u'local – Local'),
               ...
               'git': u'git – Git'}
STORAGE_PERMS = {'modifier': _('File editor'), 'viewer': _('Viewer')}
REFRESH = 3600
...
class Storage(Base):
    ...
    # -------------------------------------------------------------------------
    def __init__(self, settings, storage_id, label, description='',
    ...
    def load(cls, settings, storage_elt, error_if_exists=True):
        ...
        record = {
            'label': label,
            'description': storage_elt.findtext('description') is not None
                and storage_elt.findtext('description').strip() or '',
            ...
            if login not in done:
                ...
                if user is not None:
                    storage.users.append(StorageUser(
                        ...
                        item.get('in_menu'), item.get('permission', 'viewer')))
                done.append(login)
