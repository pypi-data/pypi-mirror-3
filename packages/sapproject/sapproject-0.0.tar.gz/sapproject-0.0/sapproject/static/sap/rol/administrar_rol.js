Ext.define('sap.administrarRolPanel', {
    extend: 'Ext.form.Panel',
    alias : 'sap.administrarRolPanel',

    title :   'Administrar Rol',
    id:       'panel_administrar_rol',
    name:     'panel_administrar_rol',
    layout:   'fit',
    frame:    true,
    closable: true,
    border:   false,
    ...
    initComponent: function() {
        var grid_roles = Ext.create('sap.rolGrid');
        this.items = [{
            xtype:      'form',
            title:      'Administrar Rol',
            autoScroll: true,
        ...
        this.buttons =[{
            ...
            id:      'asignar_permiso_btn',
            iconCls: 'asignar-icon',
            hidden:  true,
            grid:    grid_roles,
            handler: this.form_asignar_permiso_handler
        }, {
            text:    'Modificar',
            id:      'modificar_rol_btn',
            iconCls: 'update-icon',
            hidden:  true,
            grid:    grid_roles,
        ...
        this.listeners = {
            close : function() {
                ...
                Ext.getCmp('area-central').cerrar_pestanhas([
                    'panel_modificar_rol',
                    'panel_crear_rol',
                    'panel_asignar_permiso'
                ]);
            },
            render : function(){
                // mostramos los botones segun los permisos
                var permisos = Ext.getCmp('funcionalidades').permisos;
                if(permisos.indexOf('cr:ro') != -1){
                    Ext.getCmp('nuevo_rol_btn').setVisible(true);
                }
                if(permisos.indexOf('mo:ro') != -1){
                    Ext.getCmp('modificar_rol_btn').setVisible(true);
                }
                if(permisos.indexOf('el:ro') != -1){
                    Ext.getCmp('eliminar_rol_btn').setVisible(true);
                }
                if(permisos.indexOf('as:pe-ro') != -1){
                    Ext.getCmp('asignar_permiso_btn').setVisible(true);
                }
            }
        };
    ...
    form_asignar_permiso_handler : function() {
        var records = this.grid.getSelectionModel().getSelection();
        if(records == null || records.length == 0){
            Ext.Msg.alert('ERROR','Debe seleccionar un rol para asignar / desasignar permisos');
        }
        else{
            Ext.getCmp('area-central').agregar_pestanha('panel_asignar_permiso','sap.asignarPermisoPanel');
            var panel  = Ext.getCmp('panel_asignar_permiso');
    ...
    form_update_handler : function() {
        ...
        if(records == null || records.length == 0){
            ...
            Ext.getCmp('area-central').agregar_pestanha('panel_modificar_rol','sap.modificarRolPanel');
            Ext.getCmp('panel_modificar_rol').form.loadRecord(records[0]);
        }
    },
    ...
    form_buscar_handler: function(){
        ...
        if(valor == '' || campo == null){
            ...
            Ext.StoreMgr.lookup('rolStore').load({
                params: {
                    filtro : campo,
                    valor:   valor
                }
    ...
    form_new_handler: function() {
        Ext.getCmp('area-central').agregar_pestanha('panel_crear_rol','sap.crearRolPanel');
    },
    ...
    form_reset_handler: function() {
        ...
        // recargamos el store
        Ext.data.StoreManager.lookup('rolStore').load();
    },
