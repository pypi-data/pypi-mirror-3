...
class LexStat(object):
    ...
    def _init_csv(
        ...
        for i,x in enumerate(txt[0]):
            ...
            try:
                ...
                if y.lower() not in list(converter.values()):
        ...
        for r in required:
            if r[0] not in list(self._ifs.keys()):
                print('[!] Missing "{0}"-column in input file!'.format(r[1]))
        ...
        lines = 0
        for key,vals in sorted(list(tmp.items()),key=lambda x: x[0]):
            ...
            for j in range(self.words.shape[1]):
                if self.words[i][j] != '-':
                    try:
                        ...
                        print("[!] Some error with string <{0}> (ID: {1}).".format(
                            self.words[i][j],
                            self.idxs[i][j]
                            ))
    ...
    def output(
                ...
                for j in range(self.width):
                    if i < j:
                        ...
                        for pairA,pairB in self._pairs[i,j]:
                            ...
                            for k,(a,b) in enumerate(list(zip(almA,almB))):
                                if a == '-':
                                    almA[k] = str(i)+'.X.-'
                                if b == '-':
                                    almB[k] = str(j)+'.X.-'
                            ...
                            # write the data to file
                            out.write(item+'\n')
                            out.write(taxA+'\t'+'\t'.join(outA)+'\n')
                            out.write(taxB+'\t'+'\t'.join(outB)+'\n')
        ...
        if fileformat == 'msa':
            ...
            for key,val in list(self.etym_dict.items()):
                cog = key
        ...
        if fileformat == 'alm':
            ...
            for key,val in sorted(list(self.etym_dict.items()),key=lambda x:x[0]):
                cog = key
                entries = [self[i] for i in val[4]]
                taxa = [self[i,'l'] for i in val[4]]
                item = self[val[4][0],'i']
        ...
        if fileformat == 'csv':
            ...
            if sum(self.cogs.flatten()) != 0:
                out.write('\t'.join(self.dataById[0])+"\tCogID"'\n')
                for key,value in sorted(
                        list(self.dataById.items()),
                        key=lambda x:x[0]
                        )[1:]:
                    out.write(
                ...
                for key,value in sorted(list(self.dataById.items()),key=lambda x:x[0]):
                    out.write(
                            '\t'.join(value)+'\n'
                            )
            out.close()
