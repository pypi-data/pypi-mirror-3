 ...
 *          DO NOT MODIFY THIS FILE, MODIFY include/lamellarFF_HG.h
...
static PyObject * CLamellarFFHGModelError = NULL;
...
static void
CLamellarFFHGModel_dealloc(CLamellarFFHGModel* self)
{
...
static PyObject *
CLamellarFFHGModel_new(PyTypeObject *type, PyObject *args, PyObject *kwds)
{
    CLamellarFFHGModel *self;
    
    self = (CLamellarFFHGModel *)type->tp_alloc(type, 0);
   
    return (PyObject *)self;
}

static int
CLamellarFFHGModel_init(CLamellarFFHGModel *self, PyObject *args, PyObject *kwds)
{
    if (self != NULL) {
        ...
        self->model->t_length.dispersion->accept_as_source(visitor, self->model->t_length.dispersion, disp_dict);
        PyDict_SetItemString(self->dispersion, "t_length", disp_dict);
        disp_dict = PyDict_New();
        self->model->h_thickness.dispersion->accept_as_source(visitor, self->model->h_thickness.dispersion, disp_dict);
        PyDict_SetItemString(self->dispersion, "h_thickness", disp_dict);
...
static PyMemberDef CLamellarFFHGModel_members[] = {
    {name_params, T_OBJECT, offsetof(CLamellarFFHGModel, params), 0, def_params},
	{name_dispersion, T_OBJECT, offsetof(CLamellarFFHGModel, dispersion), 0, def_dispersion},     
    {name_log, T_OBJECT, offsetof(CLamellarFFHGModel, log), 0, def_log},
    {NULL}  /* Sentinel */
};
...
/** Read double from PyObject
    ...
    @return double
*/
double CLamellarFFHGModel_readDouble(PyObject *p) {
...
static PyObject *evaluateOneDim(LamellarFFHGModel* model, PyArrayObject *q){
 ...
 * @return: numpy array object 
 */
 static PyObject * evaluateTwoDimXY( LamellarFFHGModel* model, 
...
#pragma omp parallel for
        ...
        return PyArray_Return(result); 
		    ...
		    PyErr_SetString(CLamellarFFHGModelError, 
                   "CLamellarFFHGModel.evaluateTwoDimXY couldn't run.");
	        return NULL;
		}      	
}
/**
 ...
 *
 */ 
static PyObject * evalDistribution(CLamellarFFHGModel *self, PyObject *args){
	PyObject *qx, *qy;
	PyArrayObject * pars;
	int npars ,mpars;
    ...
    self->model->h_thickness = PyFloat_AsDouble( PyDict_GetItemString(self->params, "h_thickness") );
    ...
    self->model->t_length = PyFloat_AsDouble( PyDict_GetItemString(self->params, "t_length") );
    self->model->sld_head = PyFloat_AsDouble( PyDict_GetItemString(self->params, "sld_head") );
    self->model->background = PyFloat_AsDouble( PyDict_GetItemString(self->params, "background") );
    // Read in dispersion parameters
    PyObject* disp_dict;
    DispersionVisitor* visitor = new DispersionVisitor();
    disp_dict = PyDict_GetItemString(self->dispersion, "t_length");
    self->model->t_length.dispersion->accept_as_destination(visitor, self->model->t_length.dispersion, disp_dict);
    disp_dict = PyDict_GetItemString(self->dispersion, "h_thickness");
    self->model->h_thickness.dispersion->accept_as_destination(visitor, self->model->h_thickness.dispersion, disp_dict);

	
	// Get input and determine whether we have to supply a 1D or 2D return value.
	if ( !PyArg_ParseTuple(args,"O",&pars) ) {
	    PyErr_SetString(CLamellarFFHGModelError, 
	    	"CLamellarFFHGModel.evalDistribution expects a q value.");
		return NULL;
	}
    // Check params
    ...
    if(PyArray_Check(pars)==1) {
	    ...
	    if(npars==1) {
		    ...
		    PyErr_SetString(CLamellarFFHGModelError, 
                   "CLamellarFFHGModel.evalDistribution expect numpy array of one dimension.");
	        return NULL;
		}
	    ...
	    if(mpars!=2) {
	    	PyErr_SetString(CLamellarFFHGModelError, 
	    		"CLamellarFFHGModel.evalDistribution expects a list of dimension 2.");
	    	return NULL;
	    }
	     ...
	     if (PyArray_Check(qx) && PyArray_Check(qy)) {
		    ...
		    PyErr_SetString(CLamellarFFHGModelError, 
                   "CLamellarFFHGModel.evalDistribution expect 2 numpy arrays in list.");
	        return NULL;
	     }
	}
	PyErr_SetString(CLamellarFFHGModelError, 
 ...
 */
static PyObject * run(CLamellarFFHGModel *self, PyObject *args) {
	...
	if ( !PyArg_ParseTuple(args,"O",&pars) ) {
	    PyErr_SetString(CLamellarFFHGModelError, 
	    	"CLamellarFFHGModel.run expects a q value.");
		return NULL;
	}
	...
	if( PyList_Check(pars)==1) {
	    ...
	    if(npars!=2) {
	    	PyErr_SetString(CLamellarFFHGModelError, 
	    		"CLamellarFFHGModel.run expects a double or a list of dimension 2.");
	    	return NULL;
	    }
	    // We have a vector q, get the q and phi values at which
	    // to evaluate I(q,phi)
	    q_value = CLamellarFFHGModel_readDouble(PyList_GET_ITEM(pars,0));
	    phi_value = CLamellarFFHGModel_readDouble(PyList_GET_ITEM(pars,1));
	    // Skip zero
	    if (q_value==0) {
		...
		q_value = CLamellarFFHGModel_readDouble(pars);		
 ...
 */
static PyObject * calculate_ER(CLamellarFFHGModel *self) {
 ...
 */
static PyObject * calculate_VR(CLamellarFFHGModel *self) {
 ...
 * @return: function value
 */
static PyObject * runXY(CLamellarFFHGModel *self, PyObject *args) {
	...
	if( PyList_Check(pars)==1) {
	    ...
	    // to evaluate I(qx,qy)
	    qx_value = CLamellarFFHGModel_readDouble(PyList_GET_ITEM(pars,0));
	    qy_value = CLamellarFFHGModel_readDouble(PyList_GET_ITEM(pars,1));
	    return Py_BuildValue("d",(*(self->model))(qx_value,qy_value));

	} else {

		// We have a scalar q, we will evaluate I(q)
		qx_value = CLamellarFFHGModel_readDouble(pars);		
		
		return Py_BuildValue("d",(*(self->model))(qx_value));
	}	
}

static PyObject * reset(CLamellarFFHGModel *self, PyObject *args) {
    

    return Py_BuildValue("d",0.0);
}

static PyObject * set_dispersion(CLamellarFFHGModel *self, PyObject *args) {
	PyObject * disp;
	const char * par_name;

	if ( !PyArg_ParseTuple(args,"sO", &par_name, &disp) ) {
	    PyErr_SetString(CLamellarFFHGModelError,
	    	"CLamellarFFHGModel.set_dispersion expects a DispersionModel object.");
		return NULL;
	}
	...
	// Ugliness necessary to go from python to C
	    // TODO: refactor this
    if (!strcmp(par_name, "t_length")) {
        self->model->t_length.dispersion = dispersion;
    } else    if (!strcmp(par_name, "h_thickness")) {
        self->model->h_thickness.dispersion = dispersion;
    } else {
	    PyErr_SetString(CLamellarFFHGModelError,
	    	"CLamellarFFHGModel.set_dispersion expects a valid parameter name.");
		return NULL;
	}
...
static PyMethodDef CLamellarFFHGModel_methods[] = {
...
static PyTypeObject CLamellarFFHGModelType = {
    PyObject_HEAD_INIT(NULL)
    0,                         /*ob_size*/
    "CLamellarFFHGModel",             /*tp_name*/
    sizeof(CLamellarFFHGModel),             /*tp_basicsize*/
    0,                         /*tp_itemsize*/
    (destructor)CLamellarFFHGModel_dealloc, /*tp_dealloc*/
    0,                         /*tp_print*/
    0,                         /*tp_getattr*/
    0,                         /*tp_setattr*/
    0,                         /*tp_compare*/
    ...
    "CLamellarFFHGModel objects",           /* tp_doc */
    0,		               /* tp_traverse */
    0,		               /* tp_clear */
    0,		               /* tp_richcompare */
    0,		               /* tp_weaklistoffset */
    0,		               /* tp_iter */
    0,		               /* tp_iternext */
    CLamellarFFHGModel_methods,             /* tp_methods */
    CLamellarFFHGModel_members,             /* tp_members */
    0,                         /* tp_getset */
    0,                         /* tp_base */
    0,                         /* tp_dict */
    0,                         /* tp_descr_get */
    0,                         /* tp_descr_set */
    0,                         /* tp_dictoffset */
    (initproc)CLamellarFFHGModel_init,      /* tp_init */
    0,                         /* tp_alloc */
    CLamellarFFHGModel_new,                 /* tp_new */
};
...
void addCLamellarFFHGModel(PyObject *module) {
    ...
    if (PyType_Ready(&CLamellarFFHGModelType) < 0)
        return;

    Py_INCREF(&CLamellarFFHGModelType);
