...
def jslint(files, fix = False, relax = False):
    ...
    options.append('--custom_jsdoc_tags=version,ignore,returns,example,function,requires,name, \
                                        namespace,property,static,constant,default,location, \
                                        copyright,memberOf,lends,fileOverview')
    ...
    sh(command, header = "%s (%s files)" % (header, len(files)) )
