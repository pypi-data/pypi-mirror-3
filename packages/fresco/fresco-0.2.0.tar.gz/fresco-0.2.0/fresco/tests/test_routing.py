...
from fresco import FrescoApp, urlfor, GET, Response
...
class TestResolveViewSpec(object):
    ...
    def test_relative_import(self):
        assert resolve_viewspec('fixtures:CBV', 1) is CBV

    def test_attribute(self):
        assert resolve_viewspec('fixtures:CBV.index_html', 1) == CBV.index_html
        assert id(resolve_viewspec('fixtures:CBV.index_html', 1)) == \
                id(CBV.index_html)
        # Curiously the object identity test fails. Irritating, but I don't
        # think it matters.
        #assert resolve_viewspec('fixtures:CBV.index_html', 1) \
        #       is CBV.index_html
