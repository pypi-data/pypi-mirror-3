...
def get_ids(args, model, fields, op='&'):
    ...
    def _get_ids(self, b_model, b_field, b_value, b_line):
        print self, b_model, b_field, b_value, b_line
        search = []
        try:
        ...
        res = self.server.execute(self.db, self.uid, self.pwd,
               model, 'search', search) 
        if not res:
            self._logger.warning(u'get_ids(%s, %s): no object found for value `%s` on model' %
                    (model, fields, b_value), extra=STATS)
        return res
