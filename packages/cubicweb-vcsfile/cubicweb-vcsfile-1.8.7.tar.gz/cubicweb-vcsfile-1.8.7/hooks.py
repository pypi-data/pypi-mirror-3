...
class VCSFileStartHook(hook.Hook):
    ...
    def __call__(self):
        repo = self.repo
        def refresh_local_repo_caches(repo=repo, osp=osp, bridge=bridge):
            session = repo.internal_session()
            try:
                reposrset = session.execute(
                    'Any R, RT, RE, RLC, RSU, RP, RIRC WHERE R is Repository, '
                    'NOT (R local_cache NULL AND R path NULL),'
                    'R type RT, R encoding RE, R local_cache RLC, R source_url RSU, R path RP,'
                    'R import_revision_content RIRC, R cw_source S, S name "system"')
