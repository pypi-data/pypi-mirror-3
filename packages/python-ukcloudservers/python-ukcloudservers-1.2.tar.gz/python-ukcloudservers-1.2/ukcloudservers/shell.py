...
# Choices for flags.
DAY_CHOICES = [getattr(ukcloudservers, i).lower() 
               for i in dir(ukcloudservers)
               if i.startswith('BACKUP_WEEKLY_')]
HOUR_CHOICES = [getattr(ukcloudservers, i).lower()
                for i in dir(ukcloudservers)
...
class CloudserversShell(object):
    ...
    _api_class = ukcloudservers.CloudServers
    
    def __init__(self):
        self.parser = argparse.ArgumentParser(
            prog = 'ukcloudservers',
            description = __doc__.strip(),
            epilog = 'See "ukcloudservers help COMMAND" for help on a specific command.',
            add_help = False,
    ...
    def main(self, argv):                
        ...
        except ukcloudservers.Unauthorized:
    ...
    def do_backup_schedule(self, args):
        ...
        backup = {}
        if args.daily:
            backup['daily'] = getattr(ukcloudservers, 'BACKUP_DAILY_%s' % args.daily.upper())
        if args.weekly:
            backup['weekly'] = getattr(ukcloudservers, 'BACKUP_WEEKLY_%s' % args.weekly.upper())
    ...
    @arg('--flavor',
         default = None, 
         metavar = '<flavor>',
         help = "Flavor ID (see 'ukcloudservers flavors'). Defaults to 256MB RAM instance.")
    @arg('--image', 
         default = None,
         metavar = '<image>',
         help = "Image ID (see 'ukcloudservers images'). Defaults to Ubuntu 10.04 LTS.")
    @arg('--ipgroup',
         default = None, 
         metavar = '<group>',
         help = "IP group name or ID (see 'ukcloudservers ipgroup-list').")
    @arg('--meta', 
         metavar = "<key=value>", 
         action = 'append',
         default = [],
    ...
    @arg('--hard',
        dest = 'reboot_type',
        action = 'store_const',
        const = ukcloudservers.REBOOT_HARD,
        default = ukcloudservers.REBOOT_SOFT,
    ...
    def _find_flavor(self, flavor):
        ...
        except ukcloudservers.NotFound:
