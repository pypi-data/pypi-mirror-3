# $Id: storages.py c8eb72ad66ac 2012/08/06 10:56:35 patrick $
# -*- coding: utf-8 -*-
...
from ..lib.utils import _, encrypt, normalize_name, normalize_spaces
...
class Storage(Base):
    ...
    def load(cls, settings, storage_elt, error_if_exists=True):
        ...
        record = {
            ...
            vcs_elt.get('engine'),
            'vcs_url':
            vcs_elt.findtext('url') is not None
            and vcs_elt.findtext('url').strip() or None,
            'vcs_user':
            vcs_elt.findtext('user') is not None
            and vcs_elt.findtext('user').strip() or None,
            'vcs_password':
            vcs_elt.findtext('password') is not None
            and vcs_elt.findtext('password').strip() or None,
            'public_url':
            vcs_elt.findtext('public') is not None
            and vcs_elt.findtext('public').strip() or None,
            'access':
            storage_elt.findtext('access') is not None
            and storage_elt.findtext('access').strip() or 'open',
            'refresh':
            storage_elt.findtext('refresh') is not None
