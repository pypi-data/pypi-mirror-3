...
Please specify something like '?charset=utf8' explicitly.""")
    ...
    @staticmethod
    def one_to_one(foreign_table_name):
        """Class decorator, and a one-to-one relationship between two SQLAlchemy models.
        Parameters:
        foreign_table_name is the name of the table other than the one decorated by this method.
        Notice: this method is untested!
        """
        foreign_model_name = foreign_table_name.capitalize()
        foreign_key = '{0}_id'.format(foreign_table_name)
        def ref_table(cls):
            table_name = cls.__name__.lower()
            setattr(cls, foreign_key, Column(Integer, ForeignKey('{0}.id'.format(foreign_table_name))))
            setattr(cls, foreign_table_name, relationship(foreign_model_name, backref = backref(table_name, uselist = False)))
            return cls
        return ref_table
