...
def multifunc(data,axisref,deffunc):
    ...
    dataouttrns = zeros(getshapeout(datatrns,trnsaxisref,deffunc))
    scandata(datatrns,trnsaxisref,deffunc,dataouttrns)
    ...
    print('shapedataout: ',shape(dataout))
...
def getshapeout(datain,axisref,deffunc,dimidx=0):
    if ((axisref[dimidx] == False) & (dimidx <= (len(shape(datain))))):
        axidx = 0
        shapeout = getshapeout(datain[0],axisref,deffunc,dimidx = dimidx+1)
        shapeout.insert(0,datain.shape[0])
    else:
...
def scandata(datain,axisref,deffunc,dataout,dimidx=0):
    if ((axisref[dimidx] == False) & (dimidx <= (len(shape(datain))))):
        ...
        for axidx in range(datain.shape[0]):
            scandata(datain[axidx],axisref,deffunc,dataout[axidx],dimidx = dimidx+1)
    else:
