...
class LayoutViewer(Tkinter.Frame):              
    ...
    """
    def __init__(self, cells=None, hidden_layers=[], width=800, height=600, colors=None, outlines=None, background=(0,0,0), depth=0):
    ...
    def _update_canvas(self, *args):
        ...
        if len(layers) > 0:
            if len(self.colors) < max(layers):
                self.colors = self.colors * (max(layers) // len(self.colors) + 1)
            if len(self.outlines) < max(layers):
                self.outlines = self.outlines * (max(layers) // len(self.outlines) + 1)
        ...
        for i in layers:
            ...
            if i < 0:
                for pol in pol_dict[i]:
                    self.canvas.create_polygon(*list((numpy.array((1, -1)) * pol / self.scale).flatten()), fill=self.colors[0], outline=self.outlines[0], dash=(8,8), stipple='gray25', activeoutline='#FFF', activewidth=2, tag=('L' + str(i), 'V%d' % (pol.shape[0],)), state=state)
            else:
                for pol in pol_dict[i]:
    ...
    def _change_other_layers_visibility(self, evt):
        exception = int(self.listbox.get(self.listbox.nearest(evt.y)))
        layers = self.listbox.get(0, Tkinter.END)
        if len(self.hidden_layers) * 2 > len(layers):
            for i,layer in enumerate(layers):
                ilayer = int(layer)
                if ilayer != exception and ilayer in self.hidden_layers:
                    self.listbox.itemconfigure(i, foreground='#000000')
                    self.hidden_layers.remove(ilayer)
                    for j in self.canvas.find_withtag('L' + layer):
                        self.canvas.itemconfigure(j, state='normal')
        else:
            for i,layer in enumerate(layers):
                ilayer = int(layer)
                if ilayer != exception and not ilayer in self.hidden_layers:
                    self.listbox.itemconfigure(i, foreground='#aaaaaa')
                    self.hidden_layers.append(ilayer)
                    for j in self.canvas.find_withtag('L' + layer):
