...
class SetupConvert(zope.location.location.Location):
    ...
    def convert(self, key, value):
        ...
        Note: one important part of this converter concept is that we don't
        use covnerter defined in super classes. This means you can set your own
        converter in your own class but take care that you don't loose existing
        covnerters defined in your inherited classes.
...
class MongoItemBase(MongoParentAware, SetupConvertDump):
    ...
    def __init__(self, data):
        ...
        # update object with given (non IMongoItem) key/value data
        self.setup(data)
...
class MongoMappingBase(MongoItemAware, zope.location.location.Location):
    ...
    @property
    def collection(self):
        """Returns a mongodb collection based on the default"""
        raise NotImplementedError("Subclas must implement collection attribute")
...
# TODO: this will force to load an item, just lookup in caches and then 
#      only check for __name__ in mongodb, prevent loading all the data
#      and cache them in _cache_loaded
    ...
    def getBatchData(self, query=None, page=1, size=25, sortName=None,
        sortOrder=None, searchText=None, fields=None):
        ...
        Note: this method will call doBatchDataFilter whcih could enhance
        ...
        NOTE: searchText is not implemented yet, we will implement searhText
    ...
    def vote(self):
        ...
        if len(self._t_cache_update):
            self.doUpdate(collection, self._t_cache_update, upsert=False,
                manipulate=False, safe=False, multi=False)
        ...
        if obj is None:
            try:
                data = self.doFindOne(self.collection,
                    {'_id': pymongo.objectid.ObjectId(str(key))})
                if data is None:
                    raise KeyError(key)
