#!/usr/bin/python
from biotools.annotation import *
...
parameters that the Sequence constructor can handle are:
  ...
  * annotations => a set of Annotation objects that are related to this 
                   sequence in some way (support for this is lacking),
...
Constructs a new Sequence that is a subsequence as described by the way this
function is called. This function will automatically fill in the name, 
sequence, start, stop, step, original, and type of the subsequence. It also
tries to fill in the annotations, but annotations are handled pretty poorly
right now, so it's probably best not to worry about those, but it will work if
you really want to.'''
...
* sequence.upper() => technically does nothing, but returns the sequence with
                    a capitalized version of the sequence (which is done on 
                    instantiation, now).
* for character in sequence: => allows you to loop over each character in the 
                                sequence.
* dictionary[sequence] => allows sequences to be used as keys for dictionaries
                          and allows you to have sequences in sets. This relies
                          on the test seqA == seqB, described next.
* seqA == seqB     => compare two sequences. The sequences are the same if they
                      have the same sequence AND name. Therefore, two sequences 
                      with different names are treated as separate items in a set
                      and separate keys in a dictionary. If you need to match only
                      the sequence, use seqA.seq == seqB.seq.
* print sequence   => print a fasta / fastq (depending on whether there are any
                      quality scores) representation of the sequence. Sequence
                      objects in any other data structure (e.g., list, dictionary)
                      are printed as (e.g., <Sequence 0x000000>). If you want to
...
def dict(filename):
  '''dict( filename )
Transforms the output of open into a dictionary with the key as 
the sequence name and the value the corresponding Sequnece.'''
  return __builtin__.dict((s.name,s) for s in open(filename, 'r'))
  
def SequenceAnnotation(seq,source,type,**kwargs):
  '''SequenceAnnotation( sequence, source, type, ... )
