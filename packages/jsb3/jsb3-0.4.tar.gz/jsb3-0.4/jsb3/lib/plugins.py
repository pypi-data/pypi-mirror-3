...
from .jsbimport import force_import, myimport
...
class Plugins(LazyDict):
    ...
    def reloadfile(self, filename, force=True):
        logging.debug("reloading from file %s" % filename)
    ...
    def loadall(self, paths=[], force=True):
        ...
        new = []
        for module in paths:
            if module.endswith("__"): continue
            try: imp = myimport(module)
            ...
            try:
                for plug in imp.__plugs__:
                    ...
                    except KeyError: logging.warn("failed to load plugin package %s" % mod) ; continue
                    except Exception as ex: handle_exception() ; continue
                    new.append(mod)
            except AttributeError as ex: logging.error("no plugins in %s .. define __plugs__ in __init__.py: %s" % (module, str(ex)))
    ...
    def loaddeps(self, modname, force=False, showerror=False, loaded=[]):
        ...
        for dep in deps:
            if dep not in loaded:
                ...
                #if dep in self and force: self.unload(dep)
                try:
    ...
    def reload(self, modname, force=False, showerror=False):
        ...
        logging.debug("reload %s" % modname)
