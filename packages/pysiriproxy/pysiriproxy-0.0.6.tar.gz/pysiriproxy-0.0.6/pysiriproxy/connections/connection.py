...
class Connection(LineReceiver):
    ...
    iPhone or from the Guzzoni web server. The iPhone and Guzzoni web server
    transmit plist objects which are compressed using zlib compression. This
    class implements the necessary functionality for receiving the data, and
    ...
    def __hasNextObject(self):
        ...
        if pattern.match(unpacked) is not None:
            self.log.warn("ROGUE PACKET!!! WHAT IS IT?! TELL US!!! " \
                                "IN IRC!! COPY THE STUFF FROM BELOW")
        ...
        * obj -- The object that was received
        ...
        if refId is not None:
            ...
            if refId == self.__lastRefId and self.__blockRestOfSession:
                self.log.debug("Dropping object from Guzzoni: %s" % \
