...
from .utils import *
from .utils import bcrypt, scrypt, hash_old_sha256, hash_old_sha512
...
class Auth(object):
    ...
    def set_password(self, user, passw):
        passw = passw.encode('utf8', 'ignore')
    ...
    def create_user(self, login, passw, **data):
        ...
        passw = passw.encode('utf8', 'ignore')
        user_exists = self._get_user(login)
        if user_exists:
    ...
    def render(self, template, context):
        try:
            return self.settings.render(template, context)
        except TemplateNotFound:
            return auth_render(template, context)
    

    def render_to_string(self, template, context):
        try:
            return self.settings.render(template, context, to_string=True)
        except TemplateNotFound:
    ...
    def change_password_view(self, request, token=None, **kwargs):
        ...
        if token:
            ...
            if user:
                if request.user and (request.user != user):
                    self.sign_out(request)
                self.sign_in(request, user)
                return self.reset_password_view(request, user_requested=False)
    ...
    def email_token(self, user, token):
        ...
        self.render_to_string(template, {'user': user, 'token': token})
    ...
    def reset_password_view(self, request, user_requested=True, **kwargs):
