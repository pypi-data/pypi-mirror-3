...
try:
    import json # py2.6 or later
except ImportError:
    import simplejson as json

from eventful.proto.http import RESTServer, HttpHeaders
from eventful import log, Deferred, MessageProtocol, call_every
import eventful
import yaml
...
class EggmonsterControlServer(RESTServer):
    ...
    def _get_client_version(self, req):
        ...
        if not ua.startswith('Eggmonster/'):
            # Older versions of Eggmonster won't override the httplib2
            # User-Agent header. So we'll remain backwards compatible
            # for a little while and assume this is an older Eggmonster
            # client that is communicating with us.
            if ua.startswith('Python-httplib2/'):
                return (4, 1, 17)
...
def update_config(contents):
    ...
    g_config = config.load_config(fn)
...
def refresh_config():
    ...
    """
    global g_config
    config_ = config.load_config(config_fn(latest_config()))
    if config_ == g_config:
