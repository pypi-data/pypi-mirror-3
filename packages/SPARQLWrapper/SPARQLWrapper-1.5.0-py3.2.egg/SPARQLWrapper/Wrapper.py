...
class SPARQLWrapper :
    ...
    def _getURI(self) :
        ...
        return self.baseURI + "?" + urllib.parse.urlencode(dict([k, v.encode("utf-8")] for k, v in list(finalQueryParameters.items())))
...
##                acceptHeader = ",".join(_RDF_XML)
        ...
        if self.method == POST :
            # by POST
            request = urllib.request.Request(self.baseURI)
            ...
            request = urllib.request.Request(self._getURI())
