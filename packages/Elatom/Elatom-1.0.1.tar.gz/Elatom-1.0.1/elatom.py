...
class Download(threading.Thread):
    def __init__(self, url, filename, group=None,
...
class FeedReader(threading.Thread):
    ...
    def run(self):
        try:
            ...
            self.log('HTTP Error: ' + self._url + " " + HTTP_RESPONSES[e.code][0])
        except urllib.error.URLError as e:
...
class Elatom(threading.Thread):
    ...
    def run(self):
        ...
        for name in urls:
            ...
            for (url, pubdate, title) in urls[name]:
                ...
                filename = os.path.join(self._config.downloadDirectory(),
        ...
        while i < total:
            for name in downloadThreads:
                for downloadThread in downloadThreads[name]:
                    if (downloadThread.size != None and
                        not downloadThread.getUrl() in sizes[name]["urls"]):
    ...
    def formatTitle(title):
        ...
        return title.replace('FranceInter', '')\
                    .replace('FranceCulture', '')\
                    .replace('Podcast', '')\
                    .replace('podcast', '').strip()[0:20]
...
class ProgressBar(BaseProgressBar):
    ...
    def __init__(self, minValue=0, maxValue=100, width=80):
        BaseProgressBar.__init__(self, minValue=minValue, maxValue=maxValue,
                                    width=width)
...
class TkAbout(tkSimpleDialog.Dialog):
    ...
    def body(self, master):
        ...
        if os.path.exists(readme_path):
            file_handler = open(readme_path, 'r')
            readme_text = file_handler.read()
            file_handler.close()
        else:
            readme_text = _("""This file "%s" does not exist.""" % "README.txt")
...
class TkConfig(tkSimpleDialog.Dialog):
    ...
    def addFeed(self):
        ...
        if url:
            try:
                ...
                title = results['title']
                name = self._config.addFeed(name=title, url=url)
                if name != None:
                    self.widgets[name] = TkFeedWidget(self.frame, name, url, 'True')
                    self.widgets[name].pack()
                    self.update()
            except urllib.error.HTTPError as e:
                tkMessageBox.showwarning("Yes", 'HTTP Error: ' + url + " " + HTTP_RESPONSES[e.code][0])
            except urllib.error.URLError as e:
                try:
                    self.log("URL Error: " + url + HTTP_RESPONSES[e.code][0])
    ...
    def body(self, master):
        ...
        self.download_directory.set(self._config.downloadDirectory())
    ...
    def buttonbox(self):
        ...
        if add_width < 20:
            add_width = 20
        tkinter.Button(self, text=add_text, width=add_width, command=self.addFeed, default=tkinter.ACTIVE).pack(side=tkinter.LEFT)
    ...
    def save(self):
        ...
        for name in self.widgets:
            if self.widgets[name].e_name.get() != name or self.widgets[name].e_url.get() != self.widgets[name].url:
                self._config.addFeed(name=self.widgets[name].e_name.get(), url=self.widgets[name].e_url.get())
                self._config.deleteFeed(name)
            if self.widgets[name].ckb_active.get() == 1:
                self._config.activateFeed(self.widgets[name].e_name.get())
            else:
                self._config.inactivateFeed(self.widgets[name].e_name.get())
        if self.download_directory.get() != self._config.downloadDirectory():
            self._config.downloadDirectory(self.download_directory.get())
        self.close()
