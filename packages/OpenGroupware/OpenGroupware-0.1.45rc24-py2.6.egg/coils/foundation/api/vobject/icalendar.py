...
import string
import behavior
import coils.foundation.api.dateutil.rrule as coils_rrule
import coils.foundation.api.dateutil.tz as coils_tz
            ...
            for rule in completed[transitionTo]:
                ...
                if rule['end'] is not None:
                    if rule['hour'] is None:
                        ...
                        weekday = coils_rrule.weekday(rule['weekday'], num)
                        du_rule = coils_rrule.rrule(coils_rrule.YEARLY,
        ...
        for name in DATESANDRULES:
            ...
            for line in self.contents.get(name, ()):
                ...
                if name in DATENAMES:
                    ...
                    rule = coils_rrule.rrulestr(value, dtstart=dtstart)
                    until = rule._until 
            ...
            if name in DATENAMES:
                ...
                for rule in setlist:
                    ...
                    if (rule._byweekday is not None and (
                                  coils_rrule.WEEKLY != rule._freq or 
                    ...
                    if rule._bymonthday is not None and len(rule._bymonthday) > 0:
                        if not (rule._freq <= coils_rrule.MONTHLY and
                    ...
                    if rule._bymonth is not None and len(rule._bymonth) > 0:
                        if (rule._byweekday is not None or
                            ...
                            not (rule._freq == coils_rrule.YEARLY and
