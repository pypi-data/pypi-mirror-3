...
def dt_formater_from_format(date_format):
    ...
    return cache.cache("date",expire=_EXPIRE)(cdt_format)
...
def grouped_shooting(
    ...
    dt_log_formater = cache.cache("dtime", expire=_EXPIRE)(
        dt_formater_from_format(date_pattern[option.log_format])
    )
    date_formater = cache.cache("date", expire=_EXPIRE)(
        lambda dt: dt.strftime('%Y-%m-%d'))

    if "geo_ip" in option.skill:
        from pygeoip import GeoIP
        gi = GeoIP(option.geoip)
        country_by_ip = cache.cache("country",expire=_EXPIRE)(gi.country_code_by_addr)
