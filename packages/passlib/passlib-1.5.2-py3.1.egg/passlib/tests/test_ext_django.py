...
class PatchTest(TestCase):
    ...
    def assert_patched(self, context=Undef):
        ...
        self.assertIs(dam.User.check_password.__func__, state['user_check_password'])
        self.assertIs(dam.User.set_password.__func__, state['user_set_password'])
    ...
    def check_hashes(self, tests, new_hash=None, deprecated=None):
        ...
        for test in tests:
            for secret, hash in test.all_correct_hashes:
                ...
                u.password = hash
                if has_django0 and isinstance(secret, str):
