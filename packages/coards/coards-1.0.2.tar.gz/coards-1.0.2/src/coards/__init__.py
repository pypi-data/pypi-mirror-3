...
def parse(value, units):
    ...
    seconds = value*parse_units(parts[0])
    offset = parse_date(parts[1])

    try:
        date = EPOCH + timedelta(seconds=offset+seconds)
    except OverflowError:
        warnings.warn(
                "Shifted data 366 days to the future, since year zero does not exist.",
                UserWarning)
        date = EPOCH + timedelta(seconds=offset+seconds+LEAP_YEAR)
...
def format(date, units):
    ...
    dt = (date - EPOCH)
    value = dt.days*DAY + dt.seconds + dt.microseconds*MICROSECOND - offset
    return value / parse_units(parts[0])
