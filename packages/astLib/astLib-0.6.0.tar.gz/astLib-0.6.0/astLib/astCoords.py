...
def decimal2hms(RADeg, delimiter):
    """Converts decimal degrees to string in Hours:Minutes:Seconds format with user specified
    delimiter.
...
def decimal2dms(decDeg, delimiter):
    """Converts decimal degrees to string in Degrees:Minutes:Seconds format with user specified
    delimiter.
    ...
    if decDeg>0:
        ...
        secs=(decDeg-(float(sDeg)-float(sMins)/60.0))*3600.0
        if secs>-10 and secs<0:
            sSecs="0"+str(secs)[1:str(secs).find(".")+3]	# so don't get minus sign
        elif secs>-0.001:
            sSecs="00.00"
        else:
...
def calcAngSepDeg(RADeg1, decDeg1, RADeg2, decDeg2):
    """Calculates the angular separation of two positions on the sky (specified in decimal
    degrees) in decimal degrees, assuming a tangent plane projection (so separation has to be
    <90 deg). Note that RADeg2, decDeg2 can be numpy arrays.
    ...
    dRA=cRA-gRA
    dDec=gDec-cDec
    cosC=(numpy.sin(gDec)*numpy.sin(cDec))+(numpy.cos(gDec)*numpy.cos(cDec)*numpy.cos(gRA-cRA))
    x=(numpy.cos(cDec)*numpy.sin(gRA-cRA))/cosC
    y=((numpy.cos(gDec)*numpy.sin(cDec))-(numpy.sin(gDec)*numpy.cos(cDec)*numpy.cos(gRA-cRA)))/cosC
...
def convertCoords(inputSystem, outputSystem, coordX, coordY, epoch):	
    """Converts specified coordinates (given in decimal degrees) between J2000, B1950, and
    Galactic.
    
    @type inputSystem: string
    @param inputSystem: system of the input coordinates (either "J2000", "B1950" or "GALACTIC")
    @type outputSystem: string
    @param outputSystem: system of the returned coordinates (either "J2000", "B1950" or
    "GALACTIC")
    @type coordX: float
    ...
    if inputSystem=="J2000" or inputSystem=="B1950" or inputSystem=="GALACTIC":
        if outputSystem=="J2000" or outputSystem=="B1950" or outputSystem=="GALACTIC":
    ...
    raise Exception, "inputSystem and outputSystem must be 'J2000', 'B1950' or 'GALACTIC'"
        
#---------------------------------------------------------------------------------------------------
def calcRADecSearchBox(RADeg, decDeg, radiusSkyDeg):
    """Calculates minimum and maximum RA, dec coords needed to define a box enclosing a circle of
    radius radiusSkyDeg around the given RADeg, decDeg coordinates. Useful for freeform queries 
    of e.g. SDSS, UKIDSS etc.. Uses L{calcAngSepDeg}, so has the same limitations.
    ...
    @param radiusSkyDeg: radius in degrees on the sky used to define search region
    @rtype: list
    @return: [RAMin, RAMax, decMin, decMax] - coordinates in decimal degrees defining search box
