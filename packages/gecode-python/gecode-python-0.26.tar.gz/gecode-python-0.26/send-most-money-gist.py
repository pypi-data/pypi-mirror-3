...
def showme(s2):
    print "hello"
    print (s2.values(money), s2.values(letters))
@textinspector("Show Me 2")
def showme2(s2):
    return "%s, %s" % (s2.values(money), s2.values(letters))
@textinspector("Show Me 3")
def showme3(s2):
    return "%s, %s" % (s2.values(money), s2.values(letters))
s.gist(onclick=(showme,showme2,showme3))
