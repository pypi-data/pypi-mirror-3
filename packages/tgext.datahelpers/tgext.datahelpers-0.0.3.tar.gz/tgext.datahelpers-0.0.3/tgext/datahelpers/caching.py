...
class entitycached(object):
    def __init__(self, key_argument, expire=3600*24*3, cache_type='memory', namespace=None):
    ...
    def _object_primary_key(self, model):
        if hasattr(model, '_id'):
            ...
            return model.__mapper__.primary_key[0].key
...
if you are multiple decorators check that @entitycached is the first.')
        ...
        def wrapped_function(*args, **kw):
            ...
            return cache.get_value(cache_key, createfunc=call_function, expiretime=self.cache_expire)
