...
EmailMessageSchema = folder.ATFolderSchema.copy() + atapi.Schema((
        atapi.StringField('sender', label='Sender', required=True,
                          ...
                          widget=atapi.StringWidget(label='Sender')
                          ),
...
class EmailMessage(folder.ATFolder, MegamanicEditable.MegamanicEditable):
    ...
    def setup_for_edit(self, *a, **kw):
        ...
        if not self.getSender().strip():
            ...
            self.setSender('"%s" <%s>' % (member.fullname, member.email))

    def mail_enabled(self):
        "Returns true if mail is enabled."
        # Nowhere better to put it, manage_afterAdd won't
        # do the setup
        self.setup_for_edit()
        return EMAIL_4
