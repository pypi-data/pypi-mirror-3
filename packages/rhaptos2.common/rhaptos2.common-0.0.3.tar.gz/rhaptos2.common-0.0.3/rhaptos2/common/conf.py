...
confd = conf.get_config('rhaptos2',__file__)
#must be a default.ini next to this module
...
def get_config(appname):
    ...
    and at whatever location CONFIGFILE= is in os.environ
    
    
    We expect to receive ConfigParser compiliant files at these locations
    We expect there to be one section only, called appname

    We will turn these into one dict, and return it
 
    If we have *any* keys in os.environ that match those keys already specified in a file
    os.environ will overwrite the file values.  So don't call your key PATH


    '''
    print "################## Starting config for %s" % appname
    confd = {}
    #todo: error check
    inifile = appname + ".ini"
    for loc in ('/etc', '/usr/local/etc'):
        ...
        try:
            print "Reading %s" % confpath
            d = read_ini(confpath, appname)
            #brtue update - todo: log this stage carefully
            confd.update(d)    
        except Rhaptos2Error, e:
            print 'No file at %s found.' % (confpath)
    

    if 'CONFIGFILE' in os.environ.keys(): 
        try:   
            print "Reading CONFIGFILE from %s" %  os.environ['CONFIGFILE']
            d =  read_ini(os.environ['CONFIGFILE'], appname)
            confd.update(d)
        except Rhaptos2Error, e:
            print 'No file at %s found.' % (os.environ['CONFIGFILE'])
 
    ### overwrite from environment 
    for k in os.environ:
        if k in confd.keys():
            confd[k] = os.environ[k]
            print "overwriting %s -> %s (%s)" % (k, k, os.environ[k])

    ### snaffle namespave from env
    ### If we find a rhaptos2_myvar: in the env, grab that too.
    for k in os.environ:
        if k.find(appname + "_") == 0:
            k1 = k.replace(appname + "_", '')
            confd[k1] = os.environ[k]
            print "overwriting %s -> %s (%s)" % (k, k1, os.environ[k])

    import pprint 
    print "********** RESULTING CONF SETTINGS ***************"
    pprint.pprint(confd)
    print "********** RESULTING CONF SETTINGS ***************"
    print "################## Ending config for %s" % appname
    return confd    
