...
from cspace.network.locationcache import ES_NOTIFYING 

def fromPEM_PublicKeyFixed(src):
    logger.debug('service.fromPEM_PublicKeyFixed')
...
class CSpaceService:
    ...
    class ContactStatusChecker:
        ...
        def onContactRemoved(self, contact):
            try:
                ...
                logger.exception('onContactRemoved: %s not in probeOps:%s' % (contact.name, str(self.probeOps)))
        ...
        def probeContactStatus(self, cname):
            ...
            pubKeyData = contact.publicKey.toDER_PublicKey()
            entry = self.service.session.locationCache.d.get( pubKeyData )
            if entry is not None and entry.state == ES_NOTIFYING:
                return
            self.probeOps[cname] = True
            self.service.session.probeUserOnline(contact.publicKey,
        ...
        def onStatusProbed(self, cname, status):
            ...
            if contact:
                ...
                #logger.debug("[%s]: %s" % (contact.name, self.service.STATII[contact.status]))
            else:
                logger.debug("Contact removed before status returned.")

            #logger.debug('want to remove ' + cname)
            #logger.debug('list: ' + str(self.probeOps))
            try:
                del self.probeOps[cname]
            except:
                logger.exception('%s not in probeOps:%s' % (cname, str(self.probeOps)))
    ...
    class ActionManager:
        ...
        def registerAction( self, action, actionCallback, actionOrder ):
            logger.info("Registering action: %s" % action)
        ...
        def execAction( self, actionDir, contactName ) :
            ...
            for info in self.actions :
                if info[4] == actionDir :
                    #logger.debug("Executing action %s on contact %s" % (actionDir, contactName))
                    info[3]( contactName )
                    return True
                else:
    ...
    def online(self):
        ...
        logger.info("Going online as %s..." % profile.name)
    ...
    def action(self, buddy, action):
        ...
        for actionItem in actionManager.actions:
            if actionItem[4] == action:
                #logger.debug("Running action %s on %s" % (action, buddy))
