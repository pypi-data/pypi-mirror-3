# $Id: front.py 77a5c869abcc 2012/09/02 18:01:52 patrick $
...
class FrontBuildManager(object):
    ...
    def __init__(self, settings):
        ...
        for index in range(0, 100):
            ...
            if weight:
                ...
                total += weight
        if total:
            self._concurrent = int(
                self._concurrent * total /
    ...
    def start_build(self, request, processing, processor, pack, tasks=None):
        ...
        :param tasks: (tuple, optional)
            A tuple such as ``(task_back, task_next)``. If it is not ``None``,
        ...
        if agent_url is None:
            request.session.flash(
                _('No available agent for ${p}.', {'p': processing.processor}),
                'alert')
            return
        ...
        pack_dict = {
            ...
            'task_back': tasks and tasks[0] or '',
            'task_next': tasks and tasks[1] or ''}
        if request.session.peek_flash('alert'):
            return
        ...
        self._builds[build_id] = FrontBuild(
            self, build_id, agent_url, user_id, request.session['lang'],
            processing_dict, pack_dict)
    ...
    def _variables2dict(cls, request, processing, processor):
        ...
        for var in processor.findall('processor/variables/group/var'):
            ...
            value = values[name] if name in values \
                else (defaults[name] if name in defaults
                      and defaults[name] is not None
                      else var.findtext('default', '').strip())
...
class FrontBuild(object):
    ...
    # -------------------------------------------------------------------------
    def __init__(self, build_manager, build_id, agent_url, user_id, lang,
                 processing, pack):
        """Constructor method.
    ...
    def _thread_start(self, request):
        ...
        error = self._build_manager.call(
            request, self._agent_url, 'start', self.uid, self._processing,
    ...
    def _dir2buildspace(self, request, directory):
        ...
        error = self._build_manager.call(
            request, self._agent_url, 'buildspace_cleanup', directory,
            file_list)[0]
        if self.stopped(error):
            return
    ...
    def _file2storage(self, request, root, filename):
        ...
        # Transfer full file
        if not exists(fullname):
            error, content = self._build_manager.call(
                request, self._agent_url, 'output_send_file', self.uid,
                filename)
            if self.stopped(error):
                return
        ...
        error, delta = self._build_manager.call(
            request, self._agent_url, 'output_send_delta', self.uid, filename,
    ...
    def _result2pack(self, base, result):
        ...
        """
        if not self._processing['add2pack']:
            return
        ...
        # File list
        file_list = []
        if self._processing['add2pack'][:6] == 'result':
            file_list = self.result.get('files')
        elif not self._agent_url:
            for path, name, files in walk(result):
                for name in files:
                    if name not in EXCLUDED_FILES and '~' not in name:
                        file_list.append(relpath(join(path, name), result))
        else:
            for name in result:
                if name not in EXCLUDED_FILES and '~' not in name:
        ...
        # Add to database
        file_type = self._processing['add2pack'][7:-1]
        done = [k.path for k in pack.files if k.file_type == file_type]
        for name in file_list:
            name = join(base, name)
            if not name in done:
        ...
        self._log(_(
            'result added to pack (${t})', {'t':
            self._translate(ADD2PACK_TARGETS[self._processing['add2pack']])}),
            'get', 100)
    ...
    def _move2task(self, request, error=False):
        ...
        :param error: (boolean, default=False)
            ``True`` if error.
        """
        target_task_id = (error and self._pack['task_back']) \
            or (not error and self._pack['task_next'])
        if not error and not target_task_id:
            return
        ...
        pack = DBSession.query(Pack).filter_by(
            ...
            pack_id=self._pack['pack_id']).first()
        if pack is not None:
            pack2task(request, pack, error, target_task_id)
