...
that starts with a start codon and contains no stop codon other than the final
codon.'''
...
def PutativeGenesInORF(orf):
	'''PutativeGenesInORF(sequence)
Takes an ORF and yields progressively longer subsequences that start with
a start codon. This function also trims the terminal stop codon from the ORF.'''

	l = len(orf)
	for i in xrange(l-300,-1,-3):
		if orf.seq[i:i+3] in options.START_CODONS:
			pg = orf[i:-3]
			yield pg
	raise StopIteration

def _genepredict_target(qin,qout,orfs,subj):
	'''_genepredict_target(queuein, queueout, ORFs, queries)
Takes BLAST results from queuein, finds the sequence in queries to which the
result corresponds, finds ORFs that span the BLAST hit area, and aligns the
successively longer putative genes in those ORFs until the optimal alignment
is found. If such an alignment exists, and passes the length and percent identity
requirements, then it is reported via queueout.'''
	...
	while not qin.empty():
		...
		alignments = []
		#for gene in (orf for s in o for orf in PutativeGenesInORF(s) \
		#	if _genepredict_in_range(s,start,end) and len(orf) > length * options.MIN_LENGTH / 100.0):
...
def _genepredict_in_range(seq,start,end):
	'''_genepredict_in_range(ORF, start, end)
Judges whether or not the given ORF spans the region (start, end).'''
...
whether the sequnece is amino acid or nucleotide.'''
	...
	gh = open( wd + pref + '.gff3', 'w' )
	gh.write('##gff-version 3\n')

	sa = sequ.SequenceAnnotation
	for id in seqs:
		fh.write(seqs[id])
		ah.write(translate(s) for s in seqs[id])
		gh.write(str(sa(seqs[id].copy().pop(),pref,'gene',homologs=','.join(s.name for s in seqs[id]))) + '\n')
	fh.close()
	ah.close()
	gh.close()
...
def run(*args):
	'''run(*args)
This function is just setup for the GeneFromBLAST function, parsing options, etc.'''
	if len(args) < 3:
		options.help()
		raise RuntimeError
	else:
