...
class URLCallerTask(BaseTask):
    ...
    def run(self, db, root, site, report):
        ...
        credentials = self.username and (self.username, self.password) or ()
        proxy = self.proxy_server and (self.proxy_server, self.proxy_port) or ()
        proxy_auth = self.proxy_username and (self.proxy_username, self.proxy_password) or ()
