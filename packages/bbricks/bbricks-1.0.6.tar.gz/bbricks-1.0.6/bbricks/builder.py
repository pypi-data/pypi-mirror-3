...
class Builder:
    ...
    @staticmethod
    def ensure_dependency_exists(path):
    ...
    @staticmethod
    def __check_installed_deps(target):
        """Check all dependencies exist for target
        ...
        if isinstance(deps, str):
            Builder.ensure_dependency_exists(deps)
            return

        if isinstance(deps, list):
            for dep in deps:
                Builder.ensure_dependency_exists(dep)
            return
    ...
    def build(self, target_names, banner=None):
        ...
        self.__check_installed_deps(target)
