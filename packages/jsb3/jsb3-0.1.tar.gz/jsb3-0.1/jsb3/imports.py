...
def getjson():
    try:
        ...
        try: mod = myimport("json")
        except:
            try:
                mod = myimport("simplejson")
            except:
                mod = myimport("jsb.contrib.simplejson")
...
def getfeedparser():
    ...
    except: mod = myimport("jsb.contrib.feedparser")
...
def getoauth():
    try: mod = myimport("oauth")
        ...
        mod = myimport("jsb.contrib.oauth")
...
def getBeautifulSoup():
    ...
    except: mod = myimport("jsb.contrib.BeautifulSoup")
    logging.info("BeautifulSoup module is %s" % str(mod))
    return mod

def getsleek():
    try: mod = myimport("sleekxmpp")
    except: mod = myimport("jsb.contrib.sleekxmpp")
