# cython: profile=True
# Time-stamp: <2012-04-22 17:59:17 Tao Liu>
...
cdef inline int compare_int(const_void *a, const_void *b): return (<int *>a)[0] - (<int *>b)[0]
...
cdef int * build_start_poss ( plus_tags, minus_tags, long five_shift, long three_shift, long l ):
    cdef long i
    cdef int * start_poss = <int *>malloc( l*sizeof(int) )
    ...
    for i in xrange(lp):
        # shift to get start positions. To 5' side.
        # since start positions may be smaller than 0, take the max with 0
        start_poss[i] = int_max(plus_tags[i]-five_shift,0)

    # for minus tags
    for i in xrange(lm):
        # shift to get start positions by extending to d. To 3' side.
        # since start positions may be smaller than 0, take the max with 0
        start_poss[i+lp] = int_max(minus_tags[i]-three_shift,0)
            
    # sort
    qsort(<void *>start_poss, l, sizeof(int), compare_int)
    return start_poss

cdef int * build_end_poss ( plus_tags, minus_tags, long five_shift, long three_shift, long l ):
    cdef long i
    cdef int * end_poss   = <int *>malloc( l*sizeof(int) )
    ...
    for i in xrange(lp):
        # shift to get end positions by extending to d. To 3' side.
        #end_poss.append(plus_tags[i]+three_shift)
        end_poss[i] = plus_tags[i]+three_shift

    # for minus tags
    for i in xrange(lm):
        # shift to get end positions. To 5' side.
        #end_poss.append(minus_tags[i]+five_shift)
        end_poss[i+lp] = minus_tags[i]+five_shift
            
    # sort
    qsort(<void *>end_poss, l, sizeof(int), compare_int)
    return end_poss

#cdef pileup_a_chromosome ( int * start_poss, int * end_poss, long l, float scale_factor = 1 ):
