# $Id: download.py,v 1.14 2012/02/17 10:26:50 irees Exp $
import Queue
import collections
import datetime
import functools
...
class BaseDownload(emdash.upload.BaseTransport):
	...
	def begin_session(self):
		...
		for target in self._targets:
			self.set_target(int(target))


	@QtCore.pyqtSlot(int)
	def set_target(self, name):
		super(BaseDownload, self).set_target(name)
		recs = self.db.getchildren(self.target, recurse=-1)
		recs = set(recs)
		recs.add(self.target)
		bdos = self.db.findbinary(record=list(recs))
		for bdo in bdos:
			bdo['_auto'] = True
			self.signal_newfile.emit(bdo.get('name'), bdo)
