...
from .util import funcname, validate_func_arguments
...
class Cloud(object):
    ...
    def _getJobParameters(self, func, mappings, ignore=[]):
        ...
        if '_type' in mappings and '_type' not in ignore:
            if isinstance(mappings['_type'], str):
                _type = mappings['_type']
            else:
    ...
    def status(self, jids):
        ...
        #print 'jids %s. status_dict %s' % (jids, status_dict) 
      
        if deseq:
            if len(status_dict) > 1:
                raise ValueError('Expected 1 status; got %s. Results %s', len(status_dict), str(status_dict.items()))
    ...
    def result(self, jids, timeout = None, ignore_errors = False):
        ...
        if needed_jids:
            try:                
                aresults = self.adapter.jobs_result(jids=needed_jids)
            except _catchCloudException, e:
                raise e                                           
            
            for needed_jid, result in izip(maybe_xrange_Iter(needed_jids),aresults):
                self.cacheManager.putCached(needed_jid, status='done', result=result)
        else:
            aresults = []
        ...
        *num_in_parallel* controls how many results are read-ahead from the cloud
        ...
        def result_loop(iterator_ref):
            ...
            while self.__isopen:                
                ...
                new_results  = self.result(jids_to_access) 
                with result_cv:
    ...
    def info(self, jids, info_requested=None):
        ...
        *info_requested*.  Each key maps to key-specific information
	    ...
	    'status', 'stdout', 'stderr', 'exception', 'runtime'- which return job status,  
	    standard output,, standard error, exception raised (if applicable) and real 
	    runtime of job respectively.  If *info_requested* is None/empty, all info will be requested
	    
	    The job must have completed for 'exception' to be meaningful
        ...
        if not info_requested:
            info_requested = ['stdout', 'stderr', 'exception', 'status', 'runtime']
    ...
    def map(self, func, *args, **kwargs):
        ...
        * _type:
        ...
        for arg in args:
            try:
                argIters.append(maybe_xrange_Iter(arg))
