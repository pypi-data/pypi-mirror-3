...
def fromDER_PublicKeyFixed(src):
    print 'appletserver.fromDER_PublicKeyFixed'
...
class BridgeThread( threading.Thread ) :
    ...
    def run( self ) :
        logger.info('BridgeThread %s started' % self.getName())
        self.reactor.run()
        logger.info('BridgeThread %s finished' % self.getName())
...
#            return
        try:
            pubKeyData = hexDecode( hexPubKey )
            pubKey = fromDER_PublicKeyFixed(pubKeyData)
        except:
            self._writeError( 'Malformed publickey' )
            return
