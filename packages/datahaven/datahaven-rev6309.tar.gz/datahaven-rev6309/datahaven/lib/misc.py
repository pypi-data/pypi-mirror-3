        ...
        if len(filesList) > 0:
            ...
            for filename in filesList:
                if os.path.isfile(filename):
                    zfile.write(filename, os.path.basename(filename), zipfile.ZIP_DEFLATED)
            zfile.close()
...
def UpdateRegistryUninstall():
    try:
        import _winreg
    except:
        return False

    regpath = "SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\DataHaven.NET"
    values = {
