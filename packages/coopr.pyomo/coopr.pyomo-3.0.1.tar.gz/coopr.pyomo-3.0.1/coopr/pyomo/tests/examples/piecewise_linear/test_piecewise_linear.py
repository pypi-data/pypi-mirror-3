...
import coopr.pyomo.base.numtypes as nt
import cPickle
...
def createTestMethod(pName,problem,solver,writer,kwds):
    ...
    def testMethod(obj):
        ...
        for name, var in [(n,v) for (n,v) in results.solution(1).variable.iteritems() if (n[:2] == 'Fx') or (n[:1] == 'x')]:
            bF = float(baseline_results['Solution'].Variable[name]['Value'])
            F = float(var['Value'])
            if abs(F-bF) > 0.00001:
            ...
            for AUX in aux_list:
                for REPN in ['SOS2','BIGM_SOS1','BIGM_BIN','DCC','DLOG','CC','LOG','MC','INC']:
                    for BOUND_TYPE in ['UB','LB','EQ']:
                        for SENSE in [nt.maximize,nt.minimize]:
                            if not(((BOUND_TYPE == 'LB') and (SENSE == nt.maximize)) or \
                                   ((BOUND_TYPE == 'UB') and (SENSE == nt.minimize))):
                                kwds = {}
                                kwds['sense'] = SENSE
                                kwds['pw_repn'] = REPN
                                kwds['pw_constr_type'] = BOUND_TYPE
                                if SENSE == nt.maximize:
                                    attrName = "test_{0}_{1}_{2}_{3}_{4}_{5}".format(PROBLEM,REPN,BOUND_TYPE,'maximize',solver,writer)
                                elif SENSE == nt.minimize:
