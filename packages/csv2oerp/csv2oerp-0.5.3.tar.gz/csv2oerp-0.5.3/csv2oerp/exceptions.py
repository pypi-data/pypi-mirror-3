...
class RequiredFieldError(IndexError):
    """Exception used when a column index has not be found 
    
    """
    def __init__(self, model, field, log=None):
        """
        ...
        """
        msg = "Required model(%s) field(%s) has null value. Skipping." % (model, field)
        if log:
            log.info("<%-20s> %s" % msg, extra=STATS)
