...
class Doc(Gtk.TextView):
	def __init__(self, notebook, handler, contents):
	...
	def set_default_font(self):
		font = Pango.FontDescription('Ubuntu')
	...
	def buffer_changed(self, data):
		...
		self.handler.notebook.indicate_unsaved_changes()
		self.do_highlight()
	...
	def do_highlight(self):
		if self.handler.get_file_type() == 'text/x-python':
			...
			if result:
				for iter in result:
					self.buffer.apply_tag(self.tags['boldblue'], iter[0], iter[1])
		elif self.handler.get_file_type() in ['text/x-csrc', 'text/x-c++src', 'text/x-chdr']:
			for code in self.c:
				result = self.find_text(code)
				if result:
					for iter in result:
