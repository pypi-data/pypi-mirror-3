...
class HasGrades(JSONTransforms):
    ...
    def value(self):
        return float(self._json.get('value', 1.0))
...
class HasItems(JSONTransforms):
    ...
    def subitems_values(self):
        return iter([ float(item.get('value', 0.0)) for item in self.subitems() ])
        ...
        try:
            ...
            if isinstance(response, int):
                try:
                    ...
                    value = float(self._json['items'][idx].get('value', 0.0))

                text = self._json['items'][idx]['text']
                value = max([ x.get('value', 0) for x in self._json['items'] if x['text'] == text ])
...
class LearningObj(HasGrades, HasContent):
    ...
    def value(self):
        ...
        if self.value_denorm() == 0.0:
            ...
            return float(self.get('value', 1.0))
        ...
        for i in range(100):
            try:
                ...
                if 'y' in ans:
                    ...
                    answers = ''
            except (MissingAnswer, IncompleteAnswer, InvalidAnswer) as ex:
                if isinstance(ex, InvalidAnswer):
                    print '  %s invalid answer: %s' % (ex.answer, ex.ans_value)
                ans_list = []
                for node_idx, node in Exam(exam).flat_nodes_info():
                    if node_idx == ex.answer:
                        break
                    ans_list.append(node.iresponse_marked(short=False))
                answers = ''.join(ans_list)
                if isinstance(ex, (IncompleteAnswer, MissingAnswer)):
...
def correct_repeated_exam(obj):
    for qst in obj['content']:
        correct_repeated_qst(qst)

def correct_repeated_qst(obj):
    for obj in obj['content']:
        if obj['type'] == 'multiple-choice':
            correct_repeated_mc(obj)

def correct_repeated_mc(obj):
    # format text
    for item in obj['items']:
        ...
        txt.replace('$ ', '$')
        item['text'] = txt
    txts = [ item['text'] for item in obj['items'] ]
    if len(txts) > len(set(txts)):
        items = obj['items']
        new_items = []
        txts = {}
        for item in items:
            ...
            if text in txts:
                item0_idx = txts[text]
                item0 = items[item0_idx]
                values = item.get('value', 0), item0.get('value', 0)
                if values[0] != values[1]:
                    max_value = max(values)
                    item0['value'] = item['value'] = max_value
                print 'repeated: %s' % text
            else:
                txts[text] = len(new_items)
...
def correct_dir(dir):
    ...
    for f in files:
        if f.endswith('.json') and 'all_exams' not in f:
            print 'applying changes to %s' % f
            f = J(dir, f)
            with open(f) as F:
                json = simplejson.load(F)
                correct_repeated_exam(json)
