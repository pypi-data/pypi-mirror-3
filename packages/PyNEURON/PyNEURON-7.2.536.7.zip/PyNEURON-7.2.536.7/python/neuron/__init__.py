...
POSSIBLE_HOME_PATH = [
    os.path.join(os.path.dirname(__file__), '../neuronhome'), # EPD install
    os.path.join(sys.prefix, 'neuronhome'),                   # VirtualEnv
    os.path.join(os.path.dirname(__file__), '../../neuronhome'), #
    os.path.join(os.path.dirname(__file__), '../../../neuronhome'), # 
    ]
...
for p in POSSIBLE_HOME_PATH:
    neuronhome = p
    if os.path.isdir(neuronhome):
        print "Found NEURON at", neuronhome
        break
try:
  os.putenv('NEURONHOME', neuronhome)
except:
    print "Setting NEURONHOME failed, set it manually to: " + neuronhome
os.environ['NEURONHOME']=neuronhome

# Add $NEURONHOME/bin to PYTHONPATH
import sys
bindir=os.path.abspath(os.path.join(neuronhome, 'bin'))
sys.path.append(bindir)

# Add $NEURONHOME/bin to PATH, this is needed for x64 as some dependent DLLs
# which are not python modules are located there.
newpath = bindir + ";" + os.environ['PATH']
try:
  os.putenv('PATH', newpath)
except:
    print "Setting PATH failed, you need to manually add: " + bindir
