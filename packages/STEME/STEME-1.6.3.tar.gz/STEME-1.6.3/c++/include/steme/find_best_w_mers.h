...
struct find_best_w_mers_storage_base {
    ...
    inline
    bool
    operator()( size_t W, int global_pos, bool rev_comp, double Z ) {
        ...
        w_mer_evaluation eval( Z, global_pos, rev_comp );
...
struct find_best_w_mers_storage_Z_sorted
{
    ...
    inline
    bool
    does_better_overlap( size_t W, int global_pos, double Z, it wmer ) const {
        BOOST_FOREACH(
            const w_mer_evaluation & better,
...
: find_best_w_mers_storage_base<
    find_best_w_mers_sorted_vector_storage,
    std::vector< w_mer_evaluation >
>,
find_best_w_mers_storage_Z_sorted<
    find_best_w_mers_sorted_vector_storage,
    std::vector< w_mer_evaluation >
>
{
    ...
    inline
    iterator
    insertion_point( w_mer_evaluation eval ) {
        return std::upper_bound( c.begin(), c.end(), eval, std::greater< w_mer_evaluation >() );
    }
    ...
    better_wmers( iterator wmer ) const {
        return boost::make_iterator_range( c.begin(), wmer );
    }
    ...
    inline
    double
    insert( iterator insertion_point, const w_mer_evaluation & eval ) {
        if( num_stored == max_to_store ) {
            c.resize( max_to_store - 1 );
            c.insert( insertion_point, eval );
        } else {
            c.insert( insertion_point, eval );
            ++num_stored;
        }
...
: find_best_w_mers_storage_base<
    find_best_w_mers_sorted_list_storage,
    std::list< w_mer_evaluation >
>,
find_best_w_mers_storage_Z_sorted<
    find_best_w_mers_sorted_list_storage,
    std::list< w_mer_evaluation >
...
: find_best_w_mers_storage_base<
    find_best_w_mers_set_storage,
    std::set< w_mer_evaluation, std::greater< w_mer_evaluation > >
>,
find_best_w_mers_storage_Z_sorted<
    find_best_w_mers_set_storage,
    std::set< w_mer_evaluation, std::greater< w_mer_evaluation > >
...
public:
    ...
    typedef std::vector< w_mer_evaluation > container;
...
public:
    ...
    bool
    operator()( size_t W, int global_pos, bool rev_comp, double Z ) {
        c.push_back( w_mer_evaluation( Z, global_pos, rev_comp ) );
        if( worst > Z ) {
            worst = Z;
        }
        return true;
    ...
    finalise( size_t W, size_t new_size ) {
        ...
        boost::timer t;
#endif // STEME_LOG_TIMING_INFO
        boost::sort( c, w_mer_evaluation::comp_score() );
#ifdef STEME_LOG_TIMING_INFO
        std::cout << "Sort took " << t.elapsed() << " seconds.\n";
        t.restart();
#endif // STEME_LOG_TIMING_INFO
        w_mer_evaluation::vec _c;
...
protected:
    ...
    struct comp_position
    {
        ...
        bool operator()( int global_pos, const w_mer_evaluation & eval ) const { return global_pos < eval.global_pos; }

        // compare an eval and a score
        bool operator()( const w_mer_evaluation & eval, int global_pos ) const { return eval.global_pos < global_pos; }
    };
    ...
    struct comp_score
    {
        ...
        bool operator()( double Z, const w_mer_evaluation & eval ) const { return Z < eval.Z; }

        // compare an eval and a score
        bool operator()( const w_mer_evaluation & eval, double Z ) const { return eval.Z < Z; }
    };
    ...
    typedef boost::multi_index_container<
        ...
        ::boost::multi_index::indexed_by<
            ::boost::multi_index::ordered_non_unique< score_tag   , ::boost::multi_index::member< w_mer_evaluation, double, &w_mer_evaluation::Z          > >, // w-mers by score
            ::boost::multi_index::ordered_non_unique< position_tag, ::boost::multi_index::member< w_mer_evaluation, int   , &w_mer_evaluation::global_pos > >  // w-mers by position
        >
    > mi_container;

    typedef mi_container::index< score    >::type   wmers_by_score;    ///< W-mers indexed by score.
    typedef mi_container::index< position >::type   wmers_by_position; ///< W-mers indexed by position.
    typedef wmers_by_position::iterator             score_iterator;    ///< Iterator by score.
    typedef wmers_by_position::iterator             position_iterator; ///< Iterator by position.
...
public:
    ...
    operator()( size_t W, int global_pos, bool rev_comp, double Z ) {
        ...
        if( ! does_better_overlap( W, global_pos, Z, insertion_point ) ) {
            sorted_by_position().insert( insertion_point, w_mer_evaluation( Z, global_pos, rev_comp ) );
            return true;
        }
...
protected:
    ...
    wmers_by_position &
    sorted_by_position() {
        return mi.get< position >();
    }

    /// Does a better one overlap?
    inline
    bool
    does_better_overlap( int W, int global_pos, double Z, position_iterator insert_point ) {
        // check those after
        position_iterator i = insert_point;
        while( sorted_by_position().end() != i && i->global_pos - global_pos < W ) {
            if( i->Z >= Z ) {
                return true;
            }
            ++i;
        ...
        if( sorted_by_position().begin() != insert_point ) {
            i = insert_point;
            --i;
            while( global_pos - i->global_pos < W ) {
                if( i->Z >= Z ) {
                    return true;
                }
...
protected:
public:
    typedef std::vector< w_mer_evaluation > container; ///< Container type that is part of the interface.

protected:
    typedef myrrh::store_n_best< container, w_mer_evaluation::comp_score > best_n_t; ///< Stores best W-mers.
...
public:
    ...
    bool
    operator()( size_t W, int global_pos, bool rev_comp, double Z ) {
        return best_n( w_mer_evaluation( Z, global_pos, rev_comp ) );
    }
    ...
    finalise( size_t W, size_t new_size ) {
        ...
        boost::timer t;
#endif // STEME_LOG_TIMING_INFO
        boost::sort( best_n._storage, w_mer_evaluation::comp_score() );
#ifdef STEME_LOG_TIMING_INFO
        std::cout << "Sort took " << t.elapsed() << " seconds.\n";
        t.restart();
#endif // STEME_LOG_TIMING_INFO
        erase_overlapping_from_score_sorted( best_n._storage, W, new_size );
#ifdef STEME_LOG_TIMING_INFO
...
template< find_best_w_mers_storage_enum Storage >
...
template<>
struct find_best_w_mers_storage_selector< store_all > {
    typedef find_best_w_mers_store_all type; ///< Type that holds best W-mers.
};


/// Specialisation to choose set storage for find best W-mers.
template<>
struct find_best_w_mers_storage_selector< store_in_set > {
    typedef find_best_w_mers_set_storage type; ///< Type that holds best W-mers.
};


/// Specialisation to choose sorted vector storage for find best W-mers.
template<>
struct find_best_w_mers_storage_selector< store_in_sorted_vector > {
    typedef find_best_w_mers_sorted_vector_storage type; ///< Type that holds best W-mers.
};


/// Specialisation to choose sorted list storage for find best W-mers.
template<>
struct find_best_w_mers_storage_selector< store_in_sorted_list > {
    typedef find_best_w_mers_sorted_list_storage type; ///< Type that holds best W-mers.
};
...
template<>
struct find_best_w_mers_storage_selector< store_using_multi_index > {
    typedef find_best_w_mers_multi_index_storage type; ///< Type that holds best W-mers.
};
...
: descender_preferred_order< find_best_w_mers< Storage, Spec >, Spec >
{
	...
	typedef Spec                                                          spec;              ///< Specifications.
	...
	typedef typename find_best_w_mers_storage_selector< Storage >::type   storage_policy;    ///< Storage policy type.
	...
	double                              lZtt;             ///< The log Z threshold term.
	...
	on_descent_begin() {
		base_t::stats.reset();
		worst_Z = lZtt = -std::numeric_limits< double >::max();
		storage.initialise();
	}


	/**
	 * \return The threshold on log(<Z>/(1-<Z>)).
	 */
	double
	get_log_Z_threshold_term() {
	...
	update_worst_Z( double new_worst_Z ) {
		if( new_worst_Z != worst_Z ) {
			worst_Z = new_worst_Z;
			lZtt = std::log( new_worst_Z / ( 1. - new_worst_Z ) );
		}
	}
