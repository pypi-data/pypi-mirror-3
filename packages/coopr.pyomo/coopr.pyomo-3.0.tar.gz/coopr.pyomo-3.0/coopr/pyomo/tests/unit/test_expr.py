...
class Expression_EvaluateVarValue(Expression_EvaluateNumericConstant):
    ...
    def create(self,val,domain):
        tmp=coopr.pyomo.base.var._VarValue("unknown",domain)
        tmp.value=val
        return tmp
...
class PrettyPrinter(pyutilib.th.TestCase):
    ...
    def test_prod(self):
        ...
        buf = StringIO.StringIO()
        expr.pprint(ostream=buf)
        self.assertEqual("0", buf.getvalue())
    ...
    def test_inequality(self):
        ...
        buf = StringIO.StringIO()
        expr.pprint(ostream=buf)
        self.assertEqual( "5  <=  a  <  10 \n", buf.getvalue() )

        expr = 5 <= model.a + 5
        buf = StringIO.StringIO()
        expr.pprint(ostream=buf)
        self.assertEqual( "5  <=  sum( 5 , a ) \n", buf.getvalue() )

        expr = expr < 10
        buf = StringIO.StringIO()
        expr.pprint(ostream=buf)
        self.assertEqual( "5  <=  sum( 5 , a )  <  10 \n", buf.getvalue() )
