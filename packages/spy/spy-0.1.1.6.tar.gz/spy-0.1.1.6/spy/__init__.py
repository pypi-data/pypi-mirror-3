...
def parse_status():
    def parse_file(file_name):
        try:
            return list(p.split(':') for p in open(file_name, 'r').read().strip().split('\n'))
        except:
            return None
    return [status for status in imap(parse_file, glob.glob(os.path.join(BASE, '*'))) if status is not None]
