...
${fullns(ns)}.Descriptor = {
    "url": ${rootns}_wsroot + "extdirect/router/${ns}",
    "namespace": "${fullns(ns)}",
    "type": "remoting",
...
def dict_fromjson(datatype, value):
    if value is None:
        return value
    key_type, value_type = datatype.items()[0]
    return dict((
        (fromjson(key_type, key), fromjson(value_type, value))
...
def dict_tojson(datatype, value):
    ...
    key_type, value_type = datatype.items()[0]
    return dict((
        (tojson(key_type, key), tojson(value_type, value))
        for key, value in value.items()
    ))
...
@fromjson.when_object(unicode)
def unicode_fromjson(datatype, value):
    ...
    def read_form_arguments(self, context):
        ...
        for argdef in context.funcdef.arguments:
            value = from_params(argdef.datatype,
                context.request.params, argdef.name)
