...
class Stache(object):
    ...
    def render_template_iter(self, template_name, data={}):
        return self._parse(iter(list(self.templates[template_name])), data)

    def render_template(self, template_name, data={}):
        return ''.join(self.render_template_iter(template_name, data))
    ...
    def _tokenize(self, template):
        ...
        while rest and len(rest) > 0:
            ...
            block_tag      = _checkprefix(taglabel, '<') if not partial_tag else None
            delim_tag      = re.match('=(.*?) (.*?)=', taglabel) if not block_tag else None
