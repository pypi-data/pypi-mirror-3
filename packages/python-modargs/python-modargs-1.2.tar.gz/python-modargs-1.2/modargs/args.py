...
S_BOOL = lambda x, token:  ['bool', token in ['True', 'true', 'yes']]
...
SCANNER = re.Scanner([
    ...
    (r"True|False|true|false|yes|no", S_BOOL), # matching single letters causes problems
    (r"[0-9]+", S_INT),
    (r"--", S_TRAILING),
    (r"[a-z\-]+", S_WORD),
    (r"\s", S_EMPTY),
...
def help_text(prog, mod, default_command=None, on=False):
    ...
    if not on:
        ...
        if command_help_text:
            for line in command_help_text.split("\n"):
                text.append(leading_spaces + line.strip())
