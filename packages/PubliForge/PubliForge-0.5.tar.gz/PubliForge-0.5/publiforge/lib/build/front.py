# $Id: front.py 87b7d52fda37 2012/02/23 17:08:44 patrick $
...
class FrontBuild(object):
    ...
    def complete(self, request, key):
        ...
        # Update request
        user = DBSession.query(User.name, User.lang)\
               .filter_by(user_id=int(self.result['user_id'])).first()
        if user is None:
            self.stopped(_('unknown user'))
            return False
        request.session['user_id'] = self.result['user_id']
        request.session['name'] = user[0]
        request.session['lang'] = user[1]
        DBSession.close()
   ...
   # -------------------------------------------------------------------------
    def _storage4output(self, request):
        ...
        if not has_permission(request, 'stg_manager') \
                ...
                and (not user or user.perm == 'viewer'):
