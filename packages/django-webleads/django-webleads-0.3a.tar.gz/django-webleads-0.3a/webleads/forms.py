...
class LeadActionForm(forms.Form):
    ...
    def _send_mail(self, comment, trans_msg):
        if isinstance(settings.MARKETING_EMAIL, basestring):
...
class BrochureRequestForm(LeadActionForm):
    ...
    def __init__(self, request, product_key, data=None, brochure_url=None, *args, **kwargs):
        super(BrochureRequestForm, self).__init__(request, product_key,
                                                  data=data, *args, **kwargs)
        if brochure_url:
            if brochure_url.startswith('/'):
                brochure_url = 'http://%(site)s%(path_url)s' % {'site': Site.objects.get_current().domain,
                                                                'path_url': brochure_url}
        else:
            brochure_url = 'http://%(site)s%(media)s/products/%(product)s.pdf' % {'site': Site.objects.get_current().domain,
    ...
    def save_action(self):
        ...
        self._customer_mail(ugettext('You request about %(product)s, we will contact you back as soon as possible. ') + self.brochure_url)
