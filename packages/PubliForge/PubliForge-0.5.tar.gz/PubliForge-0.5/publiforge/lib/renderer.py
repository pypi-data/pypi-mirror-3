# $Id: renderer.py 9ddfe78f66e6 2012/03/01 18:26:03 patrick $
# -*- coding: utf-8 -*-
"""Renderer functions."""
...
class Menu(object):
    ...
    def xhtml(self):
        ...
        # Storages
        if has_permission(self._request, 'stg_viewer'):
        ...
        # Projects
        if has_permission(self._request, 'prj_viewer'):
        ...
        if has_permission(self._request, 'usr_modifier'):
            submenu.append(self._entry(_('Users'), 'user_admin'))
        if has_permission(self._request, 'grp_modifier'):
            submenu.append(self._entry(_('Groups'), 'group_admin'))
        if has_permission(self._request, 'stg_modifier'):
            submenu.append(self._entry(_('Storages'), 'storage_admin'))
        if has_permission(self._request, 'prj_modifier'):
    ...
    def _storage_entries(self, user_id, submenu):
        ...
        for storage in DBSession.query(Storage)\
                ...
                .filter(StorageUser.in_menu == True)\
                .filter(Storage.storage_id == StorageUser.storage_id)\
                .order_by(Storage.label):
    ...
    def _project_entries(self, user_id, submenu):
        ...
        for project in DBSession.query(
                ...
                .filter(ProjectUser.in_menu == True)\
...
class Paging(paginate.Page):
    ...
    def __init__(self, request, paging_id, collection, **kwargs):
        ...
        # Return page
        super(Paging, self).__init__(collection, params['page'],
            params['size'], url=paginate.PageURL_WebOb(request), **kwargs)
