...
class ChessproblemLatexParserTest(unittest.TestCase):
    ...
    def test_diagramWithSingleKing(self):
        ...
        field = board.fields[4][3]
        self.assertEqual(PIECE_TYPE_KING, field.piece_type)
        self.assertEqual(PIECE_COLOR_WHITE, field.piece_color)
        self.assertEqual(PIECE_ROTATION_NORMAL, field.piece_rotation)
    ...
    def test_diagramWithEquihopperAndImitator(self):
        ...
        field = board.fields[0][1]
        self.assertEqual(PIECE_TYPE_EQUIHOPPER, field.piece_type)
        self.assertEqual(PIECE_COLOR_BLACK, field.piece_color)
        self.assertEqual(PIECE_ROTATION_LEFT, field.piece_rotation)
        field = board.fields[1][0]
        self.assertEqual(PIECE_TYPE_CIRCLE, field.piece_type)
        self.assertEqual(PIECE_COLOR_NEUTRAL, field.piece_color)
        self.assertEqual(PIECE_ROTATION_NORMAL, field.piece_rotation)
    ...
    def test_diagramWithMultiplePieces(self):
        ...
        self.assertEqual(PIECE_COLOR_WHITE, field.piece_color)
        self.assertEqual(PIECE_ROTATION_NORMAL, field.piece_rotation)
        field = board.fields[6][2]
        self.assertEqual(PIECE_TYPE_QUEEN, field.piece_type)
        self.assertEqual(PIECE_COLOR_BLACK, field.piece_color)
        self.assertEqual(PIECE_ROTATION_LEFT, field.piece_rotation)
        field = board.fields[2][7]
        self.assertEqual(PIECE_TYPE_BISHOP, field.piece_type)
        self.assertEqual(PIECE_COLOR_NEUTRAL, field.piece_color)
        self.assertEqual(PIECE_ROTATION_UPSIDEDOWN, field.piece_rotation)
        field = board.fields[3][7]
        self.assertEqual(PIECE_TYPE_BISHOP, field.piece_type)
        self.assertEqual(PIECE_COLOR_NEUTRAL, field.piece_color)
        self.assertEqual(PIECE_ROTATION_UPSIDEDOWN, field.piece_rotation)
        field = board.fields[4][7]
        self.assertEqual(PIECE_TYPE_BISHOP, field.piece_type)
        self.assertEqual(PIECE_COLOR_NEUTRAL, field.piece_color)
        self.assertEqual(PIECE_ROTATION_UPSIDEDOWN, field.piece_rotation)
        field = board.fields[0][0]
        self.assertEqual(PIECE_TYPE_CIRCLE, field.piece_type)
        self.assertEqual(PIECE_COLOR_BLACK, field.piece_color)
        self.assertEqual(PIECE_ROTATION_NORMAL, field.piece_rotation)
    ...
    def test_diagramWithConditions(self):
        ...
        self.assertEqual('Circe', problem.condition[0])
        self.assertEqual('Madrasi', problem.condition[1])
        document = self.parser.parse_latex_str('\\begin{diagram}\n\\condition{Circe; Madrasi}\n\\end{diagram}')
        self.assertEqual(1, document.get_problem_count())
        problem = document.get_problem(0)
        self.assertEqual(2, len(problem.condition))
        self.assertEqual('Circe', problem.condition[0])
    ...
    def test_diagramWithNofields(self):
        ...
        self.assertEqual(1, document.get_problem_count())
        problem = document.get_problem(0)
        self.assertEqual('f7, c4, g5', problem.nofields)
        document = self.parser.parse_latex_str('\\begin{diagram}\n\\nosquares{f3, c5, g8}\\end{diagram}')
        self.assertEqual(1, document.get_problem_count())
        problem = document.get_problem(0)
        self.assertEqual('f3, c5, g8', problem.nofields)
    ...
    def test_diagramWithFieldframe(self):
        ...
        self.assertEqual(1, document.get_problem_count())
        problem = document.get_problem(0)
        self.assertEqual('f2, d4, h5', problem.fieldframe)
    ...
    def test_diagramWithVerticalcylinder(self):
        logger.debug('test_diagramWithVerticalcylinder')
        document = self.parser.parse_latex_str('\\begin{diagram}\n\\verticalcylinder\\end{diagram}')
        self.assertEqual(1, document.get_problem_count())
        problem = document.get_problem(0)
        self.assertEqual(True, problem.verticalcylinder)

    def test_diagramWithHorizontalcylinder(self):
        logger.debug('test_diagramWithHorizontalcylinder')
        document = self.parser.parse_latex_str('\\begin{diagram}\n\\horizontalcylinder\\end{diagram}')
        self.assertEqual(1, document.get_problem_count())
        problem = document.get_problem(0)
        self.assertEqual(True, problem.horizontalcylinder)

    def test_diagramWithGridchess(self):
        logger.debug('test_diagramWithGridchess')
        document = self.parser.parse_latex_str('\\begin{diagram}\n\\gridchess\\end{diagram}')
        self.assertEqual(1, document.get_problem_count())
        problem = document.get_problem(0)
        self.assertEqual(True, problem.gridchess)

    def test_diagramWithNoframe(self):
        logger.debug('test_diagramWithNoframe')
        document = self.parser.parse_latex_str('\\begin{diagram}\n\\noframe\\end{diagram}')
        self.assertEqual(1, document.get_problem_count())
        problem = document.get_problem(0)
        self.assertEqual(True, problem.noframe)
    ...
    def test_BigDiagram(self):
        ...
        self.assertEqual('\n', problem.after_command_text['{diagram}'])
        self.assertTrue(problem.board.fields[4][8] != None)
        self.assertTrue(problem.board.fields[9][7] != None)
