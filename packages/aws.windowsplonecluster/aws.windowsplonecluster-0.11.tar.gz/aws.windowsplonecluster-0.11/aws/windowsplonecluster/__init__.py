...
class myURLOpener(urllib.FancyURLopener):
    ...
    follow_redirection = True ## by default follow_redirection
    use_proxies = True ## by default use environ for proxy

    def http_error_302(self, url, fp, errcode, errmsg, headers, data=None):
        """Error 302 -- relocated (temporarily)."""
        if self.follow_redirection is True:
            return urllib.FancyURLopener.http_error_302(self, url,
                                                        fp, errcode,
                                                        errmsg, headers, data)
        else:
    ...
    def open( self, fullurl, data=None, timeout = None):
        t = time.time()
        if self.use_proxies is False:
             ### erase all config about that
             self.proxies = dict()
...
def main():
    ...
    if config.has_option('DEFAULT','follow_redirection'):
        ## in some case we dont want follow redirection
        if  config.get('DEFAULT','follow_redirection') in FALSE_VALUE:
             myURLOpener.follow_redirection = False
    if config.has_option('connect','use_http_proxy') and\
            config.get('connect','use_http_proxy') in FALSE_VALUE:
        myURLOpener.use_proxies = False
