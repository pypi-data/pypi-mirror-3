...
from secobj.principal    import AUTHENTICATED, EVERYONE, OWNER, SUBJECT_NAME
...
class Rule(object):
    ...
    def __init__(self, action, principal, *permissions):
        ...
        elif principal in self._BUILTIN:
            self._principalname  = principal
            self._principal      = self._BUILTIN[principal]
        elif SUBJECT_NAME.match(principal) is not None:
            self._principalname  = principal
