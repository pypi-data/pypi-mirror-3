...
try:
    ...
    sys.exit('Error initializing twisted.internet.reactor in dhnpacket.py')
...
class dhnpacket:
#    PacketID - string of the above 4 "Number"s with "-" seperator to uniquely identify a packet
#                     on the local machine.  Can be used for filenames, and to prevent duplicates.
    def __init__(self, Command, OwnerID, CreatorID, PacketID, Payload, RemoteID,):# SignInThread=False):
        ...
        self.OwnerID = OwnerID               # who owns this data and pays bills - http://cate.com/id1.xml
        self.CreatorID = CreatorID           # signer - http://cate.com/id1.xml - might be an authorized scrubber
        self.PacketID = PacketID             # functions for making and reading parts of this
        self.Date = datetime.datetime.now().strftime("%Y/%m/%d %I:%M:%S %p")
        self.Payload = Payload               #  main body of binary data
        self.RemoteID = RemoteID             #  want full IDURL for other party so troublemaker could not
...
# if n = -1 then localID all the way, else supplier N
def testpacket():
    OwnerID = misc.getLocalID()
    CreatorID = OwnerID
    PacketID = "xyzzyaa"
    Payload = "This is a short test\nabsdfs\ndfhvcnrtrh\ndaaaaaaaaa"
    RemoteID = 'http://identity.datahaven.net/dhncentral.xml'
    return dhnpacket(commands.Ack(), OwnerID, CreatorID, PacketID, Payload, RemoteID, False)

def testpacketwithdefer():
    import random
    OwnerID = misc.getLocalID()
    CreatorID = OwnerID
    PacketID = packetid.UniqueID()
    Payload = open('profile.report', 'r').read()
    for i in range(random.randrange(2, 7)):
        Payload += Payload
    RemoteID = contacts.getSupplierID(0)
    p = dhnpacket(commands.Data(), OwnerID, CreatorID, PacketID, Payload, RemoteID, True)
    return p

def testsigning1():
    import sys
    import time
    print 'preparing 64 Ack packets:'
    tstart = time.time()
    loops = 64
    l = []
    ...
    print '\ntotal each: ', totaltime/loops , 'seconds'
    print 'test all packets is Valid:'
    tstart = time.time()
    for p in l:
        v = p.Valid()
        if not v:
...
def testsigning2():
    ...
    import time
    commands.init()
    print 'preparing 1Mb garbage data:'
    d1kb = ''
    for i in range(128):
        d1kb += '01234567'
    d1mb = ''
    for i in range(1024):
        d1mb += d1kb
    print 'test size: ', len(d1mb)
    print 'preparing 64 Data packets with 1Mb size:'
    l = []
    tstart = time.time()
    for i in range(64):
        MyID = misc.getLocalID()
...
def testsigning3():
    ...
    import random
    import string
    print 'preparing 64 "random" Ack packets:'
    loops = 64
    sl = []
    for i in range(loops):
        s = string.join(random.sample(string.ascii_letters,  16), '')
        sl.append(s)
    tstart = time.time()
    l = []
    for i in range(loops):
