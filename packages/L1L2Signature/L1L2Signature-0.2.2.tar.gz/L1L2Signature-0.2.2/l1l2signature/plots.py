...
def heatmap(submatrix, labels, sample_names=None, var_names=None,
            ...
            var_preorder=None, fig_num=None):
    ...
    clustering_metric : str, optional (default 'euclidean')
        ...
        See :func:`scipy.cluster.hierarchy.linkage` function.
    var_preorder : :class:`numpy.ndarray` like, optional (default None)
        If given, variables are not clustered but given indexes are used to
        order them.
    fig_num : int, optional
        Figure Number. If not given a new figure is initialized.
    ...
    # Ordering Samples by hierarchical clustering
    if scipy_found:
    ...
    if var_preorder is None:
        ...
        var_preorder = np.asanyarray(var_preorder)
        heatmap = heatmap[var_preorder]
        if not var_names is None:
            var_names = var_names[var_preorder]
