...
from collective.contentlicensing.browser.copyrightbyline import CopyrightBylineView as DefaultCopyrightBylineView
...
class PositionView(BrowserView):
    ...
    def __call__(self):
        ...
        if position == 'up':
            self.moveUp(context, self.request, position, id, url)
        elif position == 'down':
            self.moveDown(context, self.request, position, id, url)
        else:
            return


    def moveUp(self, context, request, position, id, path):
        ...
        request.response.redirect('order_courseobjs')
                

    def moveDown(self, context, request, position, id,path):
        objs = self.getNavObjects(context)
    ...
    def swapPosition(self, cur_obj, swp_obj):
        if ICourseOrderable.providedBy(cur_obj):
            cobj = ICourseOrder(cur_obj)
            if ICourseOrderable.providedBy(swp_obj):
                sobj = ICourseOrder(swp_obj)
                if cobj.position == sobj.position:
                    # Objects somehow have the same position
                    # Force the current object down
                    cobj.position += 1
                else:
                    tmp = cobj.position
                    cobj.position = sobj.position
                    sobj.position = tmp
