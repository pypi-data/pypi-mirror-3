...
def smarts(atm={}):
    ...
    def runner(atm={}):
        myatm.update(atm)
        with _rtm.Working() as working:
            working.link(resources, relative_path=resource_path)
            working.write(input_file, cardify(translate(myatm)))
            working.run('%s > %s' % (command, output_log))
            smout = working.get(output_file)
            raw = numpy.genfromtxt(smout, skip_header=header_lines)
