...
def cull_constraints_from_instance(model):
    ...
    # TBD: ADD OTHER CONSTRAINT TYPES, INCLUDING SOS AND CONSTRAINT LISTS.
...
def preprocess_scenario_instance(scenario_instance, instance_variables_fixed, \
    ...
    from coopr.pyomo.preprocess.compute_canonical_repn import preprocess_objectives, preprocess_constraints, preprocess_constraint
    ...
    if (instance_objective_modified is True):
       ...
       if (solver_type == "asl") or (solver_type == "ipopt"):
          ...
          preprocess_objectives(scenario_instance, None)
    ...
    if (instance_variables_fixed is True):
        ...
        if (solver_type == "asl") or (solver_type == "ipopt"):
            ...
            for constraint_name in instance_ph_constraints:
                preprocess_constraint(scenario_instance, getattr(scenario_instance, constraint_name), var_id_map)
