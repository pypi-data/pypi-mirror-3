...
class MongoItemsData(zope.location.location.Location):
    ...
    def order(self, orderNames):
        ...
        newOrder = [self.data.pop(name) for name in orderNames]
        if len(self.data) > 0:
    ...
    def append(self, item):
        """Add a new item"""
        if item.__name__ not in self._order:
...
class MongoListData(zope.location.location.Location):
    ...
    def order(self, orderedNames):
        """Order items by the given index order."""
        newOrder = [self.data.pop(idx) for idx in orderedNames]
        if len(self.data) > 0:
            raise ValueError("Not all names used for order items")
        self.data = newOrder
        # mark as changed
