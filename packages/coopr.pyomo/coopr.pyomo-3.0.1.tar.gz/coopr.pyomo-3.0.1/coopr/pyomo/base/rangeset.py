    ...
    def __iter__(self):
        if self.filter is None and self.validate is None:
            ...
            for i in xrange(int((self._end_val-self._start_val+self._step_val+1e-7)//self._step_val)):
                ...
                if not self.filter is None and not apply_indexed_rule(self, self.filter, self.model(), val):
                    continue
                if not self.validate is None and not apply_indexed_rule(self, self.validate, self.model(), val):
                    continue
                yield val
