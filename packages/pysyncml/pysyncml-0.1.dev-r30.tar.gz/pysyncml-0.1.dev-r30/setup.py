#!/usr/bin/env python
#------------------------------------------------------------------------------
# file: $Id: setup.py 30 2012-07-01 12:31:47Z griff1n $
# lib:  pysyncml
...
if sys.hexversion < 0x02060600:
  raise RuntimeError('This package requires python 2.6.6 or later')
...
setup(
  ...
  url                   = 'http://pypi.python.org/pypi/pysyncml/',
  # these did NOT work:
  ...
  # tbd: pull in a changelog from subversion?...
  long_description      = open(os.path.join(os.path.dirname(__file__), 'README.txt'), 'rb').read()

)

#------------------------------------------------------------------------------
# end of $Id: setup.py 30 2012-07-01 12:31:47Z griff1n $
#------------------------------------------------------------------------------
