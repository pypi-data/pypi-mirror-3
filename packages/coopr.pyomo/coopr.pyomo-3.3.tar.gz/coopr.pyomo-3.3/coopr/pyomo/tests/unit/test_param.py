...
class ParamTester(object):
    ...
    def validateDict(self, ref, test):
        ...
        for key in test.keys():
            ...
            if ref[key] is None:
                ...
                self.assertEquals( ref[key], value( test[key] ) )
        ...
        try:
            ...
            if self.expectNegativeDomainError:
                self.fail("Expected setitem[%s] to fail with negative data"
                          % (idx,))
            self.assertEquals( self.instance.A[idx], -4.3 )
        except ValueError:
        ...
        try:
            ...
            if self.expectTextDomainError:
                self.fail("Expected setitem[%s] to fail with text data",
                          (idx,))
            self.assertEquals( value(self.instance.A[idx]), 'x' )
        except ValueError:
    ...
    def test_values(self):
        expectException = len(self.sparse_data) < len(self.data) and self.instance.A._default_val is None
        try:
    ...
    def test_iterkeys(self):
        ...
        self.assertTrue('next' in dir(test))
        self.assertEqual( sorted(test), sorted(self.data.keys()) )
    ...
    def test_len(self):
        ...
        self.assertEqual( len(self.instance.A.sparse_keys()),
                          len(self.sparse_data) )

    def test_index(self):
        #"""Check the use of index"""
        self.assertEqual( len(self.instance.A.index()),
                          len(self.instance.A.keys()) )
        self.assertEqual( len(self.instance.A.index()),
                          len(self.data.keys()) )
