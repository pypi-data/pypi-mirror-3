...
# -*- coding: utf-8 -*-
#
# Calculates (U-V)z slopes, scatters and intercepts of a given color--magnitude relation.
# Follows the procedure in Appendix II of Mei et al. 2009 (ApJ, 690, 42), except here we convert
...
BOOTSTRAPS=1000 # number of bootstrap samples, for estimating errors
NGAL=25         # number of galaxies, gets * number of models, having same age in each metallicity bin
FILTER_DIR="../../../../testingData/filters/"

# Map between short filter names on command line and paths, labels etc..
filterMap=[]
filterMap.append({'shortName': 'r625', 'filePath': FILTER_DIR+'F625W_WFC.res', 'plotLabel': 'r625'})
filterMap.append({'shortName': 'i775', 'filePath': FILTER_DIR+'F775W_WFC.res', 'plotLabel': 'i775'})
filterMap.append({'shortName': 'z850', 'filePath': FILTER_DIR+'F850LP_WFC.res', 'plotLabel': 'z850'})
filterMap.append({'shortName': 'U', 'filePath': FILTER_DIR+'U_Johnson.res', 'plotLabel': 'U'})
filterMap.append({'shortName': 'V', 'filePath': FILTER_DIR+'V_Johnson.res', 'plotLabel': 'V'})
...
def GetCMR(nameFragment, dictList):
    """Finds the CMR dictionary in the litCMRs or results list, by looking for nameFragment in name.
...
def CalcTransformedCMRWithZM(obsCMR, fitMags, fitCols):
    """Calculates the CMR transformed to the rest frame, using the results of the magnitude and colour
    conversion fits. See handwritten notes for the tedious algebra involved.
    
    """
    
    # we use the following notation s, zp for slope, zeropoint, append Err for errors
    ...
    e=d/a
    f=sCol*e
    g=zpCol+f+(b*transformedZeroMag)    # this last term is if we want to transform to e.g. match Mei et al.
...
def BootstrapTransformedCMRErrorsWithZM(obsCMR, fitMags, fitCols):
    """Estimates errors on transformed CMR fit (i.e., into rest frame), by assuming the errors on
    the observed CMR fit, colour transformation fit, and mag. transformation fit have Gaussian
    distributions.
...
def LoadModels(fileNameList, modelType = "bc03"):
    """Creates a list of stellar population models from the given list of model file names.
...
def GetPassbandFileNames(inputColour):
    """Given a mag (e.g. i775) or colour string e.g. r625-z850, lookup the appropriate file name(s) in the 
    filterMap, and return the paths in a list.
...
def LoadPassbands(fileNameList, redshift = None, redshiftPassbands = False):
    """Creates a list of passband objects from the given list of passband file names.
...
def CalcColourMagTransformation(cmr, restColPassbands, restMagPassband):
    """Calculates the transformation equations needed to convert the given cmr into the rest frame at Coma,
    for the given passbands.
    
    """
    
    print ">>> Calculating colour, mag transform for CMR "+litCMR['name']+" ..."
    ...
    # Generate galaxy models, we'll hold them all in memory here and use them all in a bit
    ...
    for n in range(NGAL):
        ...
        for i in range(len(models)):
            ...
            observedGalaxies.append(models[modelChoice].getSED(ageChoice, z=zCluster))
    ...
    for o, r in zip(observedGalaxies, restGalaxies):
        ...
        restColours.append(r.calcColour(restColPassbands[0], restColPassbands[1], magType="Vega"))
        observedColours.append(o.calcColour(observedColPassbands[0], observedColPassbands[1], magType=cmr['magType']))
    ...
    scatter=astStats.biweightScale(res, 6.0)    # scatter of residuals, use as fit error
    
    # Fit for mag conversion
    restMinusObservedAppMags=[]
    for o, r, obsCol, restCol in zip(observedGalaxies, restGalaxies, observedColours, restColours):

        restMinusObservedAppMags.append(r.calcMag(restMagPassband, magType="Vega")-o.calcMag(observedMagPassband, magType=cmr['magType']))
...
def GetCSPModel(labelToFind, models, modelLabels):
    """Given a list of models and a matching list of labels, returns the model matching the given label.
...
def ApplyCMRTransformation(cmr, trans):
    """Applies the magnitude and colour transformations stored in trans to the CMR.
    
    """
    
    print ">>> Transforming CMR "+cmr['name']+" to Coma rest frame "+restColour+" ..."
    ...
    transformedCMRErrs=BootstrapTransformedCMRErrorsWithZM(cmr, fitMags, fitCols)
...
restColPassbandFileNames=[FILTER_DIR+"U_Johnson.res", FILTER_DIR+"B_Johnson.res"]
...
transformations=[]
for litCMR in litCMRs:  
    trans=CalcColourMagTransformation(litCMR, restColPassbands, restMagPassband)
...
outFile=file("output_CMRRestConversion.txt", "w")
...
keyOrder=["name", "redshift", "colour", "mag", "zeroMag", "slope", "slopeErr", "intercept", 
          "interceptErr", "scatter", "scatterErr"]
for r in results:
    for k in keyOrder:
        for key in r.keys():
            if str(key) == k:
