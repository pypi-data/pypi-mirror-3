...
class Mock(object):
    ...
    def verify(self, matcher):
        ...
        message indicating that no call has been invoked from it up to now :
            >>> otherMethod.verify(thatCall('2ndCall')).invokedInOrder()
            Traceback (most recent call last):
            ...
        ...
        other examples :
            >>> method.verify(thatCall('1stCall')).invokedInOrder()
            ...
            >>> method.verify(thatCall('3rdCall')).invokedInOrder()
            Traceback (most recent call last):
            ...
            ...
            >>> anotherMethod.verify(thatCall('3rdCall')).invokedInOrder()
            Traceback (most recent call last):
            ...
...
class AutoMocks(object):
    ...
    To use it, you first need to put this decorator in front of your function :
        >>> from mocki.expectations import once
        >>> from mocki.matchers import anyCall

        >>> @AutoMocks()
        ... def testSomething(a, b, c, oneMock, anotherMock, x, y, z):
        ...     anotherMock.verify(anyCall()).invoked(once())

    Now, all arguments given to this function whose names ends with 'mock' will
    automatically get new mocks on invocation :
        >>> testSomething(1, 2, 3, x=7, y=8, z=9)
        Traceback (most recent call last):
        ...
        AssertionError: No call invoked from anotherMock up to now.

    And if you follow another naming convention for mocks, e.g. with names that
    start with 'mock' instead of ending with it, you can replace the default
    argument name matcher with your own one :
        >>> @AutoMocks(lambda argumentName: argumentName.lower().startswith('mock'))
        ... def testSomething(a, b, c, mockOne, mockAnother, x, y, z):
        ...     mockAnother.verify(anyCall()).invoked(once())

        >>> testSomething(1, 2, 3, x=7, y=8, z=9)
        Traceback (most recent call last):
        ...
        AssertionError: No call invoked from mockAnother up to now.

    """
    def __init__(self, argumentNameMatcher=lambda argumentName: argumentName.lower().endswith('mock')):
        self.argumentNameMatcher = argumentNameMatcher

    def __call__(self, func):
        def decoratedFunc(*args, **kwargs):
            modifiedKwargs = dict(kwargs)

            for argumentName in inspect.getargspec(func).args:
                if self.argumentNameMatcher(argumentName):
                    modifiedKwargs[argumentName] = Mock(argumentName)

            func(*args, **modifiedKwargs)
