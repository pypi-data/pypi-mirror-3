...
class ShardFileStorage(object):
    ...
    def write_to_file(self, index, data):
        if self.output_files[index] is None:
            filename = self.output_dir + os.path.sep + "%d.csv" % index
    ...
    def do_scatter(self):
        ...
        for (filename, connection) in zip(
            ...
            target_path = os.path.join(
                self.output_name,
                config.HOSTNAME+'_'+os.path.basename(filename)
            )
            print "%s => %s" % (filename, connection.host)
            network.send_file(connection, source_path, target_path)

    def do_sort(self, unique=False):
        inputs = self.input_files()
        output = os.path.join(self.output_dir, 'sorted.csv')
        process.sort(inputs, output, unique=unique)
    
    def do_sort_unique(self):
        self.do_sort(unique=True)
    ...
    def do_map(self, mapper):
        ...
        for line in self:
            try:
                ...
                raise ValueError("no tab character in %r" % line)
...
def run(command, *args):
    ...
    if command in ('scatter', 'sort', 'sort_unique', 'delete'):
        ...
        [source, target] = args
        if command == 'delete':
            storage = ShardFileStorage(source, target, makedirs=False)
        else:
            storage = ShardFileStorage(source, target, makedirs=True)
