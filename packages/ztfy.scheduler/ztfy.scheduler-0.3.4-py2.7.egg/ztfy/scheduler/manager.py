...
class Scheduler(Folder, SiteManagerContainer):
    ...
    __roles__ = ('ztfy.SchedulerManager', 'ztfy.SchedulerOperator')
...
def handleRegisteredScheduler(scheduler, event):
    scheduler.start()


@adapter(IScheduler, IUnregistered)
def handleUnregisteredScheduler(scheduler, event):
    scheduler.stop()
