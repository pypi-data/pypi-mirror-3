...
def run(Tester):
    ...
    #cmdoutfile = cmdLineQuote(settings.LocalTesterLogFilename())
    #cmderrfile = cmdoutfile.replace('.log', 'err.log')
    ...
    #fout = open(cmdoutfile, 'w')
    #ferr = open(cmderrfile, 'w')

    #cmd = string.join(cmdargs, ' ')
    #cmd += ' 1>%s 2>>%s ' % (cmdoutfile, cmderrfile)
...
##        del _CurrentProcess
    try:
        ...
        err_rep = 'unknown error'
        if _CurrentProcess:
            try:
                err_rep = _CurrentProcess.err_report
            except:
                pass
        dhnio.Dprint(1, 'localtester.run ERROR executing: %s, report: %s' % (str(cmd), err_rep))
