...
- It is possible to set a timeout while listening for requests
  ...
  *client*, talking to the server through a socket. (And if you
  ...
  plib.stdlib.io.async sub-package.)
...
from plib.stdlib.io import SelfPipeServerMixin
...
class SocketServer(SelfPipeServerMixin, BaseServer, SocketIOBase):
    ...
    def server_loop(self, callback=None):
        # We assume that self.terminate_pipe exists here, since
        # SelfPipeServerMixin wraps the serve_forever method that
        # calls this one
        ...
        while _keep_running():
            try:
                term_fd = self.terminate_pipe.fileno()
                sock_fd = self.socket.fileno()
                r, w, e = select.select([term_fd, sock_fd],
                    [], [], self.poll_timeout)
                if term_fd in r:
                    self.terminate_pipe.read_pipe()
                elif sock_fd in r:
