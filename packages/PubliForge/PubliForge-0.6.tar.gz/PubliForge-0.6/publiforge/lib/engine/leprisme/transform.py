# $Id: transform.py 1b8b69120590 2012/05/29 23:17:07 patrick $
"""XML Transformation via XSL stylesheet."""

from os import walk, remove
from os.path import exists, join, dirname, relpath, commonprefix
...
class Transform(object):
    ...
    def start(self, filename, fid, data):
        ...
        self.fid = fid
        self.data = data
        self.output = self._initialize(filename)
    ...
    def _regex_transform(self, kind):
        ...
        if not isinstance(self.data, basestring):
            self.data = etree.tostring(self.data, encoding='utf-8',
    ...
    def _xsl_transform(self, variables):
        ...
        self.data = (self.data.getroot() is not None
                     and etree.ElementTree(self.data.getroot())) \
                     or str(self.data)

        # Clean xmlns=""
        if not isinstance(self.data, basestring):
            self.data = etree.tostring(self.data, encoding='utf-8',
                pretty_print=True, xml_declaration=self._xml_decl)
    ...
    def _save_data(self, fmt):
        ...
        # Save XML/HTML file
            ...
            try:
                self.data.write(filename, encoding='utf-8',
                    xml_declaration=self._xml_decl, pretty_print=True)
