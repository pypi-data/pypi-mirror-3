...
class eccmap:
    ...
    def check(self):
        print "paritysegments =", self.paritysegments
        print "datasegments =", self.datasegments
        print "Doing range sanity check"
        for map in self.DataToParity:
            for parity in map:
                if (parity>self.paritysegments):
                    print "error parity "
                    print  parity
        for map in self.ParityToData:
            for data in map:
                if (data>self.datasegments):
                    print "error data "
                    print data

    def FixableNode(self, NodeMap):              # case where each node has one parity and one data so same missing
