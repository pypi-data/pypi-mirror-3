...
def mtm(x, NW=None, k=None, NFFT=None, e=None, v=None, method='adapt', show=True):
...
class pmtm(FourierSpectrum):
    """The MTM provides an interface to :func:`mtm`.
    ...
    .. plot::
        ...
        from spectrum import *
        p = pmtm(data_cosine(N=1024))
        p()
        p.plot()
        p.plot(sides='twosided')
    
    """
    def __init__(self, data, NW=2.5, k=3, NFFT=None, method='adapt', sampling=1.):
        ...
        """
        super(pmtm, self).__init__(data, NFFT=NFFT, sampling=sampling)
        self.method = method
        self.NW = NW
        self.k = k

    def __call__(self):
        psd = mtm(self.data, NW=self.NW, k=self.k, method=self.method, NFFT=self.NFFT, show=False)
    ...
    def _str_title(self):
        return "Multi tapering estimate\n"
    
    def __str__(self):
        return super(pmtm, self).__str__()
