// $Id: base.js c9b32baf8fa9 2012/07/28 21:07:17 patrick $

// State
var idx = {"closed": 0, "tabUser": 1, "tabGroup": 2, "tabStorage": 3,
           "tabProject": 4, "tabRole": 5, "tabProcessing": 6, "tabTask": 7,
           "tabPack": 8}
var state = Cookie.read("PF_STATE");
if (!state) { state = "false|0|0|0|0|0|0|0|0"; }
state = state.split('|');
...
window.addEvent("domready", function() {
    ...
    if (flash) {
        flash = new Fx.Slide(flash, {duration: 'long'});
        flash.hide().slideIn();
        (function(){flash.slideOut();}).delay(12000);
    }

    // Action parameters
    var actionParams = $("actionParams");
    if (actionParams) {
        (new Fx.Slide(actionParams, {duration: 'long'})).hide().slideIn();
    }

    // ToolTip
    var toolTip = $("toolTip");
    if (toolTip) {
        toolTip = new Fx.Slide(toolTip, {duration: 'long'});
