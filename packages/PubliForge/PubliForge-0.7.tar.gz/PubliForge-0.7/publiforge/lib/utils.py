# $Id: utils.py e2c913b1a1e3 2012/09/09 19:47:40 patrick $
# -*- coding: utf-8 -*-
...
MIME_TYPES = (
    'css', 'csv', 'epub+zip', 'folder', 'font-woff', 'html', 'javascript',
    'jpeg', 'mpeg', 'msword', 'ogg', 'pdf', 'plain', 'png', 'postscript',
    'relaxng', 'tiff', 'vnd.ms-excel', 'vnd.ms-opentype',
...
TEXT_TYPES = (
    'css', 'csv', 'html', 'plain', 'relaxng', 'x-python', 'xml', 'xml-dtd')
...
# =============================================================================
def copy_content(src_dir, dst_dir, exclude=()):
...
# =============================================================================
def make_id(name, mode=None):
    """Make an ID with name.

    :param name: (string)
        Name to use.
    :param mode: ('standard' or 'token', optional)
    :return: (string)
    """
    token = name.strip().lower()
    if mode in ('standard', 'token'):
        token = re.sub(r'[*?"\'/]', '_', u'_'.join(token.split()))
    if mode == 'token':
        token = normalize('NFKD', token)
...
def execute(command, cwd=None, no_exit_code=False):
    ...
    If the return code is not zero, return output and and error message.
    ...
    :return: (tuple)
        A string (output) and a :class:`pyramid.i18n.TranslationString`
        (error message).
    """
    ...
    try:
        ...
        if process.poll() or (no_exit_code and output):
            return output.decode('utf8'), _('"${c}" failed', {'c': command})
