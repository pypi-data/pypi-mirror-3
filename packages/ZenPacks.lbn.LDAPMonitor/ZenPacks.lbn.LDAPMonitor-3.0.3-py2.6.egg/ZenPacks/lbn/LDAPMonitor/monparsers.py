...
def _isOpenLDAP(base):
    ...
    # this sucks - why no version string ...
    return 'monitorServer' in base['objectClass']

def _OpenLDAP(ldapresults, results={}):
    """
    OpenLDAP mappings
    """
    results['addentryops'] = _dn('cn=add,cn=operations,cn=monitor', ldapresults).get('monitorOpCompleted', 0)
    results['monitorentryops'] = _dn('cn=modify,cn=operations,cn=monitor', ldapresults).get('monitorOpCompleted', 0)
    results['removeentryops'] = _dn('cn=delete,cn=operations,cn=monitor', ldapresults).get('monitorOpCompleted', 0)
    results['searchentryops'] = _dn('cn=search,cn=operations,cn=monitor', ldapresults).get('monitorOpCompleted', 0)
    results['referrals'] = _dn('cn=referrals,cn=statistics,cn=monitor', ldapresults).get('monitorCounter', 0)
