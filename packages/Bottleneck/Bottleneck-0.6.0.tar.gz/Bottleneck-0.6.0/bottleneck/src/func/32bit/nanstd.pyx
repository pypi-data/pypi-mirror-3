...
def nanstd_2d_int32_axis1(np.ndarray[np.int32_t, ndim=2] a, int ddof):
    "Valriance of 2d array with dtype=int32 along axis=1 ignoring NaNs."
    cdef np.float64_t asum = 0, amean, ai
    cdef Py_ssize_t i0, i1
    cdef np.npy_intp *dim
    dim = PyArray_DIMS(a)
...
def nanstd_2d_int64_axis1(np.ndarray[np.int64_t, ndim=2] a, int ddof):
    "Valriance of 2d array with dtype=int64 along axis=1 ignoring NaNs."
    cdef np.float64_t asum = 0, amean, ai
    cdef Py_ssize_t i0, i1
    cdef np.npy_intp *dim
    dim = PyArray_DIMS(a)
...
def nanstd_3d_int32_axis1(np.ndarray[np.int32_t, ndim=3] a, int ddof):
    "Valriance of 3d array with dtype=int32 along axis=1 ignoring NaNs."
    cdef np.float64_t asum = 0, amean, ai
    cdef Py_ssize_t i0, i1, i2
    cdef np.npy_intp *dim
    dim = PyArray_DIMS(a)
...
def nanstd_3d_int32_axis2(np.ndarray[np.int32_t, ndim=3] a, int ddof):
    "Valriance of 3d array with dtype=int32 along axis=2 ignoring NaNs."
    cdef np.float64_t asum = 0, amean, ai
    cdef Py_ssize_t i0, i1, i2
    cdef np.npy_intp *dim
    dim = PyArray_DIMS(a)
...
def nanstd_3d_int64_axis1(np.ndarray[np.int64_t, ndim=3] a, int ddof):
    "Valriance of 3d array with dtype=int64 along axis=1 ignoring NaNs."
    cdef np.float64_t asum = 0, amean, ai
    cdef Py_ssize_t i0, i1, i2
    cdef np.npy_intp *dim
    dim = PyArray_DIMS(a)
...
def nanstd_3d_int64_axis2(np.ndarray[np.int64_t, ndim=3] a, int ddof):
    "Valriance of 3d array with dtype=int64 along axis=2 ignoring NaNs."
    cdef np.float64_t asum = 0, amean, ai
    cdef Py_ssize_t i0, i1, i2
    cdef np.npy_intp *dim
    dim = PyArray_DIMS(a)
...
def nanstd_1d_float32_axisNone(np.ndarray[np.float32_t, ndim=1] a, int ddof):
    "Variance of 1d array with dtype=float32 along axis=None ignoring NaNs."
...
def nanstd_1d_float64_axisNone(np.ndarray[np.float64_t, ndim=1] a, int ddof):
    "Variance of 1d array with dtype=float64 along axis=None ignoring NaNs."
    cdef Py_ssize_t count = 0
    cdef np.float64_t asum = 0, amean, ai
    cdef Py_ssize_t i0
    cdef np.npy_intp *dim
...
def nanstd_2d_float32_axisNone(np.ndarray[np.float32_t, ndim=2] a, int ddof):
    "Variance of 2d array with dtype=float32 along axis=None ignoring NaNs."
    cdef Py_ssize_t count = 0
    cdef np.float32_t asum = 0, amean, ai
    cdef Py_ssize_t i0, i1
    cdef np.npy_intp *dim
...
def nanstd_2d_float64_axisNone(np.ndarray[np.float64_t, ndim=2] a, int ddof):
    "Variance of 2d array with dtype=float64 along axis=None ignoring NaNs."
    cdef Py_ssize_t count = 0
    cdef np.float64_t asum = 0, amean, ai
    cdef Py_ssize_t i0, i1
    cdef np.npy_intp *dim
...
def nanstd_3d_float32_axisNone(np.ndarray[np.float32_t, ndim=3] a, int ddof):
    "Variance of 3d array with dtype=float32 along axis=None ignoring NaNs."
    cdef Py_ssize_t count = 0
    cdef np.float32_t asum = 0, amean, ai
    cdef Py_ssize_t i0, i1, i2
    cdef np.npy_intp *dim
...
def nanstd_3d_float64_axisNone(np.ndarray[np.float64_t, ndim=3] a, int ddof):
    "Variance of 3d array with dtype=float64 along axis=None ignoring NaNs."
    cdef Py_ssize_t count = 0
    cdef np.float64_t asum = 0, amean, ai
    cdef Py_ssize_t i0, i1, i2
    cdef np.npy_intp *dim
...
def nanstd_2d_float32_axis1(np.ndarray[np.float32_t, ndim=2] a, int ddof):
    "Variance of 2d array with dtype=float32 along axis=1 ignoring NaNs."
    cdef Py_ssize_t count = 0
    cdef np.float32_t asum = 0, amean, ai
    cdef Py_ssize_t i0, i1
    cdef np.npy_intp *dim
...
def nanstd_2d_float64_axis1(np.ndarray[np.float64_t, ndim=2] a, int ddof):
    "Variance of 2d array with dtype=float64 along axis=1 ignoring NaNs."
    cdef Py_ssize_t count = 0
    cdef np.float64_t asum = 0, amean, ai
    cdef Py_ssize_t i0, i1
    cdef np.npy_intp *dim
...
def nanstd_3d_float32_axis1(np.ndarray[np.float32_t, ndim=3] a, int ddof):
    "Variance of 3d array with dtype=float32 along axis=1 ignoring NaNs."
    cdef Py_ssize_t count = 0
    cdef np.float32_t asum = 0, amean, ai
    cdef Py_ssize_t i0, i1, i2
    cdef np.npy_intp *dim
...
def nanstd_3d_float32_axis2(np.ndarray[np.float32_t, ndim=3] a, int ddof):
    "Variance of 3d array with dtype=float32 along axis=2 ignoring NaNs."
    cdef Py_ssize_t count = 0
    cdef np.float32_t asum = 0, amean, ai
    cdef Py_ssize_t i0, i1, i2
    cdef np.npy_intp *dim
...
def nanstd_3d_float64_axis1(np.ndarray[np.float64_t, ndim=3] a, int ddof):
    "Variance of 3d array with dtype=float64 along axis=1 ignoring NaNs."
    cdef Py_ssize_t count = 0
    cdef np.float64_t asum = 0, amean, ai
    cdef Py_ssize_t i0, i1, i2
    cdef np.npy_intp *dim
...
def nanstd_3d_float64_axis2(np.ndarray[np.float64_t, ndim=3] a, int ddof):
    "Variance of 3d array with dtype=float64 along axis=2 ignoring NaNs."
    cdef Py_ssize_t count = 0
    cdef np.float64_t asum = 0, amean, ai
    cdef Py_ssize_t i0, i1, i2
    cdef np.npy_intp *dim
...
def nanstd_1d_int64_axisNone(np.ndarray[np.int64_t, ndim=1] a, int ddof):
    "Valriance of 1d array with dtype=int64 along axis=None ignoring NaNs."
    cdef np.float64_t asum = 0, amean, ai
    cdef Py_ssize_t size
    cdef Py_ssize_t i0
    cdef np.npy_intp *dim
...
def nanstd_2d_int32_axisNone(np.ndarray[np.int32_t, ndim=2] a, int ddof):
    "Valriance of 2d array with dtype=int32 along axis=None ignoring NaNs."
    cdef np.float64_t asum = 0, amean, ai
    cdef Py_ssize_t size
    cdef Py_ssize_t i0, i1
    cdef np.npy_intp *dim
...
def nanstd_2d_int64_axisNone(np.ndarray[np.int64_t, ndim=2] a, int ddof):
    "Valriance of 2d array with dtype=int64 along axis=None ignoring NaNs."
    cdef np.float64_t asum = 0, amean, ai
    cdef Py_ssize_t size
    cdef Py_ssize_t i0, i1
    cdef np.npy_intp *dim
...
def nanstd_3d_int32_axisNone(np.ndarray[np.int32_t, ndim=3] a, int ddof):
    "Valriance of 3d array with dtype=int32 along axis=None ignoring NaNs."
    cdef np.float64_t asum = 0, amean, ai
    cdef Py_ssize_t size
    cdef Py_ssize_t i0, i1, i2
    cdef np.npy_intp *dim
...
def nanstd_3d_int64_axisNone(np.ndarray[np.int64_t, ndim=3] a, int ddof):
    "Valriance of 3d array with dtype=int64 along axis=None ignoring NaNs."
    cdef np.float64_t asum = 0, amean, ai
    cdef Py_ssize_t size
    cdef Py_ssize_t i0, i1, i2
    cdef np.npy_intp *dim
...
nanstd_dict[(2, NPY_int64, 1)] = nanstd_2d_int64_axis1
nanstd_dict[(3, NPY_int32, 0)] = nanstd_3d_int32_axis0
nanstd_dict[(3, NPY_int32, 1)] = nanstd_3d_int32_axis1
nanstd_dict[(3, NPY_int32, 2)] = nanstd_3d_int32_axis2
nanstd_dict[(3, NPY_int64, 0)] = nanstd_3d_int64_axis0
nanstd_dict[(3, NPY_int64, 1)] = nanstd_3d_int64_axis1
nanstd_dict[(3, NPY_int64, 2)] = nanstd_3d_int64_axis2
nanstd_dict[(1, NPY_float32, 0)] = nanstd_1d_float32_axisNone
nanstd_dict[(1, NPY_float32, None)] = nanstd_1d_float32_axisNone
nanstd_dict[(1, NPY_float64, 0)] = nanstd_1d_float64_axisNone
nanstd_dict[(1, NPY_float64, None)] = nanstd_1d_float64_axisNone
nanstd_dict[(2, NPY_float32, None)] = nanstd_2d_float32_axisNone
nanstd_dict[(2, NPY_float64, None)] = nanstd_2d_float64_axisNone
nanstd_dict[(3, NPY_float32, None)] = nanstd_3d_float32_axisNone
nanstd_dict[(3, NPY_float64, None)] = nanstd_3d_float64_axisNone
...
nanstd_dict[(3, NPY_float32, 0)] = nanstd_3d_float32_axis0
nanstd_dict[(3, NPY_float32, 1)] = nanstd_3d_float32_axis1
nanstd_dict[(3, NPY_float32, 2)] = nanstd_3d_float32_axis2
nanstd_dict[(3, NPY_float64, 0)] = nanstd_3d_float64_axis0
nanstd_dict[(3, NPY_float64, 1)] = nanstd_3d_float64_axis1
nanstd_dict[(3, NPY_float64, 2)] = nanstd_3d_float64_axis2
...
nanstd_dict[(3, NPY_int32, None)] = nanstd_3d_int32_axisNone
nanstd_dict[(3, NPY_int64, None)] = nanstd_3d_int64_axisNone
