...
-------------
  controller:
    available:
      TrackdController:
        working_directory: /usr/bin/trackd
      AnotherController:
...
class Controllers:
    ...
    def __init__(self, available, storage):
        ...
        :param available: Dictionary of controller facades and their configurations to
        be loaded. Typically taken from the config file.
        :param storage: filestorage.StorageFacade
        """
        self.controllers = {}
        self.storage = storage
        for controller, settings in available.iteritems():
            cls = self._map_controller(controller)
            if cls is not None:
                inst = cls(self.storage, settings)
