...
class ResourceDefaultViewAdapter(object):

    adapts(IResource, IZTFYBlogBackLayer, Interface)
...
class ResourceContainerPreviewColumn(Column):
    ...
    def renderCell(self, item):
        ...
        return '''<img src="%s/++display++64x64.jpeg" alt="%s" />''' % (absoluteURL(image, self.request), alt)
...
class ResourceContainerContentsViewActionsColumnCellAdapter(object):
    ...
    adapts(IResource, IZTFYBlogLayer, IResourceContainerContentsView, IActionsColumn)
...
class ResourceAddForm(BaseDialogAddForm):
    """Resource add form"""
...
class ResourcesFromZipAddForm(BaseDialogAddForm):
...
class ResourceEditForm(BaseDialogEditForm):
    """Resource edit form"""
