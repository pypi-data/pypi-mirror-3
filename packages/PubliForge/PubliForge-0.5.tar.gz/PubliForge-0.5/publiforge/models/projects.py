# $Id: projects.py e8da66c6d933 2012/02/18 22:34:11 patrick $
...
PROJECT_STATUS = {'draft': _('Draft'), 'active': _('Active'),
                  'archived': _('Archived')}
PROJECT_PERMS = {'modifier': _('Leader'), 'viewer': _('User')}
...
class Project(Base):
    ...
    # -------------------------------------------------------------------------
    def __init__(self, label, description='', status=None, deadline=None):
        """Constructor method."""
    ...
    def load(cls, project_elt, error_if_exists=True):
        ...
        project = cls(project_elt.findtext('label'),
            project_elt.find('description') is not None
                and project_elt.findtext('description') or '',
            project_elt.get('status', 'active'),
            project_elt.find('deadline') is not None
                and project_elt.findtext('deadline') or None)
            ...
            if not login in done:
                ...
                if user is not None:
                    project.users.append(ProjectUser(
                        user.user_id, item.get('in_menu'),
                        item.get('permission', 'viewer')))
                done.append(login)
        DBSession.commit()
...
class ProjectUser(Base):
    ...
    # -------------------------------------------------------------------------
    def __init__(self, user_id, in_menu=False, perm=None):
        """Constructor method."""
...
class ProjectProcessing(Base):
    ...
    def load(cls, project_id, processing_elt, check_if_exists=True):
        ...
        processing = cls(
            project_id, label,
            processing_elt.findtext('description') is not None
                and processing_elt.findtext('description').strip() or '',
...
class ProjectPack(Base):
    ...
    def load(cls, project_id, pack_elt):
        ...
        pack = cls(
            ...
            pack_elt.findtext('description') is not None
                and pack_elt.findtext('description') or '',
