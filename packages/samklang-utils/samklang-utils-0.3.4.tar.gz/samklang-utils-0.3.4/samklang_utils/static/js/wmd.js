...
(function () {
	...
	WMDEditor.defaults = { // {{{
		...
		// entering links.
		imageDialogText: '<iframe name="image-upload" id="image-upload" src="http://localhost:8000/files/images/"></iframe><div id="wmd-dialog-toggle">Toggle</div>',
		...
		prompt: function (text, defaultInputText, makeLinkMarkdown, promptType) {
			...
			var close = function (isCancel) {
                ...
                if (titleInput === undefined) {
                    ...
                    makeLinkMarkdown(text, titleInput.value);
                }
				return false;
			};
			...
			var createDialog = function () {
				...
				form.appendChild(label);
                    ...
                    input.name = input.id = "prompt-address";
					input.type = "text";
				...
				form.appendChild(label);
                    ...
                    titleInput.name = titleInput.id = "prompt-description";
	...
	var wmdBase = function (wmd, wmd_options) { // {{{
		...
		command.doLinkOrImage = function (chunk, postProcessing, isImage) {
			...
			if (chunk.endTag.length > 1) {
				...
				var makeLinkMarkdown = function (link, alt) {
					console.log(link);
                    alt = alt || wmd_options.description;
					if (link !== null) {
