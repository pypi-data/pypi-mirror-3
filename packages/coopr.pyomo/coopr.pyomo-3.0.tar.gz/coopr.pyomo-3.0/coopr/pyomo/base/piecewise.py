...
from constraint import Constraint, SOSConstraint, ConstraintBase
...
_DONE_ *) arbitrary number index sets of arbitrary dimension
       *) params in f_rules? (in particular when the they don't have value() around them)
...
def characterize_function(f,model,points):
    
    # determines convexity or concavity
    # return 1 (convex), -1 (concave), 0 (neither)
    
    SLOPES = [(f(model,points[i])-f(model,points[i-1]))/(points[i]-points[i-1]) for i in xrange(1,len(points))]
...
-f_rule=f(model,x)
	  ...
	  First argument must be a Pyomo model, second argument is the domain value at which the function evaluates. 
	  *** ALWAYS REQUIRED ***
	  Examples:
		   + def f(model,x):
			  return x**2 + 1
...
-force_pw=True/False
	...
	if self.model()._defer_construction:
	    self.model().concrete_mode()
	    self.add()
	    self.model().symbolic_mode()
	else:
	    self.add()
	...
	#If a piecewise list given, construct the piecewise constraints
	...
	#the constraints are created using the _construct_single_bound function.
	
	if self.PW_PTS != None:
	    
	    #check that lists are sorted and for special case
	    #'LOG','DLOG' pw_repn that list of 2^n + 1 points.
	    ...
	    force_pw = False
	    if self._element_mode:
		result = characterize_function(self.F, self.model(), self._pw_pts())
		if (result == -1):
		...
		for t in self.index:
		    result = characterize_function(self.F, self.model(), self._pw_pts(t))
		    if (result == -1):
	...
	if candidate == None:
	    ...
	    test_point = 0.0
	    if self.PW_PTS != None:
		if isinstance(self.PW_PTS,dict):
		    test_point = self.PW_PTS.items()[0][1][0]
		elif isinstance(self.PW_PTS,tuple) or isinstance(self.PW_PTS,list):
		    test_point = self.PW_PTS[0]
		else:
		    raise TypeError, "Piecewise kwd 'pw_pts' must be of type dict, list, or tuple"
	    elif self.GRAD_PTS != None:
		if isinstance(self.GRAD_PTS,dict):
		    test_point = self.GRAD_PTS.items()[0][1][0]
		elif isinstance(self.GRAD_PTS,tuple) or isinstance(self.GRAD_PTS,list):
	    ...
	    try:
		candidate(self.model(),test_point)
	    except Exception:
	...
	#    self.F = lambda model,x: eval(expr_string)
	...
	if (candidate == None) and (self.GRAD_PTS != None):
		...
		raise TypeError, "Piecewise kwd 'd_f_rule' must be callable with the form f(model,x),\n" \
		                ...
		                +"'x' is a number"
	    self.d_F = candidate	
	#elif isinstance(expr_string, dict):
	#    self.d_F = lambda model,x: dict([(key,eval(expr_string[key])) for key in expr_string.keys()])

    def _update_dicts(self,name,_object_):
	if isinstance(_object_,ConstraintBase):
	    ...
	    setattr(self.model(),constraint_name,_object_)
	    self._constraints_dict[constraint_name] = getattr(self.model(), constraint_name)
	    return None
	    ...
	    setattr(self.model(),var_name,_object_)
	    self._vars_dict.update([(v.name,v) for (n,v) in getattr(self.model(), var_name).iteritems()])
	    return var_name
		...
		raise AssertionError, "Error: "+self.name+" trying to overwrite model attribute: "+var_name
	    setattr(self.model(),var_name,_object_)
	    self._vars_dict[var_name] = getattr(self.model(), var_name)
	    return var_name
	elif isinstance(_object_,_BaseSet):
	    set_name = self.name+'_'+name
	    if hasattr(self.model(),set_name):
		raise AssertionError, "Error: "+self.name+" trying to overwrite model attribute: "+set_name
	    setattr(self.model(),set_name,_object_)
	    self._sets_dict[set_name] = getattr(self.model(), set_name)
	    return set_name
	...
	def linear_estimators_constraint_rule(model,*args):
		...
		return self._getvar(model,self.YVAR.name,t) >= self.F(model,self._grad_pts(t)[i]) + self.d_F(model,self._grad_pts(t)[i])*(self._getvar(model,self.XVAR.name,t)-self._grad_pts(t)[i])
	    elif self.GRAD_TYPE == 'UB':
		return self._getvar(model,self.YVAR.name,t) <= self.F(model,self._grad_pts(t)[i]) + self.d_F(model,self._grad_pts(t)[i])*(self._getvar(model,self.XVAR.name,t)-self._grad_pts(t)[i])

	self._update_dicts('tangent_constraint' , Constraint(getattr(self.model(),tangent_indices),rule=linear_estimators_constraint_rule))
	...
	def SIMPLE_LINEAR_constraint1_rule(model,*t):
	    if self.PW_TYPE == 'UB':
		return self._getvar(model,self.YVAR.name,t) <= (self.F(model,min(self._pw_pts(t))) + (float(self.F(model,max(self._pw_pts(t)))-self.F(model,min(self._pw_pts(t))))/float(max(self._pw_pts(t))-min(self._pw_pts(t))))*(self._getvar(model,self.XVAR.name,t)-min(self._pw_pts(t))))	
	...
	def SIMPLE_LINEAR_constraint1_rule(model,*args):
	    ...
	    i = args[-1]
	    if self.PW_TYPE == 'UB':
		return self._getvar(model,self.YVAR.name,t) <= (self.F(model,self._pw_pts(t)[i]) + (float(self.F(model,self._pw_pts(t)[i+1])-self.F(model,self._pw_pts(t)[i]))/float(self._pw_pts(t)[i+1]-self._pw_pts(t)[i]))*(self._getvar(model,self.XVAR.name,t)-self._pw_pts(t)[i]))	
	    elif self.PW_TYPE == 'LB':
		return self._getvar(model,self.YVAR.name,t) >= (self.F(model,self._pw_pts(t)[i]) + (float(self.F(model,self._pw_pts(t)[i+1])-self.F(model,self._pw_pts(t)[i]))/float(self._pw_pts(t)[i+1]-self._pw_pts(t)[i]))*(self._getvar(model,self.XVAR.name,t)-self._pw_pts(t)[i]))	

	self._update_dicts('simplified_bounds_constraint',Constraint(getattr(self.model(),simple_bounds_indices),rule=SIMPLE_LINEAR_constraint1_rule))
	...
	#Add SOS2 weighting variable to self.model()
	y = self._update_dicts('y_SOS2',Var(getattr(self.model(),y_indices),within=NonNegativeReals))
	
	#Add SOS2 constraints to self.model()
	...
	def SOS2_constraint2_rule(model,*t):
	    if self.PW_TYPE == 'UB':
		return self._getvar(model,self.YVAR.name,t) <= sum(self._getvar(model,y,t,i)*self.F(model,self._pw_pts(t)[i]) for i in xrange(len(self._pw_pts(t))))
	    elif self.PW_TYPE == 'LB':
		return self._getvar(model,self.YVAR.name,t) >= sum(self._getvar(model,y,t,i)*self.F(model,self._pw_pts(t)[i]) for i in xrange(len(self._pw_pts(t))))
	    elif self.PW_TYPE == 'EQ':
		return self._getvar(model,self.YVAR.name,t) == sum(self._getvar(model,y,t,i)*self.F(model,self._pw_pts(t)[i]) for i in xrange(len(self._pw_pts(t))))
	...
	if self._element_mode is True:
	    ...
	    self._update_dicts('SOS2_constraint4',SOSConstraint(var=getattr(self.model(),y), sos=2))
	else:
	    ...
	    self._update_dicts('SOS2_constraint4',SOSConstraint(INDEX_SET_SOS2, var=getattr(self.model(),y), set=getattr(self.model(),SOS_indices), sos=2))

    def _construct_BIGM(self,BIGM_SET):
	
	#Add dictionary of optimal big M values as attribute to self.model()
	...
	if len(all_keys) > 0:
	    ...
	    #Add indicator variable to self.model()
	    def y_domain():
		if self.PW_REP == 'BIGM_BIN':
		    return Binary
		elif self.PW_REP == 'BIGM_SOS1':
		    return NonNegativeReals
	    y += self._update_dicts('y_'+self.PW_REP,Var(getattr(self.model(),y_indices),within=y_domain()))
	...
	def BIGM_LINEAR_constraint1_rule(model,*args):
		...
		return self._getvar(model,self.YVAR.name,t) - (self.F(model,self._pw_pts(t)[i-1]) + ((self.F(model,self._pw_pts(t)[i])-self.F(model,self._pw_pts(t)[i-1]))/(self._pw_pts(t)[i]-self._pw_pts(t)[i-1]))*(self._getvar(model,self.XVAR.name,t)-self._pw_pts(t)[i-1])) <= rhs
	    elif self.PW_TYPE == 'LB':
		rhs = 1.0
	...
	if len(all_keys) > 0:
		...
		if self._element_mode:
		    self._update_dicts('BIGM_SOS1_constraintSOS',SOSConstraint(var=getattr(self.model(),y), sos=1))
		else:
		    ...
		    self._update_dicts('BIGM_SOS1_constraintSOS',SOSConstraint(BIGM_SET, var=getattr(self.model(),y), set=getattr(self.model(),SOS_indices), sos=1))
	...
	DCC_vert = self._update_dicts('DCC_vert',Set(getattr(self.model(),DCC_POLYTOPES),ordered=True, \
	                                             initialize=dict([(args,DCC_vert_init(args)) for args in getattr(self.model(),DCC_POLYTOPES)])))	
	...
	lmda = self._update_dicts('DCC_LAMBDA',Var(getattr(self.model(),DCC_LAMBDA_SET),within=PositiveReals))
	
	
	y = self._update_dicts('DCC_BIN_y',Var(getattr(self.model(),DCC_POLYTOPES),within=Binary))
	...
	def DCC_LINEAR_constraint2_rule(model,*t):
	    if self.PW_TYPE == 'UB':
		return self._getvar(model,self.YVAR.name,t) <= sum(self._getvar(model,lmda,t,p,v)*self.F(model,self._pw_pts(t)[v-1]) for p in self._getset(model,DCC_poly,t) for v in self._getset(model,DCC_vert,t,p))
	    elif self.PW_TYPE == 'LB':
		return self._getvar(model,self.YVAR.name,t) >= sum(self._getvar(model,lmda,t,p,v)*self.F(model,self._pw_pts(t)[v-1]) for p in self._getset(model,DCC_poly,t) for v in self._getset(model,DCC_vert,t,p))
	    elif self.PW_TYPE == 'EQ':
		return self._getvar(model,self.YVAR.name,t) == sum(self._getvar(model,lmda,t,p,v)*self.F(model,self._pw_pts(t)[v-1]) for p in self._getset(model,DCC_poly,t) for v in self._getset(model,DCC_vert,t,p))
	...
	if self._element_mode:
	    ...
	    self._update_dicts('DCC_constraint3',Constraint(getattr(self.model(),DCC_POLYTOPES),rule=DCC_LINEAR_constraint3_rule))
	...
	DLOG_poly_one = self._update_dicts('DLOG_poly_one',Set(getattr(self.model(),DLOG_LENGTH_POLY),ordered=True, \
	                                                       initialize=dict([(args,DLOG_poly_one_init(args)) for args in getattr(self.model(),DLOG_LENGTH_POLY)])))
	DLOG_poly_zero = self._update_dicts('DLOG_poly_zero',Set(getattr(self.model(),DLOG_LENGTH_POLY),ordered=True, \
	                                                         initialize=dict([(args,DLOG_poly_zero_init(args)) for args in getattr(self.model(),DLOG_LENGTH_POLY)])))
	DLOG_VERTICES = self._update_dicts('DLOG_VERTICES',Set(ordered=True,dimen=self._index_dimen+1,initialize=DLOG_VERTICES_init))
	DLOG_vert = self._update_dicts('DLOG_vert',Set(getattr(self.model(),DLOG_POLYTOPES),ordered=True, \
	                                               initialize=dict([(args,DLOG_vert_init(args)) for args in getattr(self.model(),DLOG_POLYTOPES)])))	
	...
	lmda = self._update_dicts('DLOG_LAMBDA',Var(getattr(self.model(),DLOG_LAMBDA_SET),within=PositiveReals))
	y = self._update_dicts('DLOG_BIN_y',Var(getattr(self.model(),DLOG_LENGTH_POLY),within=Binary))
	...
	def DLOG_LINEAR_constraint2_rule(model,*t):
	    if self.PW_TYPE == 'UB':
		return self._getvar(model,self.YVAR.name,t) <= sum(self._getvar(model,lmda,t,p,v)*self.F(model,self._pw_pts(t)[v-1]) for p in self._getset(model,DLOG_poly,t) for v in self._getset(model,DLOG_vert,t,p))
	    elif self.PW_TYPE == 'LB':
		return self._getvar(model,self.YVAR.name,t) >= sum(self._getvar(model,lmda,t,p,v)*self.F(model,self._pw_pts(t)[v-1]) for p in self._getset(model,DLOG_poly,t) for v in self._getset(model,DLOG_vert,t,p))
	    elif self.PW_TYPE == 'EQ':
		return self._getvar(model,self.YVAR.name,t) == sum(self._getvar(model,lmda,t,p,v)*self.F(model,self._pw_pts(t)[v-1]) for p in self._getset(model,DLOG_poly,t) for v in self._getset(model,DLOG_vert,t,p))
	...
	if self._element_mode:
	    ...
	    self._update_dicts('DLOG_constraint4',Constraint(getattr(self.model(),DLOG_LENGTH_POLY),rule=DLOG_LINEAR_constraint4_rule))
	    self._update_dicts('DLOG_constraint5',Constraint(getattr(self.model(),DLOG_LENGTH_POLY),rule=DLOG_LINEAR_constraint5_rule))
	else:
	...
	CC_poly = self._update_dicts('CC_poly',Set(getattr(self.model(),CC_VERTICES),ordered=True, \
	                                           initialize=dict([(args,CC_poly_init(args)) for args in getattr(self.model(),CC_VERTICES)])))
	CC_LAMBDA_SET = self._update_dicts('CC_LAMBDA_SET',Set(ordered=True,dimen=self._index_dimen+1,initialize=CC_lamba_set_init))
	
	lmda = self._update_dicts('CC_LAMBDA',Var(getattr(self.model(),CC_LAMBDA_SET),within=NonNegativeReals))
	y = self._update_dicts('CC_BIN_y',Var(getattr(self.model(),CC_POLYTOPES),within=Binary))
	...
	def CC_LINEAR_constraint2_rule(model,*t):
	    if self.PW_TYPE == 'UB':
		return self._getvar(model,self.YVAR.name,t) <= sum(self._getvar(model,lmda,t,v)*self.F(model,self._pw_pts(t)[v-1]) for v in self._getset(model,CC_vert,t))
	    elif self.PW_TYPE == 'LB':
		return self._getvar(model,self.YVAR.name,t) >= sum(self._getvar(model,lmda,t,v)*self.F(model,self._pw_pts(t)[v-1]) for v in self._getset(model,CC_vert,t))
	    elif self.PW_TYPE == 'EQ':
		return self._getvar(model,self.YVAR.name,t) == sum(self._getvar(model,lmda,t,v)*self.F(model,self._pw_pts(t)[v-1]) for v in self._getset(model,CC_vert,t))
	...
	if self._element_mode:
	    ...
	    self._update_dicts('CC_constraint4',Constraint(getattr(self.model(),CC_VERTICES),rule=CC_LINEAR_constraint4_rule))
	...
	LOG_poly = self._update_dicts('LOG_poly',Set(getattr(self.model(),LOG_VERTICES),ordered=True, \
	                                                     initialize=dict([(args,LOG_poly_init(args)) for args in getattr(self.model(),LOG_VERTICES)])))
	...
	LOG_BRANCHING_LEFT = self._update_dicts('LOG_BRANCHING_LEFT',Set(getattr(self.model(),LOG_BRANCHING_SCHEME),ordered=True, \
	                                                                 initialize=dict([(args,LOG_BRANCHING_LEFT_init(args)) for args in getattr(self.model(),LOG_BRANCHING_SCHEME)])))
	LOG_BRANCHING_RIGHT = self._update_dicts('LOG_BRANCHING_RIGHT',Set(getattr(self.model(),LOG_BRANCHING_SCHEME),ordered=True,\
	                                                                   initialize=dict([(args,LOG_BRANCHING_RIGHT_init(args)) for args in getattr(self.model(),LOG_BRANCHING_SCHEME)])))
	
	lmda = self._update_dicts('LOG_LAMBDA',Var(getattr(self.model(),LOG_LAMBDA_SET),within=NonNegativeReals))
	y = self._update_dicts('LOG_BIN_y',Var(getattr(self.model(),LOG_BRANCHING_SCHEME),within=Binary))
	...
	def LOG_LINEAR_constraint2_rule(model,*t):
	    if self.PW_TYPE == 'UB':
		return self._getvar(model,self.YVAR.name,t) <= sum(self._getvar(model,lmda,t,v)*self.F(model,self._pw_pts(t)[v-1]) for v in self._getset(model,LOG_vert,t))
	    elif self.PW_TYPE == 'LB':
		return self._getvar(model,self.YVAR.name,t) >= sum(self._getvar(model,lmda,t,v)*self.F(model,self._pw_pts(t)[v-1]) for v in self._getset(model,LOG_vert,t))
	    elif self.PW_TYPE == 'EQ':
		return self._getvar(model,self.YVAR.name,t) == sum(self._getvar(model,lmda,t,v)*self.F(model,self._pw_pts(t)[v-1]) for v in self._getset(model,LOG_vert,t))
	...
	if self._element_mode:
	    ...
	    self._update_dicts('LOG_constraint4',Constraint(getattr(self.model(),LOG_BRANCHING_SCHEME),rule=LOG_LINEAR_constraint4_rule))
	    self._update_dicts('LOG_constraint5',Constraint(getattr(self.model(),LOG_BRANCHING_SCHEME),rule=LOG_LINEAR_constraint5_rule))
	else:
	...
	if self._element_mode:
		...
		SLOPE[i] = (self.F(self.model(),self._pw_pts()[i])-self.F(self.model(),self._pw_pts()[i-1]))/(self._pw_pts()[i]-self._pw_pts()[i-1])
		INTERSEPT[i] = self.F(self.model(),self._pw_pts()[i-1]) - (SLOPE[i]*self._pw_pts()[i-1])
	else:
	    for t in MC_SET:
		SLOPE[t] = {}
		INTERSEPT[t] = {}
		for i in xrange(1,len(self._pw_pts(t))):
		    SLOPE[t][i] = (self.F(self.model(),self._pw_pts(t)[i])-self.F(self.model(),self._pw_pts(t)[i-1]))/(self._pw_pts(t)[i]-self._pw_pts(t)[i-1])
		    INTERSEPT[t][i] = self.F(self.model(),self._pw_pts(t)[i-1]) - (SLOPE[t][i]*self._pw_pts(t)[i-1])
		SLOPE[(t,)] = SLOPE[t]
	...
	xp = self._update_dicts('MC_X_POLY',Var(getattr(self.model(),MC_POLYTOPES)))
	y = self._update_dicts('MC_BIN_y',Var(getattr(self.model(),MC_POLYTOPES),within=Binary))
	...
	def MC_LINEAR_constraint2_rule(model,*t):
	    if self._element_mode:
		if self.PW_TYPE == 'UB':
		    return self._getvar(model,self.YVAR.name,t) <= sum((self._getvar(model,xp,t,p)*SLOPE[p])+(self._getvar(model,y,t,p)*INTERSEPT[p]) for p in self._getset(model,MC_poly,t))
		elif self.PW_TYPE == 'LB':
		    return self._getvar(model,self.YVAR.name,t) >= sum((self._getvar(model,xp,t,p)*SLOPE[p])+(self._getvar(model,y,t,p)*INTERSEPT[p]) for p in self._getset(model,MC_poly,t))
		elif self.PW_TYPE == 'EQ':
		    return self._getvar(model,self.YVAR.name,t) == sum((self._getvar(model,xp,t,p)*SLOPE[p])+(self._getvar(model,y,t,p)*INTERSEPT[p]) for p in self._getset(model,MC_poly,t))
		...
		if self.PW_TYPE == 'UB':
		    return self._getvar(model,self.YVAR.name,t) <= sum((self._getvar(model,xp,t,p)*SLOPE[t][p])+(self._getvar(model,y,t,p)*INTERSEPT[t][p]) for p in self._getset(model,MC_poly,t))
		elif self.PW_TYPE == 'LB':
		    return self._getvar(model,self.YVAR.name,t) >= sum((self._getvar(model,xp,t,p)*SLOPE[t][p])+(self._getvar(model,y,t,p)*INTERSEPT[t][p]) for p in self._getset(model,MC_poly,t))
		elif self.PW_TYPE == 'EQ':
		    return self._getvar(model,self.YVAR.name,t) == sum((self._getvar(model,xp,t,p)*SLOPE[t][p])+(self._getvar(model,y,t,p)*INTERSEPT[t][p]) for p in self._getset(model,MC_poly,t))
	...
	if self._element_mode:
	    ...
	    self._update_dicts('MC_constraint3',Constraint(getattr(self.model(),MC_POLYTOPES),rule=MC_LINEAR_constraint3_rule))
	    self._update_dicts('MC_constraint4',Constraint(getattr(self.model(),MC_POLYTOPES),rule=MC_LINEAR_constraint4_rule))	    
	...
	delta = self._update_dicts('INC_DELTA',Var(getattr(self.model(),INC_POLYTOPES)))
	if self._element_mode:
	    self._getvar(self.model(),delta,1).setub(1)
	    self._getvar(self.model(),delta,len(self._pw_pts())-1).setlb(0)
	else:
	    for t in INC_SET:
		self._getvar(self.model(),delta,t,1).setub(1)
		self._getvar(self.model(),delta,t,len(self._pw_pts(t))-1).setlb(0)
	y = self._update_dicts('INC_BIN_y',Var(getattr(self.model(),INC_YSET),within=Binary))
	...
	def INC_LINEAR_constraint2_rule(model,*t):
	    if self.PW_TYPE == 'UB':
		return self._getvar(model,self.YVAR.name,t) <= self.F(model,self._pw_pts(t)[0]) + sum(self._getvar(model,delta,t,p)*(self.F(model,self._pw_pts(t)[p])-self.F(model,self._pw_pts(t)[p-1])) for p in self._getset(model,INC_poly,t))
	    elif self.PW_TYPE == 'LB':
		return self._getvar(model,self.YVAR.name,t) >= self.F(model,self._pw_pts(t)[0]) + sum(self._getvar(model,delta,t,p)*(self.F(model,self._pw_pts(t)[p])-self.F(model,self._pw_pts(t)[p-1])) for p in self._getset(model,INC_poly,t))
	    elif self.PW_TYPE == 'EQ':
		return self._getvar(model,self.YVAR.name,t) == self.F(model,self._pw_pts(t)[0]) + sum(self._getvar(model,delta,t,p)*(self.F(model,self._pw_pts(t)[p])-self.F(model,self._pw_pts(t)[p-1])) for p in self._getset(model,INC_poly,t))
	...
	if self._element_mode:
	    ...
	    self._update_dicts('INC_constraint3',Constraint(getattr(self.model(),INC_POLYTOPES),rule=INC_LINEAR_constraint3_rule))
	    self._update_dicts('INC_constraint4',Constraint(getattr(self.model(),INC_POLYTOPES),rule=INC_LINEAR_constraint4_rule))
	else:
    ...
    def _M_func(self,a,b,c):
	return self.F(self.model(),a) - self.F(self.model(),b) - ((a-b) * (float(self.F(self.model(),c)-self.F(self.model(),b)) / float(c-b)))
	...
	if self._element_mode:
		...
		index = j
		if (BOUND == 'LB'):
		    #M_final[index] = min([self._M_func(PTS[k],PTS[j-1],PTS[j]) for k in xrange(len(PTS))])
		    M_final[index] = min( [0.0, min([self._M_func(PTS[k],PTS[j-1],PTS[j]) for k in xrange(len(PTS))])] )
		elif (BOUND == 'UB'):
		    #M_final[index] = max([self._M_func(PTS[k],PTS[j-1],PTS[j]) for k in xrange(len(PTS))])
		...
		for j in xrange(1,len(PTS[t])):
		    ...
		    if (BOUND == 'LB'):
			M_final[index] = min( [0.0, min([self._M_func(PTS[t][k],PTS[t][j-1],PTS[t][j]) for k in xrange(len(PTS[t]))])] )
		    elif (BOUND == 'UB'):
			M_final[index] = max( [0.0, max([self._M_func(PTS[t][k],PTS[t][j-1],PTS[t][j]) for k in xrange(len(PTS[t]))])] )
		    if M_final[index] == 0.0:
			del M_final[index]
	return M_final
    ########
