# $Id: users.py b4cc2e0d2b84 2012/01/30 09:59:49 patrick $
...
USER_PERMS = {'manager': _('Manager'), 'modifier': _('Editor'),
    'viewer': _('User')}
...
class User(Base):
    ...
    def __init__(self, settings, login, status, password, name, email,
        ...
        self.name = u' '.join(name.split()).strip()
        self.email = email
        self.lang = lang
    ...
    def setup_environment(self, request):
        ...
        * ``home``: home page content
        ...
        # Permissions
        perms = {}
        levels = {'viewer': 1, 'modifier': 2, 'manager': 3, 'admin': 4}
