...
def get_upload_list(path, prefix):
  r = []
  for f in get_controlled_files():
    if f in EXCLUDE_FILES:
      continue
    relpath = os.path.relpath(f, DIR)
...
def print_upload_list(before_symlinks, upload_list, root, branch, prev_commit):

  col = len(max(map(lambda x:x[0], before_symlinks), key=len))
  for index, pair in enumerate(upload_list):
    if os.path.islink(pair[0]):
      color = ''
      name = before_symlinks[index][0]
      dest = os.path.join(root, branch, colors.OKBLUE + prev_commit + colors.ENDC, before_symlinks[index][0])
    ...
    print '{color}{0:{col}} -> {1}{end}'.format(name, dest, col=col, color=color, end=colors.ENDC)
...
def mark(args):
  ...
  # download prev data
  print colors.HEADER + 'Downloading commit data ...' + colors.ENDC
  ...
  def do_symlinks(upload_list):
    ...
    print '%sPrevious marked commit is %s%s%s%s'%(colors.HEADER, colors.ENDC, colors.OKBLUE, prev_commit, colors.ENDC)
    ...
    if changed_files == False:
      print colors.FAIL + 'Skipping symlink step. Could not read changed files.' + colors.ENDC
      return
  ...
  if use_symlinks:
    upload_list_linked, symlink_dir = do_symlinks(upload_list)
  else:
    upload_list_linked = upload_list

  print_upload_list(upload_list, upload_list_linked, config['root'], branch_name, prev_commit)

  upload_list_linked.append( (temp_history_file.name, config['root'], HOSTERY_FILE)  )
  upload_list_linked.append( (temp_index_file.name, config['root'], 'index.html') )

  print colors.HEADER + 'Uploading files ...' + colors.ENDC
  connection.upload(upload_list_linked)
  ...
  print colors.HEADER + 'Done.' + colors.ENDC
