...
def multifunc(data,axisref,deffunc):
    ...
    shapeout, typeout = getshapeout(datatrns,trnsaxisref,deffunc)
...
def getshapeout(datain,axisref,deffunc,dimidx=0):
    iterate = False
    if (dimidx <= (len(shape(datain)))):
        if (axisref[dimidx] == False):
            iterate = True
    if iterate == True:
        axidx = 0
        shapeout,typeout = getshapeout(datain[0],axisref,deffunc,dimidx = dimidx+1)
        shapeout.insert(0,datain.shape[0])
        dataout = None
    else:
        dataout = deffunc(datain)
        typeout = type(dataout).__name__
        if ((typeout == 'list') or (typeout == 'tuple')):
            dataout = dataout[0]
        shapeout = list(shape(dataout))
    return (shapeout, typeout)
