...
: boost::noncopyable
{
	...
	, boost::equality_comparable< start >
	{
		...
		w_mer_evaluation::vec_ptr                best_w_mers;          ///< The best W-mers for the seed used to align the start.
		...
		/** Constructor. */
		start( double score, size_t num_sites, std::string w_mer, w_mer_evaluation::vec_ptr best_w_mers, typename model_t::ptr _model )
		: score( score )
		, num_sites( num_sites )
		, w_mer ( w_mer )
	...
	typedef boost::function<
		void (
		    w_mer_evaluation::vec_ptr best_w_mers,
    ...
    find_best_instances_for_model() {
        ...
        boost::sort( *instance_finder.instances, bind( &w_mer_evaluation::Z, boost::lambda::_1 ) > bind( &w_mer_evaluation::Z, boost::lambda::_2 ) );
    ...
    inline
    bool
    do_instances_overlap( const w_mer_evaluation::vec & instances1, const w_mer_evaluation::vec & instances2 ) {
	...
	examine_start( const Prefix & pre ) {
		...
		if( ! ignore_this_start ) {
			...
			w_mer_evaluation::vec_ptr instances = find_best_w_mers_for_model();
            ...
            if( instances->size() >= candidate_num_sites[ 0 ] ) {
                ...
                BOOST_FOREACH( size_t num_sites, candidate_num_sites ) {
                    ...
                    if( starts.size() < num_to_find ) {
                        ...
                        w_mer_evaluation::vec_ptr start_instances(
                            new w_mer_evaluation::vec( instances->begin(), instances->begin() + actual_num_sites )
                        );
