...
if USE_QTHREADS:
    pass
from PyQt4.QtCore import pyqtSlot, pyqtSignal, QObject, QThread, QRunnable
...
class WebServiceProxy(QObject):
    ...
    """
    successSignal = pyqtSignal( object, dict)
    errorSignal = pyqtSignal(object, dict)
    ...
    def send(self, params_data):
        ...
        #print "I'm about to enter queue of size %d" %WebServiceProxy.queue.qsize()
        WebServiceProxy.queue.put(self)
        # Create request
        ...
        while Done == False:
            try:
                response_obj = urllib2.urlopen(request_obj, timeout=5)
                response_data = loads(response_obj.read())
            except socket.error as e:
                if isinstance(e, socket.timeout):
                    print '1 Request took too long - trying again...'
            except urllib2.URLError as e:
                if isinstance(e, socket.timeout):
                    print '2 Request took too long - trying again...'
                elif e.reason[0] != 104:
                    print '2 Connection reset by user - trying again...'
                else:
                    Done = True
                    print 'aaagh!!'
            else:
                ...
                Done = True
        WebServiceProxy.queue.get(self)
        #WebServiceProxy.queue.task_done()
        #print "I'm done! Queue is not as size %d" %WebServiceProxy.queue.qsize()
        ...
        if 'result' in response_data and self.handler != None:
            ...
            """
            self.successSignal.connect(self.handler.__onRemoteResponse__)
            self.successSignal.emit(self.request_info, response_data)
...
class AutoAPI(object):
    ...
    def __call__(self, cls):
        ...
        """
        Decorating a method in ``cls`` with ``onChange(attrs)`` adds
        a list to that method that contains ``attrs``.  Here we find all
        functions that have that list, then save their names to 
        ``NewClass.__handler_names__``.
        The next step is in ``NewClass.__init__`` ...
        """
        
        """
        handlers = [ 
                    getattr(self, attr) for attr in set(dir(self)).difference(dir(self.__client_cls__))
                    if ismethod(getattr(self, attr)) \
                        and hasattr(getattr(self, attr), 'TGInterface_onChange')
                    ]
        """
        handlers = []
        for attr_name in dir(NewClass):
            attr = getattr(NewClass, attr_name)
            if ismethod(attr) and hasattr(attr, 'TGInterface_onChange'):
                expected_attrs = self.api_class.__exposed_attrs__ + ['LoadedRequiredAttrs']
                unexpected_attrs = set(attr.TGInterface_onChange).difference(expected_attrs)
                if len(unexpected_attrs)==0:
                    handlers.append(attr_name)
                else:
                    raise AutoAPIError("In class %s are methods decorated to respond to changes in %s, but those attributes aren't defined in the API." % (cls.__name__, `unexpected_attrs`) )
        
        NewClass.__handler_names__ = handlers
...
if USE_QTHREADS:
    class QSignalProxy(QObject):
        signal = pyqtSignal()
        def QHandler(self, handler):
...
def CreateNewClass(ClientClass, APIClass, autoload):
    ...
    class NewClass(ClientClass):
        ...
        __hidden__ = {}
        __old__ = {}
        __attrs_to_copy__ = ['__hidden__', '__old__', '__handlers_by_attr__', '__autoload__']
        __handlers_by_attr__ = defaultdict(list)
        __handler_names__ = []
        ...
        def __init__(self, *args, **kwargs):
            ...
            """
            Now we populate the defaultdict ``self.__handlers_by_attr__``.
            If using Qt, the handlers need to be called via ``pyqtSignal``s
            """    
            for handler_name in self.__handler_names__:
                handler = getattr(self, handler_name)
                for handled_attr in handler.TGInterface_onChange:
                    if USE_QTHREADS:
                        self.__handlers_by_attr__[handled_attr].append(QSignalProxy().QHandler(handler))
                    self.__handlers_by_attr__[handled_attr].append(handler)
            ...
            # I'll explain later...
            if not hasattr(self.__client_cls__, '__require_to_load__'):
                self.__client_cls__.__require_to_load__ = []
            self.__require_to_load__ = copy(self.__client_cls__.__require_to_load__)
            self.loaded = True
            ...
            if self.__autoload__:
                self._update_()
            if len(self.__require_to_load__)>0:
                """
                I'll update the attributes in 
                ``self.__require_to_load__``, then when each
                attribute successfully pulls its value it removes itself from
                that list.  The last one to remove itself will call
                the method decorated with ``onLoad()``
                """
                self._update_(*self.__require_to_load__)
                self.loaded = False
        ...
        def __onRemoteResponse__(self, request_info, response):
            ...
            * Call a server-side function
            ...
            if attr in self.__api__.__exposed_attrs__ and operation=='pull':
                ...
                """
                Remove 'attr' from ``self.__require_to_load__``
                when it pulls successfully.  If that's the last one, load
                the object!
                """
                if attr in self.__require_to_load__ and attr != None:
                    if len(self.__require_to_load__) == 1:
                        self.loaded = True
                        self.__onAttrChange__('LoadedRequiredAttrs')
                    self.__require_to_load__.remove(attr)
                ...
                """
                Remove 'attr' from ``self.__require_to_load__``
                when it posts successfully.  If that's the last one, load
                the object!
                """
                if attr in self.__require_to_load__ and attr != None:
                    if len(self.__require_to_load__) == 1:
        ...
        def __onAttrChange__(self, attr):
            """
            This calls all methods that were decorated with ``onChange(attr)``.
            """
            for changeHandler in self.__handlers_by_attr__[attr]:
                changeHandler()
...
class onChange:
    ...
    def __call__(self, func):
        if not hasattr(func, 'TGInterface_onChange'):
            func.TGInterface_onChange = []
        func.TGInterface_onChange.extend(self.attrs)
        if USE_QTHREADS:
...
class onLoad:
    def __call__(self, func):
        func = onChange('LoadedRequiredAttrs')(func)
        return func
...
def CompleteTheModel():
    ...
    all_api_classnames = set([api_classname for api_classname in dir(API) if type(getattr(API, api_classname))==type])
