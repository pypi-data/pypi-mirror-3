...
class VirtualEnv(object):
	...
	def create(self, path, python = "python", force = False):
		...
		if not force and fileexists(os.path.join(path, 'bin', 'activate')):
			self.__path = path
			console.confirm('   Env "%s" already created. Use "force=True" to override it' % path)
			return True

		check = check_sys_package('virtualenv')
		version = check_sys_package_version(python)
	...
	def remove(self):
		 if not self.__path or not fileexists(os.path.join(self.__path, 'bin', 'activate')):
