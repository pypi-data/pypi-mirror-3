...
from ._stempy_build import _dummy_fn, _debug, _using_old_bg_model, _python_debug_build
...
from cookbook.script_basics import log_exception, setup_logging

import glob, os, math, numpy as N, itertools, time, subprocess, pylab as P, copy
...
def logo(dist, tag, d, make_png=False, make_eps=True, write_title=True):
...
def turn_on_google_profiling_if_asked_for(options):
    ...
    if options.google_profile:
        logger.info('Starting google profiler.')
        __google_profiler_start(os.path.join(options.output_dir, 'google.prof'))
...
def read_sequences(fasta_file, options):
    ...
    if not options.max_num_sites:
        # allow number of sites up to 3 * # sequences / 2, with hard limit of 10000 if not user-specified
        hard_limit = 10000
        options.max_num_sites = min(3 * len(seqs) / 2, hard_limit)
        if hard_limit == options.max_num_sites:
        ...
        logger.info('Set maximum number of sites to %d', options.max_num_sites)
    if not options.min_num_sites:
        options.min_num_sites = max(len(seqs) / 10, 2) # always look for at least 2 sites
        logger.info('Set minimum number of sites to %d', options.min_num_sites)
...
class StartingPointFinder(object):
    ...
    def __init__(self, input_sequences, significance, options, output_dir=None):
        "Construct."
        ...
        self.central_widths = self.options.widths and self.options.widths or list(geometric_progression(self.options.min_w, self.options.max_w, N.sqrt(2.)))
    ...
    def _create_start_finder(self, W):
        ...
        model = self.input_sequences.create_model(W)
...
class MotifFinder(object):
    ...
    def __call__(self, starting_point_finder):
        ...
        # 
        background = N.array([best_model.bg.freqs.freq(i) for i in xrange(4)])
        log_odds = make_log_odds(pssm=N.exp(best_model.bs.pssm.log_probs.values()), background=background)
        log_odds_range = 100
        scaled_log_odds = scale_log_odds(log_odds, range_=log_odds_range)
        rounded_back = background.round(3) # round background so results same as MEME (and MAST)
        pssm_cdf = calc_pssm_cdf(scaled_log_odds, range_=log_odds_range, background=rounded_back)
        ...
        if self.bg_sequences:
            bg_prediction_model = self.bg_sequences.create_model(best_model.W)
            ...
            logger.info(
                'Found %d instances in %d bases in %d BACKGROUND sequences for %.1e instances/base for a %.1f-fold enrichment in the input',
                bg_stats.count, bg_stats.num_bases, bg_stats.num_seqs, bg_stats.per_base, input_stats.per_base/bg_stats.per_base
            )
...
class SequenceSet(object):
    ...
    def __init__(self, fasta_filename, options):
        ...
        self.index = build_index(self.seqs)
        self.occs = occurrences_from_index(self.index)
    ...
    def _bg_model_creator_fn(self):
        """
        @return: The function to make a background model for the given order.
        """
        try:
            logger.info('Using a Markov model of order %d' % self.options.bg_model_order)
            return globals()['create_markov_model_order_from_index_%d' % self.options.bg_model_order]
        except AttributeError:
            raise ValueError('STEME does not support Markov background models of order %d' % self.options.bg_model_order)
    

    def build_model_of(self):
        """
        Build a Markov model of the sequences.
        """
        self.mm, _ = self._bg_model_creator_fn()(self.data.index, self.options.back_dist_prior)
        return self.mm, self.freqs, self.freqs_with_pseudo_counts
        
    
    def calculate_likelihoods(self, mm):
        self.lls = mm.calculate_likelihoods(self.data)
    ...
    def get_bg_model(self, W):
        ...
        if W not in self.bg_models:
            self.bg_models[W] = BackgroundModelFromLikelihoods(self.data, self.lls, self.freqs_with_pseudo_counts, W)
...
class Algorithm(object):
    ...
    def __init__(self, options):
        "Initialise."
        self.options = options
        if self.options.widths:
            self.options.max_w = max(self.options.widths) # make sure max_w is correct
...
#        file_handler.handleError = handleError
    ...
    def _initialise_bg(self):
        ...
        if self.options.bg_fasta_file:
            ...
            self.mm, self.freqs, self.freqs_with_pseudo_counts = self.bg_sequences.build_model_of()
            self.bg_sequences.calculate_likelihoods(self.mm)
        else:
            ...
            self.mm, self.freqs, self.freqs_with_pseudo_counts = self.input_sequences.build_model_of()
        # calculate likelihood of input sequences using background model.
        self.input_sequences.calculate_likelihoods(self.mm)
    ...
    def _finalise(self):
        ...
        for msg, duration in self.timings:
            logging.info('%20s took %9.1f seconds (%2.0f%% of total) to complete', msg, duration, 100*duration/self.total_duration)


    def _find_starts(self):
        "Find the starts we will use."
        with Timer(msg='find starts') as timer:
            self.start_finder = StartingPointFinder(self.input_sequences, self.significance, self.options)
