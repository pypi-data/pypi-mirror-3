...
def makeFilename(custid, custpacketid):
    if not contacts.IsCustomer(custid):  # SECURITY
        dhnio.Dprint(4, "customerservice.makeFilename WARNING %s not a customer: %s" % (custid, str(contacts.getCustomerIDs())))
        return ''
...
def Save(request):
    dhnio.Dprint(6, "customerservice.Save %s from %s" % (request.PacketID, nameurl.GetName(request.OwnerID)))
    filename = makeFilename(request.OwnerID, request.PacketID)
    if filename == "":
#        if request.OwnerID == misc.getLocalID():
#            dhnio.Dprint(6,"customerservice.Save " + request.PacketID)
#        else:
