...
class ScalarData(object):
    ...
    def pprint(self, ostream, option, prefix="", repn=None):
        ...
        if isinstance(value, dict) and (len(value) > 0) and isinstance(value[value.keys()[0]], dict) and ("Id" in value[value.keys()[0]]):
            id_dict_map = {}
            ...
            for entry_name, entry_dict in value.iteritems():
                ...
                for attr_name, attr_value in entry_dict.iteritems():
            ...
            if entries_to_print is True:
                print >>ostream, ""
                for entry_id in sorted(id_dict_map.keys()):
                    if id_nonzeros_map[entry_id] is True:
                        print >>ostream, prefix+str(id_name_map[entry_id])+":"
                        entry_dict = id_dict_map[entry_id]
                        for attr_name, attr_value in entry_dict.iteritems():
                            if isinstance(attr_value,float) and (math.floor(attr_value) == attr_value):
                                attr_value = int(attr_value)
                            print >>ostream, prefix+"  "+string.capitalize(attr_name)+": "+str(attr_value)
            else:
