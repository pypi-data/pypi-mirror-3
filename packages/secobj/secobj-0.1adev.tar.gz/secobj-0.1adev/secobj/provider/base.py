...
class SecurityProvider(object):
    ...
    @property
    def builtin_subjects(self):
        return self._BUILTIN_SUBJECTS

    @property
    def builtin_principals(self):
        return self._BUILTIN_PRINCIPALS
    ...
    def _getnamedacl(self, name):
        ...
        if not self.config.has_option(section, option):
            ...
            return tuple()
        rules = tuple(map(lambda x: Rule(*x), json.loads(self.config.get(section, option))))
        return rules
    ...
    @memoize(cache=_cached_permissions, method=True, callback=lambda args,kwargs: args)
    ...
    def _getsubject(self, name):
        try:
            if name in Principal._principals:
                subject = Principal._principals[name]
                if isinstance(subject, Subject):
                    return subject
            return self.builtin_subjects[name]
        except KeyError:
    ...
    def _getprincipal(self, name):
        try:
            if name in Principal._principals:
                return Principal._principals[name]
            return self.builtin_principals[name]
        except KeyError:
