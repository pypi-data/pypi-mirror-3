...
class ProgressiveHedging(object):
    ...
    def compute_blended_variable_counts(self):
        ...
        for stage, tree_node, variable, index, is_fixed, is_stale in scenario_tree_node_variables_generator(self._scenario_tree, self._instances, includeLastStage=False):
    ...
    def __init__(self, options):
        ...
        self._generate_symbolic_labels = False
        ...
        self._generate_symbolic_labels          = options.symbolic_solver_labels
        ...
        self._solver.generateSymbolicLabels = self._generate_symbolic_labels
    ...
    def initialize(self, model_directory_name=".", scenario_data_directory_name=".", model=None, model_instance=None, scenario_tree=None, converger=None, linearize=False, retain_constraints=False):
        ...
        for scenario in self._scenario_tree._scenarios:
            ...
            if isinstance(self._solver_manager, coopr.plugins.smanager.phpyro.SolverManager_PHPyro):
                # TBD/IMPT: we technically only have to execute the identify_variables preprocessor
                #           (and maybe not even that preprocessor) - this will save significant time.
        ...
        if self._scenario_tree.contains_bundles():
            ...
            while (num_results_so_far < len(self._scenario_tree._scenarios)):
                ...
                if isinstance(self._solver_manager, coopr.plugins.smanager.phpyro.SolverManager_PHPyro):                
                    ...
                    load_variable_values(instance, results)
                else:
                    # a temporary hack - if results come back from pyro, they won't
                    # have a symbol map attached. so create one.
            ...
            for tree_node in stage._tree_nodes:
                ...
                for variable_name, variable_indices in tree_node._variable_indices.iteritems():
                    ...
                    for scenario in tree_node._scenarios:
                        ...
                        for index, value in new_weight_values.iteritems():
                            weight_parameter[index] = value
    ...
    def iteration_k_solves(self):
        ...
        if isinstance(self._solver_manager, coopr.plugins.smanager.phpyro.SolverManager_PHPyro):        
            transmit_weights_and_averages(self)
