...
def do_backupdb(backup_dir=None, updatelink=False):
    ...
    elif 'postgresql_psycopg2' in engine or 'postgresql' in engine or 'postgis' in engine:
...
def get_args(engine, db):
    ...
    elif 'postgresql_psycopg2' in engine or 'postgresql' in engine:
...
def do_postgresql_backup(args, passwd, outfile):
    ...
    cmd = ' '.join(args)
    if passwd:
        child = pexpect.spawn(' '.join(args), timeout=1000)
