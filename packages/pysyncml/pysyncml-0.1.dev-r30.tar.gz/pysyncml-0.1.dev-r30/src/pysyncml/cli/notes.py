...
# -*- coding: utf-8 -*-
#------------------------------------------------------------------------------
# file: $Id: notes.py 29 2012-06-30 22:52:55Z griff1n $
...
class DatabaseObject(object):
  # TBD: is having a "global" Note._db really the best way?...
  _db    = None
  @declared_attr
...
class Note(DatabaseObject, pysyncml.NoteItem):
  ...
  def dump(self, stream, contentType, version, rootdir):
    # TBD: convert this to a .body @property...
...
class FilesystemNoteAgent(pysyncml.BaseNoteAgent):
  ...
  def __init__(self, root, index, options, ignoreRoot=None, ignoreAll=None,
    ...
    # TBD: how to detect if my schema has changed?...
    if not os.path.isfile('%s%s' % (root, index)):
      DatabaseObject.metadata.create_all(self.dbengine)
    # tbd: is having a global really the best way?...
    DatabaseObject._db = self.db
    if syncstore is not None:
      self.scan(syncstore)

    # TBD: adding this for funambol-compatibility (to remove multiple "VerCT" nodes)...
    self.contentTypes = [
  ...
  #----------------------------------------------------------------------------
  def scan(self, store):
    # tbd: this scan assumes that the note index (not the bodies) will
  ...
  def _scanindex(self, store, reg):
    ...
    #            for deleted files...
    # TBD: this is somewhat of a simplistic algorithm... this comparison
  ...
  def _scanfile(self, path, name, store, reg):
    ...
    if inode != note.inode:
      ...
      note.inode = inode
    if name != note.name:
      # tbd: a rename should prolly trigger an update...
      log.debug('locally renamed note: %s => %s (not synchronized)', note.name, name)
      note.name = name
    # TBD: i *should* store the last-modified and check that instead of
...
def main():
  ...
  # # TBD: stop ignoring ``opts.remoteUri``...
  ...
  if adapter.devinfo is None:
    ...
    adapter.devinfo = context.DeviceInfo(
      ...
      # TBD: adding this for funambol-compatibility...
  ...
  # TBD: this check should be made redundant... (ie. once the
...
#------------------------------------------------------------------------------
# end of $Id: notes.py 29 2012-06-30 22:52:55Z griff1n $
#------------------------------------------------------------------------------
