...
class TestGetTag(unittest.TestCase):
    ...
    def test_text_item(self):
        """
        Ensures a TEXT_ITEM is parsed to a textarea.
        """
        token = Token('TEXT_ITEM', 'Foo')
        result = get_tag(token, 'name_value')
        expected = ('<label>Foo </label>'+
            '<textarea name="foo" rows="5" style="width: 100%">' +
            '</textarea><br/>')
        self.assertEqual(expected, result)

    def test_text_item_with_roles(self):
        """
        Ensures a TEXT_ITEM with associated roles is parsed correctly.
        """
        token = Token('TEXT_ITEM', 'Foo', roles=['bar', 'baz'])
        result = get_tag(token, 'name_value')
        expected = ('<label>Foo <span class="roles">(bar, baz)</span></label>'+
            '<textarea name="foo" rows="5" style="width: 100%">' +
            '</textarea><br/>')
        self.assertEqual(expected, result)

    def test_text_item_is_name_safe(self):
        """
        Ensures a TEXT_ITEM is parsed to a textarea with the correct name
        attribute.
        """
        token = Token('TEXT_ITEM', 'Foo&bar baz!')
        result = get_tag(token, 'name_value')
        expected = ('<label>Foo&amp;bar baz! </label>'+
            '<textarea name="foo-bar-baz" rows="5" style="width: 100%">' +
            '</textarea><br/>')
        self.assertEqual(expected, result)
