...
if __name__ == "__main__":
    ...
    c = MongoDB_Connection(server_address = ("1.2.3.4", 5678), auth_database = "db")
    with expected(timeout("timed out")):
        c.connect(Timeout(0.1))

    c = MongoDB_Connection(server_address = ("127.0.0.1", 27017), auth_database = "db",
                           username = "user", password = "wrong")
    try:
        c.connect(Timeout(3.0))
    except MongoDB_Error as e:
        assert str(e) == "auth fails"
    ...
    try:
        ...
        rq = OP_QUERY("db.system.users", {}, ("user", "pwd"))
        rs = c.sync_request(rq, Timeout(10.0))
        for user in rs.documents:
            if user.pop("user") == "user":
                assert user.pop("pwd") == md5(b"user:mongo:pass").hexdigest()
                break
        else:
            assert False
        ...
        rq = OP_QUERY(".", {})
        try:
            c.sync_request(rq, Timeout(10.0))
        except MongoDB_Error as e:
            assert str(e) == "db name can't be empty"
            assert e.code == 13075
        else:
            assert False
