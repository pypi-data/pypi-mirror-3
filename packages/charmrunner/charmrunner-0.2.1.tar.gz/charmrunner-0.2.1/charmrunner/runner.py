...
def setup_parser():
    ...
    parser.add_argument("-p", "--test-plan",
                        help="Path to the plan for the test")
    ...
    group.add_argument("-c", "--consumer-name",
                        help="Worker name")
    group.add_argument("--poll-interval", default=40,
...
def main():
    ...
    if options.test_plan and os.path.exists(options.test_plan):
        plan_iter = iter([json.read(open(options.test_plan))])
    else:
        assert options.repository and options.series, \
               "Repository and series required for generating plans"
    ...
    success, result = run(
        options.work_dir, plan_iter.next(), options.test_record)
