...
never are multiple non-terminals (so one at maximum) in a row.  Usually 
...
In any :py:class:`Grammar`-descending grammar operation precedence is 
accessible via rule ``EXPR``::
...
class OperatorPrecedenceParser(CustomRule):
    ...
    def __init__(self, compiler, name, args):
    ...
    def parse_args(self, args):
        ...
        if 'W' not in aliases: aliases['W'] = compiler.sequence(['<.ws>'])
        ...
        self.precircumfix  = precircumfix  = {}
        self.circumfix     = circumfix     = {}
        self.postcircumfix = postcircumfix = {}
            ...
            while 1:
                if len(args) == 0:  # this is either the start of expr or a rhs
                    ...
                    if mob.name == 'op': # prefix, precircumfix, circumfix
                        ...
                        if op_s == CLOSE_STACK[-1]: break

                        is_postfix = False
                        if peeked is StopIteration: is_postfix = True
                        elif peeked.name == 'op'  : is_postfix = True
                        ...
                        if is_postfix:
                            ...
                            mob = MatchObject(
                                start = mob.start, end = op.end, string = s,
                                lexems = _lexems )
                            args = [ P.lex(name=rules[rule], mob=mob) ]
                        ...
                        if op_s in infix:
                            ...
                            args = _parse([mob], min_precedence, nest, is_rhs=is_rhs)
