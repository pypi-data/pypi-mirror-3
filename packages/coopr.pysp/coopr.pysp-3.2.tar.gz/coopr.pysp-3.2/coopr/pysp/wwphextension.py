...
def slam_priority_descend_compare(a, b):
    ...
    value_a = a.get_suffix_value("SlammingPriority")
    value_b = b.get_suffix_value("SlammingPriority")
        ...
        for line in suffix_file.readlines():
            ...
            if len(line) > 0 and line[0] != '#':
                ...
                if isVariableNameIndexed(variable_string) is True:
                    ...
                    if variable.suffix_defined(suffix_name) is False:
                        variable.declare_suffix(suffix_name)
                    ...
                    for index in match_indices:
                        ...
                        variable_value.set_suffix_value(suffix_name, converted_value)
        ...
        for line in annotation_file.readlines():
            ...
            if len(line) > 0 and line[0] != '#':
                ...
                if isVariableNameIndexed(variable_string) is True:
                    ...
                    if variable.suffix_defined(annotation_name) is False:
                        variable.declare_suffix(annotation_name)
                    ...
                    for index in match_indices:
                        ...
                        variable_value.set_suffix_value(annotation_name, annotation_value)
        ...
        for stage in ph._scenario_tree._stages[:-1]:
            ...
            for tree_node in stage._tree_nodes:
                ...
                for variable_name, (reference_variable, index_template) in stage._variables.iteritems():
                    ...
                    if not reference_variable.suffix_defined("my_stage"):
                        reference_variable.declare_suffix("my_stage")
                    ...
                    if (len(parameter_index_set) is 1) and (None in parameter_index_set):
                        new_stat_parameter = Param(name=new_stat_parameter_name, default=0.0, mutable=True)
                    else:
                        new_stat_parameter = Param(parameter_index_set, name=new_stat_parameter_name, default=0.0, mutable=True)
                    ...
                    if (len(parameter_index_set) is 1) and (None in parameter_index_set):
                        new_conv_parameter = Param(name=new_conv_parameter_name, default=0.0, mutable=True)
                    else:
                        new_conv_parameter = Param(parameter_index_set, name=new_conv_parameter_name, default=0.0, mutable=True)
                    for newindex in parameter_index_set:
                    ...
                    if (len(parameter_index_set) is 1) and (None in parameter_index_set):
                        new_fix_parameter = Param(name=new_fix_parameter_name, default=0.0, mutable=True)
                    else:
                        new_fix_parameter = Param(parameter_index_set, name=new_fix_parameter_name, default=0.0, mutable=True)
                    for newindex in parameter_index_set:
                    ...
                    if (len(parameter_index_set) is 1) and (None in parameter_index_set):
                        new_hash_parameter = Param(ph._iteration_index_set, name=new_hash_parameter_name, default=0.0, mutable=True)
                        for new_index in ph._iteration_index_set:
                            new_hash_parameter[new_index] = 0                        
                    else:
                        new_hash_parameter = Param(parameter_index_set * ph._iteration_index_set, name=new_hash_parameter_name, default=0.0, mutable=True)
                    ...
                    for index in variable_indices:
                        ...
                        reference_variable[index].set_suffix_value("my_stage",stage)
                        ...
                        if is_used is True:
                            ...
                            if instance_fixed_count == 0:
                                ...
                                if isinstance(variable_type, IntegerSet) or isinstance(variable_type, BooleanSet):
                                    ...
                                    if (attrvariable.suffix_defined('Iter0FixIfConvergedAtLB') is True) and (attrvarvalue.get_suffix_value('Iter0FixIfConvergedAtLB') is not None):
                                        lb = attrvarvalue.get_suffix_value('Iter0FixIfConvergedAtLB')
                                    else:
                                        lb = self.Iter0FixIfConvergedAtLB

                                    if (attrvariable.suffix_defined('Iter0FixIfConvergedAtUB') is True) and (attrvarvalue.get_suffix_value('Iter0FixIfConvergedAtUB') is not None):
                                        ub = attrvarvalue.get_suffix_value('Iter0FixIfConvergedAtUB')
                                    else:
                                        ub = self.Iter0FixIfConvergedAtUB

                                    if (attrvariable.suffix_defined('Iter0FixIfConvergedAtNB') is True) and (attrvarvalue.get_suffix_value('Iter0FixIfConvergedAtNB') is not None):
                                        nb = attrvarvalue.get_suffix_value('Iter0FixIfConvergedAtNB')
                                    else:
                            ...
                            if self.ComputeRhosWithSEP is True:
                                ...
                                cost_for_rho = reference_variable[index].get_suffix_value("CostForRho")
        ...
        for stage, tree_node, variable, index, is_fixed, is_stale in scenario_tree_node_variables_generator(ph._scenario_tree, ph._instances, includeLastStage=False):
            ...
            if (is_stale is False) and (is_fixed is False):
                ...
                if isinstance(variable.domain, IntegerSet) or isinstance(variable.domain, BooleanSet):
                    node_min = self.Int_If_Close_Enough(ph, value(tree_node._minimums[variable.name][index]))
                    node_max = self.Int_If_Close_Enough(ph, value(tree_node._maximums[variable.name][index]))
                    ...
                    attrvariable = ph._model_instance.active_components(Var)[variable.name]
                    attrvarvalue = attrvariable[index]

                    if (attrvariable.suffix_defined('FixWhenItersConvergedAtLB') is True) and (attrvarvalue.get_suffix_value('FixWhenItersConvergedAtLB') is not None):
                        lb = attrvarvalue.get_suffix_value('FixWhenItersConvergedAtLB')
                    else:
                        lb = self.FixWhenItersConvergedAtLB

                    if (attrvariable.suffix_defined('FixWhenItersConvergedAtUB') is True) and (attrvarvalue.get_suffix_value('FixWhenItersConvergedAtUB') is not None):
                        ub = attrvarvalue.get_suffix_value('FixWhenItersConvergedAtUB')
                    else:
                        ub = self.FixWhenItersConvergedAtUB

                    if (attrvariable.suffix_defined('FixWhenItersConvergedAtNB') is True) and (attrvarvalue.get_suffix_value('FixWhenItersConvergedAtNB') is not None):
                        nb = attrvarvalue.get_suffix_value('FixWhenItersConvergedAtNB')
                    else:
                    ...
                    node_min = value(tree_node._minimums[variable.name][index])
                    node_max = value(tree_node._maximums[variable.name][index])
...
#=========================
    def _pick_one_and_slam_it(self, ph):
        ...
        for vl in self.slam_list:
            ...
            if variable.suffix_defined("my_stage") is True:
                variable_stage = variable[full_index].get_suffix_value("my_stage")
