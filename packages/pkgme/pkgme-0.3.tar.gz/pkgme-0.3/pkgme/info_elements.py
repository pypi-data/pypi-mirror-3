...
class Description(InfoElement):
    ...
    def clean(cls, value):
        ...
        If ``value`` looks to already be formatted according to Debian policy,
        then we pass it through.  If it does not, then we do our best to clean
        it up.

        We determine whether ``value`` is already Debian-formatted by checking
        whether the second line is indented by a space. If so, then we assume
        it's Debian-formatted.  If later lines break Debian formatting, then
