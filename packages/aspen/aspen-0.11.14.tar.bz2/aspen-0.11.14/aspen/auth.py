...
def startup(website):
    ...
    anonymous_can_hit   the path below which anonymous can hit; empty for 
                         none, * for all. This is used for assets for 401.html.

    """
    global backend
    log.info('polaris.authentication.startup')
    ...
    if website.conf.polaris['ldap'] is polaris.MissedConnection:
        log.warning("ldap not configured, using stub authentication")
        backend = StubBackend()
    else:
