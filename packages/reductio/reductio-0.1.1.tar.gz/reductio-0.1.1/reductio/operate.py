...
class ShardFileStorage(object):
    ...
    def write_to_shard(self, key, value):
        """
    ...
    def do_transform(self, transformer):
        ...
        file-like object to write it to.
    ...
    def do_map(self, mapper):
        ...
        for line in self:
            ...
            for result_key, result_value in mapper(key, value):
                self.write_to_shard(result_key, result_value)
    ...
    def do_reduce(self, reducer):
        ...
        multiple stages. It just gives you an iterator and assumes you can handle
        things from there.
        ...
        for key, pairs in itertools.groupby(
            ...
            for out_key, out_value in reducer(key, values):
                self.write_to_shard(out_key, out_value)
...
def run(command, *args):
    name = 'do_'+command
    if command in ('scatter', 'sort', 'sort_unique'):
        if len(args) != 2:
            raise ValueError(
