...
class TrackingWizard:
    ...
    def redraw( self ):
        ...
        for wind_data in winds_to_show:
            ...
            if self.showing_image_type == params.SHOW_RAW_FRAME:
                ...
                img, fmt, line_segs, line_clrs, a,b,c,d,e = self.bg.get_sub_image( self.showing_image_type, wind_data['framenumber'] )
                self.img_size = img.shape
    ...
    def reset_frame_numbers( self ):
        ...
        for i in range( len( self.img_winds ) ):
            ...
            self.img_winds[i]['stext'].SetLabel( "frame %d"%framenumbers[i] )
