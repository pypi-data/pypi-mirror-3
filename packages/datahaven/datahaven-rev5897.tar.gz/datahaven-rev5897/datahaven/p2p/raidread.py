...
def raidread(OutputFileName, backupId, blockNumber, eccmapname):
    INTSIZE=settings.IntSize()
    ...
    while(MakingProgress==1):
        MakingProgress=0
        for PSegNum in range(0,myeccmap.paritysegments):
#                PFileName = os.path.join(settings.TempDir(), backupId + '-' + blockNumber + '-' + str(PSegNum) + '-Parity')
                PFileName = os.path.join(tmpfile.subdir('data-par'), backupId + '-' + blockNumber + '-' + str(PSegNum) + '-Parity')
                ...
                if os.path.exists(PFileName):
                        ...
                        for DSegNum in Map:
                                TotalDSegs+=1
#                                FileName = os.path.join(settings.TempDir(), backupId + '-' + blockNumber + '-' + str(DSegNum) + '-Data')
                                FileName = os.path.join(tmpfile.subdir('data-par'), backupId + '-' + blockNumber + '-' + str(DSegNum) + '-Data')
...
##                                print "We can rebuild one " + BadName + "\n"
    ...
    for DSegNum in range(0,myeccmap.datasegments):
#        FileName = os.path.join(settings.TempDir(), backupId + '-' + blockNumber + '-' + str(DSegNum) + '-Data')
        FileName = os.path.join(tmpfile.subdir('data-par'), backupId + '-' + blockNumber + '-' + str(DSegNum) + '-Data')
        if os.path.exists(FileName):
                GoodDSegs+=1
...
##    def test_ecc(self):
##        import raidmake 
##        raidmake.raidmake("../sample.tar", "ecc/64x64")
##        os.remove(settings.RaidDir() + "/dseg.8")
##        os.remove(settings.RaidDir() + "/dseg.9")
##        os.remove(settings.RaidDir() + "/dseg.10")
##        os.remove(settings.RaidDir() + "/dseg.11")
##        os.remove(settings.RaidDir() + "/dseg.12")
##        os.remove(settings.RaidDir() + "/pseg.13")
##        os.remove(settings.RaidDir() + "/pseg.14")
##        os.remove(settings.RaidDir() + "/pseg.15")
##        raidread("sample2.tar", "ecc/64x64")
##        print "Doing diff for 64x64 case"
##        os.system("diff ../sample.tar sample2.tar")
##
##        if False:
##                raidmake.raidmake("../sample.tar", "ecc/2x2")
##                os.remove(settings.RaidDir() + "/dseg.1")
##                raidread("sample3.tar", "ecc/2x2")
##                print "Doing diff for 2x2 case"
##                os.system("diff ../sample.tar sample3.tar")
