# -*- coding: utf-8 -*-
#------------------------------------------------------------------------------
# file: $Id: protocol.py 29 2012-06-30 22:52:55Z griff1n $
# lib:  pysyncml.protocol
...
class Protocol(object):
  ...
  def initialize(self, adapter, session, xsync=None):
    cmd = state.Command(
      cmd         = constants.CMD_SYNCHDR,
      cmdID       = 0,
      version     = constants.SYNCML_VERSION_1_2,
      source      = adapter.devID,
    ...
    if session.isServer:
      ...
      if adapter.peer is None or adapter.peer.devID != peerID:
        for peer in adapter.getKnownPeers():
          # TBD: i should delete unused peers here... ie. anything that
    ...
    if session.msgID == 1:
      ...
      # the pysyncml world where it is much more peer-oriented instead of
      # client/server, i send this as well... the idea being, if two "client"
      # peers are communicating in the event of server unavailability, then
      # they may need to know each-others limitations...
      cmd.maxMsgSize = sys.maxint
      cmd.maxObjSize = sys.maxint
    return [cmd]

  #----------------------------------------------------------------------------
  def negotiate(self, adapter, session, commands):

    # tbd: determine why i decided to copy the commands...
    commands = commands[:]

    if len(commands) > 0 and commands[-1].cmd == constants.CMD_FINAL:
      return commands

    if len(commands) > 0 \
       and commands[-1].cmd == constants.CMD_ALERT \
       and commands[-1].data == constants.STATUS_NEXT_MESSAGE:
      # tbd: should i add a "final" here?...
      # commands.append(state.Command(cmd=constants.CMD_FINAL))
      return commands
  ...
  def commands2tree(self, adapter, session, commands):
    ...
    if hdrcmd.cmd != constants.CMD_SYNCHDR:
      raise common.InternalError('unexpected first command "%s" (expected "%s")'
                                 % (hdrcmd.cmd, constants.CMD_SYNCHDR))
    ...
    xhdr  = ET.SubElement(xsync, hdrcmd.cmd)
    if hdrcmd.version == constants.SYNCML_VERSION_1_2:
    ...
    for cmdidx, cmd in enumerate(commands):
      ...
      if cmd.cmd == constants.CMD_STATUS:
      ...
      if cmd.cmd in [constants.CMD_GET, constants.CMD_PUT]:
      ...
      if cmd.cmd == constants.CMD_RESULTS:
        ET.SubElement(xcmd, 'MsgRef').text    = cmd.msgRef
        ET.SubElement(xcmd, 'CmdRef').text    = cmd.cmdRef
        ET.SubElement(ET.SubElement(xcmd, 'Meta'), 'Type',
                      {'xmlns': constants.NAMESPACE_METINF}).text = cmd.type
      ...
      if cmd.cmd == constants.CMD_FINAL:
        if cmdidx + 1 < len(commands):
          raise common.InternalError('command "%s" not at tail end of commands' % (cmd.cmd,))
        continue

      raise common.InternalError('unexpected command "%s"' % (cmd.cmd,))
  ...
  def tree2commands(self, adapter, session, lastcmds, xsync):
    ...
    # tbd: do i really want to be using assert statements?...
    ...
    try:
      ...
      if not session.isServer:
        raise
      # TBD: make this configurable as to whether or not any error
      #      is sent back to the peer as a SyncML "standardized" error
      #      status...
      log.exception('failed while interpreting command tree')
      ...
      return [
        ...
        state.Command(cmd=constants.CMD_FINAL)]
  ...
  def _tree2commands(self, adapter, session, lastcmds, xsync, ret):
    ...
    chkcmds = [e for e in lastcmds if e.cmd not in (constants.CMD_STATUS, constants.CMD_FINAL)]

    # for each "sync" command, search for sub-commands
    # tbd: should this be generalized to search for any sub-commands?...
    for chkcmd in chkcmds:
      if chkcmd.cmd != constants.CMD_SYNC:
        continue
      chkcmds.extend(chkcmd.data or [])

    for chkcmd in chkcmds:
      log.debug('outstanding command node "s%s.m%s.c%s.%s"',
                session.id, lastcmds[0].msgID, chkcmd.cmdID, chkcmd.cmd)

    # first, check all the 'Status' commands
    for child in xsync[1]:
      if child.tag != constants.CMD_STATUS:
        continue
      cmd = child.findtext('Cmd')

      log.debug('checking status node "s%s.m%s.c%s.%s"',
                session.id, child.findtext('MsgRef'), child.findtext('CmdRef'), cmd)
      ...
      # TBD: check for unknown elements...

      code = int(child.findtext('Data'))
      # tbd: any other common elements?...
      ...
      if cmd == constants.CMD_SYNCHDR:
        # assert adapter.peer.cleanUri(child.findtext('TargetRef')) == chkcmd.target
        # assert adapter.cleanUri(child.findtext('SourceRef')) == chkcmd.source
        if code not in [constants.STATUS_OK, constants.STATUS_AUTHENTICATION_ACCEPTED]:
          raise common.ProtocolError('unexpected status code %d for command "%s"' % (code, cmd))
        if code == constants.STATUS_AUTHENTICATION_ACCEPTED:
          session.authAccepted = True
        continue

      if cmd == constants.CMD_ALERT:
        # assert adapter.peer.cleanUri(child.findtext('TargetRef')) == chkcmd.target
        # assert adapter.cleanUri(child.findtext('SourceRef')) == chkcmd.source
        if code not in [constants.STATUS_OK]:
          raise common.ProtocolError('unexpected status code %d for command "%s"' % (code, cmd))
        # TBD: do something with the Item/Data/Anchor/Next...
        continue

      if cmd == constants.CMD_GET:
        # assert adapter.peer.cleanUri(child.findtext('TargetRef')) == chkcmd.target
        if code not in [constants.STATUS_OK]:
          raise common.ProtocolError('unexpected status code %d for command "%s"' % (code, cmd))
        continue

      if cmd == constants.CMD_PUT:
        # assert adapter.cleanUri(child.findtext('SourceRef')) == chkcmd.source
        if code not in [constants.STATUS_OK]:
          raise common.ProtocolError('unexpected status code %d for command "%s"' % (code, cmd))
        continue

      if cmd == constants.CMD_RESULTS:
        # assert adapter.cleanUri(child.findtext('SourceRef')) == chkcmd.source
        if code not in [constants.STATUS_OK]:
          raise common.ProtocolError('unexpected status code %d for command "%s"' % (code, cmd))
        continue

      if cmd == constants.CMD_SYNC:
        # assert adapter.peer.cleanUri(child.findtext('TargetRef')) == chkcmd.target
        # assert adapter.cleanUri(child.findtext('SourceRef')) == chkcmd.source
        if code not in [constants.STATUS_OK]:
          raise common.ProtocolError('unexpected status code %d for command "%s"' % (code, cmd))
      ...
      if cmd in [constants.CMD_ADD, constants.CMD_REPLACE, constants.CMD_DELETE]:
        data = child.findtext('Data')
        # TBD: check all valid values of ``data``...
        if cmd == constants.CMD_ADD:
          if data is not None and int(data) not in (constants.STATUS_ALREADY_EXISTS,):
            session.dsstates[chkcmd.uri].stats.peerAdd += 1
        elif cmd == constants.CMD_REPLACE:
          session.dsstates[chkcmd.uri].stats.peerMod += 1
        elif cmd == constants.CMD_DELETE:
          session.dsstates[chkcmd.uri].stats.peerDel += 1
        # assert adapter.cleanUri(child.findtext('SourceRef')) == chkcmd.source
        # tbd: anything else in common?...
        # tbd: trap errors...
        res = getattr(self, 't2c_status_' + cmd.lower())(adapter, session, chkcmd, xsync, child)
        ret.extend(res or [])
        continue

      if cmd == constants.CMD_MAP:
        # assert adapter.peer.cleanUri(child.findtext('TargetRef')) == chkcmd.target
        # assert adapter.cleanUri(child.findtext('SourceRef')) == chkcmd.source
      ...
      raise common.ProtocolError('unexpected status node for command "%s"' % (cmd,))

    # TBD: is this right?... or should i be getting pissed off and
  ...
  #----------------------------------------------------------------------------
  def t2c_status_add(self, adapter, session, chkcmd, xsync, xnode):
    cmd  = xnode.findtext('Cmd')
    code = int(xnode.findtext('Data'))
    if code not in [constants.STATUS_OK, constants.STATUS_ITEM_ADDED, constants.STATUS_ALREADY_EXISTS]:
      raise common.ProtocolError('unexpected status code %d for command "%s"' % (code, cmd))
    peerStore = adapter.peer.stores[adapter.router.getTargetUri(chkcmd.uri)]
    # tbd: also potentially check Change.updatedDate... although, in theory,
    #      this is an *ADD* and therefore can only happen once.
    adapter._context._model.Change.q(store_id=peerStore.id, itemID=chkcmd.source,
                                     state=constants.ITEM_ADDED).delete()

  #----------------------------------------------------------------------------
  def t2c_status_replace(self, adapter, session, chkcmd, xsync, xnode):
    cmd  = xnode.findtext('Cmd')
    code = int(xnode.findtext('Data'))
    if code not in [constants.STATUS_OK]:
      raise common.ProtocolError('unexpected status code %d for command "%s"' % (code, cmd))
    peerStore = adapter.peer.stores[adapter.router.getTargetUri(chkcmd.uri)]
    # tbd: also potentially check Change.updatedDate...
    adapter._context._model.Change.q(store_id=peerStore.id, itemID=chkcmd.source,
                                     state=constants.ITEM_MODIFIED).delete()

  #----------------------------------------------------------------------------
  def t2c_status_delete(self, adapter, session, chkcmd, xsync, xnode):
    cmd  = xnode.findtext('Cmd')
    code = int(xnode.findtext('Data'))
    if code not in [constants.STATUS_OK, constants.STATUS_ITEM_NOT_DELETED]:
      raise common.ProtocolError('unexpected status code %d for command "%s"' % (code, cmd))
    if code == constants.STATUS_ITEM_NOT_DELETED:
    ...
    # tbd: also potentially check Change.updatedDate...
    adapter._context._model.Change.q(store_id=peerStore.id, itemID=chkcmd.source,
                                     state=constants.ITEM_DELETED).delete()
  ...
  def t2c_put_devinf12(self, adapter, session, lastcmds, xsync, xnode):
    ...
    # TBD: THIS DESTROYS ANY BINDINGS!... I SHOULD DO A MERGE INSTEAD!...
    log.critical('TBD: destroying pre-existing bindings... need smarter merge!')
    adapter.peer._stores = []
    for store in stores:
  ...
  def t2c_alert(self, adapter, session, lastcmds, xsync, xnode):
    ...
    if code not in (
      ...
      # tbd: these should only be received out-of-band, right?...
    ...
    # TBD: this should really be done by the synchronizer... as it can
    ...
    if session.isServer:
      if uri in session.dsstates:
        ...
        ds = common.adict(
          # TBD: perhaps share this "constructor" with router/adapter?...
          peerUri    = ruri,
    ...
    # TBD: verify that the peer's lastAnchor matches what we have in the
    #      database, otherwise trigger a slow-sync...
    ...
    return [state.Command(
      ...
      statusOf   = xnode.tag,
      statusCode = constants.STATUS_OK,
      # tbd: syncevolution does not echo the remote last anchor... why not?
    ...
    if not session.isServer:
      if ds.action != 'recv':
        raise common.ProtocolError('unexpected sync state for URI "%s": action=%s'
                                   % (uri, ds.action))
      # tbd: is there really nothing else to do?...
      ds.action = 'done'
    else:
      if ds.action != 'alert':
        raise common.ProtocolError('unexpected sync state for URI "%s": action=%s'
  ...
  def t2c_xnode2item(self, adapter, session, lastcmds, store, xsync, xnode):
    ...
    # tbd: can the version be specified in the Meta tag?... maybe create an
  ...
  def t2c_map(self, adapter, session, lastcmds, xsync, xnode):
    # TBD: should this be moved into the synchronizer?...
    ...
    # tbd: should i verify that the GUID is valid?...
    for xitem in xnode.findall('MapItem'):
      luid = xitem.findtext('Source/LocURI')
      guid = xitem.findtext('Target/LocURI')
      # TBD: is there a better way of doing this than DELETE + INSERT?...
...
#------------------------------------------------------------------------------
# end of $Id: protocol.py 29 2012-06-30 22:52:55Z griff1n $
#------------------------------------------------------------------------------
