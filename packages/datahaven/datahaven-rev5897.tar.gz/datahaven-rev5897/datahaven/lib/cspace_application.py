...
def init_log(logLevel=0, logFilename='cspace.log', logWriteMode='w'):
    ...
    if logFilename == 'dhnio':
        logging.basicConfig(level=logLevel,
                            stream=CSpace_stdout(),
                            format='%(name)30s    %(levelname)5s   %(message)s', #   [%(threadName)s]',
                            )
    else:
        logging.basicConfig(level=logLevel,
                            filename=logFilename,
                            filemode=logWriteMode,
                            format='%(name)30s    %(levelname)5s   %(message)s', #   [%(threadName)s]',
                            )
    _AppLogger = logging.getLogger('cspace.application')
...
class FileSender(object):
    ...
    def __init__(self, reactor, stream, peer):
        ...
        self.state = 'ready'
        self.doWaitNewFiles()
        #print 'FileSender.__init__ peer=%s' % self.peer

    def newFile(self, filename):
        self.files2send.append(filename)
    ...
    def doWaitNewFiles(self):
        ...
        if len(self.files2send) == 0:
            ...
            return
        self.filename = self.files2send.pop(0)
        try:
    ...
    def doSend(self):
        ...
        if not chunk:
            #print 'FileSender.doSend all data for %s were read' % self.peer
            self.close_file()
            self.state = 'waiting'
...
#        global _SendingStreams
        self.reactor.callLater(0.1, self.ProcessSending)
...
#        self.doSend(contactName)
    ...
    def onStarted(self):
        ...
        if self.service.online():
            ...
            _AppLogger.info('all contacts was removed')
#        global _StatusNotifyFunc
