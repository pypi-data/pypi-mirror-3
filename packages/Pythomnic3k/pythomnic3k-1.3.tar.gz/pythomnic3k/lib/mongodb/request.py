...
#
# Pythomnic3k project
# (c) 2005-2011, Dmitry Dvoinikov <dmitry@targeted.org>
...
if __name__ == "__main__":
    ...
    assert str(op) == "OP_UPDATE foo.bar SET {} WHERE {}"
    assert op.serialize() == \
    ...
    assert str(op) == "OP_UPDATE foo.bar SET {k: 'foo'} WHERE {v: 'bar'} WITH UPSERT, MULTI_UPDATE"
    assert op.serialize() == \
    ...
    assert str(op) == "OP_INSERT INTO foo.bar VALUES ({})"
    assert op.serialize() == \
    ...
    op = OP_QUERY("foo.bar", {})
    assert str(op) == "OP_QUERY * FROM foo.bar WHERE {}"
    assert op.serialize() == \
    ...
    assert str(op) == "OP_GET_MORE FROM foo.bar USING CURSOR 0x0000000000000000"
    assert op.serialize() == \
    ...
    assert str(op) == "OP_GET_MORE FROM foo.bar USING CURSOR 0x1122334455667788 RETURN 1"
    assert op.serialize() == \
    ...
    assert str(op) == "OP_DELETE FROM foo.bar WHERE {}"
    assert op.serialize() == \
    ...
    assert str(op) == "OP_DELETE FROM foo.bar WHERE {k: 'v'} WITH SINGLE_REMOVE"
    assert op.serialize() == \
    ...
    op = OP_KILL_CURSORS([])
    assert str(op) == "OP_KILL_CURSORS"
    assert op.serialize() == \
        b"\x18\x00\x00\x00\n\x00\x00\x00\x00\x00\x00\x00\xd7\x07\x00\x00" \
        b"\x00\x00\x00\x00\x00\x00\x00\x00"

    op = OP_KILL_CURSORS([0, 0x7fffffffffffffff])
    assert str(op) == "OP_KILL_CURSORS 0x0000000000000000, 0x7fffffffffffffff"
