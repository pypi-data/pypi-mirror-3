# $Id: file.py f6a06566f45f 2012/07/31 22:58:36 patrick $
...
from ..lib.utils import _, EXCLUDED_FILES, MIME_TYPES, TEXT_TYPES
...
class FileView(object):
    ...
    def search(self):
        ...
        return {
            'form': form, 'scope': scope, 'MIME_TYPES': MIME_TYPES,
            'results': results}
    ...
    def info(self):
        ...
        log = handler.vcs.log(
            path, filename, int(self._request.session.get('page_size', 20)))
        ...
        if action[0:4] == 'shw!':
            ...
            if content is None:
                ...
                content = {
                    'route': 'file_revision', 'revision': action,
        ...
        return {
            ...
            'TEXT_TYPES': TEXT_TYPES, 'MIME_TYPES': MIME_TYPES, 'log': log,
    ...
    def _search_storage_by_filename(cls, root, files):
        ...
        results = []
        files = files.split()
        cmd = ['nice', 'find', root, '-type', 'f', '-name', files[0]]
        for item in files[1:]:
            cmd += ['-or', '-name', item]
