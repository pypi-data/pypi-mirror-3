...
def hgtime2datetime(hgdate):
    hgtime, hgtz = hgdate
    dateinfo = list(time.gmtime(float(hgtime) - hgtz)[:-2]) + [utc]
    return datetime.datetime(*dateinfo)
...
class TicketUpdater(object):
    ...
    def compose_msg(self, changectx):
        styles = {
        ...
        try:
            ...
            self.ui.warn('Unsupported changeset style: "%s". Currently '
                         ...
                         'Using default (short-hex)' % self.changeset_style)
    ...
    def __call__(self, changectx):
        msg = self.compose_msg(changectx)
        ...
        author = unicode(changectx.user(), 'utf-8')
        date = hgtime2datetime(changectx.date())
