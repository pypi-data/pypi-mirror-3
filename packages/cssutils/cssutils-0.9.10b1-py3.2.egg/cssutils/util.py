...
class _Namespaces(object):
    ...
    def __delitem__(self, prefix):
        ...
        for i, rule in enumerate([r for r in self.parentStyleSheet.cssRules if r.type == r.NAMESPACE_RULE]):
            if rule == delrule:
    ...
    def __findrule(self, prefix):
        ...
        for rule in [r for r in reversed(self.parentStyleSheet.cssRules) if r.type == r.NAMESPACE_RULE]:
