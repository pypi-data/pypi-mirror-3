...
class GetPackageFilesTests(TestCase):
    ...
    def test_ignores_metadata(self):
        path = self.make_tree([MetadataBackend.METADATA_FILE, 'a', 'b'])
        self.assertEqual(set(['a', 'b']), get_package_files(path, {}))

    def test_ignores_debian(self):
        path = self.make_tree(['debian/', 'a', 'b'])
        self.assertEqual(set(['a', 'b']), get_package_files(path, {}))

    def test_ignores_icons(self):
        metadata = {'icons': {'48x48': 'foo.png'}}
        path = self.make_tree(['foo.png', 'a', 'b'])
        self.assertEqual(set(['a', 'b']), get_package_files(path, metadata))
