...
def WeightedMeanFrame(x,wx=None):
    ...
    for i in range(len(nx)):
        numx = numpy.add(numx,numpy.multiply(nx[i],numpy.power(nwx[i],-2)))
        denw = numpy.add(denw,numpy.power(nwx[i],-2))
    # mean
    ...
    #
    for i in range(len(nx)):
        numx = numpy.add(numx,numpy.power(numpy.subtract(nx[i],wa),2))
    if len(nx) > 1:
