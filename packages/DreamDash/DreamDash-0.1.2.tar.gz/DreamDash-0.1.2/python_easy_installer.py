...
conn = httplib.HTTPConnection("http://ginis.dreamdash.net")
conn.request("POST", "/api/login", postdata, headers)
response = conn.getresponse()
print response.status, response.reason 
if response.reason == '200 OK':
   print "Invalid Data"
else:
...
database=raw_input("datasource_database")

print "Enter the Host:"

host=raw_input("datasource_host")

print "Enter Server name:"

server_username=raw_input("datasource_server_username")

print "Enter Server password:"

server_password = get_value('datasource_server_password', is_password=True)

print "Enter the Database name:"

database_name=raw_input("datasource_database_name")


print "Enter the Database User:"

user=raw_input("datasource_user")

print "Enter the Database Password:"

password = get_value('datasource_password', is_password=True)

print "Enter the Database port:"

database_port=raw_input("datasource_database_port")

postdata_second={}
postdata_second["database"]=database
postdata_second["host"]=host
postdata_second["server_username"]=server_username
postdata_second["server_password"]=server_password
postdata_second["database_name"]=database_name
postdata_second["user"]=user
postdata_second["password"]=password
postdata_second["database_port"]=database_port
...
headers = {"Content-type": "application/x-www-form-urlencoded", "Accept": "text/plain"}
conn1 = httplib.HTTPConnection("http://ginis.dreamdash.net")
conn1.request("POST", "/api/get_dbdetails", params, headers)
