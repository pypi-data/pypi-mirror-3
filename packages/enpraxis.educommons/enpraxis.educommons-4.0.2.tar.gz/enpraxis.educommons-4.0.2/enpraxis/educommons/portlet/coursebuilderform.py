...
def schoolsvocab(context):
    ...
    return SimpleVocabulary([SimpleVocabulary.createTerm(x.getPath(), x.getPath(), x.Title) for x in brains])
    ...
    def buildCourse(self, context):
        ...
        if type(templates) == type(u''):
            ...
            for x in self.request.form['form.templates']:
                self.createObject(course, 'Document', self.templates[x], x, count)
                count += 1
