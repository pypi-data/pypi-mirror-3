...
/**
 * A background model for a particular motif width, W
 */
struct bg_model
{
...
struct likelihood_calculator
{
	...
	calculate( Data & _data ) {
		...
		visit_node( _data, static_cast< Derived * >( this )->index_root( _data ) );
	}
	...
	>
	double
	visit_node( Data & _data, It it ) {
		using namespace seqan;

		MS_DEBUG_STRING_VAR( w_mer, representative( it ) );

		static_cast< Derived * >( this )->entered_node( it );
		...
		const size_t w = std::min( _data.get_rep_length( it ), likelihoods.W );
        ...
        static_cast< Derived * >( this )->left_node( it );
...
: likelihood_calculator< markov_wmer_likelihoods_calculator< MarkovModel > >
{
    ...
    /** The iterator used for the descent. */
    template< typename Data >
    typename Data::top_down_history_it
    index_root( Data & _data ) {
        return typename Data::top_down_history_it( _data.index );
    }
    ...
    entered_node( It it ) {
        ...
        const size_t offset = repLength( it ) - parentEdgeLength( it );

        // resize evaluations
        MYRRH_ASSERT( current_evals.size() >= offset );
        current_evals.resize( offset );
    ...
    log_likelihoods_for_w_mer( It it ) {
        using namespace seqan;
        MS_DEBUG_STRING_VAR( w_mer, representative( it ) );
        MYRRH_ASSERT( W == current_evals.size() );
