# $Id: project.py 23bf3c561736 2012/05/30 19:32:09 patrick $
...
from ..lib.utils import _, has_permission, camel_case, normalize_spaces
...
class ProjectView(object):
    ...
    def view(self):
        ...
        members = DBSession.query(
            ...
            .filter(ProjectUser.perm != None).order_by(User.name)
    ...
    def results(self):
        ...
        if build_list:
            ...
            for k in processings:
                if processings[k][1] and '${user}' in processings[k][1]:
                    if login is None:
                        login = DBSession.query(User.login).filter_by(
                          user_id=self._request.session['user_id']).first()[0]
                    processings[k] = (processings[k][0], processings[k][1]
                        .replace('${user}', camel_case(login)))
            packs = [k['pack_id'] for k in build_list]
            packs = dict(DBSession.query(
                Pack.pack_id, Pack.label)
                         .filter_by(project_id=project['project_id'])
    ...
    def _action(self, project):
        ...
        :param processing: (:class:`~.models.projects.Project` instance)
