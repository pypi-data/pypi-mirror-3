...
class _BaseSet(Component):
    ...
    def _verify(self,element,use_exception=True):
        ...
        #print "HERE VERIFYING",element,self.validate,self.domain,self.model
        ...
        if self.validate is not None:
            if self.model is not None:
                flag = apply_indexed_rule(self, self.validate, self.model(), element)
            else:
...
class _SetContainer(_BaseSet):
    ...
    def construct(self, values=None):
        ...
        #
            ...
            if self.initialize.func_code.co_argcount == 1:
                ...
                #
                ctr=1
                val = apply_indexed_rule(self, self.initialize, self.model(), ctr)
                if val is None:
    ...
    def _apply_filter(self):
        ...
        for el in self.value:
            flag = apply_indexed_rule(self, self.filter, self.model(), el)
            if not flag:
                toRemove.append(el)
        for el in toRemove:
            self.remove(el)
        ...
        #
            ...
            for key in self._index:
                ...
                if self.initialize.func_code.co_argcount == len(tmp)+1:
                    rule_list = apply_indexed_rule(self, self.initialize, self.model(), tmp)
                    for val in rule_list:
                        self.value[key].add( val )
                else:
                    ctr=1
                    val = apply_parameterized_indexed_rule(self, self.initialize, self.model(), ctr, tmp)
                    if val is None:
                        #logger.warning("DEPRECATION WARNING: Set rule returned None instead of Set.Skip")
                        raise ValueError, "Set rule returned None instead of Set.Skip"
