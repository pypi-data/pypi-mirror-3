...
from model import Document, DocumentModel
...
Notice that the found document doesn't have a *UUID*, *created_at* or *updated_at* keys. This is because it was never loaded into SQlite.
  ...
  def find(self, *filters):
    ...
    for _filter in filters:
      ...
      # if type(_filter) == Document or type(_filter) == DocumentModel:
        result = filter_(result, _filter._name, _filter._predicate)
      elif callable(_filter):
        result = _filter(result)
      else:
        # print _filter, type(_filter), type(DocumentModel), 
...
.. code-block:: python
  ...
  #there are two ways to create a new collection / verse
  members = uv.members
...
A Verse or collection uses the same methodology as SubVerse to group data
