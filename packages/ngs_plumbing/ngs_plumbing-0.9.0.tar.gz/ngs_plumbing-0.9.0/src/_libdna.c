...
"Takes a bytes object with 2-bit packed DNA values, a slice in the DNA space,"
" and returns a new bytes object with 2-bit packed DNA values."
" As with other Python slices the last element is not included.");
...
static PyObject* slicebit2_frombit2bytes(PyObject *self, PyObject *args)
{
  ...
  if (ihigh < ilow) {
    PyErr_Format(PyExc_IndexError, 
		 "Lowest index must be lower that the highest.");
    return NULL;
  }
  ...
  SliceBoundaries slbo;
  make_sliceboundaries(ilow, ihigh, &slbo);

  if (slbo.ihigh_packed > n) {
    PyErr_Format(PyExc_IndexError, 
		 "Highest limit for the interval out of bound "
		 "(requesting %zd while length is %zd)", slbo.ihigh_packed, n);
    return NULL;
  }
  ...
  Py_ssize_t n_slice = (ihigh - ilow) / NGSP_WORD_SIZE;
  if (((ihigh - ilow) % NGSP_WORD_SIZE) > 0)
    n_slice++;
  char *string = NULL;
  PyObject *slice_bytes = PyBytes_FromStringAndSize(string, n_slice);
  unsigned char *slice_char = PyBytes_AS_STRING(slice_bytes);
  packed_char = PyBytes_AS_STRING(packed_bytes);
  Py_ssize_t i;
  for (i = 0; i < n_slice; i++) {
    slice_char[i] = (char)0; 
  }
  unsigned short remainder_src, remainder_dest, shift_src, shift_dest;
  Py_ssize_t i_src, i_dest;
  unsigned char src;
  /* printf("%i\n", packed_char[i / NGSP_WORD_SIZE]); */
  for (i = ilow; i < ihigh; i++) {
    /* printf("%i\n", i); */
    remainder_src = i % NGSP_WORD_SIZE;
    remainder_dest = (i-ilow) % NGSP_WORD_SIZE;
    i_src = i / NGSP_WORD_SIZE;
    i_dest = (i-ilow) / NGSP_WORD_SIZE;
    shift_src = (NGSP_WORD_SIZE - remainder_src - 1) * 2;
    shift_dest = (NGSP_WORD_SIZE - remainder_dest - 1) * 2;
    src = (unsigned char)((packed_char[i_src] & (0x03 << shift_src)) >> shift_src);
    slice_char[i_dest] |= (src << shift_dest);
    /* printf("src: i=%i - reste=%i - shift=%i - value=%i\n", */
    /* 	   i_src, remainder_src, shift_src, src); */
    /* printf("dest: i=%i - reste=%i - shift=%i - new=%i - value=%i\n", */
    /* 	   i_dest, remainder_dest, shift_dest, (src << shift_dest), slice_char[i_dest]); */
  }
  return slice_bytes;
}
...
static PyObject* slice_frombit2bytes(PyObject *self, PyObject *args)
{
  ...
  char *string = NULL;
  PyObject *buf_bytes = PyBytes_FromStringAndSize(string, n_up);
  char *unpacked = PyBytes_AS_STRING(buf_bytes);
  ...
  char *packed = PyBytes_AS_STRING(packed_bytes);
#ifdef LIBDNA_DEBUG
  printf("-->(%zd, %zd) low: offest %i, pad: %zd, ilow_packed:%zd\n", 
	 ilow, ihigh, slbo.low_offset, slbo.low_n_padbytes, 
	 slbo.ilow_packed, slbo.ihigh_packed);
  printf("-->(%zd, %zd) high: offest %i, pad: %zd, ihigh_packed:%zd\n", 
	 ilow, ihigh, slbo.high_offset, slbo.high_n_padbytes, 
	 slbo.ihigh_packed);
#endif
  if ( slbo.ilow_packed + slbo.low_offset < slbo.ihigh_packed ) {
#if defined(LIBDNA_DEBUG)
    printf(" packed[%zd], length = %zd\n", 
	   slbo.ilow_packed + slbo.low_offset, 
	   (slbo.ihigh_packed - slbo.ilow_packed - slbo.low_offset - slbo.high_offset));
#endif
    bit2_to_bytes(&(packed[slbo.ilow_packed + slbo.low_offset]),
		  (slbo.ihigh_packed - slbo.ilow_packed - slbo.low_offset - slbo.high_offset),
		  &(unpacked[slbo.low_n_padbytes]), 
		  ihigh - ilow);
  }
  ...
  if (slbo.low_offset) {
    //int foo = ((ihigh_packed - ilow_packed) > 1)?1:0;
    int foo = 0;
    bit2_to_bytes(&(packed[slbo.ilow_packed - foo]), 1,
		  (char *)(&padbytes), NGSP_WORD_SIZE);
    for (k = 0; k < slbo.low_n_padbytes; k++) {
#ifdef LIBDNA_DEBUG
      printf("  --> %i : padbytes[%i] = %c\n", k, 
	     NGSP_WORD_SIZE - slbo.low_n_padbytes + k, 
	     padbytes[NGSP_WORD_SIZE - slbo.low_n_padbytes + k]);//;k + NGSP_WORD_SIZE - low_n_padbytes);
#endif
      unpacked[k] = padbytes[NGSP_WORD_SIZE - slbo.low_n_padbytes + k];
    }
#ifdef LIBDNA_DEBUG
  printf("post-low-fix\n-");
  for (k = 0; k < n_up; k++) {
  ...
  if (slbo.high_offset) {
    bit2_to_bytes(&(packed[slbo.ihigh_packed-1]), 1, // 10/3
		  (char *)(&padbytes), NGSP_WORD_SIZE);
    for (k = 0; k < slbo.high_n_padbytes; k++) {
#ifdef LIBDNA_DEBUG
      printf("  --> %i : padbytes[%i] = %c -> %zd\n", k, 
	     k, 
	     padbytes[k], ihigh - ilow - slbo.high_n_padbytes + k);//;k + NGSP_WORD_SIZE - low_n_padbytes);
#endif
      unpacked[ihigh - ilow - slbo.high_n_padbytes + k] = padbytes[k];
    }

#ifdef LIBDNA_DEBUG
  printf("post-high-fix\n-");
  for (k = 0; k < n_up; k++) {
    printf("%c", unpacked[k]);
  }
  printf("-\n");
