# $Id: upload.py,v 1.76 2012/07/31 09:57:36 irees Exp $
import Queue
import collections
import datetime
import functools
...
class BaseTransport(QtGui.QMainWindow):
    ...
    # Files
    signal_set_path = QtCore.pyqtSignal(unicode)
...
class BaseUpload(BaseTransport):
    ...
    @QtCore.pyqtSlot(unicode)
    def check_path(self, path, onlynew=False):
        ...
        for f in handler.poll():
            emdash.log.msg("Found", f)
            jsonfile = '%s.json'%f
            data = {}
            if os.path.exists(jsonfile):
                data = json.load(file(jsonfile,"r")) or {}
                emdash.log.msg("... already uploaded to record:", data.get('name'))
    ...
    def update_path(self, path):
        ...
        self.signal_set_path.emit(unicode(path))
...
__version__ = "$Revision: 1.76 $".split(":")[1][:-1].strip()
