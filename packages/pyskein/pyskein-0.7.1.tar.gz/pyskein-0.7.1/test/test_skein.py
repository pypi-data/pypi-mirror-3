...
class TestSkeinBase:
    ...
    def testMacAlias(self):
        # conflict if given both 'key' and 'mac'
        self.assertRaises(TypeError, self.HASHER,
                          b"foo", key=b"bar1", mac=b"bar2")
        # test same result and deprecation
        with warnings.catch_warnings(record=True) as w:
            warnings.simplefilter("always")
            self.assertEqual(self.HASHER(b"foo", key=b"bar").digest(),
                             self.HASHER(b"foo", mac=b"bar").digest())
