...
class ExtDirectProtocol(object):
    ...
    def extract_path(self, req):
        ...
        req._wsme_extdirect_data = data

        return path

    def read_arguments(self, funcdef, req):
        ...
        args = req._wsme_extdirect_data[0]['data']
    ...
    def encode_result(self, funcdef, result):
        return json.dumps({
            ...
            'action': '',
            'method': '',
            'result': tojson(funcdef.return_type, result)
        })
    ...
    def api(self):
        namespaces = {}
        for funcdef in self.root.getapi():
            if len(funcdef.path) > 0:
                namespace = '.'.join(funcdef.path[:-1])
                action = funcdef.path[-1]
            else:
                namespace = ''
                action = ''
