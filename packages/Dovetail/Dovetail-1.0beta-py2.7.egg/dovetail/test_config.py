...
from dovetail.util.logging import LEVEL
...
class TestConfig(object):
    ...
    def test_logger(self):
        ...
        assert option is not None
        assert option.map("debug") is LEVEL.DEBUG
        assert option.map("iNfO")  is LEVEL.INFO
        assert option.map("major") is LEVEL.MAJOR
        assert option.map("WARN")  is LEVEL.WARN
        assert option.map("ERROR") is LEVEL.ERROR
