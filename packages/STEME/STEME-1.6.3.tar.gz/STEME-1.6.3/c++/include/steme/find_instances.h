...
: descender_preferred_order< find_instances< Spec >, Spec >
{
    ...
    typedef std::vector< w_mer_evaluation >                               instances_vec;     ///< Vector of instances.
    typedef boost::shared_ptr< instances_vec >                            instances_vec_ptr; ///< Pointer to instances.
    ...
    , lZtt( std::log( Z_threshold / ( 1 - Z_threshold ) ) )
    {
        if( Z_threshold < 0 ) {
    ...
    handle_W_mer_occurrence_one_strand(
        ...
        if( Z_strand >= Z_threshold ) {
            instances->push_back( w_mer_evaluation( Z_strand, global_pos, RevComp ) );
        }
    }
