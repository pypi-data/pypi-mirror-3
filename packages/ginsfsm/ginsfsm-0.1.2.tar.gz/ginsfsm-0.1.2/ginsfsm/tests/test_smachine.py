...
class TestEmptySMachine(unittest.TestCase):
    ...
    def test_set_trace_mach(self):
        prev = self.fsm.set_trace_mach(True)
        self.assertEqual(prev, False)
        prev = self.fsm.set_trace_mach(False)
        self.assertEqual(prev, False) # 1rst set_trace_match fails by mising log

        prev = self.fsm.set_trace_mach(True, pprint)
        self.assertEqual(prev, False)
        prev = self.fsm.set_trace_mach(False)
...
FSM_TEST1 = {
    ...
    'state_list': ('ST_STATE1', 'ST_STATEXX'),
    'machine': {
        'ST_STATE1':
        (
...
FSM_TEST9 = {
    'output_list': ('EV_TEST3',),
    'event_list': ('EV_TEST1', 'EV_TEST2'),
    'state_list': ('ST_STATE1', 'ST_STATE2'),
    'machine': {
        'ST_STATE1':
        (
...
class Test9SMachineState(unittest.TestCase):
    ...
    def test_event_list(self):
        self.assertEqual(self.fsm._event_index.get('EV_TEST1'), 0)
        self.assertEqual(self.fsm._event_index.get('EV_TEST2'), 1)
        input_event_list = self.fsm.get_input_event_list()
        self.assertEqual(input_event_list, list(FSM_TEST9['event_list']))
        output_event_list = self.fsm.get_output_event_list()
        self.assertEqual(output_event_list, list(FSM_TEST9['output_list']))

    def test_state_list(self):
        self.assertEqual(self.fsm._state_index.get('ST_STATE1'), 0)
        self.assertEqual(self.fsm._state_index.get('ST_STATE2'), 1)
