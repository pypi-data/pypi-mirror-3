...
def verify_password(password, hash_encoded_password):
    ...
    * ``hashed_password`` the result of applying the hash function ``digestmod``
    ...
    if len(method) == 0:
        ...
        return b_hashed_password == pseudo_pbkdf2(b_password, b_salt, iter, HASHES[digestmod])
    elif method[0] == 'pbkdf2':
        return b_hashed_password == pbkdf2(b_password, b_salt, iter, KEY_LENGTH, HASHES[digestmod])
    else:
