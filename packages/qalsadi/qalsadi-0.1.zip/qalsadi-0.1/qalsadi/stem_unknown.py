...
class unknownStemmer:
	...
	def __init__(self, debug=False):
		...
		self.compStemmer.set_infix_letters(stem_unknown_const.COMP_INFIX_LETTERS);
		self.compStemmer.set_prefix_letters(stem_unknown_const.COMP_PREFIX_LETTERS);
		self.compStemmer.set_suffix_letters(stem_unknown_const.COMP_SUFFIX_LETTERS);
		self.compStemmer.set_max_prefix_length(stem_unknown_const.COMP_MAX_PREFIX);
		self.compStemmer.set_max_suffix_length(stem_unknown_const.COMP_MAX_SUFFIX);
		self.compStemmer.set_min_stem_length(stem_unknown_const.COMP_MIN_STEM);
		self.compStemmer.set_prefix_list(stem_unknown_const.COMP_PREFIX_LIST);
		self.compStemmer.set_suffix_list(stem_unknown_const.COMP_SUFFIX_LIST);
		...
		# configure the stemmer object
		self.conjStemmer.set_infix_letters(stem_unknown_const.CONJ_INFIX_LETTERS);
		self.conjStemmer.set_prefix_letters(stem_unknown_const.CONJ_PREFIX_LETTERS);
		self.conjStemmer.set_suffix_letters(stem_unknown_const.CONJ_SUFFIX_LETTERS);
		self.conjStemmer.set_max_prefix_length(stem_unknown_const.CONJ_MAX_PREFIX);
		self.conjStemmer.set_max_suffix_length(stem_unknown_const.CONJ_MAX_SUFFIX);
		self.conjStemmer.set_min_stem_length(stem_unknown_const.CONJ_MIN_STEM);
		self.conjStemmer.set_prefix_list(stem_unknown_const.CONJ_PREFIX_LIST);
		self.conjStemmer.set_suffix_list(stem_unknown_const.CONJ_SUFFIX_LIST);
		# noun dictionary
...
#			noun_list.append(HAMZA+ALEF+noun[1:])
		...
		for noun in noun_list:
			...
			list_seg_comp=self.verify_affix(noun,list_seg_comp,stem_unknown_const.COMP_NOUN_AFFIXES);
	...
	def steming_second_level(self,noun,noun2,procletic,encletic):
		...
		list_seg_conj=self.verify_affix(noun2,list_seg_conj,stem_unknown_const.NOMINAL_CONJUGATION_AFFIX);
		...
		for seg_conj in list_seg_conj:
			...
			for vocalized_suffix in stem_unknown_const.CONJ_SUFFIX_LIST_TAGS[suffix_conj]['vocalized']:
		...
		for seg_conj in list_seg_conj:
			...
			has_plural_suffix=((u"جمع" in stem_unknown_const.CONJ_SUFFIX_LIST_TAGS[suffix_conj]['tags']) or( u"مثنى" in stem_unknown_const.CONJ_SUFFIX_LIST_TAGS[suffix_conj]['tags']))
...
##							print ' *****','-'.join(listsingle).encode('utf8')
			...
			for id in infnoun_form_list:
				...
				'tags':u':'.join(noun_tags+stem_unknown_const.COMP_PREFIX_LIST_TAGS[procletic]['tags']+stem_unknown_const.COMP_SUFFIX_LIST_TAGS[encletic]['tags']+stem_unknown_const.CONJ_SUFFIX_LIST_TAGS[suffix_conj]['tags']),
	...
	def is_compatible_proaffix_affix(self,procletic, encletic, suffix):
		...
		procletic_tags=stem_unknown_const.COMP_PREFIX_LIST_TAGS[procletic]['tags'];
		encletic_tags=stem_unknown_const.COMP_SUFFIX_LIST_TAGS[encletic]['tags'];
		#prefix_tags=CONJ_PREFIX_LIST_TAGS[procletic]['tags'];
		suffix_tags=stem_unknown_const.CONJ_SUFFIX_LIST_TAGS[suffix]['tags'];		
	...
	def vocalize(self,noun, proclitic,prefix,suffix,enclitic):
		...
		@rtype: unicode.
		"""
		enclitic_voc=stem_unknown_const.COMP_SUFFIX_LIST_TAGS[enclitic]["vocalized"][0];
		proclitic_voc=stem_unknown_const.COMP_PREFIX_LIST_TAGS[proclitic]["vocalized"][0];
