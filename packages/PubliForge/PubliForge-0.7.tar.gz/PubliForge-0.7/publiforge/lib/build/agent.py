# $Id: agent.py 3fa7b8bd944e 2012/09/06 17:23:18 patrick $
...
class AgentBuildManager(object):
    ...
    def __init__(self, settings):
        ...
        self._processors = [
            ...
            tuple(settings.get('processor.list', '').split())]
...
class AgentBuild(object):
    ...
    def start(self):
        ...
        if self._build_manager.develop and exists(path):
            shutil.rmtree(path)
        if not exists(path):
            makedirs(path)
    ...
    def _thread_engine(self):
        ...
        # Launch engine
        if not self.stopped():
            self.log(_('start engine'), 'a_build', 1)
