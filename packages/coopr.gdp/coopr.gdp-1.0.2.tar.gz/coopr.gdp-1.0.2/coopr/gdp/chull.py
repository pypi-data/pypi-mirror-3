...
class ConvexHull_Transforamtion(Plugin):
    ...
    def _transform_disjunct(self, disjunct, instance, disaggregatedVars):
        ...
        fullName = ''
        d = disjunct
        while d.parent is not None and d.parent() is not None:
    ...
    def _xform_constraint(self, _name, constraint, varMap, disjunct, instance):
        for cname, c in constraint._data.iteritems():
            ...
            if (c.lower_ineq_strict and c.lower is not None) or \
               (c.upper_ineq_strict and c.upper is not None):
                raise GDP_Error, 'GDP(cHull) disallows strict inequality '\
                      'constraints (found at ' + name + ').'
