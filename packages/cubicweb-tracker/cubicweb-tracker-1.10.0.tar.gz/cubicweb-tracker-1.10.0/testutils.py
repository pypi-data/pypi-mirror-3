...
def security_tc_init_project(tc, name, user_prefix=None):
    req = tc.request()
    project = req.execute(*create_project_rql(name)).get_entity(0, 0)
    if user_prefix is None:
        user_prefix = name
    for perm in (u'developer', u'client'):
        group = "%s%ss" % (name, perm)
        req.create_entity('CWGroup', name=group)
        tc.grant_permission(tc.session, project, group, perm)
        tc.create_user(req, '%s%s' % (user_prefix, perm), groups=('users', group,))
    return project
...
class SecurityTC(TrackerTCMixIn, CubicWebTC):
    ...
    def setUp(self):
        ...
        self.cubicweb = security_tc_init_project(self, 'cubicweb', user_prefix='prj1')
        self.project2 = security_tc_init_project(self, 'project2', user_prefix='prj2')
