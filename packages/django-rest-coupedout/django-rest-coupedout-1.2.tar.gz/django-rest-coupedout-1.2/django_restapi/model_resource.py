...
from django.forms import ModelForm, models, ValidationError
...
class Collection(ResourceBase):
    ...
    def create(self, request, **kwargs):
        ...
        ResourceForm = models.modelform_factory(self.queryset.model, form=self.form_class, exclude=['coupons'])
        #try:
        data = self.receiver.get_post_data(request)
        #except ValidationError, e:
        #    error_dict = ErrorDict()
        #    error_list = ErrorList()
        #    error_list.append('You have passed data of an invalid format:' + e.message)
        #    error_dict['general'] = error_list
        #    return self.responder.error(request,400,error_dict)
