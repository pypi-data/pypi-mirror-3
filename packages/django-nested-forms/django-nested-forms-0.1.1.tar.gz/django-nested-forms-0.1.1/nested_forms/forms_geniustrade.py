...
class ComplexModelForm(forms.ModelForm):
    ...
    def _get_formset(self, name, form, extra=None, initial=None, can_delete=True, \
        ...
        if data:
            ...
            #    if not data.has_key("%s-%d-%s" % (self.add_prefix(name), i, instance_pk)):
            ...
            #print update_button, data.has_key(update_button), data
            ...
            if "%s-%s" % (prefix, TOTAL_FORM_COUNT) not in data.keys():
                #print "NO", "%s-%s" % (prefix, TOTAL_FORM_COUNT), "in data.keys :", data.keys()
                data = files = None
        ...
        if instance.pk:
            if isinstance(field, RelatedObject):
                ...
                #print prefix, queryset.query

                #print name, ": model formset"
            ...
            formset = formset_class(
                ...
                files,
                prefix = prefix,
                queryset = queryset, #to.objects.none(),
                initial = not self.instance.pk and isinstance(initial, list) and initial or None,
            )
        ...
        #try:
        #    tmp_objs = getattr(instance, "_%s" % name)
        ...
        for form in formset.forms:
            try:
                if form.instance.pk not in deleted_instance_pks:
                    ...
                    #if form.instance not in tmp_objs:
                    #print "++++++++++++ (%s) appending" % prefix , form.instance
                    tmp_objs.append(form.instance)
                    #else:
                    #    print "------------ (%s) not appending" % prefix , form.instance
