        ...
        for i in range(256):
            try:
                ...
                if len(checkID) == 0:
                    s.close()
                    time.sleep(0.1)
                    s = serial.Serial(i,baudrate,timeout=1, dsrdtr = 1)
                    s.write('i' + chr(1) + chr(0))
                    checkID = s.read(6)
                if 'pymcu' in checkID:
        ...
        if port == None:
            ...
            if len(findmcu) > 0:
                ...
                if len(checkID) == 0:
                    ...
                    time.sleep(0.1)
                    self.mcuserial = serial.Serial(findmcu.items()[0][0],baudrate, timeout=1, dsrdtr = 1)
                    self.mcuserial.write('i' + chr(1) + chr(0))
                    checkID = self.mcuserial.read(6)
                ...
                for baud in self.baudList:
                    ...
                    if len(findmcu) > 0:
                        ...
                        if len(checkID) == 0:
                            self.mcuserial.close()
                            time.sleep(0.1)
                            self.mcuserial = serial.Serial(findmcu.items()[0][0],baud, timeout=1, dsrdtr = 1)
                            self.mcuserial.write('i' + chr(1) + chr(0))
                            checkID = self.mcuserial.read(6)
                        self.mcuType = int(checkID[5:])
                        self.active = self.mcuserial.isOpen()
            ...
            if self.active == False:
                ...
                if len(checkID) == 0:
                    self.mcuserial.close()
                    time.sleep(0.1)
                    self.mcuserial = serial.Serial(port,baudrate,timeout=1, dsrdtr = 1)
                    self.mcuserial.write('i' + chr(1) + chr(0))
                    checkID = self.mcuserial.read(6)
                self.mcuType = int(checkID[5:])
