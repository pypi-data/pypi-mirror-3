# -*- coding: utf-8 -*-
#------------------------------------------------------------------------------
# file: $Id: test_server.py 29 2012-06-30 22:52:55Z griff1n $
...
import sqlalchemy
import pysyncml
from .common import adict, ts, ts_iso, getIntSize
...
#------------------------------------------------------------------------------
class TestServer(unittest.TestCase, test_helpers.XmlEqual):

  #----------------------------------------------------------------------------
  def assertIntsNear(self, chk, val):
    if chk == val or ( chk - 1 == val ) or ( chk + 1 == val ):
      return
  ...
  def makeRequestInit(self, targetID, sourceID,
                      ...
                      alertCode=201, nextAnchor=None, lastAnchor=None,
                      ):
...
''' % locals()
  ...
  def test_new_peer_nodevinfo(self):
    ...
    chk = adict(headers=dict((('content-type', 'application/vnd.syncml+xml; charset=UTF-8'),)),
                ...
                '  <Meta>'
                '   <MaxMsgSize xmlns="syncml:metinf">2147483647</MaxMsgSize>'
                ...
                '       <MaxObjSize>2147483647</MaxObjSize>'
  ...
  def test_new_peer_store(self):
    ...
    response = pysyncml.Response()
    stats = self.server.handleRequest(session, request, response)
    self.assertEqual(dict(srv_note=dict(mode=pysyncml.SYNCTYPE_SLOW_SYNC, conflicts=0,
    ...
    self.assertEqual(chk.headers['content-type'], response.contentType)
    self.assertEqualXml(chk.body, response.body)
    # TBD: confirm that the session object looks good...
  ...
  def test_sync_update(self):
    ...
    self.initServer()
    self.items.replace(NoteItem(id=item.id, name='foo', body='and now it is the bar'))
    ...
    #         (but sleep 2 seconds first so that anchors are forced to change)
    self.initServer()
    time.sleep(2)
    peerAnchor2 = ts_iso()
    request = adict(headers=dict((('content-type', 'application/vnd.syncml+xml'),)),
    ...
    response = pysyncml.Response()
    stats = self.server.handleRequest(session, request, response)
    self.assertEqual(dict(srv_note=dict(mode=pysyncml.SYNCTYPE_TWO_WAY, conflicts=0,
                                        hereAdd=0,hereMod=0,hereDel=0,hereErr=0,
                                        peerAdd=0,peerMod=0,peerDel=0,peerErr=0)),
                     stats)
    ...
    self.assertEqual(dict(srv_note=dict(mode=pysyncml.SYNCTYPE_TWO_WAY, conflicts=0,
                                        hereAdd=0,hereMod=0,hereDel=0,hereErr=0,
                                        peerAdd=0,peerMod=1,peerDel=0,peerErr=0)),
                     stats)

#------------------------------------------------------------------------------
# end of $Id: test_server.py 29 2012-06-30 22:52:55Z griff1n $
#------------------------------------------------------------------------------
