...
.. note::
    ...
    `$PREFIX/lib/python2.7/site-packages/pycom-0.6.0-py2.7.egg/share/pycom/`.
...
.. data:: service
   ...
   .. note::
      ...
      You can get into trouble with generated service names. If you restart
      such service, for some time (no more than a minute) you'll have
      two services with the same interface registered, as old one will not
      expire (on nameserver) yet.
...
.. data:: authentication
   ...
   A dictionary with authentication configuration. The only supported property
   now is:

   .. data:: policy

      Authentication policy, the only meaningful values is string
      ``"required"`` which forces authentication check for all services.
