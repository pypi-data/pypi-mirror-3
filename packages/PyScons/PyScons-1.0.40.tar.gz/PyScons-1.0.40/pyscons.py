...
class PyScanner(MF):
    ...
    def __call__(self, node, env, path):
        ...
        imports = [i for i in imports if "/site-packages/" not in i]
        imports = [i for i in imports if "/SCons/" not in i]
        imports = [i for i in imports if "/dist-packages/" not in i]
...
def _PyScons(env, target, source, scons, options="-Q", sig_options="", clean = True, tempdir = None):
    target = ToList(target) 
    source = ToList(source) 
    ...
    options += " temp=" + TEMP + " n=" + scons  
    cmd = "scons %s %s -f%s %s" % (options, sig_options, scons, args)
    cleancmd = cmd + " --clean"
    ...
    if clean: 
        cmd = [Mkdir(TEMP), cmd, clean, Delete(TEMP)]
    else:
        cmd = [Mkdir(TEMP), cmd]
...
def PySconsSetup(env=None, args=ARGUMENTS, tools=['default', PYTOOL()]):
    ...
    myname = args["n"]
    N = str(len(os.path.split(myname)))
    ...
    env.Command(ship, targets + sources + [myname], "tar cjvf $TARGET $SOURCES")

    import SCons.Node
    files = set(targets)
    for t in targets:
        W = SCons.Node.Walker(T[0])
        while not W.is_done():
            N = W.get_next()
            try:
                if N.exists(): files.add(N)
            except: pass

    env.Command(out, list(files) + [os.join(basename, ".sconsign.dblite")], "tar cjvf $TARGET $SOURCES")
