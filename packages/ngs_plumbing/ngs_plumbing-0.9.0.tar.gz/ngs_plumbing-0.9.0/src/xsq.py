...
_ACGT = bytes('ACGT')
FRAGMENT_NAMES = ('F3', 'F5-DNA', 'F5-RNA', 'R3', 'R5')
...
def make_csfasta(lib, fragment, buf_cs, buf_qual, f, flowcell, 
    ...
    for tile_i, (tile_n, a) in enumerate(iter_valuearrays(lib, fragment, 
        ...
        for row_i in _progress_bar_iter(xrange(a.shape[0]), 
                                        suffix = ' tile: %i'%(tile_i+1)): 
            seq, qual = colourqual_frombuffer(a[row_i])
            # turn integers to their ASCII representations
            ...
            buf_qual.write('\n')
            buf_qual.write(' '.join( str(q) for q in qual))
            buf_qual.write('\n')
...
def make_csfastq(lib, fragment, buf, f, flowcell, 
    ...
    for tile_i, (tile_n, a) in enumerate(iter_valuearrays(lib, fragment, 
        ...
        for row_i in _progress_bar_iter(xrange(a.shape[0]), 
            ...
            buf.write(seq)
            buf.write('\n+\n')
            buf.write(' '.join( str(q) for q in qual))
            buf.write('\n')
