...
#    Portions copyright 2010 Cynapse India Pvt. Ltd.
#                                                                                 
...
from interfaces import IZipFileTransportUtility, IZipFileAdapter
...
class ZipFileTransportUtility(SimpleItem):
    ...
    def exportContent(self, context, obj_paths=None, filename=None, comments=False):
        """ Export content to a zip file.
        """      
        objects_list = self._createObjectList(context, obj_paths)
        zip_path = self._getAllObjectsData(context, objects_list, comments)        
        return zip_path
    ...
    def _getAllObjectsData(self, context, objects_listing, comments):
        ...
        mappings = pt.zipfile_properties.getProperty('content_mappings')
        mappings = dict([x.split(':') for x in mappings])

        for obj in objects_listing:
            if obj.meta_type in mappings:
                ob = getAdapter(obj, IZipFileAdapter, name=mappings[obj.meta_type])
                ob.writeObjectToZipArchive(context, zipfile, comments)                
