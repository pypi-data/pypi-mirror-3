...
class RealKernel( Kernel ):
    """A kernel that is defined on R^n (n dimensional space of real numbers)"""
    def __init__(
            self,
            params,
            param_priors,
        ...
        If dimensions = 0 then the arguements to the kernel can take any
        dimension
        """
...
def distance_2( x1, x2, params ):
    """Calculates distance squared between two points in R^n
    """
    assert type(x1) == type(x2) == numpy.ndarray
    if len(x1) != len(x2):
        raise RuntimeError(
...
def distance( x1, x2, params ):
    """Calculates distance between two points in R^n
    """
    import math
    assert type(x1) == type(x2) == numpy.ndarray
