"""
Underverse is named after the destination and afterlift of the Necromonger's from the 
...
At it's core, the Underverse is a JSON-based document store built on top of SQlite. JSON was chosen over pickle because of it's size, simplicity, readability and speed. These features, combined with SQlite, make for a simple and elegant solution.
...
Here's the relationship between the objects found in Underverse and a traditional RDBMS:
...
The Underverse's query syntax is similar to SQLAlchemy's. However, Underverse removes some of the complexity of getting started. 
...
I have really enjoyed using SQL-Alchemy in the past. However, some projects can get away with a much more 
light-weight approach to managing data. Underverse is meant to fill that nitch.
