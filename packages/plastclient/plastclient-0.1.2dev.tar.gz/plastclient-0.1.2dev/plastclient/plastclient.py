...
class StateSet(object):
    ...
    def update(self, set_uuid, key, **kwargs):
        data = {}
        for option in ['name','description','disabled']:
            if kwargs.has_key(option):
                data[option] = kwargs[option]
        if kwargs.has_key('states'):
            states = kwargs['states']
            if type(states) != str:
                states = json.dumps(states)
            data['states'] = states
        set = self.get(set_uuid)
        req = {
            'url'     : '/stateset/%s' % set['uuid'],
            'method'  : 'PUT',
            'data'    : data,
            'headers' : {
                'plast_accesskey' : key
