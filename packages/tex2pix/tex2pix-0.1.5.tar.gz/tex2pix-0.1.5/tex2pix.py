...
Usage example::
    ...
    r = Renderer("example.tex", runbibtex=True, extras=["example.bib"])
...
def check_bitmap_pkgs():
    """Test for convert*and* the PDF producing packages if we need to make a PNG
    ...
    def mkpdf(self, dest=None):
        ...
        if not self._made_pdf:
            try:
                ...
                raise Exception("pdflatex could not be run: PDF and/or PNG format mode cannot work")
        if dest:
