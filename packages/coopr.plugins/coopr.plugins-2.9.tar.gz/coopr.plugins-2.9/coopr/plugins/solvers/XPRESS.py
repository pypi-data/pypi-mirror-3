...
class XPRESS_shell(ILMLicensedSystemCallSolver):
    ...
    def executable(self):
        ...
        if executable is None:
            pyutilib.component.core.PluginGlobals.env().log.error("Could not locate the 'optimizer' executable, which is required for solver %s" % self.name)
            self.enable = False
            return None
...
class MockXPRESS(XPRESS_shell,mockmip.MockMIP):
