...
class IAddress(Interface):
    ...
    phone       = TextLine(title=u'Phone Number', description=u'Phone number',         required=False, constraint=re.compile('^[0-9()-.,\s]+$').match, max_length=25)
    fax         = TextLine(title=u'Fax Number',   description=u'Fax number',           required=False, constraint=re.compile('^[0-9()-.,\s]+$').match, max_length=25)
