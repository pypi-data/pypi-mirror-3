...
from django_factory import DummyTestCase, Factory, generators
...
class TestGenerators(TestCase):
    ...
    def setUp(self):
        ...
        self.field = test_models.ModelWithCharField._meta.get_field_by_name(
                'char_field')[0]
...
class FactoryGenerateValueTests(DjangoTestCase, TestCase):
    ...
    def get_factory_with_known_generator(self, field, expected, value):
        ...
        """
        self.assertEqual(expected, Factory.DEFAULT_MAPPING[field])
        mapping = Factory.DEFAULT_MAPPING.copy()
        def return_value(_field, _factory):
            return value
        mapping[field] = return_value
        return Factory(type_mapping=mapping)
...
class TestFactory(DjangoTestCase, TestCase):
    ...
    def test_make_one_doesnt_generate_for_null(self):
        # When a field has null=True, no value is generated for it by
        # default.
        factory = Factory()
    ...
    def test_make_one_doesnt_generates_for_null_if_factory_method(self):
        ...
        # used even if the field has null=True.
        factory = Factory()
        instance = factory.make_one(test_models.ModelWithNullCharFieldAndFactory)
        self.assertEquals(test_models.ModelWithNullCharFieldAndFactory.VALUE,
                instance.char_field)
    ...
    def test_make_one_validates_model(self):
        ...
        # releases
        if getattr(django_fields.CharField, 'validators', None) is None:
