...
def nansum_2d_int32_axis0(np.ndarray[np.int32_t, ndim=2] a):
    "Sum of 2d array with dtype=int32 along axis=0 ignoring NaNs."
...
def nansum_2d_int32_axis1(np.ndarray[np.int32_t, ndim=2] a):
    "Sum of 2d array with dtype=int32 along axis=1 ignoring NaNs."
    cdef np.int32_t asum = 0, ai
    cdef Py_ssize_t i0, i1
    cdef np.npy_intp *dim
    dim = PyArray_DIMS(a)
...
def nansum_2d_int64_axis0(np.ndarray[np.int64_t, ndim=2] a):
    "Sum of 2d array with dtype=int64 along axis=0 ignoring NaNs."
...
def nansum_2d_int64_axis1(np.ndarray[np.int64_t, ndim=2] a):
    "Sum of 2d array with dtype=int64 along axis=1 ignoring NaNs."
    cdef np.int64_t asum = 0, ai
    cdef Py_ssize_t i0, i1
    cdef np.npy_intp *dim
    dim = PyArray_DIMS(a)
...
def nansum_3d_int32_axis0(np.ndarray[np.int32_t, ndim=3] a):
    "Sum of 3d array with dtype=int32 along axis=0 ignoring NaNs."
    cdef np.int32_t asum = 0, ai
...
def nansum_3d_int32_axis1(np.ndarray[np.int32_t, ndim=3] a):
    "Sum of 3d array with dtype=int32 along axis=1 ignoring NaNs."
    cdef np.int32_t asum = 0, ai
    cdef Py_ssize_t i0, i1, i2
    cdef np.npy_intp *dim
    dim = PyArray_DIMS(a)
...
def nansum_3d_int32_axis2(np.ndarray[np.int32_t, ndim=3] a):
    "Sum of 3d array with dtype=int32 along axis=2 ignoring NaNs."
    cdef np.int32_t asum = 0, ai
    cdef Py_ssize_t i0, i1, i2
    cdef np.npy_intp *dim
    dim = PyArray_DIMS(a)
...
def nansum_3d_int64_axis0(np.ndarray[np.int64_t, ndim=3] a):
    "Sum of 3d array with dtype=int64 along axis=0 ignoring NaNs."
    cdef np.int64_t asum = 0, ai
    cdef Py_ssize_t i0, i1, i2
    cdef np.npy_intp *dim
    dim = PyArray_DIMS(a)
...
def nansum_3d_int64_axis1(np.ndarray[np.int64_t, ndim=3] a):
    "Sum of 3d array with dtype=int64 along axis=1 ignoring NaNs."
    cdef np.int64_t asum = 0, ai
    cdef Py_ssize_t i0, i1, i2
    cdef np.npy_intp *dim
    dim = PyArray_DIMS(a)
...
def nansum_3d_int64_axis2(np.ndarray[np.int64_t, ndim=3] a):
    "Sum of 3d array with dtype=int64 along axis=2 ignoring NaNs."
    cdef np.int64_t asum = 0, ai
    cdef Py_ssize_t i0, i1, i2
    cdef np.npy_intp *dim
    dim = PyArray_DIMS(a)
...
def nansum_1d_float32_axisNone(np.ndarray[np.float32_t, ndim=1] a):
    "Sum of 1d array with dtype=float32 along axis=None ignoring NaNs."
    cdef int allnan = 1
...
def nansum_1d_float64_axisNone(np.ndarray[np.float64_t, ndim=1] a):
    "Sum of 1d array with dtype=float64 along axis=None ignoring NaNs."
    cdef int allnan = 1
...
def nansum_2d_float32_axisNone(np.ndarray[np.float32_t, ndim=2] a):
    "Sum of 2d array with dtype=float32 along axis=None ignoring NaNs."
    cdef int allnan = 1
    cdef np.float32_t asum = 0, ai
    cdef Py_ssize_t i0, i1
    cdef np.npy_intp *dim
...
def nansum_2d_float64_axisNone(np.ndarray[np.float64_t, ndim=2] a):
    "Sum of 2d array with dtype=float64 along axis=None ignoring NaNs."
    cdef int allnan = 1
    cdef np.float64_t asum = 0, ai
    cdef Py_ssize_t i0, i1
    cdef np.npy_intp *dim
...
def nansum_3d_float32_axisNone(np.ndarray[np.float32_t, ndim=3] a):
    "Sum of 3d array with dtype=float32 along axis=None ignoring NaNs."
    cdef int allnan = 1
    cdef np.float32_t asum = 0, ai
    cdef Py_ssize_t i0, i1, i2
    cdef np.npy_intp *dim
...
def nansum_3d_float64_axisNone(np.ndarray[np.float64_t, ndim=3] a):
    "Sum of 3d array with dtype=float64 along axis=None ignoring NaNs."
    cdef int allnan = 1
    cdef np.float64_t asum = 0, ai
    cdef Py_ssize_t i0, i1, i2
    cdef np.npy_intp *dim
...
def nansum_2d_float32_axis0(np.ndarray[np.float32_t, ndim=2] a):
    "Sum of 2d array with dtype=float32 along axis=0 ignoring NaNs."
    cdef int allnan = 1
    cdef np.float32_t asum = 0, ai
    cdef Py_ssize_t i0, i1
    cdef np.npy_intp *dim
...
def nansum_2d_float32_axis1(np.ndarray[np.float32_t, ndim=2] a):
    "Sum of 2d array with dtype=float32 along axis=1 ignoring NaNs."
    cdef int allnan = 1
    cdef np.float32_t asum = 0, ai
    cdef Py_ssize_t i0, i1
    cdef np.npy_intp *dim
...
def nansum_2d_float64_axis0(np.ndarray[np.float64_t, ndim=2] a):
    "Sum of 2d array with dtype=float64 along axis=0 ignoring NaNs."
    cdef int allnan = 1
    cdef np.float64_t asum = 0, ai
    cdef Py_ssize_t i0, i1
    cdef np.npy_intp *dim
...
def nansum_2d_float64_axis1(np.ndarray[np.float64_t, ndim=2] a):
    "Sum of 2d array with dtype=float64 along axis=1 ignoring NaNs."
    cdef int allnan = 1
    cdef np.float64_t asum = 0, ai
    cdef Py_ssize_t i0, i1
    cdef np.npy_intp *dim
...
def nansum_3d_float32_axis0(np.ndarray[np.float32_t, ndim=3] a):
    "Sum of 3d array with dtype=float32 along axis=0 ignoring NaNs."
    cdef int allnan = 1
    cdef np.float32_t asum = 0, ai
    cdef Py_ssize_t i0, i1, i2
    cdef np.npy_intp *dim
...
def nansum_3d_float32_axis1(np.ndarray[np.float32_t, ndim=3] a):
    "Sum of 3d array with dtype=float32 along axis=1 ignoring NaNs."
    cdef int allnan = 1
    cdef np.float32_t asum = 0, ai
    cdef Py_ssize_t i0, i1, i2
    cdef np.npy_intp *dim
...
def nansum_3d_float32_axis2(np.ndarray[np.float32_t, ndim=3] a):
    "Sum of 3d array with dtype=float32 along axis=2 ignoring NaNs."
    cdef int allnan = 1
    cdef np.float32_t asum = 0, ai
    cdef Py_ssize_t i0, i1, i2
    cdef np.npy_intp *dim
...
def nansum_3d_float64_axis0(np.ndarray[np.float64_t, ndim=3] a):
    "Sum of 3d array with dtype=float64 along axis=0 ignoring NaNs."
    cdef int allnan = 1
    cdef np.float64_t asum = 0, ai
    cdef Py_ssize_t i0, i1, i2
    cdef np.npy_intp *dim
...
def nansum_3d_float64_axis1(np.ndarray[np.float64_t, ndim=3] a):
    "Sum of 3d array with dtype=float64 along axis=1 ignoring NaNs."
    cdef int allnan = 1
    cdef np.float64_t asum = 0, ai
    cdef Py_ssize_t i0, i1, i2
    cdef np.npy_intp *dim
...
def nansum_3d_float64_axis2(np.ndarray[np.float64_t, ndim=3] a):
    "Sum of 3d array with dtype=float64 along axis=2 ignoring NaNs."
    cdef int allnan = 1
    cdef np.float64_t asum = 0, ai
    cdef Py_ssize_t i0, i1, i2
    cdef np.npy_intp *dim
...
def nansum_1d_int32_axisNone(np.ndarray[np.int32_t, ndim=1] a):
    "Sum of 1d array with dtype=int32 along axis=None ignoring NaNs."
    cdef np.int32_t asum = 0, ai
...
def nansum_1d_int64_axisNone(np.ndarray[np.int64_t, ndim=1] a):
    "Sum of 1d array with dtype=int64 along axis=None ignoring NaNs."
    cdef np.int64_t asum = 0, ai
    cdef Py_ssize_t size
    cdef Py_ssize_t i0
    cdef np.npy_intp *dim
...
def nansum_2d_int32_axisNone(np.ndarray[np.int32_t, ndim=2] a):
    "Sum of 2d array with dtype=int32 along axis=None ignoring NaNs."
    cdef np.int32_t asum = 0, ai
    cdef Py_ssize_t size
    cdef Py_ssize_t i0, i1
    cdef np.npy_intp *dim
...
def nansum_2d_int64_axisNone(np.ndarray[np.int64_t, ndim=2] a):
    "Sum of 2d array with dtype=int64 along axis=None ignoring NaNs."
    cdef np.int64_t asum = 0, ai
    cdef Py_ssize_t size
    cdef Py_ssize_t i0, i1
    cdef np.npy_intp *dim
...
def nansum_3d_int32_axisNone(np.ndarray[np.int32_t, ndim=3] a):
    "Sum of 3d array with dtype=int32 along axis=None ignoring NaNs."
    cdef np.int32_t asum = 0, ai
    cdef Py_ssize_t size
    cdef Py_ssize_t i0, i1, i2
    cdef np.npy_intp *dim
...
def nansum_3d_int64_axisNone(np.ndarray[np.int64_t, ndim=3] a):
    "Sum of 3d array with dtype=int64 along axis=None ignoring NaNs."
    cdef np.int64_t asum = 0, ai
    cdef Py_ssize_t size
    cdef Py_ssize_t i0, i1, i2
    cdef np.npy_intp *dim
