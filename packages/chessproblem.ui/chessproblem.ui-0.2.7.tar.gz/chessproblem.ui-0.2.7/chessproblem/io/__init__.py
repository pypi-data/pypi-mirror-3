...
TRUE_COMMANDS = set(['verticalcylinder', 'horizontalcylinder', 'gridchess', 'noframe'])
...
class ChessProblemLatexParser(object):
    ...
    def create_problem(self, problem_content, after_command_text):
        ...
        if 'condition' in problem_content:
            result.condition = [s.strip() for s in re.split('[;,] ', problem_content['condition'])]
        ...
        if 'nofields' in problem_content:
            result.nofields = problem_content['nofields']
        if 'fieldframe' in problem_content:
            result.fieldframe = problem_content['fieldframe']
    ...
    for piece_spec in pieces.split(','):
        ...
        try:
            while index < len(piece_spec):
                ...
                board.fields[_row][_column] = piece
                index += 1
...
def _write_problem(problem, output):
    ...
    _write_simple_member(problem, 'nofields', output)
    _write_simple_member(problem, 'fieldframe', output)
...
def _write_array_member(problem, output, member_name, latex_command=None, write_member=None):
    ...
    if len(array) > 0:
        ...
        for value in array:
            ...
            if write_member == None:
                ...
                output.write(write_member(value))
        output.write('}')
...
def _has_pieces(board):
    for row in range(board.rows):
        for column in range(board.columns):
            if board.fields[row][column] != None:
                return True
    return False
