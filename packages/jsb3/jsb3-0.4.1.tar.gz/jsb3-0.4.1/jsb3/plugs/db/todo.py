...
from jsb3.utils.timeutils import strtotime, striptime, today 
...
from jsb3.lib.datadir import datadir
    ...
    def delete(self, name, indexnr):
        ...
        try:
            ...
            if warnsec:
                alarmnr = 0 - warnsec
                if alarmnr > 0:
                    alarms = plugs.get("jsb3.plugs.common.alarm")
...
def handle_settodo(bot, ievent):
    ...
    if not ttime  == None:
        ievent.reply('time detected ' + time.ctime(ttime))
        what = striptime(what)
        karma = plugs.get("jsb3.plugs.db.karma2")
