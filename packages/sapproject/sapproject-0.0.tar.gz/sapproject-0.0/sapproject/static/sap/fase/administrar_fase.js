Ext.define('sap.administrarFasePanel', {
    extend: 'Ext.form.Panel',
    alias : 'sap.administrarFasePanel',
    ...
    initComponent: function() {
        var grid_fases = Ext.create('sap.faseGrid');
        this.items = [{
            xtype:      'form',
            title:      'Administrar Fase',
            autoScroll: true,
    ...
    form_update_handler : function() {
		...
		else if(records[0].data.estado == 'INICIADO' || records[0].data.estado == 'FINALIZADO'){
		       Ext.Msg.alert('ERROR','No se puede modificar esta Fase porque se encuentra en estado '+records[0].data.estado);
        }     
        else{
            Ext.getCmp('area-central').agregar_pestanha('panel_modificar_fase','sap.modificarFasePanel');
    ...
    form_delete_handler : function() {
        ...
        if(records == null || records.length == 0){
            Ext.Msg.alert('ERROR','Debe seleccionar un registro para eliminar');
		       ...
		       Ext.Msg.alert('ERROR','No se puede eliminar esta Fase porque se encuentra en estado'+records[0].data.estado);
        }     
        else{        
			var grid = this.grid;  
            Ext.Msg.show({
    ...
    form_new_handler: function() {
        Ext.getCmp('area-central').agregar_pestanha('panel_crear_fase','sap.crearFasePanel');
    },
    ...
    form_reset_handler: function() {
        ...
        // recargamos el store
        Ext.data.StoreManager.lookup('faseStore').load();
    },
