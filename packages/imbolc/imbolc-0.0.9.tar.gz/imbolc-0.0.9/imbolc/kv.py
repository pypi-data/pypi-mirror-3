...
def main():
    db = KVBase('var/test.tc', 'a+', 
        backup={'db_dir': 'var/backup/test.tc'})
...
class KVBase(object):
    ...
    def __init__(self, fn, mode='a+', db_type='tc', autocompress=False,
            backup=None):
        '''
        *fn: файл
        ...
        **backup = {'db_dir': '/dir/name', **FSDBM_kwargs}
        ...
        if autocompress:
            import zlib
            self.compress = lambda x: zlib.compress(x, 9)
            self.decompress = lambda x: zlib.decompress(x)
        self.backup = backup
        self.open()
