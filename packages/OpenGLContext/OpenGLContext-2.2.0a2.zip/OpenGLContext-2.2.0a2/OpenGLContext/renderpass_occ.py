...
class VisitingRenderPass( RenderPass, rendervisitor.RenderVisitor ):
	...
	def children( self, node ):
		...
		Adds frustum-culling check to filter out children who
		...
		if hasattr( context, 'USE_FRUSTUM_CULLING') and context.USE_FRUSTUM_CULLING:
			if glGenQueries:
				self.__class__.children = self.__class__.culling_children 
			else:
				self.__class__.children = self.__class__.frustum_children
		else:
			del self.__class__.children
		return self.children( node )
	def frustum_children( self, node ):
		...
		for child in base:
			if hasattr( child, 'visible'):
				...
				yield child
	def culling_children( self, node ):
		"""Perform culling of children then do occlusion tests"""
		glDepthMask(GL_FALSE)
		glColorMask(GL_FALSE,GL_FALSE,GL_FALSE,GL_FALSE)
		glDisable(GL_LIGHTING)
		try:
			queries = []
			for child in self.frustum_children( node ):
				if hasattr( child, 'boundingVolume' ):
					volume = child.boundingVolume()
					if hasattr( volume, 'occlusionRender' ):
						query = glGenQueries(1)
						queries.append( (query,child) )
						glBeginQuery(GL_SAMPLES_PASSED, query)
						try:
							volume.occlusionRender()
						finally:
							glEndQuery( GL_SAMPLES_PASSED )
					else:
						queries.append( (None,child))
				else:
					queries.append( (None,child))
		finally:
			glDepthMask(GL_TRUE)
		...
		try:
			for query,child in queries:
				if query is not None:
					# this can stall, but that *should* allow other threads to run...
			...
			glDeleteQueries( [x[0] for x in queries  if x[0]])
