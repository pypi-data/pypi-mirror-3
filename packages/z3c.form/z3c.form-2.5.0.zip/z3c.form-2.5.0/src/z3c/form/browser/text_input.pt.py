...
def bind():
    ...
    def render(econtext, rcontext=None):
        ...
        u'view/alt'
        _tmp1 = _path(econtext['view'], econtext['request'], True, 'alt')
        if (_tmp1 is _default):
            _tmp1 = u''
        if ((_tmp1 is not None) and (_tmp1 is not False)):
            if (_tmp1.__class__ not in (str, unicode, int, float, )):
        ...
        u'view/accesskey'
        _tmp1 = _path(econtext['view'], econtext['request'], True, 'accesskey')
        if (_tmp1 is _default):
            _tmp1 = u''
        if ((_tmp1 is not None) and (_tmp1 is not False)):
            if (_tmp1.__class__ not in (str, unicode, int, float, )):
        ...
        u'view/onselect'
        _tmp1 = _path(econtext['view'], econtext['request'], True, 'onselect')
        if (_tmp1 is _default):
            _tmp1 = None
        if ((_tmp1 is not None) and (_tmp1 is not False)):
            if (_tmp1.__class__ not in (str, unicode, int, float, )):
            ...
            if ('"' in _tmp1):
                _tmp1 = _tmp1.replace('"', '&quot;')
            _write(((' onselect="' + _tmp1) + '"'))
        _write(u' />\n')
        return _out.getvalue()
    return render

__filename__ = 'd:\\eggdev\\source\\z3c\\z3c.form\\trunk\\src\\z3c\\form\\browser\\text_input.pt'
