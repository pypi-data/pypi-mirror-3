...
def send_file(connection, source_path, target_path):
    ...
    """
    absolute_target = os.path.join(connection.reductio_home, target_path)
...
def make_connections():
    ...
    for worker_config in config.WORKERS:
        conn = para_ssh.Connection(**worker_config)
        if worker_config.has_key('virtualenv'):
