...
class MySQLBaseSocket(object):
    ...
    def send_compressed(self, buf, pktnr):
        ...
        if pllen > 16777215:
            ...
            zpkts.append(struct.pack('<I',len(zbuf))[0:3]
                + struct.pack('<B',seqid) + b'\x00\x40\x00' + zbuf)
            tmpbuf = tmpbuf[16384:]
            pllen = len(tmpbuf)
            seqid = seqid + 1
            while pllen > MAX_PACKET_LENGTH:
                ...
                zpkts.append(struct.pack('<I',len(zbuf))[0:3]
                    + struct.pack('<B',seqid) + b'\xff\xff\xff' + zbuf)
        ...
        try:
            ...
            while buf:
                ...
                if pktsize == 0 and totalsize >= 4:
                    pktsize = struct.unpack("<I", buf[0:3]+b'\x00')[0]
    ...
    def recv_compressed(self):
        ...
        try:
            ...
            while buf:
                ...
                if zpktsize == 0 and totalsize >= 7:
                    zpktsize = struct.unpack("<I", buf[0:3]+b'\x00')[0]
                    pktsize = struct.unpack("<I", buf[4:4+3]+b'\x00')[0]
        ...
        bigbuf = bytearray()
        for pkt in pkts:
            pktsize = struct.unpack("<I", pkt[4:7]+b'\x00')[0]
            if pktsize == 0:
                bigbuf.extend(pkt[7:])
            else:
                bigbuf.extend(zlib.decompress(pkt[7:]))
            pktparts = ()
        
        while bigbuf:
            pktsize = struct.unpack("<I", bigbuf[0:3]+b'\x00')[0]
...
class MySQLUnixSocket(MySQLBaseSocket):
    ...
    def __init__(self, prtcls=None,unix_socket='/tmp/mysql.sock'):
        MySQLBaseSocket.__init__(self, prtcls=prtcls)
...
class MySQLTCPSocket(MySQLBaseSocket):
    ...
    def get_address(self):
        return "{ip}:{port}".format(ip=self.server_host,port=self.server_port)
...
class MySQLConnection(object):
    ...
    def set_converter_class(self, convclass):
        ...
        (charset_name, col) = constants.CharacterSet.get_info(self._charset)
    ...
    def get_characterset_info(self):
        """Backward compatibility. Please use charset-property"""
        return self.charset
