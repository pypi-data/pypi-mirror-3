...
class Parser(object):
    ...
    def __init__(self, nodeFactoryInstance=None):
        ...
        self._EQ_CHR = '='
        self._ATT_QUOTE_CHR = "'"
        self._EMPTY_ATT_VAL = "True"  #value to give for "flag" atts without val
    ...
    def _parseGenAttr(self, attDef):
        ...
        firstEqPos = attDef.find(self._EQ_CHR)
        if firstEqPos == -1:
            key = attDef
            val = self._EMPTY_ATT_VAL
        else:
            key = attDef[:firstEqPos]
            val = attDef[(firstEqPos+1):]
    ...
    def _tagAttrSplit(self, attsStr):
        ...
        if nextSepPos == -1: #no other attribute
            attDef = attsStr
            rest = ''
        elif sepChr == self._ATT_CHR and eqPos != -1 and \
                       findPatternPos(attsStr[:eqPos], self.TAG_PART_SEPARATORS) == -1:
