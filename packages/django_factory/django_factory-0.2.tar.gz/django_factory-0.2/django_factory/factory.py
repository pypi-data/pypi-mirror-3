...
class Factory(object):
    ...
    def _get_references_to(self, source_model, *args):
        """Get the foreign key fields referencing particular models.

        Given a source model and a list of target models, this method will
        return a list of the ForeignKey fields on the source model that
        reference the target models.

        :param source_model: the model to find the fields on.
        :param *args: other models to find references to, at least one model.
        :return: a list of fields from source_model that refer to the target
                models, in the same order as the input arguments.
        :raises ImproperlyConfigured: if any target model is not referenced,
             or is referenced twice.
        """
        target_fields = [None] * len(args)
                    ...
                    if field.related.parent_model == model:
                        if target_fields[i] is not None:
                            raise ImproperlyConfigured(
                                    "Two references to %r from %r"
                                    % (model, source_model))
                        target_fields[i] = field
        for i, model in enumerate(args):
            if target_fields[i] is None:
                raise ImproperlyConfigured("No reference to %r from %r"
                        % (model, source_model))
        return target_fields

    def _generate_through_model_instances(self, through, instance,
            related_instances, not_including):
        ...
        back_field, other_side_field = self._get_references_to(
                through, back_model, other_side_model)
