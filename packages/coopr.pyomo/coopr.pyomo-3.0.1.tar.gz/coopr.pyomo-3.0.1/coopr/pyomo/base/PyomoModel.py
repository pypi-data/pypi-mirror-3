...
class Model(Block):
 ...
 """
    ...
    def clone(self):
        ...
        #    self.components[name].model = weakref.ref(self)
        #for name in instance.components:
        #    instance.model = weakref.ref(instance)
        #    instance.components[name].model = weakref.ref(instance)
        self.model = weakref.ref(self)
        instance.model = weakref.ref(instance)
        for block in self.all_blocks():
            for compMap in block.components.components().itervalues():
                for comp in compMap.itervalues():
                    comp.parent = weakref.ref(block)
        ...
        for i in xrange(len(results.solution)):
            ...
            for label in input_soln.objective.keys():
                ...
                if obj_value.index.__class__ is tuple:
                    tmp[(obj_value.obj().name,)+obj_value.index] = (obj_value.name, entry)
                else:
                    tmp[(obj_value.obj().name,obj_value.index)] = (obj_value.name, entry)
            for key in sorted(tmp.keys()):
                value = tmp[key]
 ...
 """
    ...
    def _load_solution( self, soln, symbol_map,
        ...
        for label, entry in soln.variable.iteritems():
            ...
            if not isinstance(var_value,_VarValue):
        ...
        for label,entry in soln.constraint.iteritems():
            ...
            if not isinstance(con_value, ConstraintData):
