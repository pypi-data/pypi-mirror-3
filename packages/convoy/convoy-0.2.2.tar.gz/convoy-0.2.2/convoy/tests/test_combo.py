...
from convoy.combo import parse_path_hint
...
class ComboTest(ComboTestBase, mocker.MockerTestCase):
    ...
    def test_parse_url_paths(self):
        """Verify we parse out the path correctly for several urls"""
        path_checks = [
            ('http://yui.yahooapis.com/+combo?xxxxxx', ''),
            ('http://yui.yahooapis.com/+combo/dir?xxxxxx', 'dir'),
            ('http://yui.yahooapis.com/+combo/dir/dir2?xxxxxx', 'dir/dir2'),
            ('http://yui.yahooapis.com/combo/dir/dir2?xxxxxx', 'dir/dir2'),
        ]

        for url in path_checks:
            self.assertEquals(parse_path_hint(url[0]), url[1])
...
class WSGIComboTest(ComboTestBase, mocker.MockerTestCase):
    ...
    def test_combo_respects_path_hints(self):
        ...
        res = app.get("/+combo/%s/?%s" % (first_base,
            "yui/yui-min.js"), status=200)
        self.assertEquals(res.body.strip(), expected)

        res = app.get("/+combo/%s/?%s" % (second_base, "&".join(
            ["yui/yui-min.js"])), status=200)
