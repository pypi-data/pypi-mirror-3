 ...
 *          DO NOT MODIFY THIS FILE, MODIFY include/fractal.h
...
static PyObject * CFractalModelError = NULL;
...
static void
CFractalModel_dealloc(CFractalModel* self)
{
...
static PyObject *
CFractalModel_new(PyTypeObject *type, PyObject *args, PyObject *kwds)
{
    CFractalModel *self;
    
    self = (CFractalModel *)type->tp_alloc(type, 0);
   
    return (PyObject *)self;
}

static int
CFractalModel_init(CFractalModel *self, PyObject *args, PyObject *kwds)
{
    if (self != NULL) {
...
static PyMemberDef CFractalModel_members[] = {
    {name_params, T_OBJECT, offsetof(CFractalModel, params), 0, def_params},
	{name_dispersion, T_OBJECT, offsetof(CFractalModel, dispersion), 0, def_dispersion},     
    {name_log, T_OBJECT, offsetof(CFractalModel, log), 0, def_log},
    {NULL}  /* Sentinel */
};
...
/** Read double from PyObject
    ...
    @return double
*/
double CFractalModel_readDouble(PyObject *p) {
...
static PyObject *evaluateOneDim(FractalModel* model, PyArrayObject *q){
 ...
 * @return: numpy array object 
 */
 static PyObject * evaluateTwoDimXY( FractalModel* model, 
...
#pragma omp parallel for
        ...
        return PyArray_Return(result); 
		    ...
		    PyErr_SetString(CFractalModelError, 
                   "CFractalModel.evaluateTwoDimXY couldn't run.");
	        return NULL;
		}      	
}
/**
 ...
 *
 */ 
static PyObject * evalDistribution(CFractalModel *self, PyObject *args){
	PyObject *qx, *qy;
	PyArrayObject * pars;
	int npars ,mpars;
    ...
    self->model->fractal_dim = PyFloat_AsDouble( PyDict_GetItemString(self->params, "fractal_dim") );
    self->model->sldSolv = PyFloat_AsDouble( PyDict_GetItemString(self->params, "sldSolv") );
    self->model->cor_length = PyFloat_AsDouble( PyDict_GetItemString(self->params, "cor_length") );
    self->model->background = PyFloat_AsDouble( PyDict_GetItemString(self->params, "background") );
    self->model->radius = PyFloat_AsDouble( PyDict_GetItemString(self->params, "radius") );
    self->model->sldBlock = PyFloat_AsDouble( PyDict_GetItemString(self->params, "sldBlock") );
	...
	if ( !PyArg_ParseTuple(args,"O",&pars) ) {
	    PyErr_SetString(CFractalModelError, 
	    	"CFractalModel.evalDistribution expects a q value.");
		return NULL;
	}
    // Check params
    ...
    if(PyArray_Check(pars)==1) {
	    ...
	    if(npars==1) {
		    ...
		    PyErr_SetString(CFractalModelError, 
                   "CFractalModel.evalDistribution expect numpy array of one dimension.");
	        return NULL;
		}
	    ...
	    if(mpars!=2) {
	    	PyErr_SetString(CFractalModelError, 
	    		"CFractalModel.evalDistribution expects a list of dimension 2.");
	    	return NULL;
	    }
	     ...
	     if (PyArray_Check(qx) && PyArray_Check(qy)) {
		    ...
		    PyErr_SetString(CFractalModelError, 
                   "CFractalModel.evalDistribution expect 2 numpy arrays in list.");
	        return NULL;
	     }
	}
	PyErr_SetString(CFractalModelError, 
                   "CFractalModel.evalDistribution couln't be run.");
	return NULL;
 ...
 */
static PyObject * run(CFractalModel *self, PyObject *args) {
	...
	if ( !PyArg_ParseTuple(args,"O",&pars) ) {
	    PyErr_SetString(CFractalModelError, 
	    	"CFractalModel.run expects a q value.");
		return NULL;
	}
	...
	if( PyList_Check(pars)==1) {
	    ...
	    if(npars!=2) {
	    	PyErr_SetString(CFractalModelError, 
	    		"CFractalModel.run expects a double or a list of dimension 2.");
	    	return NULL;
	    }
	    // We have a vector q, get the q and phi values at which
	    // to evaluate I(q,phi)
	    q_value = CFractalModel_readDouble(PyList_GET_ITEM(pars,0));
	    phi_value = CFractalModel_readDouble(PyList_GET_ITEM(pars,1));
	    // Skip zero
	    if (q_value==0) {
		...
		q_value = CFractalModel_readDouble(pars);		
 ...
 */
static PyObject * calculate_VR(CFractalModel *self) {
 ...
 * @return: function value
 */
static PyObject * runXY(CFractalModel *self, PyObject *args) {
	...
	if( PyList_Check(pars)==1) {
	    ...
	    // to evaluate I(qx,qy)
	    qx_value = CFractalModel_readDouble(PyList_GET_ITEM(pars,0));
	    qy_value = CFractalModel_readDouble(PyList_GET_ITEM(pars,1));
	    return Py_BuildValue("d",(*(self->model))(qx_value,qy_value));

	} else {

		// We have a scalar q, we will evaluate I(q)
		qx_value = CFractalModel_readDouble(pars);		
		
		return Py_BuildValue("d",(*(self->model))(qx_value));
	}	
}

static PyObject * reset(CFractalModel *self, PyObject *args) {
    

    return Py_BuildValue("d",0.0);
}

static PyObject * set_dispersion(CFractalModel *self, PyObject *args) {
	PyObject * disp;
	const char * par_name;

	if ( !PyArg_ParseTuple(args,"sO", &par_name, &disp) ) {
	    PyErr_SetString(CFractalModelError,
	    	"CFractalModel.set_dispersion expects a DispersionModel object.");
		return NULL;
	}
    ...
    if (!strcmp(par_name, "radius")) {
	    ...
	    PyErr_SetString(CFractalModelError,
	    	"CFractalModel.set_dispersion expects a valid parameter name.");
		return NULL;
	}
...
static PyMethodDef CFractalModel_methods[] = {
...
static PyTypeObject CFractalModelType = {
    PyObject_HEAD_INIT(NULL)
    0,                         /*ob_size*/
    "CFractalModel",             /*tp_name*/
    sizeof(CFractalModel),             /*tp_basicsize*/
    0,                         /*tp_itemsize*/
    (destructor)CFractalModel_dealloc, /*tp_dealloc*/
    0,                         /*tp_print*/
    0,                         /*tp_getattr*/
    0,                         /*tp_setattr*/
    0,                         /*tp_compare*/
    ...
    0,		               /* tp_iter */
    0,		               /* tp_iternext */
    CFractalModel_methods,             /* tp_methods */
    CFractalModel_members,             /* tp_members */
    0,                         /* tp_getset */
    0,                         /* tp_base */
    0,                         /* tp_dict */
    0,                         /* tp_descr_get */
    0,                         /* tp_descr_set */
    0,                         /* tp_dictoffset */
    (initproc)CFractalModel_init,      /* tp_init */
    0,                         /* tp_alloc */
    CFractalModel_new,                 /* tp_new */
};
...
void addCFractalModel(PyObject *module) {
    ...
    if (PyType_Ready(&CFractalModelType) < 0)
        return;

    Py_INCREF(&CFractalModelType);
