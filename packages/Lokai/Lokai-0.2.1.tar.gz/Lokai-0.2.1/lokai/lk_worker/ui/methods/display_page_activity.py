# Name:      lokai/lk_worker/ui/methods/display_page_activity.py
...
class DisplayPage(NodeBaseForm):
    ...
    def build_history(self):
        if self.obj_ref and self.obj_ref != '**new**':
            history = engine.session.query(
                ndHistory
                ).filter(
                ndHistory.nde_idx == self.obj_ref
                ).order_by(ndHistory.hst_time_entry.desc()).all()
    ...
    def add_form_elements(self):
        ...
        self.detail.add(HtmlWidget, 'history',
                        title = '',
                        output = 'super'
                        )
    ...
    def populate_fields(self):
        ...
        history_body = self.detail.get_widget('history')
        history_body.set_title(
            render_page(self._formatted_history(),
                        self.request,
                        initial_header_level='6',
                        doctitle_xform=0))
    ...
    def _formatted_history(self):
        response_list = []
        if self.obj and self.obj.has_key('history'):
            for history_entry in self.obj['history']:
                block = "%s\n\n%s"% (
                    as_heading('h3', "%s - %s (%s)" % (
                        history_entry.hst_time_entry,
                        history_entry.hst_user,
                        history_entry.hst_type)),
                    history_entry.hst_text)
                response_list.append(block)
        return '\n\n'.join(response_list)
