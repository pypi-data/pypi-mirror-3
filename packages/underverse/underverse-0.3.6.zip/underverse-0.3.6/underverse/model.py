...
.. admonition:: Performance Hint
  :class: perf
	...
	**UDFs** are the most flexible in terms of customization, and they allow the join operation to be tailored to the data. 
	...
	# def udp(self, left, right, udp, omit=False, default=[]):
	# 	for l in self.left:
	# 		if not left._name in l:
	# 		# if not hasattr(l, left._name):
	# 			if not omit:
	...
	# 			value = []
	# 			for r in self.right:
	# 				if not right._name in r:
	# 				#if not hasattr(r, right._name):
	# 					continue
	# 				if udp(getattr(l, left._name), getattr(r, right._name)):
	# 					value.append(r)
