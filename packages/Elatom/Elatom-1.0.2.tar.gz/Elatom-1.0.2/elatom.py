    ...
    def run(self):
        try:
            ...
            for info in results["files"]:
                self._urls.append((info["url"], info["pubDate"],
                                    info["title"]))
        except urllib.error.HTTPError as error:
        ...
        while i < total:
            for name in feeds:
                ...
                if feed_reader.is_ended and not name in urls:
                    urls[name] = []
                    file_nb = 0
                    for (url, pubdate, title) in feed_reader.get_urls():
                        # clean the URL
                        url = self._clean_url(url)
                        # if downloaded, ignore
                        if self._config.is_downloaded(url):
                            continue
            ...
            for name in download_threads:
                ...
                for download_thread in download_threads[name]:
                    if download_thread.is_ended:
                        ...
                        # save
                        self._config.memory(download_thread.get_url())
