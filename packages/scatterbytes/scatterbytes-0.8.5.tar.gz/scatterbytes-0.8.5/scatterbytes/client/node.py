...
The process of uploading a file.
    ...
    1. The file is split and optionally compressed and encrypted.
    2. The client requests storage nodes to send the chunks to.
    3. The client sends the chunks to the designated storage nodes.

    The server tracks the uploads and treats them as a transaction.  The
    file is not considered uploaded until all of the storage nodes (including
    replication host nodes) have confirmed receipt and checksums.
