...
class TextBoxCell(Cell):
    def __init__(self, app, row, column):
        ...
        textAlign = self.styleGet('textAlign')
        self.inputBox = InputBox(self.app, Area(CellAttachedPoint((0,0), self,
                textAlign), self._getUsableRect().size, textAlign),
                font=self.styleGet('font'), colour=self.styleGet('foreColour'))
...
class TextButtonCell(Cell):
    def __init__(self, app, row, column):
        ...
        self.textButton = TextButton(self.app, pos, '', self.styleGet('font'), self.styleGet('foreColour'),
                                     self.styleGet('hoverColour'))
...
class Table(StyleUser):
    ...
    def addEmptyColumns(self, count, firstIndex=None):
        ...
        for i in range(count):
            self.addColumn(TextColumn(self.app, self, self._defaultWidth), index)
            if index is not None:
                index += 1
