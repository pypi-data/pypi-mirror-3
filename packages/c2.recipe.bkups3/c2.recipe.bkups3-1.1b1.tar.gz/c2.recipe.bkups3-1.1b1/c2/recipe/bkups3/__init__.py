...
class Recipe(object):
    ...
    def __init__(self, buildout, name, options):
        ...
        options.setdefault('use_scp', 'false')
        options.setdefault('scp_user', '')
        options.setdefault('scp_server', '')
        options.setdefault('scp_path', '')
        options.setdefault('use_copy_to_other_folder', 'false')
        options.setdefault('copy_path', '')
        ...
        check_for_true(options, ['use_s3', 'sync_s3_filesfolder',
                                 'use_scp', 'use_copy_to_other_folder',])
...
use_scp = %(use_scp)s
scp_user = %(scp_user)r
scp_server = %(scp_server)r
scp_path = %(scp_path)r
use_copy_to_other_folder = %(use_copy_to_other_folder)s
copy_path = %(copy_path)r
"""
        ...
        scripts = zc.buildout.easy_install.scripts(
            ...
            arguments=('bin_dir, blobstorage_path, backup_location, '+
                        ...
                        'use_blobstorage, use_scp, scp_user, scp_server, scp_path, ' +
                        'use_copy_to_other_folder, copy_path, '),
