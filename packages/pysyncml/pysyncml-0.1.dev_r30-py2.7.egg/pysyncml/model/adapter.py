# -*- coding: utf-8 -*-
#------------------------------------------------------------------------------
# file: $Id: adapter.py 29 2012-06-30 22:52:55Z griff1n $
...
# TBD: the current algorithm for finding the "local" adapter is to search
...
def decorateModel(model):
  ...
  class Adapter(model.DatabaseObject):
    ...
    #         should be the externally accessible URL that launches the
    #         SyncML transaction, and for clients this should be a unique
    #         ID, such as the IMEI number (for mobile phones). if not
    #         specified, it will if possible be auto- loaded from the
    #         database. if it cannot be loaded from the database, then it
    #         must be provided before any synchronization can begin.
    ...
    #         sets the local adapter :class:`pysyncml.devinfo.DeviceInfo`.
    #         if not specified, it will be auto-loaded from the database. if
    #         it cannot be loaded from the database, then it must be
    #         provided before any synchronization can begin.
    ...
    # TBD: look into making devID nullable - that way, i may be able to
    #      remove all the "adapter.devID is not None" checks before doing
    #      flushes or adapter.getKnownPeers() calls...
    ...
    def _initHelpers(self):
      if not self.isLocal:
        ...
        # tbd: should i be using the default opener instead?...
    ...
    def sync(self, mode=None):
      # # tbd: deal with this paranoia... perhaps move this into the synchronizer?...
      # self._context._model.session.flush()
      ...
      for store in self.stores.values():
        ...
        if peerUri is None:
          continue
        ds = common.adict(
          # TBD: perhaps share this "constructor" with router/protocol?...
        ...
        log.critical('TBD: implement auto-detect mode')
        # TBD: perhaps this should be pushed into synchronizer? it
        ...
        #   else:
        #     # tbd: anything smarter that should be done?...
    ...
    def _transmit(self, session, commands, response=None):
      ...
      # that.
      # TBD: should that only be done on successful transmit?...
    ...
    def _handleRequestRemote(self, session, request, adapter):
      # TBD: should this be broken out into a separate sub-class?...
      ...
      # tbd: add any other syncml headers?...
      # # ***********************************************************************
      # # TBD: **HACKALERT** **HACKALERT** **HACKALERT** **HACKALERT** **HACKALERT**
      # #      remove this HACK!...
      # req.adapter = adapter
      # req.session = session
      # # ***********************************************************************
      res = self._opener.open(req, request.body)
      # TBD: check response status...
...
#------------------------------------------------------------------------------
# end of $Id: adapter.py 29 2012-06-30 22:52:55Z griff1n $
#------------------------------------------------------------------------------
