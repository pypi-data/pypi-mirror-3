...
def _PyScons(env, target, source, scons, options="-Q", sig_options="", clean = True, tempdir = None):
    ...
    exe = os.path.join(tempdir, "EXECUTE")
    file = open(exe, 'w')
    print >>file, "#!/bin/bash"
    print >>file, cmd + " $*"
    os.chmod(exe, 0755)
...
def PySconsSetup(env=None, args=ARGUMENTS, tools=['default', PYTOOL()]):
    ...
    env.Command(ship, sources, "tar cjf $TARGET $SOURCES")

    env.Default(targets)


    env.Command(out, targets + [os.path.join(basename, ".sconsign.dblite")], "tar cjf $TARGET $SOURCES")
