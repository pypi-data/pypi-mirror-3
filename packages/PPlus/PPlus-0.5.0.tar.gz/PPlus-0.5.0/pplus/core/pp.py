...
class Server(object):
    ...
    def __init__(self, ncpus="autodetect", ppservers=(), secret=None,
        ...
        self.logger = logging.getLogger('pplus')
        self.logger.info("Creating server instance (pplus-%s)" % version)
        ...
        self.logger.info("pplus local server started with %d workers"
                % (self.__ncpus, ))

    def submit(self, func, tid, args=(), depfuncs=(), modules=(),
               callback=None, callbackargs=(), group='default', globals=None):
        ...
        self.logger.debug("Task %i submitted, function='%s'" %
    ...
    def connect1(self, host, port, persistent=True):
        ...
        try:
            ...
            #creating reserved rworkers: disabled prefetching!
            #for x in range(ncpus):
            #    rworker = _RWorker(host, port, self.secret, self, "EXEC", persistent)
            #    self.__update_active_rworkers(rworker.id, 1)
    ...
    def _run_remote(self, job, sfunc, sargs, rworker):
        ...
        self.logger.debug("Task (remote) %i started on %s",  job.tid, rworker.id)
