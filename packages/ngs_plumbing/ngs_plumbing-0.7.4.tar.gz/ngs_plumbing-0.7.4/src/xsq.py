    ...
    for lib_i, lib in enumerate(xsqfile.iter_lib()):
        ...
        for fragment in lib.fragments():
            ...
            data_fn = os.path.join(directory, 
                                  'fastqual_%s_%s_%i.%s' % (lib.name, fragment, lib_i, data_type))
            if verbose:
            ...
            if data_type == 'csv':
                ...
                f.write('dataf_%i_%s = ' % (lib_i, fragment) )
                f.write(tmp)
                f.write(';')
            f.close()
            if verbose:
                sys.stdout.write('done.\n')
            frags.append(FragmentReport(fragment, os.path.relpath(data_fn, start = directory)))
