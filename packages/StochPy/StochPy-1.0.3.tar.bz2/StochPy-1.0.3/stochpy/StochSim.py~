...
class SSA():
  ...
  def PlotTimeSim(self,species2plot = True,linestyle = 'solid',marker = '',colors = None,title = 'StochPy Time Simulation Plot'):
    """
    ...
    if IsPlot:
          try:
              ...
              print "Info: Use the print function: PrintTimeSim()"
  ...
  def PlotDistributions(self,species2plot = True, linestyle = 'dotted',colors=None,title = 'StochPy Probability Density Function',bin_size=1):
    ...
    if IsPlot:
        try:
            ...
            print "Info: Use the print function: PrintDistributions()"
  ...
  def PlotPropensities(self,rates2plot = True,linestyle = 'solid',marker = '',colors = None,title = 'StochPy Propensities Plot'):
    """ 
    Plot time simulation output for each generated trajectory
    ...
    if IsPlot:
        try:
            ...
            while traj <= self.SSA.Trajectories:	
                ...
                self.plot.Propensities(self.data_stochsim.getPropensities(),rates2plot,traj-1,linestyle,marker,colors,title) # Plot props time sim 
                if self.SSA.Trajectories > 1: self.plot.plotnum-=1
                traj+=1
            if traj > 1: self.plot.plotnum+=1
        except:
          ...
          for value in self.data_stochsim_interpolated.points:
              while n<len(xp):
                  if value > xp[n]: n+=1
                      ...
                      break
          if xp[-1] < self.data_stochsim_interpolated.points[-1]: temp_interpolated_output.append(temp_interpolated_output[-1])
          self.data_stochsim_interpolated.species[i].append(temp_interpolated_output)
          i+=1
      traj+=1
    self.data_stochsim_interpolated.means,self.data_stochsim_interpolated.standard_deviations,self.data_stochsim_interpolated.time = Analysis.ObtainInterpolationResults(self.data_stochsim_interpolated.species,self.data_stochsim_interpolated.points,self.data_stochsim_interpolated.points[-1])
  ...
  def Write2File(self,what='TimeSim', directory=None):
    ...
    if directory == None: directory = self.output_dir + '/' + self.model_file+'_' + what
       ...
       directory += '/' + self.model_file+'_' + what
    if what == 'TimeSim':
        if not self.IsSimulationDone: self.DoStochSim()
        traj = 1
        while traj <= self.SSA.Trajectories:
