...
from ztfy.jqueryui.browser import jquery_fancybox
...
class GalleryImageDefaultViewAdapter(object):
    ...
    def getAbsoluteURL(self):
        return """javascript:$.ZBlog.dialog.open('%s/%s')""" % (absoluteURL(self.context, self.request),
                                                                self.viewname)
...
class GalleryImagePropertiesExtension(object):
    ...
    @property
    def url(self):
        return """javascript:$.ZBlog.dialog.open('%s/@@properties.html')""" % absoluteURL(self.context, self.request)
