...
struct tree_descender
{
	...
	visit_node(
		...
		const double bg_log_likelihood_bound = _model.bg.get_min_log_likelihood( it );

		// get a bound on log(1 + g/(1-g))
		const double weight_bound = _model._prior._psp->get_term_bound( it );

		// the threshold on log(<Z>/(1-<Z>))
		const double log_Z_threshold_term = static_cast< Derived * >( this )->get_log_Z_threshold_term();

		// don't ignore the -1 in prior term even though is probably swamped by prior_term
		const double log_prior_term = weight_bound - _model._prior.lambda.get_log_lambda();
		const double log_prior_accurate = std::log( std::exp( log_prior_term ) - 1. );

		// get the bound on log p(X_n|\theta_BS)
		const double eval_threshold =
			log_Z_threshold_term
			+ log_prior_accurate
		...
		if( ! partial_eval_pair ) { // do we have an eval on either strand?
			...
			if( W == w ) {
				...
				static_cast< Derived * >( this )->descend( it, partial_eval_pair, w );
	...
	descend(
		...
		if( goDown( it ) ) {
			visit_node( it, partial_eval_pair, w );
			while( goRight( it ) ) {
				visit_node( it, partial_eval_pair, w );
			}
		}
	...
	get_log_Z_threshold_term() {
		const double lZt = static_cast< Derived * >( this )->get_log_Z_threshold();
		const double Zt = std::exp( lZt );
		MYRRH_ASSERT( 0. <= Zt );
		MYRRH_ASSERT( Zt <= 1. );
		const double lZt_term = lZt - std::log( 1. - Zt );
		return lZt_term;
	}
	...
	evaluate_W_mer(
		...
		for( size_t i = 0; num_occurrences != i; ++i ) {
			...
			const int global_pos = seqan::posGlobalize( occ, _data.get_string_set_limits() );
