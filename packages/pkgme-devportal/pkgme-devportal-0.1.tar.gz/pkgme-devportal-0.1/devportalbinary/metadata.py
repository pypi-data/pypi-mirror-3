...
class MetadataBackend(object):
    ...
    def dump_json(self, stream=sys.stdout):
        """Dump the information from this backend as JSON."""
        info = self.get_info(self.get_metadata())
        info_for_json = dict(
            (element.name, data) for element, data in info.items())
        json.dump(info_for_json, stream)
