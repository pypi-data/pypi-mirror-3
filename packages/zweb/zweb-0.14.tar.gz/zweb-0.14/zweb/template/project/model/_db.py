...
def mysql_config():
    ...
    from config import MYSQL_HOST , MYSQL_PORT , MYSQL_USER , MYSQL_PASSWD , MYSQL_PREFIX
    from os.path import dirname, abspath, join, exists, basename
    import sys
    sqlfarm = {}
    for i in glob('%s/*'%dirname(abspath(config.__file__))):
        if "." in basename(i):
            continue
        path = join(i, 'mysql.py')
        if exists(path):
            d = join(dirname(dirname(dirname(path))), 'app/%s/config'%basename(dirname(path)))
            sys.path.insert(0, d)
