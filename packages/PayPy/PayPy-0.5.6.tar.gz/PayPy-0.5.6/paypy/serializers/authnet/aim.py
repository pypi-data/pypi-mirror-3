...
class Serialize(object):
    ...
    def _to_string(self, schema):
        ...
        if trans.billing:
            ...
            if billto.firstname:
                transaction['x_first_name'] = urllib.quote(billto.firstname)
            if billto.lastname:
                transaction['x_last_name']  = urllib.quote(billto.lastname)
            if billto.company:
                transaction['x_company']    = urllib.quote(billto.company)
            if billto.address:
                transaction['x_address']    = urllib.quote(billto.address)
            if billto.city:
                transaction['x_city']       = urllib.quote(billto.city)
            if billto.state:
                transaction['x_state']      = urllib.quote(billto.state)
            if billto.postal_code:
                transaction['x_zip']        = billto.postal_code
            if billto.country:
                transaction['x_country']    = urllib.quote(billto.country)
            if billto.phone:
        ...
        if trans.shipping:
            ...
            if shipto.firstname:
                transaction['x_ship_to_first_name'] = urllib.quote(shipto.firstname)
            if shipto.lastname:
                transaction['x_ship_to_last_name']  = urllib.quote(shipto.lastname)
            if shipto.company:
                transaction['x_ship_to_company']    = urllib.quote(shipto.company)
            if shipto.address:
                transaction['x_ship_to_address']    = urllib.quote(shipto.address)
            if shipto.city:
                transaction['x_ship_to_city']       = urllib.quote(shipto.city)
            if shipto.state:
                transaction['x_ship_to_state']      = urllib.quote(shipto.state)
            if shipto.postal_code:
                transaction['x_ship_to_zip']        = shipto.postal_code
            if shipto.country:
                transaction['x_ship_to_country']    = urllib.quote(shipto.country)
            if shipto.phone:
