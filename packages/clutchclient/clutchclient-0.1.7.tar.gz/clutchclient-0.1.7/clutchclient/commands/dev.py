    ...
    if isinstance(path, basestring):
        try:
            with open(path, 'r') as f:
                conn.request('POST', parsed_url.path + uuid, f, headers)
        except IOError:
            conn.request('POST', parsed_url.path + uuid,
                'CLUTCH404DOESNOTEXIST', headers)
    else:
        conn.request('POST', parsed_url.path + uuid, json.dumps(path), headers)
