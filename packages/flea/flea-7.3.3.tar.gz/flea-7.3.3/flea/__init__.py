    ...
    def register(self, xpath, func):
        ...
        doc = 'For elements matching ``%s``:'\
              '\n%s\n\n.. comment block\n\n' % (
                    xpath,
                    '\n'.join('    %s' % line
                              for line in func.func_doc.split('\n'))
        )
        self.__doc__ += doc
...
class ElementWrapper(object):
    ...
    def fill(self, values):
        ...
        if isinstance(values, bool):
            ...
            found = set()
            values = set(unicode(v) for v in values)
    ...
    def html(self):
        ...
        """
        return tostring(self.el)

    def pretty(self):
        """
        Return an pretty-printed string representation of the element
        """
        return tostring(self.el, pretty_print=True)
...
class TestAgent(object):
    ...
    def __init__(self, app, request=None, response=None, cookies=None,
        ...
        :param validate_wsgi: If True, the application under test will be
                              wrapped in ``wsgiref.validate.wsgi_validator``
                              middleware.
        ...
        :param cookies: The cookie store to use for the TestAgent.
    ...
    def make_environ(self, REQUEST_METHOD='GET', PATH_INFO='', wsgi_input='',
                     **kwargs):
        """
    ...
    def _request(self, environ, follow=True, history=False, check_status=True):
        ...
        if self.logger:
            self.logger.info("%s %s", environ['REQUEST_METHOD'],
                                      Request(environ).request_uri)
            if environ['HTTP_COOKIE']:
                self.logger.debug("Cookie: %s", environ['HTTP_COOKIE'])
            postdata = environ['wsgi.input'].getvalue()
            environ['wsgi.input'].seek(0)
            if postdata:
                self.logger.info("wsgi.input: %r", postdata)

        response = self.response_class.from_wsgi(self.app, environ,
                                                 self.start_response)
        if self.logger:
            self.logger.info('Response: %s', response.status)
            for name, value in response.headers:
                self.logger.debug('Response: %s: %s', name, value)
    ...
    def get(self, PATH_INFO='/', data=None, charset='UTF-8', follow=True,
        ...
        if self.request:
            PATH_INFO = uri_join_same_server(self.request.request_uri,
                                             PATH_INFO)
    ...
    def reload(self, follow=True, check_status=True):
        ...
        """
        self.logger and self.logger.debug("Reload", self.request.request_path)
...
def uri_join_same_server(baseuri, uri):
    ...
    protocol/netloc then a ``ValueError`` will be raised.
        ...
        '/bar'
        >>> uri_join_same_server('http://localhost/foo',
        ...                      'http://localhost/bar')
        '/bar'
