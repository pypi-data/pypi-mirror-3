...
def addSearchOptions(optionManager):
    ...
    optionManager.add_option('--add-all-matches',
                             action='store_true',dest='allmatches',
                             default=False,
                             help='Add an extra column in the output with the the list of matches and identities')    
...
#    print "##########@",options.large
    ...
    for seq,best,match in search:
        ...
        if best[0] is not None:
            ...
            all_matches = set((x[0],x[1]) for x in match)
