...
class cl_MDIChild( Frame ):
    ...
    def __init__( self, pw_container, global_app, **kw ):
        ...
        self.__ao_mdiapp = global_app
        self.__ao_mdiparent = parent_widget
    ...
    def init_subchild( self, pv_id, pv_name ):
        ...
        lo_child = self.__ao_mdiapp.add_subchild( lv_subchildid, pv_name, self )
    ...
    def wrap_subchild( self, pv_id ):
        ...
        lo_child.propagate()
        self.__ao_mdiapp.bind_child_onActivate(lo_child, lv_subchildid)
    ...
    def show_subchild( self, pv_id ):
        ...
        self.__ao_mdiapp.look_at_child(lv_subchildid, 'show_sub')
    ...
    def fill_wp(self):
        ...
        b1 = Button( f, text='set as name',
                     command=lambda x=1: self.__ao_mdiapp.configure_wmenu_item( self.__id, 'rename', label=lw_entry.get()) )
        ...
        b3 = Button( f, text='remove help',
                     command=lambda x=1: self.set_help( [] ) )
        b3.pack(side=LEFT, padx=2)

        b4 = Button( f, text='add help',
                     command=lambda x=1: self.set_help( [lw_entry.get()] ) )
    ...
    def xbind(self, event, command):
        ...
        self.append_xbind( event, command )
        
    def get_xbinds( self ):
        """ return copy of xbinds """
        
        return self.__al_xbinds[:]
    
    def append_xbind( self, event, command ):
        """ add xbind to list """
        
        self.__al_xbinds.append( ( event, command ) )
        
    def reapply_xbinds( self ):
    ...
    def get_mdiapp( self ):
        """ return ref to mdi app """
        
        return self.__ao_mdiapp
    
    def get_mdiparent( self ):
        """ return ref to mdi parent """
        
        return self.__ao_mdiparent
...
class cl_MDIApp():
        ...
        if self.__aw_active_child is not None:
            ...
            if children_len > 0:
                ...
                for child_id in self.__al_children:
                    ...
                    if lw_child.get_mdiparent() is None:
    ...
    def del_child(self, p_windowid):
        ...
        if lw_child is not None:
            lw_mdiparent = lw_child.get_mdiparent()
            
            if lw_mdiparent is None:
                # configure menu
                ...
                # configure menu
                self.configure_wmenu_item( lw_mdiparent.get_id(), 'del_sub', wid=p_windowid )

                lw_mdiparent.close_subchild( p_windowid )
    ...
    def call_del_child(self, p_windowid):
        ...
        if self.__aw_active_child is not None:
            ...
            lv_parent = self.__aw_active_child.get_mdiparent()
    ...
    def get_active_child( self ):
        ...
        return self.__aw_active_child
        
    def get_active_child_id( self ):
        """ return id of active child """
        
        if self.__aw_active_child is not None:
            return self.__aw_active_child.get_id()
        else:
            return None
    
    def active_child_change_wstate( self, pv_action=CHILD_ACTIVE ):
        """ change state of active child window """
        
        if self.__aw_active_child is not None:
            if pv_action == CHILD_ACTIVE:
                self.__aw_active_child.activate()
            elif pv_action == CHILD_DISABLED:
                self.__aw_active_child.deactivate()
