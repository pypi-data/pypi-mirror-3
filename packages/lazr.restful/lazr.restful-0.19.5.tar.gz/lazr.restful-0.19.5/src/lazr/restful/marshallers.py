...
class DictFieldMarshaller(SimpleFieldMarshaller):
    ...
    def __init__(self, field, request):
        ...
        self.key_marshaller = getMultiAdapter(
            (field.key_type, request), IFieldMarshaller)
        self.value_marshaller = getMultiAdapter(
            (field.value_type, request), IFieldMarshaller)
    ...
    def _marshall_from_request(self, value):
        ...
        If the value isn't a list or dict, transform it into a one-element list.
        That allows web client to submit one-element list of strings
        without having to JSON-encode it.
        """
