...
def merge_css( src_list):
    result = [ ]
    for i in src_list:
        result.append("""@import url(%s);"""%(i[len(_env.PREFIX):]))
    return result
...
def merge_js( src_list):
    ...
    for i in src_list:
        result.append("""LOAD('http://%s%s%s')"""%(HOST_DEV_PREFIX, HOST_CSS_JS, i[len(_env.PREFIX):]))
    return result
