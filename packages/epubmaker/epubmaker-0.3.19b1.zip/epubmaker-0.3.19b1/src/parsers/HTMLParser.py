...
class Parser (HTMLParserBase):
    ...
    def find_coverpage (self):
        ...
        for head in xpath (self.xhtml, 'xhtml:head'):
            for link in xpath (head, 'xhtml:link[@rel = "coverpage"]'):
                # already there
                return
    ...
    def pre_parse (self):
        ...
        lightweight parse would be almost the same work. """
        ...
        # self._strip_pagenumbers ()
        self._fix_anchors () # needs relative paths
        # FIXME: lxml bug?
        # etree.fromstring quotes base_url twice
