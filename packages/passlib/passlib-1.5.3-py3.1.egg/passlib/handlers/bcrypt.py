...
class bcrypt(uh.HasManyIdents, uh.HasRounds, uh.HasSalt, uh.HasManyBackends, uh.GenericHandler):
    ...
    def to_string(self, native=True):
        hash = "%s%02d$%s%s" % (self.ident, self.rounds, self.salt, self.checksum or '')
    ...
    def _hash_needs_update(cls, hash, **opts):
        if isinstance(hash, bytes):
            hash = hash.decode("ascii")
        if hash.startswith("$2a$") and hash[28] not in BSLAST:
            return True
        return False
    ...
    def _has_backend_os_crypt(cls):
        ...
        return bool(safe_os_crypt and safe_os_crypt("test",h1)[1]==h1 and
                    safe_os_crypt("test", h2)[1]==h2)
