...
#
# status_code, headers, content = pmnc.transaction.\
...
try:
    ...
    have_gzip = True
    try:
        from gzip import compress as gzip_compress # Python 3.2+ has this
    except ImportError:
        def gzip_compress(data):
            r = BytesIO()
            with gzip.GzipFile(fileobj = r, mode = "wb") as gz:
                gz.write(data)
            return r.getvalue()
...
_valid_headers = dict_of(_valid_header, str)

_valid_header_lc = by_regex("^[a-z0-9_-]+$")
_valid_headers_lc = dict_of(_valid_header_lc, str)

_valid_http_version = by_regex("^HTTP/1\\.(?:0|1)$")

_zero_qvalue = by_regex("^[ \t]*[Qq][ \t]*=[ \t]*0(?:.0+)?[ \t]*$")
...
class Interface: # HTTP interface built on top of TCP pseudo-interface
    ...
    def __init__(self, name: str, *,
                 ...
                 gzip_content_types: tuple_of(str),
        ...
        handler_factory = lambda prev_handler: \
            pmnc.protocol_http.Handler(prev_handler, self.process_http_request,
                                       ...
                                       gzip_content_type = by_regex("^(?:{0:s})$".format("|".join(s for s in gzip_content_types))) \
                                                           if gzip_content_types else lambda s: False)
...
class Handler: # this class is instantiated from interface_tcp
    ...
    def __init__(self, prev_handler,
        ...
        self._gzip_content_type = gzip_content_type
    ...
    def process_tcp_request(self):
        ...
        try:
            ...
            response_content_type = response_headers.\
                setdefault("content-type", "application/octet-stream").lower()
            ...
            # compress the response if so configured and the client accepts gzipped response

            if response_content and self._gzip_content_type(response_content_type) and have_gzip:
                for accepted_encoding in headers.get("accept-encoding", "identity; q=1.0").split(","):
                    coding, *qvalue = accepted_encoding.split(";", 1)
                    if coding.strip().lower() in ("gzip", "*") and (not qvalue or not _zero_qvalue(qvalue[0])):
                        response_content = gzip_compress(response_content)
...
def self_test():
    ...
    def test_compression():
        ...
        def process_http_request(request, response):
            ...
            if request["url"] == "/":
                ...
                response["content"] = "foo" * 10000
            elif request["url"] == "/jpeg":
                response["headers"]["content-type"] = "image/jpeg"
                response["content"] = b"JFIF"

        with active_interface("http_1", **interface_config(process_http_request = process_http_request, gzip_content_types = ("text/.+", "image/bmp"))) as ifc:

            s = sendall(ifc, b"GET / HTTP/1.0\r\nAccept-Encoding: gzip\r\n\r\n")
            resp = recvall(s)
            assert resp.startswith(b"HTTP/1.1 200 OK\r\n")
            h, c = resp.split(b"\r\n\r\n", 1)
            h = dict(tuple(s.split(": ")) for s in h.strip().decode("ascii").split("\r\n")[1:])
            assert h["Content-Encoding"] == "gzip" and h["Content-Type"] == "text/plain; charset=ascii" and int(h["Content-Length"]) < 100
            gz = gzip.GzipFile(fileobj = BytesIO(c), mode = "rb")
            assert gz.read() == b"foo" * 10000

            s = sendall(ifc, b"GET / HTTP/1.0\r\nAccept-Encoding: identity; q=1.0, gzip;q=0\r\n\r\n")
            resp = recvall(s)
            assert resp.startswith(b"HTTP/1.1 200 OK\r\n") and b"gzip" not in resp and len(resp) > 30000
            ...
            resp = recvall(s)
            assert resp.startswith(b"HTTP/1.1 200 OK\r\n") and b"gzip" in resp and len(resp) < 1000

            s = sendall(ifc, b"GET / HTTP/1.0\r\n\r\n")
            resp = recvall(s)
            assert resp.startswith(b"HTTP/1.1 200 OK\r\n") and b"gzip" not in resp and len(resp) > 30000

            s = sendall(ifc, b"GET /jpeg HTTP/1.0\r\n\r\n")
            resp = recvall(s)
            h, c = resp.split(b"\r\n\r\n", 1)
            h = dict(tuple(s.split(": ")) for s in h.strip().decode("ascii").split("\r\n")[1:])
            assert h["Content-Type"] == "image/jpeg" and "Content-Encoding" not in h and c == b"JFIF"
    ...
    def test_resource_server_misbehaves():
        ...
        # the following two tests fail if the data if the fifth content byte
        # is sent separately, be sure to disable local firewalls
    ...
    def test_resource_transaction(): # usage in transaction, this utilizes config_resource_http_1.py
        ...
        with active_interface("http_1", **interface_config(process_http_request = process_http_request)) as ifc:
            ...
            status_code, headers, content = pmnc.transaction.\
                http_1.post("/foo?%00+bar", b"\x00\x00\x00", { "X-Foo": "Bar", "CONTENT-TYPE": "zig/zag" })
            assert status_code == 200
            ...
            status_code, headers, content = pmnc.transaction.\
                http_1.get("/foo?%00+bar", { "CONNECTION": "CLOSE" })
            assert status_code == 200
