...
class GSock(GObj):
    ...
    *Output-Events:*
    ...
    def handle_read_event(self):
        #self.logger and self.logger.debug("handle_read_event    (%s %s)" % (self, self.name))
        if self._accepting:
    ...
    def handle_connect_event(self):
        #self.logger and self.logger.debug("handle_connect_event (%s %s)" % (self, self.name))
    ...
    def handle_write_event(self):
        #self.logger and self.logger.debug("handle_write_event   (%s %s)" % (self, self.name))
        if self._accepting:
    ...
    def handle_accept(self):
        try:
            ...
            logging.warning('server accept() threw an exception', exc_info=True)
    ...
    def handle_read(self):
        ...
        self.rxed_bytes += ln
        if self.trace_dump:
            self.logger and self.logger.debug("Recv data '%s' (%d bytes)\n%s" % (
