...
#if HAS_PAM_FAIL_DELAY
static void _delay_fn(int, unsigned, void *);
#endif
...
handle_init(handle *self, PyObject *args, PyObject *kws)
{
    ...
    if (pam_delay_set)
    {
        self->rc = pam_set_item(self->handle, PAM_FAIL_DELAY, &_delay_fn);
        if (self->rc != PAM_SUCCESS)
        {
...
{
    opaque_data opq = *(opaque_data *)data;
    handle *self = opq.self;
    PyObject *delay_fn = opq.delay;
    PyObject *cbdata = opq.data;
    PyObject *args = Py_BuildValue("iIO", retval, usec_delay, cbdata);
    PyObject *result = PyObject_Call(delay_fn, args, NULL);
    Py_XDECREF(result);
}
...
static int handle_setter(handle *self, PyObject *dataobj, void *key)
{
    ...
    switch (itype)
    {
        ...
        switch (itype)
        {
            ...
            pam_set_item(self->handle, PAM_FAIL_DELAY, &_delay_fn);
            break;
#endif
        }
        Py_INCREF(dataobj);
