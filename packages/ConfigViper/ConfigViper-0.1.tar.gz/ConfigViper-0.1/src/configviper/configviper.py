...
class ConfigViper(object):
    ...
    def exists(self, path):
        ...
        return data.has_key(head)
    
            
    def get(self, path):
        """Get value for the given config-path."""
        # assume path = 'a/b/c/d', so names will be ['a', 'b', 'c', 'd']
        names = path.split(self._path_separator)
        tail = names[:-1]  # ['a', 'b', 'c']
        head = names[-1]   # 'd'
        ...
        converter = self._converter_for(path)
    ...
    def stabilize(self, values):
        ...
        for path, value, converter in values:
            ...
            self._converters_map[path] = converter
            if self.exists(path):
    ...
    def _converter_for(self, path):
        converter = self._converters_map.get(path, None)
