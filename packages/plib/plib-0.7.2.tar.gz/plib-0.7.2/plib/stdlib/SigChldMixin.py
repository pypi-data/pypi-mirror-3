...
class SigChldMixin(object):
    ...
    the ``ProcessWrapper`` class in ``plib.utils._processwrapper``,
    ...
    def child_sig_handler(self, sig, frame):
        """Respond to SIGCHLD by reaping dead children.
        
        The reaping can be delayed by setting the class flag
        ``reap_on_signal`` to ``False``; in this case,
        the derived class must call ``reap_children``
        somewhere else (see the ``SigChldServerMixin`` class
        for an example).
        """
        
        self.child_waiting = True
        if self.reap_on_signal:
            self.reap_children()
    ...
    def reap_children(self):
        ...
        *Python* signal handler to run when we want it to run, which is
        ...
        if self.child_waiting and not self.reaping:
            self.reaping = True
            self._wait_children()
            self.reaping = False
