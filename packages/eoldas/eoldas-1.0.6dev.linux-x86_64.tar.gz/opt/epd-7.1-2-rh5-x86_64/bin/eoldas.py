...
def eoldas_demo():
    '''
    Standard running of eoldas
        
    e.g. 
        
    eoldas.py --conf=default.conf --calc_posterior_unc --write_results
        
    '''
    argv = sys.argv
    this = [argv[0].replace('.py','')]
    argv[0] = '--conf=eoldas_config.conf'
    [this.append(i) for i in argv]
    #pdb.set_trace()
    self = eoldas(this)
    self.solve()
