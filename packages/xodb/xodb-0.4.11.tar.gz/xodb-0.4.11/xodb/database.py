...
class Database(object):
    ...
    def check_prefix(self, name, boolean=False):
        ...
        prefixes.union(self.boolean_prefixes.keys())
    ...
    def reopen(self, retry_limit=RETRY_LIMIT, refresh=True):
        """
        ...
        if refresh:
            try:
                op = lambda: self.backend.metadata_keys()
                # don't recurse into refresh here, just reopen and retry
                keys = self.retry_if_modified(op, retry_limit, False)
