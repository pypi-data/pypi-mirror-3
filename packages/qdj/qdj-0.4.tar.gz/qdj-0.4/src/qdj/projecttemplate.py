...
def create_django_project(project_path):
    prev_cwd = os.getcwd()
    try:
        os.chdir(os.path.dirname(project_path))
        call_command('startproject', os.path.basename(project_path))
    finally:
        os.chdir(prev_cwd)
...
class CreateProjectTemplate(object):
    ...
    def add_blank_hooks(self):
        hooks_source = textwrap.dedent('''
            def post_create():
                """
                This hook is called after a project is created.

                Your current path is the project's directory.

                Django should be configured, unless you've changed your settings layout -
                if so, you need to set os.environ['DJANGO_SETTINGS_MODULE']
                ...
                Example post_create():
                    ...
                    ...     import subprocess
                    ...
                    ...     # Initialise git, make an initial commit
                    ...     subprocess.call(['git', 'init'])
                    ...     subprocess.call(['git', 'add', '*.py', '*.txt'])
                    ...     subprocess.call(['git', 'commit', '-m', 'Initial commit'])
                    ...
                    ...     # Run syncdb (your template must have working database settings)
        ...
        open(os.path.join(self.template_path, 'hooks.py'), 'w').write(hooks_source)
