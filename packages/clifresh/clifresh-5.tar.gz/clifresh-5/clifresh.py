import os,argparse,json,dateutil.parser
...
log_parser.add_argument("-d","--date",type=dateutil.parser.parse,required=False, default=None)
...
try:
	configfile = open(CONFIG_FILE_LOCATION,"r")
except IOError:
	configfile = open(CONFIG_FILE_LOCATION,"w+")
	config = {}
...
def log_time(freshbooks,project_id,task_id,hours,notes,date):
...
def main():
	...
	if args.action == "projects":
		...
		log = log_time(freshbooks,args.project,args.task,args.hours,args.message, args.date)
