...
class ConsentWebTestCase(WebTestCase):
    ...
    def setUp(self):
        ...
        self.kuiAssemblyTitle = 'kuitesttitle'
        self.kuiAssemblyDescription = 'kuitest assembly description'
        self.kuiAssemblyLicense = '/licenses/1'
        ...
        self.urlAssemblies = self.url_for('assemblies', action='index')
        self.urlAssemblySearch = self.url_for('assemblies', action='search', id=None)
        self.urlAssemblyCreate = self.url_for('assemblies', action='create')
        self.urlAssemblyRead = self.url_for('assemblies', action='read', id=self.kuiAssemblyName)
        self.urlAssemblyUpdate = self.url_for('assemblies', action='edit', id=self.kuiAssemblyName)
        self.urlAssemblyDelete = self.url_for('assemblies', action='delete', id=self.kuiAssemblyName)

        self.urlAssemblyJoin = self.url_for('assemblies', action='join', id=self.kuiAssemblyName)

        self.urlAssemblyMembers = self.url_for('assemblies.admin', assembly=self.kuiAssemblyName,
            subcontroller='members')
        self.urlAssemblyMemberCreate = self.url_for('assemblies.admin', assembly=self.kuiAssemblyName,
            subcontroller='members', action='create')
        self.urlAssemblyMemberUpdate = self.url_for('assemblies.admin', assembly=self.kuiAssemblyName,
            subcontroller='members', action='edit', id=self.kuiPersonName)
        self.urlAssemblyMemberDelete = self.url_for('assemblies.admin', assembly=self.kuiAssemblyName,
            subcontroller='members', action='delete', id=self.kuiPersonName)

        self.urlAssemblyServices = self.url_for('assemblies.admin', assembly=self.kuiAssemblyName,
            subcontroller='services')
        self.urlAssemblyServiceCreate = self.url_for('assemblies.admin', assembly=self.kuiAssemblyName,
