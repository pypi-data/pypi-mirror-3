...
# TODO
#08:46        centralservice.inbox [ListContacts] from tcp://67.207.147.183:50105 handled
#08:46            nitro.selectreactor    ERROR   Error in timer callback
#Traceback (most recent call last):
#  File "C:\work\datahaven\nitro\selectreactor.py", line 127, in runOnce
#    ti.callback()
#  File "C:\work\datahaven\nitro\async.py", line 18, in notify
#    if self.callback : self.callback( *args, **kwargs )
#  File "C:\work\datahaven\lib\cspace_application.py", line 361, in doWaitNewFiles
#    self.stream.sendMessage(struct.pack("!Q", self.length))
#  File "C:\work\datahaven\nitro\tcp.py", line 373, in sendMessage
#    self.stream.writeData( msg )
#  File "C:\work\datahaven\nitro\tcp.py", line 273, in writeData
#    self.reactor.addWriteCallback( self.sock.fileno(), self._onWrite )
#  File "<string>", line 1, in fileno
#  File "C:\work\soft\Python25\lib\socket.py", line 146, in _dummy
#    raise error(EBADF, 'Bad file descriptor')
#error: (9, 'Bad file descriptor')
#08:47      contacts.getContact WARNING who is http://identity.datahaven.net/atg.xml ?
#08:47      identitydb.get file http###identity.datahaven.net#dzeaman.xml not exist
#08:47            cspace.application     INFO   onContactsProbed
...
#        dhnio.Dprint(4, 'transport_cspace.init.status_callback my keyID is %s, status=%s, thread is %s' % (
#            cspace_application.getKeyID(), str(x), threading.currentThread().getName()))
#        global _State
#        _State = x
...
def config():
    try:
        ...
        if misc.getLocalID() == settings.CentralID():
            cspace_application.init_log()
        else:
            logLevel = 50 - dhnio.DebugLevel * 8
            if logLevel < 0:
                logLevel = 0
            cspace_application.init_log(logLevel, 'dhnio', None)
...
def onSent(host, filename, status, proto='', error=None, message=''):
    push_cspace_event('sent', (host, filename, status, proto, error, message))


def onReceive(filename, status, proto='', host=None, error=None, message=''):
    push_cspace_event('received', (filename, status, proto, host, error, message))
...
def onStatus(status):
    ...
    if _Status != status:
        ...
        _Status = status
        push_cspace_event('status', status)


def onContactStatus(keyid, status):
    push_cspace_event('contact-status', (keyid, status))
...
def send(keyID, filename):
    dhnio.Dprint(14, 'transport_cspace.send %s %s' % (keyID, os.path.basename(filename)))
...
#                os._exit(x)
#                return
            reactor.callLater(5, send, keyID, filename)
