...
class Parser(object):
    def __init__(self, verbosity=0,
        ...
        #We need to retain all these options cause we will
        #recreate the parse_options if the parser starts failing
        ...
        self._create_parse_options()
        self.dictionary = Dictionary("en")

    def _create_parse_options(self):
        self.parse_options = ParseOptions(verbosity=self.verbosity,
                                          linkage_limit=self.linkage_limit,
                                          min_null_count=self.min_null_count,
                                          max_null_count=self.max_null_count,
                                          null_block=self.null_block,
                                          islands_ok=self.islands_ok,
                                          ...
                                          disjunct_cost=self.disjunct_cost,
    ...
    def parse_sent(self, txt):
        ...
        #This is ugly, but the parser sometiems starts failing on parses.
        #if it fails, recreate parse_options one time and try again. This will correct the issue.
        #If the sentence truly cannot be parsed, then it'll fail on the second time.
        if n == 0:
            self._create_parse_options()
            n = clg.sentence_parse(sent, self.parse_options._po)
        if n > 0:
        ...
        #linkgrammar will start to poop its pants if its 'resources' are exhausted.
        #This is a little ugly to check each time but it seems to be a cheap operation.
        #If the resources are exhausted, then reset them.
        #Extra note: apparently resetting them was not enough. If you kept parsing
        #the same sentence over and over, eventually the parser were stop returning
        #parses. I found that deleting and reinstantiating the parse_options corrected
        #this issue.
        if clg.parse_options_resources_exhausted(self.parse_options._po):
            clg.parse_options_reset_resources(self.parse_options._po)
...
class ParseOptions(object):
    ...
    def max_memory():
        doc = "Determines the maximum memory allowed during parsing. This is used just as max_parse_time is, so that the parsing process is terminated as quickly as possible after the total memory (including that allocated to all dictionaries, etc.) exceeds the maximum allowed."
        def fget(self):
            return clg.parse_options_get_max_memory(self._po)
        def fset(self, value):
            if not isinstance(value, int):
                raise TypeError("max_memory must be set to an integer")
            if value < 0:
                raise ValueError("max_memory must be positive")
            clg.parse_options_set_max_memory(self._po, value)
        return locals()
    max_memory = property(**max_memory())
