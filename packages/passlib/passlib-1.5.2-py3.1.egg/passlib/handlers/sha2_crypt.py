    ...
    * a pure python implementation of SHA256-Crypt built into passlib.
    ...
    def to_string(self, native=True):
        if self.rounds == 5000 and self.implicit_rounds:
            hash = "$5$%s$%s" % (self.salt, self.checksum or '')
        else:
            hash = "$5$rounds=%d$%s$%s" % (self.rounds, self.salt, self.checksum or '')
    ...
    def _has_backend_os_crypt(cls):
        h = "$5$rounds=1000$test$QmQADEXMG8POI5WDsaeho0P36yK3Tcrgboabng6bkb/"
        return bool(safe_os_crypt and safe_os_crypt("test",h)[1]==h)
...
class sha512_crypt(uh.HasManyBackends, uh.HasRounds, uh.HasSalt, uh.GenericHandler):
    ...
    * a pure python implementation of SHA512-Crypt built into passlib.
    ...
    def to_string(self, native=True):
        if self.rounds == 5000 and self.implicit_rounds:
            hash = "$6$%s$%s" % (self.salt, self.checksum or '')
        else:
            hash = "$6$rounds=%d$%s$%s" % (self.rounds, self.salt, self.checksum or '')
        return to_hash_str(hash) if native else hash
    ...
    @classproperty
    def _has_backend_os_crypt(cls):
        h = "$6$rounds=1000$test$2M/Lx6MtobqjLjobw0Wmo4Q5OFx5nVLJvmgseatA6oMnyWeBdRDx4DU.1H3eGmse6pgsOgDisWBGI5c7TZauS0"
        return bool(safe_os_crypt and safe_os_crypt("test",h)[1]==h)
