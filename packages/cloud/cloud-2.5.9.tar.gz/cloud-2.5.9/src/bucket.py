...
def put(source, name=None):
    ...
    If ``name`` is specified, the bucket object will be stored as name on PiCloud.
    ...
    if name is None:
        ...
        raise ValueError('Cannot upload bucket object with name "%s"' % name)
...
def putf(f, name):
    ...
    if name[-1] == '/':
        raise ValueError('Cannot end a name with a forward slash')
    
    if name[0] == '/':
        raise ValueError('Cannot begin a name with a forward slash')
    
    if '//' in name:
        raise ValueError('Cannot have consecutive forward slashes in name')
    ...
    try:
        cloudLog.debug('bucket object name in client: %s' % name)
        # get a file ticket
        resp = conn.send_request(_bucket_new_query, {'name': name})
        ticket = resp['ticket']
        params = resp['params']
...
def list(prefix=None, delimiter=None, marker=None, max_keys=1000):
    """List all bucket objects stored on PiCloud."""
...
def _file_info(name):
    ...
    resp = conn.send_request(_bucket_exists_query, {'name':name})
    return resp

def exists(name):
    """Check if a bucket object named ``name`` is stored on PiCloud."""
    conn = _getcloudnetconnection()
...
def get_md5(name, log_missing_file_error = True):
    """Return the md5 checksum of the bucket object named ``name`` stored on PiCloud"""
    conn = _getcloudnetconnection()
    resp = conn.send_request(_bucket_md5_query, {'name': name},
                             log_cloud_excp = log_missing_file_error)
    md5sum = resp['md5sum']
    return md5sum
    
def remove(name):
    """Removes the bucket object named ``name`` from PiCloud."""
    conn = _getcloudnetconnection()

    resp = conn.send_request(_bucket_remove_query, {'name': name})
    removed = resp['removed']
    return removed

def make_public(name):
    """Makes the bucket object named ``name`` publicly accessible by a URL"""
    conn = _getcloudnetconnection()
    
    resp = conn.send_request(_bucket_make_public_query, {'name': name})
    public_url = S3_URL+resp['url']
    return public_url

def is_public(name):
    """Checks if the bucket object named ``name`` is publicly accessible by a URL"""
    conn = _getcloudnetconnection()
    
    resp = conn.send_request(_bucket_is_public_query, {'name': name})
    if resp['status']:
        public_url = S3_URL+resp['url']
        return public_url
    else:
...
def make_private(name):
    """Removes the publically accessible URL (if any) associated with the bucket object named ``name``"""
    conn = _getcloudnetconnection()
    
    resp = conn.send_request(_bucket_make_private_query, {'name': name})
    
def get(name, destination=None, start_byte=0, end_byte=None):
    """
    Retrieves the bucket object named by ``name`` from PiCloud and stores it to ``destination``.
...
def sync_from_cloud(name, destination=None):
    ...
    cloud.bucket.get(name,destination) 
...
def getf(name, start_byte=0, end_byte=None):
    ...
    resp = conn.send_request(_bucket_get_query, {'name': name})
