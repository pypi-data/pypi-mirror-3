...
class TestOpenForWriting(TestCase):
    ...
    def test_directory_provided(self):
        temp_path = self.useFixture(TempdirFixture()).path
    ...
    def test_working_directory_no_name(self):
        working_dir = self.useFixture(TempdirFixture()).path
        fd, path = utils._open_file_for_writing(
...
class TestDownloadFile(TestCase):
    ...
    def test_http_error_includes_url(self):
        ...
        tempdir.create_file('thing.txt', 'whatever')
        fp = open(tempdir.abspath('thing.txt'), 'r')
        def urlopen(url):
...
class IsImageTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.img_path = os.path.join(self.tempdir.path, "foo.png")
    ...
    def test_is_not_quite_the_right_image(self):
        ...
        with ExpectedException(MismatchError):
            no_img = os.path.join(self.tempdir.path, "bar.png")
