# -*- coding: utf-8 -*-
#------------------------------------------------------------------------------
# file: $Id: router.py 28 2012-06-28 03:26:05Z griff1n $
# lib:  pysyncml.router
...
# tbd: i think this class has mostly been deprecated by the introduction
...
class Router(object):
  ...
  def getTargetUri(self, sourceUri, mustExist=True):
    ...
    if sourceUri in self.routes:
      ...
      if targetUri not in self.adapter.peer.stores:
        if mustExist:
          # tbd: should i raise a different error here?
  ...
  def addRoute(self, sourceUri, targetUri, autoMapped=False):
    ...
    # tbd: this is ignoring the manual routes... i.e. a manual route should
  ...
  def recalculate(self, session):
    ...
    for store in self.adapter.stores.values():
      ...
      newstates[store.uri] = common.adict(
        # TBD: perhaps share this "constructor" with protocol/adapter?...
...
#------------------------------------------------------------------------------
# end of $Id: router.py 28 2012-06-28 03:26:05Z griff1n $
#------------------------------------------------------------------------------
