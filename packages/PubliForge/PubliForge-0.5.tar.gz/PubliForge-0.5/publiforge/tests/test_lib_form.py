# $Id: test_lib_form.py a8f1b45b2ed8 2011/12/10 10:19:48 patrick $
...
class UnitTestLibFormBegin(unittest.TestCase):
    ...
    def test_begin_unsecure_form(self):
        ...
        expected_result = literal(u'<form action="url" method="post">')
    ...
    def test_begin_secure_form(self):
        ...
        html = form.begin(url='url')
        expected_result = \
            literal(u'<form action="url" method="post"><div><input id="_csrf"\
 name="_csrf" type="hidden" value="csrft" /></div>')
        self.assertEqual(html, expected_result)
