...
class ZenPerfLDAPTask(ObservableMixin):
    ...
    def doTask(self):
        ...
        try:
            ...
            return self._failure(Failure(ldap.LDAPError('authentication failure: %s/%s: check credentials!' % (ldapuri, ldapcreds[0]))))

        try:
            resultseq = slapd.search_s(ldapfilter, ldap.SCOPE_SUBTREE)
        except:
            return self._failure(Failure(ldap.LDAPError('search failure: %s?%s: is cn=monitor activated?' % (ldapuri, ldapfilter))))
