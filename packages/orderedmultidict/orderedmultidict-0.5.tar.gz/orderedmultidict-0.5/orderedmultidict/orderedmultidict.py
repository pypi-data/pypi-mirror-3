#
# OrderedMultiDict: Ordered Multivalue Dictionary that retains method parity
#   with dict().
#
# Arthur Grunseid
# grunseid.com
...
class OrderedMultiDict():
  ...
  key. This means that OrderedMultiDict() retains method parity with dict(), and
  a dict() object can be replaced with an OrderedMultiDict() object and all
  interaction will behave identically. Every dict() method retains parity:
  ...
  New methods have also been added to OrderedMultiDict(). Methods with 'list' in
  their name interact with lists of values, and methods with 'all' in their name
  interact with all items in the dictionary, including multiple items with the
  same key.
 
  The new methods of OrderedMultiDict() are:
