# $Id: storage.py de5826a61f18 2012/02/27 17:59:57 patrick $
...
from ..lib.views import get_action, paging_users, current_storage
from ..lib.views import vcs_user, file_download, create_pack, add2container
...
STORAGE_SETTINGS_TABS = (_('Description'), _('Authorized users'))
REFRESH = {
...
class StorageView(object):
    ...
    def admin(self):
        ...
        return {
            ...
            'i_modifier': has_permission(self._request, 'stg_modifier'),
    ...
    @view_config(route_name='storage_view',
                 ...
                 permission='stg.read')
    def view(self):
        """Show storage settings with its users."""
    ...
    def create(self):
        ...
        return {
            ...
            'STORAGE_ACCESS': STORAGE_ACCESS, 'REFRESH': REFRESH,
            'perms': STORAGE_PERMS, 'vcs_engines': self._vcs_engines,
            'storage': None, 'users': None, 'paging': None, 'groups': None}
    ...
    def edit(self):
        ...
        action = get_action(self._request)[0]
        if action == 'exp!':
            return self._export_storages((storage.storage_id,))
        elif action[0:4] == 'add!':
            ...
            DBSession.query(StorageUser).filter_by(
                storage_id=storage.storage_id, user_id=int(action[7:]))\
                .delete()
            DBSession.commit()
            del self._request.session['menu']
        ...
        if action == 'sav!' and form.validate(storage):
            ...
            for user in storage.users:
                user.perm = form.values['perm_%d' % user.user_id]
            DBSession.commit()
        ...
        return {
            ...
            'STORAGE_ACCESS': STORAGE_ACCESS, 'REFRESH': REFRESH,
            'perms': STORAGE_PERMS, 'vcs_engines': self._vcs_engines,
            'storage': storage, 'paging': paging, 'users': users,
            'groups': groups}
    ...
    def _paging_storages(self, user_id=None):
        ...
        if user_id is not None:
            ...
            query = query.filter(User.login == params['f_login'])\
                .filter(StorageUser.user_id == User.user_id)\
                .filter(Storage.storage_id == StorageUser.storage_id)
        if 'f_id' in params:
        ...
        if 'f_access' in params and params['f_access'] != '*':
            query = query.filter_by(access=params['f_access'])
        elif not 'f_access' in params:
            query = query.filter(Storage.access != 'closed')
        if 'f_engine' in params:
            query = query.filter_by(vcs_engine=params['f_engine'])
    ...
    def _export_storages(self, storage_ids):
        ...
        for storage in DBSession.query(Storage)\
                ...
                .order_by('label'):
            if i_modifier or storage.access == 'open' \
    ...
    def _browse_action_container(self, action, items, path, form):
        ...
        # Create pack
            ...
            # for item in items:
            #     pack.files.append(
            #         ProjectPackFile('file', normpath(join(path, item))))
            # DBSession.add(pack)
            # DBSession.commit()
