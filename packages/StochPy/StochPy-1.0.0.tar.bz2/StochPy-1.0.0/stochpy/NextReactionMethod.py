...
Channels", J. Phys. Chem., 2000,104,1876-1889
  ...
  def Execute(self,Trajectories,Endtime,Timesteps,TrackPropensities):       
    ...
    Input: 
     - *Trajectories*: Number of trajectories (integer)
     - *Endtime*
     - *Timesteps*
  ...
  def InitialStep(self): 
      ...
      self.taus = self.randoms_log_init[0:self.number_of_reactions]/self.sim_a_mu # Calculate all initial taus
      pairs = []
      j=0
  ...
  def BuildInits(self):
      ...
      self.sim_a_mu_zero = []
      for i in range(0,self.number_of_reactions):
