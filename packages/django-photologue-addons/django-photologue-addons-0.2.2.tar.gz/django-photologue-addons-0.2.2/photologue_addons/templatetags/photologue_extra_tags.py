...
Gallery = models.get_model('photologue', 'gallery')
...
class GetGalleriesNode(template.Node):
    '''
        Get all public galleries and load them up
    '''
    def __init__(self, context_var):
        self.obj=Gallery.objects.filter(is_public=True)
        self.context_var= context_var
...
def do_get_public_galleries(parser, token):
    '''
        Retrieves all public galleries
        {% get_public_galleries as <template var> %}
    '''
    try:
        bits = token.split_contents()
    except ValueError:
        raise template.TemplateSyntaxError(_('tag requires exactly onearguments'))
    if bits[1] != 'as':
        raise template.TemplateSyntaxError(_("first argument to tag must be 'as'"))
    return GetGalleriesNode(bits[2] )

register.tag('get_public_galleries', do_get_public_galleries)
