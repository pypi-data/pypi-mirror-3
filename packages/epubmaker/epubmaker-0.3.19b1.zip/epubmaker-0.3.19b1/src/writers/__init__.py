...
class BaseWriter (object):
    ...
    def ibiblio_local_file (self):
        """ Get the ibiblio local input file. """

        fn = self.options.candidate.filename

        if hasattr (options.config, 'PUBDIR'):
            if fn.startswith (options.config.PGURL):
                fn = fn.replace (options.config.PGURL, options.config.PUBDIR)
...
class HTMLishWriter (BaseWriter):
    ...
    def add_meta_generator (xhtml):
        """ Add our piss mark. """
        HTMLishWriter.add_meta (xhtml, 'generator', gg.GENERATOR)
