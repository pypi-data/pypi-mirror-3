# $Id: emmodels.py,v 1.9 2012/02/24 09:30:45 irees Exp $
import Queue
import collections
import datetime
import operator
...
class QueueModel(QtCore.QAbstractItemModel):
	...
	def __init__(self, headers=None, headernames=None, headerwidths=None, queue=None, parent=None):
		super(QueueModel, self).__init__(parent)
		self.queue = Queue.Queue()

		# Current microscope settings...		
		self.settings = {}
		self.settings['assess_image_quality'] = None
	...
	def set_setting(self, value, param=None):
		# print "Changing setting: %s -> %s"%(param, value)
		if not param:
			return
		if not value:
	...
	def target(self, name):
		...
		if self.names.get('_target') == int(name):
			return

		self.names["_target"] = int(name)
		data = {
			"name":name,
			"_recname":"Record: %s"%name, 
			"_status":"(view)"
		}
	...
	def newfile(self, filename, data=None):
		...
		parent = self.names.get("_target")
		data['_parent'] = parent
		data.update(self.settings)
		
		# Need to look at the handler to get some info..
		rectype = data.get('_rectype')
		handler = emdash.handlers.get_handler(rectype=rectype)
		dbt = handler(filename=filename)		
		data['_recname'] = dbt.checkrecname()
				
		######################################
		# Add the item to the model
		
		self.beginInsertRows(QtCore.QModelIndex(), self.rowCount(), self.rowCount())
		d = self.itemclass(data, parent=self.grids[parent], cols=self.headers)
		self.grids[parent].appendChild(d)
		self.endInsertRows()

		######################################
				
		# Index by filename		
		self.mdata[filename] = d

		# Stack files are currently not automatically uploaded...
		if os.path.splitext(filename)[-1] != '.st':
			self.queue.put(data)
