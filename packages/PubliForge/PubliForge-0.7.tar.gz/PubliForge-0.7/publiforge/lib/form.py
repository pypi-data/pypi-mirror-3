# $Id: form.py 0af2084a7a96 2012/08/16 03:47:55 Patrick $
...
def button(url, label='', src=None, title=None, class_='button'):
    ...
    if class_ == 'button' and not label and src:
        class_ = None
    return literal(u'<a href="{url}"{title}{class_}>{img}{label}</a> '.format(
        url=url, title=title and ' title="%s"' % title or '',
...
class Form(object):
    ...
    def _value(self, name, default=None):
        ...
        """
        return (
            name not in self._special[0] and name[0] != self._special[1]
