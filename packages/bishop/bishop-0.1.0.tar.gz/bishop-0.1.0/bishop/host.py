...
class Local(object):
    ...
    def snapshot(self, root='/', roles=None):
        '''Save a snapshot of the current node to the repository filesystem for diff to make sense of later'''
        if roles is None:
            roles = self.roles
        packages = package.Package().list()
        files = {}
        for role in roles:
            self.snapshot_files(role, files, root)

        f = open(os.path.join(self.path, '.snapshot'), 'w')
        pickle.dump({'packages': packages, 'files': files}, f)
        f.close()

    def snapshot_files(self, role, files, root):
        '''Accept a list of roles and an accumulator, and fill the accumulator with file data'''
        role = self.role(role)
        for path in [os.path.join(root, path) for path in role.files if os.path.exists(os.path.join(root, path)) and os.path.join(root, path) not in files]:
            files[path] = {'type': 'files', 'mtime': os.stat(path).st_mtime, 'owner': role.name}

        for path in [os.path.join(root, path) for path in role.templates if os.path.exists(os.path.join(root, path)) and os.path.join(root, path) not in files]:
            files[path] = {'type': 'templates', 'mtime': os.stat(path).st_mtime, 'owner': role.name}

        for mixin in role.mixins:
            self.snapshot_files(mixin, files, root)

    
    def diff(self, roles, root='/'):
        '''Compare the installed packages and role-handled files to the last snapshot'''
        snapshot = pickle.load(open(os.path.join(self.path, '.snapshot'), 'r'))
        new_packages = [p for p in package.Package().list() if p not in snapshot['packages']]

        new_files = {}
        for path, info in snapshot['files'].iteritems():
            if os.stat(path).st_mtime > info['mtime']:
                new_files[path] = info
        return new_packages, new_files
