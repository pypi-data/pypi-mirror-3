...
def before_hooks(argv):
    ...
    #if not options.compress_assets:
    #    return
    try:
        import gae_assets_bundler
        compressor = gae_assets_bundler.AssetCompressor(argv[1])
        compressor.run()
    except ImportError:
        print 'Cannot find gae_assets_bundler module, make sure it is installed in your python path'
    except Exception:
        print 'Unexpected exception from gae_asset_bundler'
