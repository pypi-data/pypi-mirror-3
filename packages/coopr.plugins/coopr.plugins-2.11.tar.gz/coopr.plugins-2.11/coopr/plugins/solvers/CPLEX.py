...
class CPLEXSHELL(ILMLicensedSystemCallSolver):
    ...
    def warm_start(self, instance):
        ...
        print >>mst_file, "<?xml version=\"1.0\" ?>"
        print >>mst_file, "<CPLEXSolution version=\"1.0\">"
        print >>mst_file, "<header/>"
        print >>mst_file, "<quality/>"
        print >>mst_file, "<variables>"
        ...
        for block in instance.all_blocks():
            for variable in block.active_components(Var).itervalues():
                for index in variable:
                    ...
                    print >>mst_file, "<variable index=\"%d\" name=\"%s\" value=\"%f\" />" % (output_index, name, var.value)
                    output_index = output_index + 1

        print >>mst_file, "</variables>"
        print >>mst_file, "</CPLEXSolution>"
        mst_file.close()
        ...
        for line in INPUT:
            ...
            if tokens[0] == "variable":
               ...
               if soln.status == SolutionStatus.unknown:
                ...
                primal_feasible = int((string.strip(" ".join(tokens).split('=')[1])).lstrip("\"").rstrip("\""))
