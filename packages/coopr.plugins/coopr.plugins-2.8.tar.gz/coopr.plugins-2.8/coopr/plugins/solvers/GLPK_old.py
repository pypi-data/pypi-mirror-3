...
class GLPKSHELL_old(SystemCallSolver):
    ...
    def create_command_line(self, executable, problem_files):
        ...
        if self._results_format is None \
           or self._results_format == ResultsFormat.soln:
        ...
        cmd.append('--output ' + self.soln_file)
    ...
    def process_soln_file(self, results):
        ...
        try:
                ...
                if (len(tokens) == 1) and (len(tokens[0]) == 0):
                    ...
                    if (len(tokens) == 2) and (len(active_constraint_name) == 0):
                        ...
                        if lp_solution is True:
                            ...
                            #print "HERE '%s' '%s' '%s'" % (state_string, upper_bound_string, marginal_string)
            ...
            INPUT.close()
            raise
            #raise RuntimeError, "Unexpected input encountered in GLPK solution file, yielding parse failure; line="+str(line)+"\nException: "+str(msg)
        #
        if soln.status is SolutionStatus.optimal:
            soln.gap = 0.0
...
class MockGLPK(GLPKSHELL_old,mockmip.MockMIP):
