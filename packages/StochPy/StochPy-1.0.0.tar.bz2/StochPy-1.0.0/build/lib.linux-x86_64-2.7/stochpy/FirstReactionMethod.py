...
Last Change: 18 September, 2011
"""
############################## IMPORTS ###################################

import sys,re,copy,time,os,getopt,cPickle
...
class FirstReactionMethod(PySCeS_Connector):
  ...
  def Execute(self,Trajectories,Endtime,Timesteps,TrackPropensities):   
    ...
    Input: 
     ...
     - *Number of time steps or the endtime* 
    """
    t1 = time.time()
  ...
  def Run(self):
      ...
      #np.random.seed(5) 
      randoms = np.random.random(self.number_of_reactions)     # Regenerate for each time step M random numbers
  ...
  def InitialStep(self):
      ...
      self.sim_taus = self.randoms_log[0:self.number_of_reactions]/self.sim_a_mu
