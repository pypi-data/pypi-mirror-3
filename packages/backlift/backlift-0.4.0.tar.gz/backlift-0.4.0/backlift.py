...
backlift_configfile = 'config.yml'
...
def register_key(key):
    # import pdb; pdb.set_trace()
    try:
        r = requests.post(backliftURL['register'], 
                          data={'access_key': key}, verify=False)
        check_response(r)
...
def get_app_meta(api_key, app_id):
    try:
        # import pdb; pdb.set_trace()
        r = requests.get(backliftURL['getappmeta'] % app_id, 
...
def create_app(api_key, name):
    try:
        # import pdb; pdb.set_trace()
        r = requests.post(backliftURL['create'], data={'appname': name},
                          auth=HTTPBasicAuth('api', api_key), verify=False)
        check_response(r)
...
def delete_app(api_key, app_id):
    try:
        # import pdb; pdb.set_trace()
        r = requests.delete(backliftURL['delete'] % app_id,
                            auth=HTTPBasicAuth('api', api_key), verify=False)
        check_response(r)
...
def execute_command_line():
    ...
    parser.add_option('-f', '--force', 
        ...
        help='Force action. Used for delete only.')
