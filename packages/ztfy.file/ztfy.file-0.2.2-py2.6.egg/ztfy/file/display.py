...
class ImageDisplayAdapter(object):
    ...
    def createDisplay(self, display, format='jpeg'):
        ...
        if thumbnailer is not None:
            if display == 'cthumb':
                thumbnail = thumbnailer.createSquareThumbnail(self.image, DEFAULT_DISPLAYS[display], format=format.upper())
            else:
                thumbnail = thumbnailer.createThumbnail(self.image, (width, height), format=format.upper())
            img = self.displays[display] = Image(thumbnail)
            notify(ObjectCreatedEvent(img))
            locate(img, self.image, '++display++%s.%s' % (display, format))
            return img
        return self.image

    def getDisplay(self, display, format='jpeg'):
        if '.' in display:
