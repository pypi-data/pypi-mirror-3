======================
ztfy.scheduler package
======================

You may find documentation in:

- Global README.txt: ztfy/scheduler/docs/README.txt
- General: ztfy/scheduler/docs
- Technical: ztfy/scheduler/doctests

More informations can be found on ZTFY.org_ web site.

This package is created and maintained by Thierry Florac_.

.. _Thierry Florac: mailto:tflorac@ulthar.net
.. _ZTFY.org: http://www.ztfy.org
