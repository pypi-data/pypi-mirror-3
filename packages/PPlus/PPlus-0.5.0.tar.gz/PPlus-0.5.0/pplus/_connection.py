...
class PPlusConnection(object):
    ...
    def __init__(self, id=None, config_file_path=None, debug=True,
                 local_workers_number=None, workers_servers=None, secret=None):
        ...
        workers_servers : list, optional (default: None)
            list of pplus servers addresses. Is possible to specify port
            numbers using the format 'address:port'.
        ...
        if debug:
            ...
            if config_file_path is None:
                config_cwd_path = os.path.abspath('pplus.cfg')
    ...
    def submit(self, function, args=(), depfuncs=(), modules=()):
        """Submit requested callable as a task to be executed in parallel
        within current PPlus environment.

        The callable **have to accept** as first parameter an instance
