...
def init(config, modules):
    ...
    :param modules: hash containing True or False for each module (its uri path respectively), if True, perform auth, if False, skip
    """
    pass
...
def _authenticate():
    ...
    >>> if module_name in modules and not modules[module_name]: return
    """
    if True:
        return True
    else:
