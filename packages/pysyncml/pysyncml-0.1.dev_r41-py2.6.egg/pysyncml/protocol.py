# -*- coding: utf-8 -*-
#------------------------------------------------------------------------------
# file: $Id: protocol.py 39 2012-07-22 16:17:58Z griff1n $
...
class Protocol(object):
  ...
  def _tree2commands(self, adapter, session, lastcmds, xsync, ret):
    ...
    for child in xsync[1]:
      ...
      if cname in (constants.CMD_ADD, constants.CMD_REPLACE, constants.CMD_DELETE,):
        ...
        # todo: trap errors...
        res = getattr(self, 't2c_status_' + cname.lower())(adapter, session, chkcmd, xsync, child)
        ret.extend(res or [])
        continue
  ...
  def t2c_status_add(self, adapter, session, chkcmd, xsync, xnode):
    ...
    if code != constants.STATUS_ALREADY_EXISTS:
    ...
    if ds.peerLastAnchor != adapter.peer.stores[ruri].binding.targetAnchor:
      ...
      if ds.mode not in (
        ...
        if session.isServer:
          ...
          raise common.ProtocolError('server-side requested inappropriate sync mode %s on unbound datastore "%s"'
...
#------------------------------------------------------------------------------
# end of $Id: protocol.py 39 2012-07-22 16:17:58Z griff1n $
#------------------------------------------------------------------------------
