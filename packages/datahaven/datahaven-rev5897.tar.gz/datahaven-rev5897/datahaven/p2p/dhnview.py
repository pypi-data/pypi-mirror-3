...
LogWindowTitle = 'DHN Log'
TrafficWindowTitle = 'DHN Traffic'
EventsWindowTitle = 'DHN Events'
...
StatesDict = {
    ...
    'p2p centralservice':       (True, 'connecting to central server'),
    'p2p propagate':            (True, 'propagate my identity'),
    'p2p incomming?':           (True, 'waiting response from others'),
    ...
    'central identity':         (True, 'sending my identity to central server'),
    'central settings':         (True, 'sending my settings to central server'),
    'central request settings': (True, 'asking my settings from central server'),
    'central suppliers':        (True, 'requesting suppliers from central server'),
    'central connected':        (True, 'connected to central server'),
    'central disconnected':     (True, 'disconnected from central server'),
...
def sharedPath(filename, subdir='logs'):
    if platform.uname()[0] == 'Windows':
        #return os.path.join(os.path.expanduser('~'), 'Application Data', 'DataHavenNet', subdir, filename)
        #return os.path.join(os.path.expanduser('~'), 'Application Data', 'DataHaven.NET', subdir, filename)
        return os.path.join(os.environ['APPDATA'], 'DataHaven.NET', subdir, filename)
    else:
        return os.path.join(os.path.expanduser('~'), 'datahavennet', subdir, filename)
...
class MyBusyInfoFrame(wx.Frame):
    def __init__(self, parent, message):
        wx.Frame.__init__(self,
                          ...
                          wx.DefaultPosition,
                          wx.DefaultSize,
                          wx.SIMPLE_BORDER|wx.FRAME_TOOL_WINDOW)
        panel = wx.Panel(self)
...
class MyEventInfoDialog(wx.Dialog):
    def __init__(self, eTime, eModule, eMessage, eBody):
        wx.Dialog.__init__(self, None, -1, eModule.capitalize() + " event", size=(400,300),
            style=wx.SIMPLE_BORDER|wx.FRAME_TOOL_WINDOW,)
...
class MyFrame(wx.Frame):
    ...
    def OnLogs(self, evt):
        ...
        def eb(x):
            d = wx.MessageDialog(
                self,
                'To enable logs go to settings->debug options->enable logs',
                'Logs disabled',
...
class MyAppBusyInfoFrame(wx.Frame):
    def __init__(self, parent, message):
        wx.Frame.__init__(self, parent, wx.ID_ANY, "Busy",
            ...
            wx.SIMPLE_BORDER|wx.FRAME_TOOL_WINDOW|wx.STAY_ON_TOP)
