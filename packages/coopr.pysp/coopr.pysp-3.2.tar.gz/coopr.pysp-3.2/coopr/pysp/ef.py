...
def create_ef_instance(scenario_tree, scenario_instances,
    ...
    for scenario_name, scenario_instance in scenario_instances.items():
        # FIXME: this is hackish, and serves only to supress a warning from block.py.
        scenario_instance._constructed = True
    ...
    if cc_indicator_var_name is not None:
        ...
        if isVariableNameIndexed(cc_indicator_var_name) is False:
            ...
            if variable_name not in scenario_instance.active_components(Var).keys():
                raise RuntimeError, "Unknown variable="+variable_name+" referenced as the CC indicator variable."

            variable = scenario_instance.active_components(Var)[variable_name]
...
def write_ef(binding_instance, scenario_instances, output_filename, symbolic_solver_labels): 
    ...
    if ef_output_file_suffix == "lp":
       ...
       symbol_map = problem_writer._print_model_LP(binding_instance, output_file, lambda x:True, symbolic_solver_labels)
       output_file.close()
       ...
       output_file = open(output_filename,'w')
       symbol_map = problem_writer._print_model_NL(binding_instance, output_file, lambda x:True, symbolic_solver_labels)  
       output_file.close()                          
...
def write_ef_from_scratch(model_directory, instance_directory, output_filename, symbolic_solver_labels,
    ...
    try:
        ...
        master_scenario_instance = master_scenario_model.create(filename=reference_scenario_filename, preprocess=False)
    ...
    symbol_map = write_ef(binding_instance, scenario_instances, output_filename, symbolic_solver_labels)
