...
from django.template import (Template, Node, Variable, TemplateSyntaxError,
...
def get_template(path):
    """
    Get correct template object depending on the TEMPLATE_LOADERS settings.
    
    Using the old style function based template loaders makes it necessary to
    call `get_template_from_string` to get the desired template, while with 
    the new class based template loaders it only requires `find_template`.
    
    """
    source, origin = find_template(path)
    if not isinstance(source, Template):
        return get_template_from_string(source, origin, path)
    return source
...
class HookNode(Node):
    ...
    def render(self, context):
        ...
        #print 'Block: %s Template: %s' % (self.block_name, self.template)
        for addon in settings.ADDONS:
            ...
            if path in template_cache:                
                ...
                try:
                    addon_template = get_template(path)
