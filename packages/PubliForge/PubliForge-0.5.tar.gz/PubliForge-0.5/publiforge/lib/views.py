# $Id: views.py de5826a61f18 2012/02/27 17:59:57 patrick $
...
from ..models.storages import VCS_ENGINES, Storage, StorageUser
from ..models.projects import Project, ProjectProcessing, ProjectProcessingFile
from ..models.projects import ProjectUser, ProjectPack, ProjectPackFile
...
def get_action(request):
    ...
    * ``upl``: upload
    ...
    ``DataHg`` and only this one". And ``add?usr`` means "propose users
    to add".
    """
    action = ''
    items = []
...
def paging_users(request, only_active=True):
    ...
    if 'f_group' in params:
        query = query.filter(User.user_id == GROUPS_USERS.c.user_id)\
            .filter(GROUPS_USERS.c.group_id == params['f_group'])
...
def current_storage(request, storage_id=None, only_dict=True):
    ...
    if not has_permission(request, 'stg_manager') \
           and not storage.access == 'open' and not user:
    ...
    request.session['storage'] = {
        ...
        'perm':
          (has_permission(request, 'stg_manager') and 'modifier')
          or (storage.access == 'open' and 'modifier')
          or (user and user.perm) or 'viewer',
...
def file_download(request, path, filenames, download_name=None):
    ...
    zip_file.close()
    download_name = filenames[0] if len(filenames) == 1 else basename(path)
...
def current_project(request, project_id=None, only_dict=True):
    ...
    # Has permission?
    user = DBSession.query(ProjectUser).filter_by(
        project_id=project_id, user_id=request.session['user_id']).first()
    if not has_permission(request, 'prj_manager') and not user:
    ...
    request.session['project'] = {'project_id': project_id,
        ...
        'perm': has_permission(request, 'prj_modifier') and 'modifier' \
            or user.perm,
