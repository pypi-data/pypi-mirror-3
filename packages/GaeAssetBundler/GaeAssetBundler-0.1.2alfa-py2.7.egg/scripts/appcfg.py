...
def before_hooks(argv):
    ...
    try:
        logging.info('--- GaeAssetBundler ---')
        import gae_asset_bundler
        compressor = gae_asset_bundler.AssetCompressor(argv[argv.index('update') + 1])
        compressor.run()
        logging.info('--- GaeAssetBundler completed ---')
    except ImportError, e:
        logging.info('Cannot find gae_assets_bundler module or dependencies: %s' % e)
    except Exception, e:
        logging.warn('Unexpected exception while generating bundles: %s' % e)
