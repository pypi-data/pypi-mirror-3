 ...
 *          DO NOT MODIFY THIS FILE, MODIFY include/sld_cal.h
...
static PyObject * CSLDCalFuncError = NULL;
...
static void
CSLDCalFunc_dealloc(CSLDCalFunc* self)
{
...
static PyObject *
CSLDCalFunc_new(PyTypeObject *type, PyObject *args, PyObject *kwds)
{
    CSLDCalFunc *self;
    
    self = (CSLDCalFunc *)type->tp_alloc(type, 0);
   
    return (PyObject *)self;
}

static int
CSLDCalFunc_init(CSLDCalFunc *self, PyObject *args, PyObject *kwds)
{
    if (self != NULL) {
        ...
        self->model = new SLDCalFunc();
        
        // Initialize parameter dictionary
        PyDict_SetItemString(self->params,"fun_type",Py_BuildValue("d",0.000000000000));
        PyDict_SetItemString(self->params,"nu_inter",Py_BuildValue("d",2.500000000000));
        PyDict_SetItemString(self->params,"shell_num",Py_BuildValue("d",0.000000000000));
        PyDict_SetItemString(self->params,"sld_right",Py_BuildValue("d",0.000000000000));
        PyDict_SetItemString(self->params,"sld_left",Py_BuildValue("d",0.000000000000));
        PyDict_SetItemString(self->params,"npts_inter",Py_BuildValue("d",21.000000000000));
        // Initialize dispersion / averaging parameter dict
        DispersionVisitor* visitor = new DispersionVisitor();
        PyObject * disp_dict;
        disp_dict = PyDict_New();
        self->model->npts_inter.dispersion->accept_as_source(visitor, self->model->npts_inter.dispersion, disp_dict);
        PyDict_SetItemString(self->dispersion, "npts_inter", disp_dict);
...
static PyMemberDef CSLDCalFunc_members[] = {
    {name_params, T_OBJECT, offsetof(CSLDCalFunc, params), 0, def_params},
	{name_dispersion, T_OBJECT, offsetof(CSLDCalFunc, dispersion), 0, def_dispersion},     
    {name_log, T_OBJECT, offsetof(CSLDCalFunc, log), 0, def_log},
    {NULL}  /* Sentinel */
};
...
/** Read double from PyObject
    ...
    @return double
*/
double CSLDCalFunc_readDouble(PyObject *p) {
...
static PyObject *evaluateOneDim(SLDCalFunc* model, PyArrayObject *q){
 ...
 * @return: numpy array object 
 */
 static PyObject * evaluateTwoDimXY( SLDCalFunc* model, 
...
#pragma omp parallel for
        ...
        return PyArray_Return(result); 
		    ...
		    PyErr_SetString(CSLDCalFuncError, 
                   "CSLDCalFunc.evaluateTwoDimXY couldn't run.");
	        return NULL;
		}      	
}
/**
 ...
 *
 */ 
static PyObject * evalDistribution(CSLDCalFunc *self, PyObject *args){
	PyObject *qx, *qy;
	PyArrayObject * pars;
	int npars ,mpars;
    ...
    self->model->fun_type = PyFloat_AsDouble( PyDict_GetItemString(self->params, "fun_type") );
    self->model->nu_inter = PyFloat_AsDouble( PyDict_GetItemString(self->params, "nu_inter") );
    self->model->shell_num = PyFloat_AsDouble( PyDict_GetItemString(self->params, "shell_num") );
    self->model->sld_right = PyFloat_AsDouble( PyDict_GetItemString(self->params, "sld_right") );
    self->model->sld_left = PyFloat_AsDouble( PyDict_GetItemString(self->params, "sld_left") );
    self->model->npts_inter = PyFloat_AsDouble( PyDict_GetItemString(self->params, "npts_inter") );
    // Read in dispersion parameters
    PyObject* disp_dict;
    DispersionVisitor* visitor = new DispersionVisitor();
    disp_dict = PyDict_GetItemString(self->dispersion, "npts_inter");
    self->model->npts_inter.dispersion->accept_as_destination(visitor, self->model->npts_inter.dispersion, disp_dict);

	
	// Get input and determine whether we have to supply a 1D or 2D return value.
	if ( !PyArg_ParseTuple(args,"O",&pars) ) {
	    PyErr_SetString(CSLDCalFuncError, 
	    	"CSLDCalFunc.evalDistribution expects a q value.");
		return NULL;
	}
    // Check params
    ...
    if(PyArray_Check(pars)==1) {
	    ...
	    if(npars==1) {
		    ...
		    PyErr_SetString(CSLDCalFuncError, 
                   "CSLDCalFunc.evalDistribution expect numpy array of one dimension.");
	        return NULL;
		}
	    ...
	    if(mpars!=2) {
	    	PyErr_SetString(CSLDCalFuncError, 
	    		"CSLDCalFunc.evalDistribution expects a list of dimension 2.");
	    	return NULL;
	    }
	     ...
	     if (PyArray_Check(qx) && PyArray_Check(qy)) {
		    ...
		    PyErr_SetString(CSLDCalFuncError, 
                   "CSLDCalFunc.evalDistribution expect 2 numpy arrays in list.");
	        return NULL;
	     }
	}
	PyErr_SetString(CSLDCalFuncError, 
                   "CSLDCalFunc.evalDistribution couln't be run.");
	return NULL;
 ...
 */
static PyObject * run(CSLDCalFunc *self, PyObject *args) {
	...
	if ( !PyArg_ParseTuple(args,"O",&pars) ) {
	    PyErr_SetString(CSLDCalFuncError, 
	    	"CSLDCalFunc.run expects a q value.");
		return NULL;
	}
	...
	if( PyList_Check(pars)==1) {
	    ...
	    if(npars!=2) {
	    	PyErr_SetString(CSLDCalFuncError, 
	    		"CSLDCalFunc.run expects a double or a list of dimension 2.");
	    	return NULL;
	    }
	    // We have a vector q, get the q and phi values at which
	    // to evaluate I(q,phi)
	    q_value = CSLDCalFunc_readDouble(PyList_GET_ITEM(pars,0));
	    phi_value = CSLDCalFunc_readDouble(PyList_GET_ITEM(pars,1));
	    // Skip zero
	    if (q_value==0) {
		...
		q_value = CSLDCalFunc_readDouble(pars);		
 ...
 */
static PyObject * calculate_ER(CSLDCalFunc *self) {
 ...
 */
static PyObject * calculate_VR(CSLDCalFunc *self) {
 ...
 * @return: function value
 */
static PyObject * runXY(CSLDCalFunc *self, PyObject *args) {
	...
	if( PyList_Check(pars)==1) {
	    ...
	    // to evaluate I(qx,qy)
	    qx_value = CSLDCalFunc_readDouble(PyList_GET_ITEM(pars,0));
	    qy_value = CSLDCalFunc_readDouble(PyList_GET_ITEM(pars,1));
	    return Py_BuildValue("d",(*(self->model))(qx_value,qy_value));

	} else {

		// We have a scalar q, we will evaluate I(q)
		qx_value = CSLDCalFunc_readDouble(pars);		
		
		return Py_BuildValue("d",(*(self->model))(qx_value));
	}	
}

static PyObject * reset(CSLDCalFunc *self, PyObject *args) {
    

    return Py_BuildValue("d",0.0);
}

static PyObject * set_dispersion(CSLDCalFunc *self, PyObject *args) {
	PyObject * disp;
	const char * par_name;

	if ( !PyArg_ParseTuple(args,"sO", &par_name, &disp) ) {
	    PyErr_SetString(CSLDCalFuncError,
	    	"CSLDCalFunc.set_dispersion expects a DispersionModel object.");
		return NULL;
	}
	...
	// Ugliness necessary to go from python to C
	    // TODO: refactor this
    if (!strcmp(par_name, "npts_inter")) {
        self->model->npts_inter.dispersion = dispersion;
    } else {
	    PyErr_SetString(CSLDCalFuncError,
	    	"CSLDCalFunc.set_dispersion expects a valid parameter name.");
		return NULL;
	}
...
static PyMethodDef CSLDCalFunc_methods[] = {
...
static PyTypeObject CSLDCalFuncType = {
    PyObject_HEAD_INIT(NULL)
    0,                         /*ob_size*/
    "CSLDCalFunc",             /*tp_name*/
    sizeof(CSLDCalFunc),             /*tp_basicsize*/
    0,                         /*tp_itemsize*/
    (destructor)CSLDCalFunc_dealloc, /*tp_dealloc*/
    0,                         /*tp_print*/
    0,                         /*tp_getattr*/
    0,                         /*tp_setattr*/
    0,                         /*tp_compare*/
    ...
    0,		               /* tp_iter */
    0,		               /* tp_iternext */
    CSLDCalFunc_methods,             /* tp_methods */
    CSLDCalFunc_members,             /* tp_members */
    0,                         /* tp_getset */
    0,                         /* tp_base */
    0,                         /* tp_dict */
    0,                         /* tp_descr_get */
    0,                         /* tp_descr_set */
    0,                         /* tp_dictoffset */
    (initproc)CSLDCalFunc_init,      /* tp_init */
    0,                         /* tp_alloc */
    CSLDCalFunc_new,                 /* tp_new */
};
...
void addCSLDCalFunc(PyObject *module) {
    ...
    if (PyType_Ready(&CSLDCalFuncType) < 0)
        return;

    Py_INCREF(&CSLDCalFuncType);
