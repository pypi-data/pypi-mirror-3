 ...
 *          DO NOT MODIFY THIS FILE, MODIFY include/fuzzysphere.h
...
static PyObject * CFuzzySphereModelError = NULL;
...
static void
CFuzzySphereModel_dealloc(CFuzzySphereModel* self)
{
...
static PyObject *
CFuzzySphereModel_new(PyTypeObject *type, PyObject *args, PyObject *kwds)
{
    CFuzzySphereModel *self;
    
    self = (CFuzzySphereModel *)type->tp_alloc(type, 0);
   
    return (PyObject *)self;
}

static int
CFuzzySphereModel_init(CFuzzySphereModel *self, PyObject *args, PyObject *kwds)
{
    if (self != NULL) {
        ...
        disp_dict = PyDict_New();
        self->model->fuzziness.dispersion->accept_as_source(visitor, self->model->fuzziness.dispersion, disp_dict);
        PyDict_SetItemString(self->dispersion, "fuzziness", disp_dict);
...
static PyMemberDef CFuzzySphereModel_members[] = {
    {name_params, T_OBJECT, offsetof(CFuzzySphereModel, params), 0, def_params},
	{name_dispersion, T_OBJECT, offsetof(CFuzzySphereModel, dispersion), 0, def_dispersion},     
    {name_log, T_OBJECT, offsetof(CFuzzySphereModel, log), 0, def_log},
    {NULL}  /* Sentinel */
};
...
/** Read double from PyObject
    ...
    @return double
*/
double CFuzzySphereModel_readDouble(PyObject *p) {
...
static PyObject *evaluateOneDim(FuzzySphereModel* model, PyArrayObject *q){
 ...
 * @return: numpy array object 
 */
 static PyObject * evaluateTwoDimXY( FuzzySphereModel* model, 
...
#pragma omp parallel for
        ...
        return PyArray_Return(result); 
		    ...
		    PyErr_SetString(CFuzzySphereModelError, 
                   "CFuzzySphereModel.evaluateTwoDimXY couldn't run.");
	        return NULL;
		}      	
}
/**
 ...
 *
 */ 
static PyObject * evalDistribution(CFuzzySphereModel *self, PyObject *args){
	PyObject *qx, *qy;
	PyArrayObject * pars;
	int npars ,mpars;
    ...
    self->model->fuzziness = PyFloat_AsDouble( PyDict_GetItemString(self->params, "fuzziness") );
    ...
    disp_dict = PyDict_GetItemString(self->dispersion, "fuzziness");
    self->model->fuzziness.dispersion->accept_as_destination(visitor, self->model->fuzziness.dispersion, disp_dict);

	
	// Get input and determine whether we have to supply a 1D or 2D return value.
	if ( !PyArg_ParseTuple(args,"O",&pars) ) {
	    PyErr_SetString(CFuzzySphereModelError, 
	    	"CFuzzySphereModel.evalDistribution expects a q value.");
		return NULL;
	}
    // Check params
    ...
    if(PyArray_Check(pars)==1) {
	    ...
	    if(npars==1) {
		    ...
		    PyErr_SetString(CFuzzySphereModelError, 
                   "CFuzzySphereModel.evalDistribution expect numpy array of one dimension.");
	        return NULL;
		}
	    ...
	    if(mpars!=2) {
	    	PyErr_SetString(CFuzzySphereModelError, 
	    		"CFuzzySphereModel.evalDistribution expects a list of dimension 2.");
	    	return NULL;
	    }
	     ...
	     if (PyArray_Check(qx) && PyArray_Check(qy)) {
		    ...
		    PyErr_SetString(CFuzzySphereModelError, 
                   "CFuzzySphereModel.evalDistribution expect 2 numpy arrays in list.");
	        return NULL;
	     }
	}
	PyErr_SetString(CFuzzySphereModelError, 
                   "CFuzzySphereModel.evalDistribution couln't be run.");
	return NULL;
 ...
 */
static PyObject * run(CFuzzySphereModel *self, PyObject *args) {
	...
	if ( !PyArg_ParseTuple(args,"O",&pars) ) {
	    PyErr_SetString(CFuzzySphereModelError, 
	    	"CFuzzySphereModel.run expects a q value.");
		return NULL;
	}
	...
	if( PyList_Check(pars)==1) {
	    ...
	    if(npars!=2) {
	    	PyErr_SetString(CFuzzySphereModelError, 
	    		"CFuzzySphereModel.run expects a double or a list of dimension 2.");
	    	return NULL;
	    }
	    // We have a vector q, get the q and phi values at which
	    // to evaluate I(q,phi)
	    q_value = CFuzzySphereModel_readDouble(PyList_GET_ITEM(pars,0));
	    phi_value = CFuzzySphereModel_readDouble(PyList_GET_ITEM(pars,1));
	    // Skip zero
	    if (q_value==0) {
		...
		q_value = CFuzzySphereModel_readDouble(pars);		
 ...
 */
static PyObject * calculate_ER(CFuzzySphereModel *self) {
 ...
 */
static PyObject * calculate_VR(CFuzzySphereModel *self) {
 ...
 * @return: function value
 */
static PyObject * runXY(CFuzzySphereModel *self, PyObject *args) {
	...
	if( PyList_Check(pars)==1) {
	    ...
	    // to evaluate I(qx,qy)
	    qx_value = CFuzzySphereModel_readDouble(PyList_GET_ITEM(pars,0));
	    qy_value = CFuzzySphereModel_readDouble(PyList_GET_ITEM(pars,1));
	    return Py_BuildValue("d",(*(self->model))(qx_value,qy_value));

	} else {

		// We have a scalar q, we will evaluate I(q)
		qx_value = CFuzzySphereModel_readDouble(pars);		
		
		return Py_BuildValue("d",(*(self->model))(qx_value));
	}	
}

static PyObject * reset(CFuzzySphereModel *self, PyObject *args) {
    

    return Py_BuildValue("d",0.0);
}

static PyObject * set_dispersion(CFuzzySphereModel *self, PyObject *args) {
	PyObject * disp;
	const char * par_name;

	if ( !PyArg_ParseTuple(args,"sO", &par_name, &disp) ) {
	    PyErr_SetString(CFuzzySphereModelError,
	    	"CFuzzySphereModel.set_dispersion expects a DispersionModel object.");
		return NULL;
	}
    ...
    } else    if (!strcmp(par_name, "fuzziness")) {
        self->model->fuzziness.dispersion = dispersion;
    } else {
	    PyErr_SetString(CFuzzySphereModelError,
	    	"CFuzzySphereModel.set_dispersion expects a valid parameter name.");
		return NULL;
	}
...
static PyMethodDef CFuzzySphereModel_methods[] = {
...
static PyTypeObject CFuzzySphereModelType = {
    PyObject_HEAD_INIT(NULL)
    0,                         /*ob_size*/
    "CFuzzySphereModel",             /*tp_name*/
    sizeof(CFuzzySphereModel),             /*tp_basicsize*/
    0,                         /*tp_itemsize*/
    (destructor)CFuzzySphereModel_dealloc, /*tp_dealloc*/
    0,                         /*tp_print*/
    0,                         /*tp_getattr*/
    0,                         /*tp_setattr*/
    0,                         /*tp_compare*/
    ...
    "CFuzzySphereModel objects",           /* tp_doc */
    0,		               /* tp_traverse */
    0,		               /* tp_clear */
    0,		               /* tp_richcompare */
    0,		               /* tp_weaklistoffset */
    0,		               /* tp_iter */
    0,		               /* tp_iternext */
    CFuzzySphereModel_methods,             /* tp_methods */
    CFuzzySphereModel_members,             /* tp_members */
    0,                         /* tp_getset */
    0,                         /* tp_base */
    0,                         /* tp_dict */
    0,                         /* tp_descr_get */
    0,                         /* tp_descr_set */
    0,                         /* tp_dictoffset */
    (initproc)CFuzzySphereModel_init,      /* tp_init */
    0,                         /* tp_alloc */
    CFuzzySphereModel_new,                 /* tp_new */
};
...
void addCFuzzySphereModel(PyObject *module) {
    ...
    if (PyType_Ready(&CFuzzySphereModelType) < 0)
        return;

    Py_INCREF(&CFuzzySphereModelType);
