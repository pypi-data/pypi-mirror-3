...
def create_model(data):
    ...
    if not data.options.save_model is None:
        ...
        if data.options.save_model == True:
            ...
            format=None
        (fname, symbol_map) = instance.write(filename=fname, format=format, generate_symbolic_labels=data.options.symbolic_solver_labels)
        if not data.options.quiet:
...
def apply_optimizer(data, instance=None):
    ...
    opt.generateSymbolicLabels = data.options.symbolic_solver_labels
