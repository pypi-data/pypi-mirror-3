...
class ModelSOS(object):
    ...
    def count_constraint(self,con,symbol_map, labeler,level,index=None):
        ...
        prefix = symbol_map.getSymbol(con.sos_vars().model(), labeler)
        ...
        for x in tmpSet:
            ...
            if strX[0] == "(":
                ...
                #strX = varName+strX.replace(" ","")
                strX = varName+strX.replace(" ","").replace("(","[").replace(")","]")
            else:
                ...
                for name, con in constrs.iteritems():
                    ...
                    if (level == 1 and sos1) or (level == 2 and sos2) or (sosn):
                        ...
                        if None in masterIndex:
                            modelSOS.count_constraint(con,name,level)
                        else:
