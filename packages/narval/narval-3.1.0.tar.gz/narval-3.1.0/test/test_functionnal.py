...
class NarvalFunctionalTC(NarvalBaseTC):
    ...
    def test_plan_arguments(self):
        ...
        self.assertTrue([x for x in memelements if isinstance(x, ArbitraryObject)])
    ...
    def test_step_outdates(self):
        ...
        a, b = [x for x in plan.p_elements if isinstance(x, ArbitraryObject)]
        self.assertFalse(a is b)
        self.assertFalse(a._narval_outdated and b._narval_outdated)
        self.assertTrue(a._narval_outdated or b._narval_outdated)
        self.assertFalse(getattr(a, '_narval_tags', None) or getattr(b, '_narval_tags', None))
    ...
    def test_step_use(self):
        ...
        a, b = [x for x in plan.p_elements if isinstance(x, ArbitraryObject)]
        self.assertFalse(a is b)
        self.assertFalse(a._narval_outdated or b._narval_outdated)
        self.assertTrue(getattr(a, '_narval_tags', None) or getattr(b, '_narval_tags', None))
    ...
    def test_added_to_narval_memory_hook_1(self):
        ...
        memelements = main.ENGINE.memory.elements.values()
        self.assertEqual(len(memelements), 3, memelements)
        self.assertTrue([x for x in memelements if x.__class__ is EnsureOptions])
        self.assertTrue([x for x in memelements if x.__class__ is Options])
        self.assertTrue([x for x in memelements if isinstance(x, EXPR_CONTEXT['Plan'])])
    ...
    def test_added_to_narval_memory_hook_2(self):
        ...
        self.assertFalse([x for x in memelements if x.__class__ is EnsureOptions])
        self.assertTrue([x for x in memelements if x.__class__ is Options])
        self.assertTrue([x for x in memelements if isinstance(x, EXPR_CONTEXT['Plan'])])
        opt = (x for x in memelements if x.__class__ is Options).next()
        self.assertEqual(opt['value'], 1)
