# $Id: initialize.py 0af2084a7a96 2012/08/16 03:47:55 Patrick $
...
from .lib.views import connect_user
...
class Initialize(object):
    ...
    def all(self):
        ...
        if not self._lang or not settings.get('languages'):
...
def new_request(event):
    ...
    if not 'lang' in request.session:
        request.session['lang'] = request.accept_language.best_match(
            request.registry.settings.get('languages').split(),
...
def add_routes_front(config):
    ...
    config.add_view(
        ...
        or 'Templates/login.pt', permission=NO_PERMISSION_REQUIRED)
    ...
    config.add_route(
        'storage_root_ajax', '/storage/browse_ajax/{storage_id}')
    config.add_route(
        'storage_browse_ajax', '/storage/browse_ajax/{storage_id}/*path')
    ...
    config.add_route('build_results_ajax', '/build/results_ajax/{project_id}')
...
def add_routes_agent(config):
    ...
    config.add_xmlrpc_method(
        '.views.xmlrpc.processor_list',
        endpoint='xmlrpc', method='processor_list',
        permission=NO_PERMISSION_REQUIRED)
    config.add_xmlrpc_method(
        '.views.xmlrpc.processor_xml',
