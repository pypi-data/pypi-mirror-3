# $Id: views.py 23bf3c561736 2012/05/30 19:32:09 patrick $
...
from .utils import hash_sha, camel_case
...
def current_processing(request, project_id=None, processing_id=None,
    ...
    if processor is None:
        raise HTTPNotFound(comment=_('Unknown processor "${p}"!',
            mapping={'p': processing.processor}))
    processings = None
    ...
    output = current_storage(request, processing.output.partition('/')[0])[0]
    output = {
        'storage_id': output['storage_id'],
        'storage_label': output['label'],
        'path': processing.output.partition('/')[2]}

    if '${user}' in output['path']:
        login = DBSession.query(User.login).filter_by(
            user_id=request.session['user_id']).first()[0]
        output['path'] = output['path'].replace('${user}', camel_case(login))
...
def variable_description(request, processor, variables, name,
    ...
    if processor_default and var.findtext('default') is not None:
        html += grid_item('', translate(_('Factory:')),
            var.findtext('default'), class_='')
