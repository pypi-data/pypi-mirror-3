...
def read_fasta(stream):
    ...
    sequence = ''.join(sequence)
    if header is None:
        raise RuntimeError('Missng FASTA header.')
    yield Entry(header, sequence)

EntryBin = namedtuple('Entry', 'header sequence bits')

def tofastab(fasta, stream):
    """
    ...
    while True:
        ...
        if len(raw) < 8+2:
            if len(raw) == 0:
                ...
                raise RuntimeError("Truncated FASTAB file (incomplete entry descriptor).")
        ...
        if len(raw) < (nseq):
            raise RuntimeError("Truncated FASTAB file (incomplete sequence).")
