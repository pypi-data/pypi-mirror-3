...
: tree_descender< EM_descender< Spec >, Spec >
{
	...
	double                     log_delta;                ///< Threshold on log expected Z_n.
	...
	, log_delta( -std::numeric_limits< double >::max() )
	, wnsites( wnsites )
	...
	double
	do_iteration() {
		tree_descender< EM_descender >::descend_tree();
	...
	on_descent_begin() {
		// set the threshold
		log_delta = std::log( epsilon ) + base_t::_model._prior.lambda.get_log_lambda();
