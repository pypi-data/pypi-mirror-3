...
for f in os.listdir(FONTS_PATH):
    try:
        FONTS[f] = ImageFont.truetype(os.path.join(FONTS_PATH, f), 30)
    except:
...
def getCaptchaImage(text, format='JPEG'):
    ...
    for c in text:
        ...
        w, h = font.getsize(c)
        d.text((x, y), c, font=font, fill=0x333333)
        x += w
        height = max(height, h)
    for _i in range(5):
    ...
    return img.crop((0, 0, x, height)).filter(ImageFilter.SMOOTH_MORE)
