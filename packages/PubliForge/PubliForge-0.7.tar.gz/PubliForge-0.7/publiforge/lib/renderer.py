# $Id: renderer.py 4c5a8816200c 2012/09/06 09:04:56 patrick $
# -*- coding: utf-8 -*-
"""Renderer functions."""
    ...
    def _project_entries(self, user_id, submenu):
        ...
        for project in DBSession\
            ...
            subentries = [
                ...
                self._entry(_('Tasks'), 'task_index',
                            None, True, project_id=pid),
                self._entry(_('Packs'), 'pack_index',
                            None, True, project_id=pid),
                self._entry(_('Last results'), 'build_results',
                            None, True, project_id=pid)]
...
class Paging(paginate.Page):
    ...
    page_sizes = ('', 2, 5, 10, 20, 40, 80, 160, 320)
    ...
    def pager_top(self, icon_name=None, label=None):
        ...
        if icon_name is not None and label is not None:
            icon = literal(
                u'<img src="/Static/Images/{i}.png" alt="{l}"'
...
class TabSet(object):
    ...
    def tab_begin(self, index, access_key=None):
        ...
        """
        return literal(
            '<fieldset class="tabContent" id="tab%d">\n'
