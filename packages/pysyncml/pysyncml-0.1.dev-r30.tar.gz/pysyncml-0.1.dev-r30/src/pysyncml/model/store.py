# -*- coding: utf-8 -*-
#------------------------------------------------------------------------------
# file: $Id: store.py 29 2012-06-30 22:52:55Z griff1n $
# lib:  pysyncml.store
...
def decorateModel(model):
  ...
  class Store(model.DatabaseObject):
    ...
    def __init__(self, **kw):
      # TBD: this is a little hack... it is because the .merge() will
    ...
    def merge(self, store):
      ...
      # TBD: what about bindings?...
      log.critical('TBD: handle bindings during Store.merge()...')
    ...
    def clearChanges(self):
      if self.adapter.isLocal:
        # TBD: THIS NEEDS TO BE SIGNIFICANTLY OPTIMIZED!... either:
    ...
    def toSyncML(self):
      ...
      if self.maxGuidSize is not None:
        # tbd: this should ONLY be sent by the client... (according to the
  ...
  class Binding(model.DatabaseObject):
    # tbd: since store <=> binding is one-to-one, shouldn't this be a primary key?...
    store_id          = Column(Integer, ForeignKey('%s_store.id' % (model.prefix,),
                                                   onupdate='CASCADE', ondelete='CASCADE'),
                               nullable=False, index=True)
    targetStore       = relation('Store', backref=backref('binding', uselist=False,
                                                          cascade='all, delete-orphan',
                                                          passive_deletes=True))
    # tbd: this uri *could* be replaced by an actual reference to the Store object...
...
#------------------------------------------------------------------------------
# end of $Id: store.py 29 2012-06-30 22:52:55Z griff1n $
#------------------------------------------------------------------------------
