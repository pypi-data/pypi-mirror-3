...
def main():
	parser = argparse.ArgumentParser(version='0.2.0', description='CodeShow turns a directory of code into a static website suitable for use in presentations.', usage='codeshow [path] [<options>]')
	...
	Generator(
		...
		map(lambda path: os.path.join(source_directory, path), itertools.chain(*map(glob2.glob, arguments.ignore_paths)))
		+ [output_directory],
		arguments.site44,
