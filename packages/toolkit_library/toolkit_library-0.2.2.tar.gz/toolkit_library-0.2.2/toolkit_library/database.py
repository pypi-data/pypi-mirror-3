...
Please specify something like '?charset=utf8' explicitly.""")
    ...
    @staticmethod
    def foreign_key(table, foreign_name = None, backref_name = None):
        ...
        """
        foreign_name = foreign_name or table
        foreign_key = '{0}_id'.format(foreign_name)        
        def ref_table(cls):
            setattr(cls, foreign_key, Column(Integer, ForeignKey('{0}.id'.format(table))))
            ...
            my_backref_name = backref_name or '{0}s'.format(cls.__name__.lower())
            setattr(cls, foreign_name, relationship(table.capitalize(), 
                primaryjoin = '{0}.{1} == {2}.id'.format(cls.__name__, foreign_key, table.capitalize()), 
