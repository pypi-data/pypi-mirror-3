...
def purge_old_logs(pid_log_dir, mylog):
    ...
    try:
        ...
        if e.errno != errno.ENOENT:
            cloudLog.debug('Could not purge %s due to %s', pid_log_dir, str(e))
        return
    ...
    for log in logs:
        try:
            ...
            cloudLog.warn('Could not stat %s', log, exc_info = True)
            continue        
        if  (now - stat_result.st_ctime) > allowed_difference:
            ...
            try:
                ...
                cloudLog.warn('Could not delete %s', log_path, exc_info = True)            
