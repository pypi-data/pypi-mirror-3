...
MAX_IMAGE_DIMEN = (584, 754)  # in pixels
MAX_COVER_DIMEN = (600, 800)  # in pixels

MAX_IMAGE_SIZE_KINDLE  = 127 * 1024  # in bytes
MAX_IMAGE_DIMEN_KINDLE = (600, 800)  # in pixels, (the kindle can zoom now)
MAX_COVER_DIMEN_KINDLE = (600, 800)  # in pixels
...
class TocNCX (object):
    ...
    def __unicode__ (self):
        ...
        head = ncx.head (
            ...
            ncx.meta (name = 'dtb:generator',      content = gg.GENERATOR),
        ...
        toc_ncx = "%s\n%s\n\n%s" % (gg.XML_DECLARATION, gg.NCX_DOCTYPE,
                                    etree.tostring (ncx,
                                                    encoding = unicode,
                                                    pretty_print = True))
        # debug (toc_ncx)
...
class ContentOPF (object):
  ...
  <metadata xmlns:dcterms='http://purl.org/dc/terms/'
    ...
    """   
        ...
        source = dc.source
        if hasattr (options.config, 'PUBDIR'):
            if source.startswith (options.config.PUBDIR):
                source = source.replace (options.config.PUBDIR, options.config.PGURL)
    ...
    def add_coverpage (self, ocf, url):
        ...
        for item in xpath (
            ...
            "//*[local-name () = 'item' and @media-type = 'image/jpeg' and @href = $url]", 
            url = url):
...
class Writer (writers.HTMLishWriter):
    ...
    def strip_pagenumbers (xhtml):
        ...
        ## inline_ok = set ( [NS.xhtml.div, NS.xhtml.p, NS.xhtml.pre,
        ##                    NS.xhtml.address, NS.xhtml.fieldset,
        ##                    NS.xhtml.h1, NS.xhtml.h2, NS.xhtml.h3,
        ##                    NS.xhtml.h4, NS.xhtml.h5, NS.xhtml.h6] )
        ...
        for elem in xpath (xhtml, "//xhtml:*[@class]"):
            ...
            if STRIP_CLASSES.intersection (classes):
                ...
                ## # find out if we may use <a> or have to use <div>
                ## # <a> is not allowed in <body>
                ## tag = NS.xhtml.div
                ## for ancestor in reversed (xpath (elem, 'ancestor::xhtml:*')):
                ##     if ancestor.tag in inline_ok:
                ##         tag = NS.xhtml.a
                ##         break
    ...
    # characters that are not widely understood
    translate_map = {
        0x2012: u'–',    # U+2012 FIGURE-DASH    -> U+2013 EN-DASH (ADE lacks this)
        0x2015: u'—',    # U+2015 HORIZONTAL BAR -> U+2014 EM-DASH (ADE lacks this)
    }
    ...
    def strip_links (xhtml, manifest):
        ...
        for link in xpath (xhtml, '//xhtml:a[@href]'):
            ...
            if not manifest[href] in OPS_CONTENT_DOCUMENTS:
                debug ("strip_links: Deleting <a> to non-ops-document-type." % href)
    ...
    @staticmethod    
    def fix_image_dimensions (xhtml):
        """ fix image size attributes

        Make sure image width and height attributes don't exceed the
        acceptable image dimensions for the output format.
        ...
        for img in xpath (xhtml, '//xhtml:img'):
            ...
            # src = img.get ('src')
            # p = ParserFactory.ParserFactory.create (src, '')
            # w, h = p.get_image_dimen ()
            # img.set ('width', str (w))
            # img.set ('height', str (h))


    @staticmethod    
    def html_image_width_from_url (xhtml, url):
        """ Get the image width specified in the HTML.

        """

        for img in xpath (xhtml, '//xhtml:img[@src = $src]', src = url):
            return img.get ('width')
        ...
        try:
            ...
            for p in self.spider.parsers:
                if p.mediatype in OPS_CONTENT_DOCUMENTS:
                    ...
                    if hasattr (p, 'rst2epub2'):
                        ...
                        if self.options.verbose >= 2:
                            ...
                            debugfilename = os.path.join (self.options.outputdir,
                                                          self.options.outputfile)
                            debugfilename = os.path.splitext (debugfilename)[0] + '.html'
                    ...
                    self.fix_image_dimensions (xhtml)
                    if options.coverpage_url:
