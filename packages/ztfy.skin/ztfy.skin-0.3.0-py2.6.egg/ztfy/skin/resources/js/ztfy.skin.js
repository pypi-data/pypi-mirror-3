	...
	$.ZTFY.dialog = {
		...
		options: {
			...
			onBeforeLoad: function() {
				$.ZTFY.loader.start(this.getContent());
				this.getContent().load($.ZTFY.dialog.dialogs[$.ZTFY.dialog.getCount()-1].src);
	...
	$.ZTFY.form = {
		...
		add: function(form, parent, callback) {
			$.ZTFY.form.check(function() {
				$.ZTFY.form._add(form, parent, callback);
			});
			return false;
		},
		...
		_add: function(form, parent, callback) {
			...
			$($.ZTFY.form).data('add_action', $(form).attr('action'));
			var dataType = $.browser.msie ? 'text' : 'json';
			$.ZTFY.ajax.submit(form, $(form).attr('action') + '/@@ajax/ajaxCreate', data, callback || $.ZTFY.form._addCallback, null, dataType);
		},
		...
		edit: function(form, base, callback) {
			$.ZTFY.form.check(function() {
				$.ZTFY.form._edit(form, base, callback);
			});
			return false;
		},
		...
		_edit: function(form, base, callback) {
			...
			var dataType = $.browser.msie ? 'text' : 'json';
			$.ZTFY.ajax.submit(form, action + '/@@ajax/ajaxEdit', {}, callback || $.ZTFY.form._editCallback, null, dataType);
		},
		...
		update: function(form, callback) {
			$.ZTFY.form.check(function() {
				$.ZTFY.form._update(form, callback);
			});
			return false;
		},
