...
def run(args=None):
    ...
    reference_model, reference_instance, full_scenario_tree, scenario_tree_instance = load_reference_and_scenario_models(options)
    ...
    objective_name = reference_model.active_components()[Objective].keys()[0]
    objective = reference_model.active_components()[Objective][objective_name]
    ...
    if options.solve_xhat_with_ph is False:
        ...
        objective_name = ef_results.solution(0).objective.keys()[0] # take the first one - we don't know how to deal with multiple objectives.
        xhat_obj = float(ef_results.solution(0).objective[objective_name].value)  ## DLW to JPW: we need the gap too
        """if sense == 'min':
    ...
    for k in range(1, n_g+1):
        if AllInOne is True:
            ...
            reference_model, reference_instance, scenario_tree_for_soln, scenario_tree_instance = load_reference_and_scenario_models(options)
        ...
        objective_name = ef_results.solution(0).objective.keys()[0] # take the first one - we don't know how to deal with multiple objectives.
        xstar_obj = float(ef_results.solution(0).objective[objective_name].value)  ## DLW to JPW: we need the gap too, and to add/subtract is as necessary.
        """if sense == 'min':
        ...
        if type(xstar_obj_gap) is UndefinedData:
            ...
            #EW#print "xstar_obj_bound=", xstar_obj_bound
        else:
            if sense == 'min':
                ...
                xstar_obj_bound = xstar_obj + xstar_obj_gap
            #EW#print "xstar_obj_bound=", xstar_obj_bound
            #EW#print "xstar_obj = ", xstar_obj
            #EW#print "xstar_obj_gap = ", xstar_obj_gap
        ...
        objective_name = ef_results.solution(0).objective.keys()[0] # take the first one - we don't know how to deal with multiple objectives.
        xstar_obj_given_xhat = float(ef_results.solution(0).objective[objective_name].value)
        """if sense == 'min':
    ...
    for k in range(0, n_g):
        print "g_supk_of_xhat[k]=", g_supk_of_xhat[k]
        g_var = g_var + (g_supk_of_xhat[k] - g_bar) * (g_supk_of_xhat[k] - g_bar)
    g_var = g_var / (n_g - 1)    # sample var
    print ""
    print "Raw results:"
    print "g_bar=",g_bar
    print "g_stddev=",math.sqrt(g_var)
    print "Average f(xhat)=",sum_xstar_obj_given_xhat / n_g

    if n_g in t_table_values:
        print ""
        print "Results summary:"
        t_table_entries = t_table_values[n_g]
        for key in sorted(t_table_entries.keys()):
    ...
    if options.write_xhat_solution is True:
        ...
        for key, value in root_node._solutions.items():
            for idx in value:
                if value[idx]() != 0.0:
                    print key, idx, value[idx]()
    ...
    if options.append_file is not None:
        ...
        if n_g in t_table_values:
            t_table_entries = t_table_values[n_g]
            for key in sorted(t_table_entries.keys()):
                print >>output_file, " , alpha="+str(key)+" , "+str(g_bar + (t_table_entries[key] * math.sqrt(g_var) / math.sqrt(n_g))),
        ...
        if options.write_xhat_solution is True:
            ...
            for key, value in root_node._solutions.items():
                for idx in value:
                    if value[idx]() != 0.0:
                        print >>output_file, key, idx, value[idx](),
        output_file.close()
        print ""
