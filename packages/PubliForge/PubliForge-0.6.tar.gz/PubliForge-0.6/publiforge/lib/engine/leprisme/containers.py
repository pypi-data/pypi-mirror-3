# $Id: containers.py ff7342d7a5e7 2012/05/15 11:25:44 patrick $
...
from ...utils import _, EXCLUDED_FILES, get_mime_type
...
class OcfFactory(object):
    ...
    def make(self, fid, output):
        ...
        if not exists(join(output, root)):
            self._engine.build.stopped(
                _('unknown directory "${d}"', mapping={'d': root}), 'a_error')
            return
    ...
    def _update_manifest(self, root, manifest):
        ...
        manifest_elt = tree.xpath(xpath,
            namespaces={'opf': tree.getroot().tag.split('}')[0][1:]})
        if len(manifest_elt) != 1:
            self._engine.build.stopped(
                _('${x}: incorrect XPATH', mapping={'x': xpath}), 'a_error')
            return
        ...
        for path, name, files in walk(root):
            for name in files:
                ...
                elt = etree.SubElement(manifest_elt, 'item',
                    id='x_%s' % splitext(name)[0], href=relname)
