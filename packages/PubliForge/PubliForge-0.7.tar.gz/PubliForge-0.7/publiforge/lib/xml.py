# $Id: xml.py 4c5a8816200c 2012/09/06 09:04:56 patrick $
"""XML manipulation."""
...
def load(filename, relaxngs=None, data=None):
    ...
    if not relaxng.validate(tree):
        return relaxng.error_log.last_error and \
            unicode(relaxng.error_log.last_error)\
            .replace('%s/' % dirname(filename), '')\
            .replace(':ERROR:RELAXNGV', '') or _('Invalid XML file')
