...
class ADURecord(object):
    ...
    @staticmethod
    def to_file(records, file, truncate_long_values=False):
        """
        writes the data given in a list of ADURecord objects to the given
        filename or file.
        
        if truncate_long_fields is False, raises an exception if any are too
        long.
        """
        ...
        if havefile:
            ...
            fp=open(file, 'wb')
        for r in records:
            _writeline(fp, r._data, truncate_long_values)
        if not havefile:
            fp.close()
        else:
            fp.flush()
    ...
    def to_list(self, truncate_long_values=False):
        """
        if truncate_long_fields is False, raises an exception if any are too
        long.
        """
        return _tolist(self._data, truncate_long_values)
