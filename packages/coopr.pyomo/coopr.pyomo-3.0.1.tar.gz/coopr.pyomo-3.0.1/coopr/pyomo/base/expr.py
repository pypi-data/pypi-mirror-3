...
def generate_expression(*_args):
    def _no_clone(obj, target):
        return obj
    def _clone_if_needed(obj, target):
    ...
    if etype == 'neg':
        if type(_self) is _SumExpression:
            ...
            etype = 'rmul'
            other = NumericConstant(None, None, -1)
    elif etype == 'abs':
    ...
    if etype == 'add':
        ...
        l = len(ans._args)
        if l == 0:
            return NumericConstant(None, None, ans._const)
        ...
        # Special cases for simplifying expressions
        if ans.coef == 0:
            return NumericConstant(None,None,0)
        if not ans._denominator:
            l = len(ans._numerator)
            if l == 0:
                return NumericConstant(None,None,ans.coef)
