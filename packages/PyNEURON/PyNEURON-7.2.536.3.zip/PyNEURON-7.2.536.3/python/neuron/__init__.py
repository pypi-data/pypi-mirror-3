...
"""
# Execute the lines from package.ini to locate the package installation
import os
package_ini = os.path.join(os.path.dirname(__file__), 'package.ini')
with open(package_ini) as ini:
    for line in ini.readlines():
        exec(line)

# Configure NEURONHOME
neuronhome = os.path.join(__data_dir__, 'neuronhome')
if os.path.isdir(neuronhome) == False:
    print "Could not find NEURONHOME at", neuronhome
    neuronhome = os.path.join(os.path.dirname(__file__), '../../../neuronhome')
os.environ['NEURONHOME']=neuronhome
print "NEURONHOME is", neuronhome

# Add $NEURONHOME/bin to PYTHONPATH
import sys
sys.path.append(os.path.join(__data_dir__, 'neuronhome/bin')) 
