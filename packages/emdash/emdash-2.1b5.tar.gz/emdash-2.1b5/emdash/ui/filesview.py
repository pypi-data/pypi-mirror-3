# $Id: filesview.py,v 1.30 2011/05/24 21:58:30 irees Exp $
import collections
import datetime
import operator
import os
...
class ControlMenu(QtGui.QMenu):
	...
	def __init__(self, parent=None, filename=None, data=None):
	...
	def enqueue(self):
		self.signal_enqueue.emit(self.filename, self.data)
		
	def dequeue(self):
		self.signal_dequeue.emit(self.filename, self.data)
...
class ProgressBarDelegate(QtGui.QItemDelegate):
	...
	def paint(self, painter, option, index):
		...
		# 	return QtGui.QApplication.style().drawItemText(painter, rect, 0, option.palette, True, QtCore.QString("Record: %s"%progress))
...
class Menu(QtGui.QMenu):
	...
	signal_set = QtCore.pyqtSignal(int, unicode, unicode)
...
class FilesView(QtGui.QTreeView):	
	...
	def newfile(self, filename, item):
		"""Watch for new files, scroll to bottom and expand all"""
		self.expandAll()
	...
	def load_viewer(self, filename):
		print "loading viewer:", filename
		dname = os.path.dirname(filename)
		subprocess.Popen(['python', '-m', 'e2display', unicode(filename)], cwd=dname)
	...
	def mousePressEvent(self, event):
		...
		if param == "_status":	
			...
			if data.get("name") != None:
				QtGui.QDesktopServices.openUrl(QtCore.QUrl("%s/record/%s"%(emdash.config.get('host'), data.get("name"))))

			else:
				editor = ControlMenu(parent=self, data=data, filename=filename)
