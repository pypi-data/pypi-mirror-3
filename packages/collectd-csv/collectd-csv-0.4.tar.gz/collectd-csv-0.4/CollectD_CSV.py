...
"""A python module for fetching CollectD CSV data from a CollectD data directory
matching a specified time interval and regular expressions.
"""
...
def readEpochLines(filename, header, epoch_start, epoch_end):
    ...
    with open(filename, "r") as infile:
        ...
        #length (this is true for collectd 5), and that the measurement 
        #interval (step) doesn't change in the file. If you are sure these 
        #things are true for you, then it should be safe to uncomment this.
        """   
    ...
    for f in glob.iglob(csvdatadir+(os.sep+"*")*3):
        ...
        for d in isodaylist:
            if d in ftail:
                ...
                #Try to check if the data in the directory is valid
                try:
                    datestr = ftail.rsplit("-", 3)[-3:]
                    date(int(datestr[0]), int(datestr[1]), int(datestr[2]))
                except:
                    raise MyError("The data directory '" + csvdatadir + 
                                  "' doesn't contain valid data files. ")
