...
class BackboneProject(Project):
    """
    Start a Backbone project
    """
    def init(self):
        """
        Initialize the Backbone project structure.

        We don't actually super() this explicitly as we aren't likely to
        start a Backbone project at the top level. It'll be a sub-instance of
        anothe project type. So::

        * Create root directory (root/static/js)
        * Create our lib dir
        * Create a stub Backbone application
        * Create a stub Jasmine Specs section
        * Add Javascripty thinhs to the VCS ignore file
...
def main():
 ...
 and overrides any other vcs arguments.", action = "store_true")],
    ...
    backboneparser = subparsers.add_parser('backbone', help='Start a Backbone project',
                                           description='Start a shiny Backbone project')
    backboneparser.set_defaults(func=main_backbone, flavour='backbone')
