...
__all__ = ['Block', 'active_components_generator', 'active_subcomponents_generator']
import sys
import weakref
...
# 
# we now include a number of generators for iterating over various components within a block. 
# the primary purpose is to streamline code, as this kind of iteration is done *everywhere*
# in Coopr. any generally useful iteration schemes should be moved into the following segment.
#

# iterates over all active components (e.g., Var) of the specified type in the input block.
# sorting, if specified, is by component name.
def active_components_generator(a_block, component_type, sort_by_keys=False):
    active_components = a_block.active_components(component_type)
    if sort_by_keys is True:
        for component_name in sorted(active_components): 
            component = active_components[component_name]
            if (not component.active):
                continue
            yield component
    else:
        for component in active_components.itervalues():
            if (not component.active):
                continue
            yield component

# iterates over all active sub_components (e.g., _VarData) of all active components of the 
# specified type in the input block. sorting, if specified, is by component name, and within 
# a component, by index.
def active_subcomponents_generator(a_block, component_type, sort_by_keys=False):
    active_components = a_block.active_components(component_type)
    if sort_by_keys is True:
        for component_name in sorted(active_components):
            component = active_components[component_name]
            if (not component.active):
                continue
            for index in sorted(component._data):
                sub_component = component[index]
                if (not sub_component.active):
                    continue
                yield sub_component
    else:
        for component in active_components.itervalues():
            if (not component.active):
                continue
            for sub_component in component.itervalues():
                if (not sub_component.active):
                    continue
                yield sub_component
...
class BlockComponents(object):
    ...
    def __setattr__(self,name,val):
        ...
        if name in self.__dict__:
            ...
            setter = getattr(self.__dict__[name], 'set_value', None)
            if setter is None:
                if isinstance(self.__dict__[name], Component):
                    raise Exception(
                        "Expected component %s to have a 'set_value' "
                        "method, but none was found." % \
                        ( self.__dict__[name],))
                self.__dict__[name]=val
            else:
                setter(val)
    ...
    def model(self):
        """Return the top-level block that defines this model."""
        return self._model()
...
class Block(IndexedComponent):
    ...
    # flags *all* active variables and their composite _VarData objects as stale.
    # fixed variables are flagged as non-stale. the state of all inactive variables
    # is left unchanged.
    ...
    def construct(self, data=None):
        ...
        for val in self._index:
            ...
            if val is None:
                apply_indexed_rule( self, self._rule, self, () )
                # FIXME: This may be a circular reference, but it's letting us make
                #        non-indexed blocks for now (ZF)
                block = self
            else:
                block = self.__class__()
                self._add_component(name, block)
                apply_indexed_rule( self, self._rule, block, val )
                self._data[val] = block

            data_for_this_block_index = None
            if type(data) is dict:
                data_for_this_block_index = data.get(val,{})

            if __debug__ and logger.isEnabledFor(logging.DEBUG):
                logger.debug( "Constructing %s %s, index %s, from data=%s",
                              self.__class__.__name__, self.name, str(name), str(data_for_this_block_index) )

            for tmp in block.components.itervalues():
                data_to_use = None
                if data_for_this_block_index is not None:
                    data_to_use = data_for_this_block_index.get(tmp.name, None)
                tmp.construct(data_to_use)

    def _add_temporary_set(self,val):
        """ TODO """
        # NB: only "old" IndexedComponent contain _index_set.
        # SparseIndexedComponent does not have that attribute
    ...
    def fix_all_vars(self):
        for var in self.components[Var]:
            self.components[Var][var].fix_all()
        for block in self.components[Block]:
            self.components[Block][block].fix_all_vars()

    def unfix_all_vars(self):
        for var in self.components[Var]:
            self.components[Var][var].unfix_all()
        for block in self.components[Block]:
            self.components[Block][block].unfix_all_vars()
