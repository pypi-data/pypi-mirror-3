...
class Service(object):
    ...
    def __init__(self):
        ...
        self._js_persistence = persistence(self.exports, self.LAVA_FILE)
        self._nfs_persistence = persistence(self.exports, self.NFS_FILE, serializer=ExportsSerializer)
        self._js_persistence.load()

    def __del__(self):
        if self.js_persistence.is_dirty:
            self._js_persistence.save()
            self._nfs_persistence.save()
            self._call_exports_r()

    def _call_exports_r(self):
        subprocess.call_checked(['sudo', 'exports', '-r'])
