...
valid_log_level = one_of("ERROR", "MESSAGE", "WARNING", "LOG", "INFO", "DEBUG")

###############################################################################

class MethodProxy:

    def __init__(self, method, unlock_module, src_module, call_attrs):
        self._method, self._unlock_module = method, unlock_module
        self._src_module, self._call_attrs = src_module, call_attrs
        ...
        with self._lock:
            ...
            if not attr_info:                 # no luck, go full cycle
                ...
                try:
                    ...
                    attr = get_attr(name, __source_module_name = self._name)
...
class ModuleProxy:
    ...
    def __getattr__(self, name):
        try:
            ...
            try:
                attr, requires_src_module_kwarg, requires_call_attrs_kwarg = self._module.get_attr_info(name)
                src_module = self._src_module if requires_src_module_kwarg else None
                call_attrs = [] if requires_call_attrs_kwarg else None
                method_proxy = MethodProxy(attr, unlock_module, src_module, call_attrs)
            except:
                unlock_module()
                raise
            else:
...
class ModuleLoader:
    ...
    _log_levels = { "ERROR": 1, "MESSAGE": 2, "WARNING": 3, "LOG": 4, "INFO": 5, "DEBUG": 6 }
    ...
    class _Logger:
        ...
        def __init__(self, log, log_level, src_module):
            self._log, self._log_level = log, log_level
            self._src_module = src_module

        def __call__(self, *args, msg_level = None):
            try:

                if msg_level is None: # such recursive call is required because we need exact stack depth
                    return self(*args, msg_level = "LOG")

                request = getattr(current_thread(), "_request", None)
                request_log_level = request and request.log_level or 0

                msg_level = ModuleLoader._log_levels[msg_level]
                if msg_level <= max(request_log_level, self._log_level):
                    ...
                    message = " ".join(str_ex(arg) for arg in args)
                    ...
                    self._log(message, msg_level = msg_level)

            except:
                pass # do nothing

        error = lambda self, *args: self(*args, msg_level = "ERROR")
        message = lambda self, *args: self(*args, msg_level = "MESSAGE")
        warning = lambda self, *args: self(*args, msg_level = "WARNING")
        info = lambda self, *args: self(*args, msg_level = "INFO")
        debug = lambda self, *args: self(*args, msg_level = "DEBUG")

        # this utility class supports log level modification using
        # with pmnc.log.level("LEVEL"): syntax
        ...
        # this method modifies the log level for the current request
        # to support pmnc.log.level("LEVEL") syntax
        ...
        def level(self, log_level: optional(valid_log_level) = None):
            ...
            if request:
                if log_level is not None:
                    request.push_log_level(ModuleLoader._log_levels[log_level])
                else:
                    request.pop_log_level()
            return self._PopLogLevel(request)

    # this method modifies the log level for the entire cage

    @typecheck
    def set_log_level(self, log_level: valid_log_level):
        log_level = self._log_levels[log_level]
        with self._lock:
    ...
    def __getattr__(self, module_name, src_module = None):
        ...
        if module_name == "log":
            with self._lock:
                ...
                if logger is None:
                    logger = ModuleLoader._Logger(self._log, self._log_level, src_module)
...
if __name__ == "__main__":
    ...
    pmnc = ModuleLoader(node_name, cage_name, cage_dir, log, "DEBUG")
    ...
    [
        ...
        "ERROR # logging.py:12 " + suffix,
        "MESSAGE # logging.py:13 " + suffix,
        "WARNING # logging.py:14 " + suffix,
        "LOG # logging.py:15 " + suffix,
        "INFO # logging.py:16 " + suffix,
        #"DEBUG # logging.py:17 " + suffix,

        "ERROR # logging.py:19 " + suffix,
        "MESSAGE # logging.py:20 " + suffix,
        "WARNING # logging.py:21 " + suffix,
        "LOG # logging.py:22 " + suffix,
        "INFO # logging.py:23 " + suffix,
        #"DEBUG # logging.py:24 " + suffix,

        "ERROR # logging.py:25 " + suffix,
        "MESSAGE # logging.py:26 " + suffix,
        "WARNING # logging.py:27 " + suffix,
        "LOG # logging.py:28 " + suffix,
        "INFO # logging.py:29 " + suffix,
        #"DEBUG # logging.py:30 " + suffix,

        "ERROR # logging.py:32 " + suffix,
        "MESSAGE # logging.py:33 " + suffix,
        "WARNING # logging.py:34 " + suffix,
        "LOG # logging.py:35 " + suffix,
        "INFO # logging.py:36 " + suffix,
        #"DEBUG # logging.py:37 " + suffix,

        "ERROR # logging.py:39 " + suffix,
        "MESSAGE # logging.py:40 " + suffix,
        "WARNING # logging.py:41 " + suffix,
        "LOG # logging.py:42 " + suffix,
        "INFO # logging.py:43 " + suffix,
        #"DEBUG # logging.py:44 " + suffix,

    ]
    assert log_lines[-len(_log_lines):] == _log_lines

    del log_lines[:]
    pmnc.logging.test_request_level("DEBUG")
    suffix = "in test_request_level() by {0:s}".format(r.description)
    _log_lines = \
    [
