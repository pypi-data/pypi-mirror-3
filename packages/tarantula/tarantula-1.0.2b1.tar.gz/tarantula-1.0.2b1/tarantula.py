...
def crawl(basedirectory, posters, trailer, databasepath = common.databasepath, interactive = False):
	"""
	...
	databasepath	-- The location of the database. Default set in module common.
		...
		for afile in files:
			...
			if fileextension in common.filetypes and filesize > 360*((1024)**2):
				...
				if checkdatabase(moviepath, databasepath) == False:
					...
					pass
				if trailer == True:
					checktrailer(moviepath, files, databasepath)
				if posters == True:
					checkimdbposter(moviepath, files, databasepath)
					checkrtposters(moviepath, files, 'original', databasepath)				
...
def checkjson(moviepath, databasepath):
	...
	movie = common.cleanfilename(os.path.basename(moviepath))
	movie = common.cleanbeforewrite(movie)
	jsonfile = os.path.join(moviedirectory, 'metadata', movie + '.json')
...
def startinteractive(moviepath, posters, trailers, databasepath):
	moviedirectory = os.path.dirname(moviepath)
	movie = common.cleanfilename(os.path.basename(moviepath))
	records = ()
	with lite.connect(database) as con:
		cur = con.cursor()
		request = ("select moviestext.title, moviestext.id, \
alternates.alternateid, alternates.alternatetitle from moviestext, moviesmisc, \
alternates where moviestext.tomatometer = 'Not Available' and \
moviestext.id = moviesmisc.id and moviestext.id = alternates.id")
		try:
			response = cur.execute(request)
		except lite.Error as e:
			logger.info('Database error: %s', e.args[0])
		else:
...
def checktrailer(moviepath, files, databasepath):
	"""
...
def checkimdbposter(moviepath, files, databasepath):
	"""
...
def checkrtposters(moviepath, files, quality, databasepath):
	"""
...
for easy access.', version=common.version)
	...
	crawl(arguments.directory, arguments.posters, arguments.trailer, databasepath, arguments.interactive)
