# $Id: storage.py d91bb93a2a7f 2012/07/29 09:44:59 patrick $
...
from ..lib.utils import _, MIME_TYPES, has_permission, encrypt, age
...
class StorageView(object):
    ...
    def __init__(self, request):
        ...
        self._vcs_engines = dict([
            ...
            in request.registry.settings['storage.vcs'].split()])
    ...
    def index(self):
        ...
        self._request.breadcrumbs.add(
            _('All storages'), 'container' in self._request.session and 6 or 2)
        return {
    ...
    # -------------------------------------------------------------------------
    @view_config(
        route_name='storage_index_ajax', renderer='json',
        permission='stg.read')
    def index_ajax(self):
    ...
    # -------------------------------------------------------------------------
    @view_config(
        route_name='storage_root_ajax', renderer='json', permission='stg.read')
    @view_config(
        route_name='storage_browse_ajax', renderer='json',
        permission='stg.read')
        ...
        if action[0:4] == 'upl!':
            ...
            working = handler.launch(
                ...
                (vcs_user(self._request, storage), path,
                 form.values['directory'], form.values.get('message', '-')))
        elif action[0:4] == 'ren!':
            working = handler.launch(
    ...
    def _vcs_user_form(self, storage, action):
        ...
        user.vcs_password = encrypt(
            form.values['vcs_password'],
            self._request.registry.settings.get('encryption', '-'))
        DBSession.commit()
