...
"""
    ...
    Utilities to simplify processing images in your Shake app.
...
def remove_thumbnails(base_root, src_path, src_name, rules):
    path = os.path.join(base_root, src_path)
    try:
        os.remove(os.path.join(path, src_name))
    except OSError:
        pass
    for rule in rules:
        tname = rule[0]
        try:
            os.remove(os.path.join(path, tname, src_name))
        except OSError:
            pass
...
def process_image(path, new_path, operations, format=''):
    """ """
    img = Image.open(path)
    # PIL's Image object doesn't have a notion of "transparent color index",
    ...
    if format in ('JPEG', 'JPG'):
        ...
        try:
            ...
            pass
        img.save(new_path, format=format, transparency=transparency)
    else:
