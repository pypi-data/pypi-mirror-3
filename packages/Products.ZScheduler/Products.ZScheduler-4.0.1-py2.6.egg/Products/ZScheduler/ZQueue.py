...
class ZQueue(ZCatalog):
    ...
    def pop(self, obj):
        try:
            self._lock.acquire()
            ZCatalog.uncatalog_object(self, '/'.join(obj.getPhysicalPath()))
        finally:
            self._lock.release()
    ...
    def push(self, obj):
        try:
            self._lock.acquire()
            ZCatalog.catalog_object(self, obj, '/'.join(obj.getPhysicalPath()))
        finally:
            self._lock.release()
	...
	try:
        ...
        map( lambda subfolder,q=self: q._reschedule(subfolder),
             filter( lambda x: getattr(aq_base(x), 'objectValues', None), objs ) )
