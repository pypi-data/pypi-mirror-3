...
class TestGetTokens(unittest.TestCase):
    ...
    def test_good_case(self):
        ...
        [...] This is an item that requires a textarea to be filled in.
        ...
        self.assertEqual(16, len(tokens), tokens)
        ...
        self.assertEqual('TEXT_ITEM', tokens[13].token)
        self.assertEqual('BREAK', tokens[14].token)
        self.assertEqual('TEXT', tokens[15].token)
    ...
    def test_text_item(self):
        """
        Ensures items that require textual input are identified.
        """
        data = "[...] This is a text item."
        tokens = get_tokens(data)
        self.assertEqual(1, len(tokens),
            "Got the wrong number of tokens: %s" % tokens)
        for t in tokens:
            self.assertEqual("TEXT_ITEM", t.token)
            self.assertEqual("This is a text item.", t.value)

    def test_text_item_with_roles(self):
        """
        Ensures items may have roles assigned to them via a list in curly
        brackets.
        """
        data = "[...] {doctor, nurse} This is a text item."
        tokens = get_tokens(data)
        self.assertEqual(1, len(tokens),
            "Got the wrong number of tokens: %s" % tokens)
        self.assertEqual("TEXT_ITEM", tokens[0].token)
        self.assertEqual("This is a text item.", tokens[0].value)
        self.assertEqual(['doctor', 'nurse'], tokens[0].roles)
