    ...
    def test_imported_patches(self):
        ...
        for patch in patches.entities():
            ...
            if len(files) == 1:
                ...
                bypatch[patch] = files
        self.assertEqual(len(byfile), 5, byfile)
        ...
        self.assertEqual(len(MAILBOX), 6, MAILBOX)
...
url: http://testing.fr/cubicweb/patch/%s\n''' % patch3_2.eid)
        ...
        self.assertEqual(MAILBOX[3].subject, '[patch repository] patch pending-review: patch4.diff')
        self.assertEqual(MAILBOX[3].recipients, ['admin@cwo'])
        self.assertMultiLineEqual(MAILBOX[3].content, u'''
cubicweb changed status from <in-progress> to <pending-review> for entity
'patch4.diff'

review asked by Julien Cristau <julien.cristau@logilab.fr> in #17:2f2bad1f3f12

url: http://testing.fr/cubicweb/patch/%s\n''' % patch4.eid)

        self.assertEqual(MAILBOX[4].subject, '[patch repository] patch assigned: patch4.diff')
        self.assertEqual(MAILBOX[4].recipients, ['admin@cwo'])
        self.assertMultiLineEqual(MAILBOX[4].content, u'''
You have been assigned for review of patch patch4.diff

url: http://testing.fr/cubicweb/patch/%s\n''' % patch4.eid)

        self.assertEqual(MAILBOX[5].subject, '[patch repository] patch in-progress: patch4.diff')
        self.assertEqual(MAILBOX[5].recipients, ['admin@cwo'])
        self.assertMultiLineEqual(MAILBOX[5].content, u'''
cubicweb changed status from <pending-review> to <in-progress> for entity
'patch4.diff'

modified by Julien Cristau <julien.cristau@logilab.fr> in #19:650cce0df5e2
...
url: http://testing.fr/cubicweb/task/%s\n''' % done.eid)
        ...
        self.assertEqual(MAILBOX[0].subject, '[patch repository] patch reviewed: patch3.diff')
        self.assertEqual(MAILBOX[1].subject, '[patch repository] patch applied: patch3.diff')
        self.assertEqual(MAILBOX[0].recipients, ['admin@cwo'])
        self.assertEqual(MAILBOX[1].recipients, ['admin@cwo'])
        self.assertMultiLineEqual(MAILBOX[1].content, u'''
