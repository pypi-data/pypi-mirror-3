...
def _flat_upgma(
    ...
    for i,valA in list(clusters.items()):
        for j,valB in list(clusters.items()):
            if i != j:
                score = []
                for vA in valA:
                    for vB in valB:
...
def _neighbor(
    ...
    if not tracer:
        tracer = dict([(tuple([a]),b[0]) for (a,b) in list(clusters.items())])
    # terminate when the dictionary is of length 1
    if len(clusters) == 2:
    ...
    clusters = {}
    for key,val in list(new_clusters.items()):
        clusters[key] = val
