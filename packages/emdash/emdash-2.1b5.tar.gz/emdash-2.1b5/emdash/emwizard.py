# $Id: emwizard.py,v 1.15 2012/02/24 09:30:45 irees Exp $
...
def edititem(parent=None, pd=None, value=None, db=None, name=None):
...
class DBQNoEdit(QtGui.QLabel):
	def __init__(self, parent, value=None, pd=None, db=None):
...
class DBQComboBox(QtGui.QComboBox):
	...
	def query(self):	
		self.query = self.db.findvalue(self.pd.get("name"),"")
...
# 	def __init__(self, parent=None, choices=None, db=None):
...
class Page(QtGui.QWizardPage):
	...
	def __init__(self, parent=None, db=None, title=None, helptext=None):
...
class PageRectypeSelect(Page):
	...
	def setupUi(self):		
		...
		rds = self.db.getrecorddef(self.rectypes)
		self.radios = {}
		for rd in rds:
...
class PageNew(Page):
	...
	def update(self):
		...
		if self.p.results_select.get(self.rectype) >= 0:
			rec = self.p.db.getrecord(self.p.results_select.get(self.rectype))
				
		self.map_pd_elements = {}
		name = self.db.checkcontext()[0]
		rd = self.db.getrecorddef(self.rectype)
	...
	def getvalue(self):
		...
		rec = self.db.newrecord(
			self.rectype, 
			self.p.results_select.get("project"))	
...
class PageSelect(Page):				
	...
	def update(self):
		...
		# Backwards compat.		 
		self.rectypes = [i.get('name') for i in self.db.getrecorddef(self.rectypes)]
		self.accepted = [i.get('name') for i in self.db.getrecorddef(self.accepted)]		
		
		recnames, paths = self.db.renderchildtree(
			root, 
			self.recurse, 
			self.rectypes)
...
class PageEdit_microscopy(PageEdit):
	...
	def init(self):
		self.rectype = emdash.config.get('session_rectype')
...
class SessionWizard(QtGui.QWizard):
	...
	def __init__(self, parent=None, selected=None, db=None):
...
class SelectAnyWizard(SessionWizard):
	...
	signal_any = QtCore.pyqtSignal(int)
	...
	def accept(self):
		try:
			...
			self.signal_any.emit(int(target))
			self.done(0)
		except Exception, e:
			error(e)
...
class NewGridWizard(SessionWizard):
	...
	def getrecords(self):		
		...
		records[-1]["performed_by"] = self.db.checkcontext()[0]
	...
	def getrecords(self):		
		...
		if s.get('aliquot') >= 0:
			...
			if s.get('purification') >= 0:
				...
				print """Aliquot without parent purification?? Please report this."""
...
# 	wizard = NewFreezingWizard(db=db)
# 	wizard.show()
# 	sys.exit(app.exec_())
