...
def get_comments ( issue ):
    return jiraobj.jira1.getComments( token, issue )
...
def format_issue( issue , mode = 0 ):
    ...
    try:
        ...
        if mode >= 1:
            ...
            idx = 1
            for comment in get_comments(issue["key"]):
                fields["comment_%d" % idx] = "\n" + comment["body"]
                idx+=1
        if mode < 0:
        ...
        return "\n".join( " : ".join((k.ljust(20),v)) for k,v in fields.items() ) + "\n"
            ...
            return "%s: Not found %s" % (issue["key"], e)
...
def edit_jira( issue ):
    jira = get_jira ( issue )
    for k in jira:
        new_val = raw_input("%s (value=%s): " % (k, jira[k]) )
...
"""
    ...
    parser.add_option("-e","--edit",dest="edit", help="free style editing of the jira")
    ...
    try:
        if opts.listtypes:
            ...
            if opts.issue_type:
                ...
                print format_issue ( edit_jira( opts.edit ), mode )
