...
from coopr.pyomo.base.var import  _VarValue, VarStatus, _VarArray, _VarBase, Var, VarList
...
class IdentifyVariablesPresolver(pyutilib.component.core.SingletonPlugin):
    ...
    #
    def _identify_variables_in_linear_encoding(self, encoding, var_map):

        # recall that "encoding" is a tuple: element 0 is the constant, element 1 is a list of (coefficient, _VarValue) pairs.
        for coefficient, var in encoding[1]:

            if var.fixed: # we know this is a _VarValue, by construction - do the direct query to save time.
                continue
            else:
                if var.id in var_map:
                    continue
                var.status = VarStatus.used
                var.id = self.var_count
                var_map[self.var_count] = var
                self.var_count += 1

    #
    # Recursively work through an expression (tree) to identify variables.
    #
    def _identify_variables_in_expression(self, exp, var_map):
        ...
        if is_expr:
            ...
            if exp.__class__ is expr._ProductExpression:
                for arg in exp._numerator:
                    if arg.is_constant():
                        pass
                    elif isinstance(arg,_VarValue):
                        if arg.id not in var_map:
                            arg.status = VarStatus.used
                            arg.id = self.var_count
                            var_map[self.var_count] = arg
                            self.var_count += 1
                    else:
                        self._identify_variables_in_expression(arg, var_map)
        ...
        #
            ...
            if exp.id in var_map:
                return
            exp.status = VarStatus.used
            exp.id = self.var_count
            var_map[self.var_count] = exp
            self.var_count += 1
        #
        # Connector Value
        #
            ...
            for var in exp.vars.itervalues():
                ...
                if type(var) is not VarList:
                    self._identify_variables_in_expression(var, var_map)
        #
        # Variable Base
        #
            ...
            if tmp is exp:
                raise ValueError("Variable %s appears in an expression without "
                                 ...
                                 "value." % ( exp.name, ))
            if tmp.id in var_map:
                return
            exp.status = VarStatus.used
            tmp.id = self.var_count
            var_map[self.var_count] = tmp
            self.var_count += 1
        #
        # Connector Base
        #
    ...
    def _preprocess_block(self, block, _var_map):
        ...
        for name, objective in active_objectives.iteritems():
            for index, objective_data in objective._data.iteritems():
                if not objective_data.active:
                    continue
                try:
                    self._identify_variables_in_expression(objective_data.expr, _var_map)
            ...
            for index, constraint_data in constraint._data.iteritems():
                ...
                try:
                    if constraint_data.lin_body is not None:
                        self._identify_variables_in_linear_encoding(constraint_data.lin_body, _var_map)
                    else:
                        self._identify_variables_in_expression(constraint_data.body, _var_map)
                except ValueError, err:
                    logging.getLogger('coopr.pyomo').error(
    ...
    # components, in addition to models themselves. for VarValue,
    # ConstraintData, and ObjectiveData, the routine assigns the "id"
    # attribute. in addition, for VarValues, the routine sets the
    ...
    # respective VarValue, ConstraintData, or ObjectiveData object.
    #
    ...
    def preprocess(self,model):
        ...
        for block in all_blocks:
            self._preprocess_block(block, _var_map)
