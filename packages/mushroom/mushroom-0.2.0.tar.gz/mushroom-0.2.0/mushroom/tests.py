...
class MessagingTestService(object):
    ...
    def rpc_get_answer(self, request):
        return mushroom.Response(request, 42)
...
class MessagingTestCase(unittest.TestCase):
    ...
    def test_transport(self):
        ...
        try:
            transport.start()
            message = mushroom.Notification('foo')
            transport.send(message, 'monolog')
            self.assertEquals(transport.receive().method, 'foo')
        finally:
            transport.stop()

    def test_reply_to(self):
        a = messaging.Transport(BROKER_URL, self.exchange, self.a_queue)
        b = messaging.Transport(BROKER_URL, self.exchange, self.b_queue)
        try:
            a.start()
            b.start()
            a_request = mushroom.Request('get_answer')
            a.send(a_request, 'b')
            b_request = b.receive()
            self.assertEqual(b_request.method, 'get_answer')
            b_response = mushroom.Response(b_request, 42)
            b.send(b_response)
            a_response = a.receive()
            self.assertEqual(a_response.data, 42)
        finally:
            a.stop()
            b.stop()
    ...
    def test_rpc_request(self):
        ...
        try:
            transport.start()
            engine.start()
            answer = engine.request('get_answer', routing_key='monolog')
