...
def _import_sound_classes(filename):
    ...
    for key in list(sc_dict.keys()):
        for value in sc_dict[key]:
            sc_repl_dict[str(value)] = key
...
def _import_score_tree(filename):
    ...
    for key in list(score_tree.keys()):
...
def _fop(
    ...
    if start not in graph.node:
        return []
    paths = []
    for node in list(graph.edge[start].keys()):
        if node not in path:
            ...
            if _fop(graph.to_undirected(),start,end) != []:
                ...
                for path in paths:
                    for node in path[1:-1]:
                        if start in list(shortest_paths[node].keys()) and end in\
                                list(shortest_paths[node].keys()):
...
def _export_score_dict(score_dict):
    ...
    letters = list(set([key[0] for key in list(score_dict.keys())]))
