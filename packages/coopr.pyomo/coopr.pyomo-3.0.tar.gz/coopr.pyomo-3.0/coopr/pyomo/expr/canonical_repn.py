...
__all__ = [ 'generate_canonical_repn', 'as_expr', 'is_constant', 
            'is_linear', 'is_quadratic', 'is_nonlinear' ]
...
def generate_canonical_repn(exp, idMap=None):
    ...
    if degree == 1:
        # varmap is a map from the variable hash to a VarValue.
...
def collect_variables(exp, idMap):
    if exp.is_expression():
        ...
        return {}
    elif exp.__class__ is _VarValue or exp.type() is Var:
...
def collect_linear_canonical_repn(exp, idMap):
    ...
    # constants and _VarValues are more frequent than expressions - look for them first, for efficiency.
    if exp.is_constant():
        # NB: is_constant() returns True for variables with fixed values
        return { None: exp() }, {}
        ...
        if exp.__class__ is expr._ProductExpression:
            ...
            for subexp in exp._numerator:
                ...
                elif subexp.__class__ is _VarValue:
            ...
            for i in xrange(len(exp._args)):
                ...
                if (arg.__class__ is _VarValue) and (arg.fixed is False):
