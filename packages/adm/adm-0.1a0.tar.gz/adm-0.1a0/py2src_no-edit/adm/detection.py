u"""Detection for Active Data Mapping."""
...
def total_classifier(fn):
    ...
    def wrapped(sample):
        preface = (u"Function %s() has been wrapped for use in a "
                   u"classifier.  It requires an iterable collection of "
                   u"mappings or sequences - " % fn.__name__)
        if not _is_iterable(sample):
            raise TypeError(preface + (u"'%s' type is not iterable." %
                                       type(sample).__name__))
        if isinstance(sample, unicode):
            raise TypeError(preface + u"instead, received a string (an "
                            u"iterable collection of characters) -- "
        ...
        if not _is_mapping(outcome):
            raise TypeError(u"Classifier %s() returned illegal type "
                            u"'%s' (must return a mapping)." %
                            (fn.__name__, type(outcome).__name__))
        for val in outcome.values():
            if not isinstance(val, (float, int)):
                raise TypeError(u"Returned mapping contains illegal "
                                 u"type '%s', %s" %
                                 (type(val).__name__, repr(outcome)))
            if val < 0 or val > 1:
                raise ValueError(u"Returned mapping contains illegal "
                                 u"values " + repr(val))
...
def element_classifier(fn):
    ...
    @column_classifier
    def wrapped(column):
        if not _is_iterable(column) or isinstance(column, unicode):
            raise TypeError(u"Function '%s' has been wrapped for use as "
                            u"a classifier.  It requires an iterable "
                            u"collection of items - '%s' object is not "
                            u"allowed." %
        ...
        for element in column:
            ...
            if isinstance(outcome, (float, int)) and 0 <= outcome <= 1:
                ...
                raise ValueError(u"Calling %s() classifier on type '%s' "
                                 u"returned illegal value (%s)." %
    ...
    wrapped.__doc__ = u"""Decorated function calling %s() as part of
...
class Locator(object):
    def __init__(self, *adapters, **classifiers):
        ...
        for clsname, clsfunc in classifiers.items():
            ...
            if not callable(clsfunc):
                try:
                    handler = getattr(self, clsname + u'_handler')
                    clsfunc = handler(clsfunc)
                except AttributeError:
                    raise TypeError(u"Special handling for '%s' "
                                    u"classifier is unsupported." %
                                    (clsname))
    ...
    def __repr__(self):
        ...
        requirements = [unicode(k)+u'='+repr(v) for k, v in requirements]
        requirements = u', '.join(requirements)
        if requirements:
            requirements = u', using ' + requirements
        ...
        return (u'<%s object, status: %s%s> %s' %
                (self.__class__.__name__, status, requirements, stats))

    @staticmethod
    def _filter_cutoff(cutoff, results):
        u"""Takes cutoff dict and results set and returns only those
        ...
        return set(r for r in results if meets_cutoff(r))

    @staticmethod
    def _filter_required(cutoff, results):
        u"""Takes cutoff dict and results set, returns results for only
        ...
        for objkey, group in itertools.groupby(results, key=getobjkey):
            group = set(group)
            matched_clsnames = set(result[2] for result in group)
    ...
    def _filter_maximum(results):
        ...
        return set(r for r in results if r[3]==maxvals.get(r[0]))

    @staticmethod
    def _filter_order(adapters, results):
        u"""Takes list of adapters and set of results.  Returns set of
        ...
        first_adapter = set()
        for key, group in itertools.groupby(results, key=groupfn):
            result = group.next()  # Take first result only.
    ...
    def location(self):
        ...
        locations = set(Location(x[0], adapter=x[1]) for x in stats)
    ...
    def scan(self, sample, **criteria):
        ...
        def normalize(mapping):
            if not _is_mapping(mapping):
                if isinstance(mapping, unicode):
                    raise TypeError((u"sample must be an iterable "
                                     u"collection of mappings or "
                                     u"sequences - a collection of "
                                     u"strings is not allowed.") %
                                     fn.__name__)
        ...
        def classify(adapter, classifier_key):  # !!! TODO: Replace closures with partial()
            if adapter:
                fn = lambda mapping: dict((k, adapter(v)) for k,v in mapping.items())  # Closes over adapter
        ...
        rtup = collections.namedtuple(u'result', [u'objkey',  u'adapter',
                                                 u'clsname', u'clsvalue'])
        result = []
    ...
    def form_handler(patterns):
        if isinstance(patterns, unicode) or not _is_container(patterns):
            patterns = [patterns]

        retype = type(re.compile(u'foo'))  # re module's compiled object
    ...
    def freq_handler(pmf):  # Probability mass function.
        u"""Accepts probability distribution as a dict."""

        @column_classifier
        def classify_by_freq(column):
            count = collections.Counter(column)
            total = sum(count.values())
            count = dict((k, v/total) for k,v in count.items())
...
def pearsonr(x, y):
    u"""Returns the Pearson product-moment correlation coefficient
