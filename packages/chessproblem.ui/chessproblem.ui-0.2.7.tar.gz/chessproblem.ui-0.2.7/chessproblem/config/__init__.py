...
CONFIGDIR=os.path.expanduser('~/.cpe')

LOG_DIRECTORY = os.path.join(CONFIGDIR, 'log')
LOGGING_CONFIG_FILE=os.path.join(CONFIGDIR, 'logging.config')
...
class CpeConfig(object):
    ...
    def __init__(self):
        ...
        self.database_url = 'sqlite:///' + os.path.join(CONFIGDIR, 'cpe.db')
        self.compile_menu_actions = []
        self.compiled_latex_viewer = None

DEFAULT_CONFIG = CpeConfig()

_CONFIG_TEMPLATE_FILENAME = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'template', 'config.py')

USER_CONFIG = os.path.join(CONFIGDIR, 'config.py')
