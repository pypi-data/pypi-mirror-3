...
#def AckTimeout(CallbackFunction, CreatorID, PacketID):
#    global _AckWaitingPacketsList
#    ComboID = CombineIDs(CreatorID, PacketID)
#    if ComboID in _AckWaitingPacketsList:
#        CallbackFunction(CreatorID, PacketID)
#        #dhnio.Dprint(14, "transport_control.AckTimeout [%s] found waiting=%d" % (ComboID, len(_AckWaitingPacketsList)))
#    else:
#        dhnio.Dprint(14, "transport_control.AckTimeout WARNING [%s] NOT FOUND waiting=%d" % (ComboID, len(_AckWaitingPacketsList)))

#def RegisterAckTimeout(CallbackFunction, CreatorID, PacketID, timeout=60):
#    reactor.callLater(timeout, AckTimeout, CallbackFunction, CreatorID, PacketID)
...
#def FindAckWaitingPacket(ack):
#    global _AckWaitingPacketsList
#    ComboID = CombineIDs(ack.CreatorID, ack.PacketID)
#    if ComboID in _AckWaitingPacketsList:
#        _AckWaitingPacketsList.remove(ComboID)
#        dhnio.Dprint(14, "transport_control.FindAckWaitingPacket [%s] found waiting=%d" % (ComboID, len(_AckWaitingPacketsList)))
#        return True
#    return False
...
##    dhnio.Dprint(12, "transport_control.FetchAndCallBack")
##    dhnio.Dprint(8, "transport_control.FetchAndCallBack with CreatorID =" + CreatorID)
##    dhnio.Dprint(8, "transport_control.FetchAndCallBack with PacketID =" + PacketID)
##    dhnio.Dprint(8, "transport_control.FetchAndCallBack with SupplierNumber =" + str(SupplierNumber))
...
#            # Can't get too excited about it though
    ...
    if not packetHandled and not foundInterest:
        dhnio.Dprint(1, "transport_control.inbox ERROR have unhandled packet %s pid=%s " % (Command, PacketID))
...
def identityCachingFailed(pagesrc, destinationID, outpacket, doAck, wide=False):
    dhnio.Dprint(6, 'transport_control.identityCachingFailed for %s NETERROR' % destinationID)
    def do_again():
...
def receiveStatusReport(filename, status, proto='', host_=None, error=None, message=''):
    ...
    dhnio.Dprint(12, 'transport_control.receiveStatusReport [%s] %s (%s) %s {%s}' % (proto, host, status, msg, os.path.basename(filename),))
...
def sendStatusReport(host_, filename, status, proto='', error=None, message=''):
    ...
    if error is not None:
        if proto != 'tcp':
            msg = '(' + str(error) + ') ' + msg
    dhnio.Dprint(12, "transport_control.sendStatusReport [%s] %s (%s) %s {%s}" % (proto, host, status, msg, os.path.basename(filename)))
            ...
            if workitem.doack == doAck:
                ...
                if (workitem.status == "sending" or workitem.status=="sent") and elapsedtime > workitem.timeout:
                    ...
                    if workitem.retries == 0:
                        dhnio.Dprint(10, "transport_control.GetFromSendQueue NETERROR could not send to " + workitem.remoteid)
                        SendQueueRemove(workitem)
                    else:
                        dhnio.Dprint(10, "transport_control.GetFromSendQueue NETERROR doing retry to=%s elapsedtime=%s timeout=%s retry=%s" % (workitem.remoteid, str(elapsedtime), str(workitem.timeout), str(workitem.retries)))
                        workitem.time = time.time()              # need to update time when sending
...
def ProcessSendQueue():
    ...
    tm = time.time()
    if tm < _TimeNextSend:
        #dhnio.Dprint(24, "transport_control.ProcessSendQueue not sending yet _TimeNextSend=%s" % str(_TimeNextSend))
    ...
    #dhnio.Dprint(24, "transport_control.ProcessSendQueue interval=%f" % interval)
