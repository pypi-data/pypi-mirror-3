...
from juju.control.status import StatusCommand as Collector
...
class ServiceWatcher(object):
    ...
    def poll(self):
        log.debug("polling")
        collector = yield Collector(self.client, self.provider, log)
        state = yield collector()
...
#        relations = service_info["relations"]
        ...
        for unit_id, unit_info in service_info['units'].items():
            ...
            for label, value in (
                ...
                ('unit %s' % unit_id, unit_info['agent-state'])):
            ...
            for relation_id, relation_info in unit_info.get(
...
def setup_parser():
    ...
    parser.add_argument(
        "-p", "--poll-interval", default=3, type=int,
