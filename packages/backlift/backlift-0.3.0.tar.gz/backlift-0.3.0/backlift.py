...
def register_key(key):
    # import pdb; pdb.set_trace()
    try:
        r = requests.post(backliftURL_registerkey, 
                          params={'access_key': key}, verify=False)
        check_response(r)
...
def get_app_meta(api_key, app_id):
    try:
        # import pdb; pdb.set_trace()
        r = requests.get(backliftURL_getappmeta % app_id, 
...
def create_superuser(api_key, app_id, username, password):
    try:
        ...
        headers = {'x-requested-with': 'BackliftAPI'}
        res = requests.post(backliftURL_createsuperuser % app_id, 
                          auth=HTTPBasicAuth('api', api_key), verify=False,
...
def create(path, name='', template='init'):
...
def open_browser(path, admin=False):
    ...
    cfg_path = os.path.normpath(os.path.abspath(
        os.path.join(path, backlift_configfile)))

    app_id = get_id(cfg_path)

    meta = get_app_meta(api_key, app_id)

    if admin:
        url = meta.get('admin_url', '')
    else:
...
def execute_command_line():
    ...
    backliftURL_getappmeta = os.path.join(backliftURL, 'app-admin/%s')
    ...
    backliftURL_createsuperuser = os.path.join(backliftURL, 'createsuperuser/%s')
