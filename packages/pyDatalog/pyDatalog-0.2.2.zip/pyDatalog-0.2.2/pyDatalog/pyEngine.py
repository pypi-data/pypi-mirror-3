...
Var.unify_const = lambda self, const, env : env.setdefault(self, const)
Fresh_var.unify_const = lambda self, const, env : env.setdefault(self, const)
...
def retract(clause):
    ...
    if id_ in pred.db: pred.db[id_] = None  # remove clause from pred.db
...
def fact(subgoal, literal):
    if not is_member(literal, subgoal.facts):
        ...
        for waiter in reversed(subgoal.waiters):
            ...
            if resolvent:
                sched(lambda: add_clause(waiter.subgoal, resolvent))
...
def rule(subgoal, clause, selected):
    ...
    if sg:
        ...
        todo = []
        for id, fact in sg.facts.items():
            resolvent = resolve(clause, fact)
            if resolvent: 
                todo.append(resolvent)
        for t in todo:
            sched(lambda: add_clause(subgoal, t))
    else:
        sg = make_subgoal(selected)
        sg.waiters.append(Waiter(subgoal, clause))
        merge(sg)
        return sched(lambda: search(sg))
...
def search(subgoal):
    ...
    if literal.pred.prim:
        ...
        for id, clause in literal.pred.db.items():
