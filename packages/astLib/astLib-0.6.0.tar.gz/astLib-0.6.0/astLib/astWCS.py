...
This is a higher level interface to some of the routines in PyWCSTools (distributed with astLib).
PyWCSTools is a simple SWIG wrapping of WCSTools by Doug Mink
(U{http://tdc-www.harvard.edu/software/wcstools/}). It is intended is to make this interface
complete enough such that direct use of PyWCSTools is unnecessary.

@var NUMPY_MODE: If True (default), pixel coordinates accepted/returned by routines such as 
L{astWCS.WCS.pix2wcs}, L{astWCS.WCS.wcs2pix} have (0, 0) as the origin. Set to False to make these routines 
accept/return pixel coords with (1, 1) as the origin (i.e. to match the FITS convention, default behaviour 
prior to astLib version 0.3.0).
...
# if True, -1 from pixel coords to be zero-indexed like numpy. If False, use FITS convention.
NUMPY_MODE=True

# Check for the locale bug when decimal separator isn't '.' (atof used in libwcs)
lconv=locale.localeconv()
if lconv['decimal_point'] != '.':
    print "WARNING: decimal point separator is not '.' - astWCS coordinate conversions will not work."
    print "Workaround: after importing any modules that set the locale (e.g. matplotlib), ",
...
class WCS:
    ...
    Coordinate System (WCS) contained in the header of a FITS image. Conversions
    ...
    def __init__(self, headerSource, extensionName = 0, mode = "image"):
        """Creates a WCS object using either the information contained in the header of the specified 
        .fits image, or from a pyfits.header object. Set mode = "pyfits" if the headerSource
        is a pyfits.header.
        
        @type headerSource: string or pyfits.header
        @param headerSource: filename of input .fits image, or a pyfits.header object
        @type extensionName: int or string
        @param extensionName: name or number of .fits extension in which image data 
        is stored
        @type mode: string
        @param mode: set to "image" if headerSource is a .fits file name, or set to "pyfits"
        if headerSource is a pyfits.header object
    ...
    def updateFromHeader(self):
        """Updates the WCS object using information from WCS.header. This routine should be called whenever
        changes are made to WCS keywords in WCS.header.
        
        """
        
        # Take out any problematic overly long header keyword values before creating the WCSStructure,
        # as these cause problems for WCSTools
    ...
    def getCentreWCSCoords(self):
        """Returns the RA and dec coordinates (in decimal degrees) at the centre of the
        WCS.
    ...
    def getFullSizeSkyDeg(self):
        ...
        decimal degrees on the sky (i.e., with the projection taken into account).
        
        @rtype: list
        @return: width and height of image in decimal degrees on the sky in format 
        [width, height]
    ...
    def getHalfSizeDeg(self):
        """Returns the half-width, half-height of the image according to the WCS in
        RA and dec degrees. 
        
        @rtype: list
        @return: half-width and half-height of image in R.A., dec. decimal degrees 
        in format [half-width, half-height]
    ...
    def getImageMinMaxWCSCoords(self):
        """Returns the minimum, maximum WCS coords defined by the size of the parent image (as
        defined by the NAXIS keywords in the image header).
    ...
    def wcs2pix(self, RADeg, decDeg):
        """Returns the pixel coordinates corresponding to the input WCS coordinates (given
        in decimal degrees). RADeg, decDeg can be single floats, or lists or numpy arrays.
        ...
        if type(RADeg) == numpy.ndarray or type(RADeg) == list:
            if type(decDeg) == numpy.ndarray or type(decDeg) == list:
                ...
                for ra, dec in zip(RADeg, decDeg):
                    ...
                    if pix[0] < 1:
                        xTest=(self.header['CRPIX1'])-(ra-360.0)/self.getXPixelSizeDeg()
            ...
            pixCoords=wcs.wcs2pix(self.WCSStructure, float(RADeg), float(decDeg))
            # Below handles CEA wraparounds
            if pixCoords[0] < 1:
                xTest=(self.header['CRPIX1'])-(RADeg-360.0)/self.getXPixelSizeDeg()
                if xTest >= 1 and xTest < self.header['NAXIS1']:
                    pixCoords[0]=xTest
            if NUMPY_MODE == True:
            ...
            if type(y) == numpy.ndarray or type(y) == list:
                ...
                for xc, yc in zip(x, y):
                    if NUMPY_MODE == True:
                        xc=xc+1
                        yc=yc+1
                    WCSCoords.append(wcs.pix2wcs(self.WCSStructure, float(xc), float(yc)))
        else:
            if NUMPY_MODE == True:
                x=x+1
                y=y+1
...
def findWCSOverlap(wcs1, wcs2):
    """Finds the minimum, maximum WCS coords that overlap between wcs1 and wcs2. Returns these coordinates, 
    plus the corresponding pixel coordinates for each wcs. Useful for clipping overlapping region between 
    two images.
    
    @rtype: dictionary
    @return: dictionary with keys 'overlapWCS' (min, max RA, dec of overlap between wcs1, wcs2)
            'wcs1Pix', 'wcs2Pix' (pixel coords in each input WCS that correspond to 'overlapWCS' coords)
