...
class Store(object):
    ...
    def products(cls, product_types=list(), async=True):
        ...
        # If product types not given, ask the script which ones does it handle
        # In other words: fetch all the products.
    ...
    def product_urls_and_types(cls, product_types=list(), async=True):
        ...
        # If product types not given, ask the script which ones does it handle
        # In other words: fetch all the products urls.
        if not product_types:
            product_types = cls.product_types()
        ...
        if async:
            ...
            try:
                product_urls_and_types = cls.__product_urls_and_types(
                    product_types)
    ...
    def retrieve_product(cls, url, product_type, should_log=True):
        ...
        try:
            ...
            error_message = 'Error fetching URL from {0}: {1} - {2}'.format(
                cls.__name__,
                url,
