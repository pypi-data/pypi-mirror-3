...
validTransports = ['http', 'cspace', 'q2q', 'tcp', 'ssh', 'email', 'skype', ]
...
def getLocalID():
    ...
    if _LocalIDURL is None:
        if isLocalIdentityReady():
            _LocalIDURL = getLocalIdentity().getIDURL()
    return _LocalIDURL
...
def DigitsOnly(input):
    return ''.join([c for c in input if c in '0123456789'])
...
def percent2string(percent):
    s = str(round(percent, 2))
    if s[-2:] == '.0':
        s = s[:-2]
    return s + '%'
...
def RunShellCommand(cmdstr, wait=True):
    dhnio.Dprint(4, 'misc.RunShellCommand ' + cmdstr)
    try:
        if dhnio.Windows():
            import win32process
            p = subprocess.Popen(
...
#def MakeBatFile2Restart(wait_appname='dhnmain.exe', start_cmd='dhnmain.exe', local_dir=dhnio.getExecutableDir()):
#    dhnio.Dprint(6, 'misc.MakeBatFile2Restart ')
##    batfileno, batfilename = tempfile.mkstemp("-dhn.bat")
#    batfileno, batfilename = tmpfile.make('bat', '.bat')
#    batsrc = ''
#    batsrc += 'cd "%s"\n' % local_dir
...
def RunBatFile(filename, output_filename=None):
    if not dhnio.Windows():
        return
    dhnio.Dprint(6, 'misc.RunBatFile going to execute ' + str(filename))
#    cmdargs = [filename, ]
#    # method 1
#    try:
#        import win32process
...
#    return BatProcess
    ...
    dhnio.Dprint(6, 'misc.RunBatFile cmdargs=' + str(cmdargs))
...
def MoveCustomersFiles(current_dir, new_dir, remove_old=False):
    #TODO
    return
...
##        return 'ok'
        ...
        cmdstr0 = 'mkdir "%s"' % new_dir
        cmdstr1 = 'xcopy "%s" "%s" /E /K /R /H /Y' % (os.path.join(current_dir, '*.*'), new_dir)
        cmdstr2 = 'rmdir /S /Q "%s"' % current_dir
...
#    subprocess.Popen('explorer %s' % filepath)
...
def SendDevReport(subject, body, includelogs):
    filesList = []
    if includelogs:
...
#    IdMessage1.Recipients.EMailAddresses := 'datahaven.net@gmail.com';
#    IdMessage1.From.Address := 'dhnemail1@mail.offshore.ai';
#    IdMessage1.IsEncoded := true;
#
#    IdSMTP1.AuthenticationType := atLogin;
#    IdSMTP1.Host := 'mail.offshore.ai';
#    IdSMTP1.Port := 25;
#    IdSMTP1.UserId := 'dhnemail1';
#    IdSMTP1.Password := '461tsA1';
...
#    RunBatFile(batfilename)
    ...
    #SendDevReport('subject', 'some body', True)
