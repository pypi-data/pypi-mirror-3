...
public:
	...
	double
	calculate_Z( int global_pos, bool rev_comp, boost::optional< double > g = boost::optional< double >() ) {
	    ...
	    const double log_p_binding =
            rev_comp
                ? w_mer_binding_log_likelihood< true  >( bs, _data.get_W_mer_new( W(), global_pos ) )
                : w_mer_binding_log_likelihood< false >( bs, _data.get_W_mer_new( W(), global_pos ) )
                ;
	    const double log_p_background = bg.w_mer_log_likelihood( seq, offset );
	 ...
	 */
	//typename seqan::Infix< raw_text_t >::Type
	typename seqan::Infix< string_t >::Type
	get_W_mer( int pos ) {
		return _data.get_W_mer_new( W(), pos );
	}
    ...
    update_with_W_mers( const Instances & W_mers, size_t num_sites, bool use_pseudo_counts = false ) {
        ...
        size_t sites_used = 0;
        BOOST_FOREACH( const w_mer_evaluation & W_mer, W_mers ) {
