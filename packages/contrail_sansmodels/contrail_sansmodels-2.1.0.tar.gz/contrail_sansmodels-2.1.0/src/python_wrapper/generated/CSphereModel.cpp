...
CSphereModel_init(CSphereModel *self, PyObject *args, PyObject *kwds)
{
    if (self != NULL) {
        ...
        PyDict_SetItemString(self->params,"sldSph",Py_BuildValue("d",0.000002000000));
...
static PyMemberDef CSphereModel_members[] = {
    {name_params, T_OBJECT, offsetof(CSphereModel, params), 0, def_params},
	{name_dispersion, T_OBJECT, offsetof(CSphereModel, dispersion), 0, def_dispersion},     
    {name_log, T_OBJECT, offsetof(CSphereModel, log), 0, def_log},
    {NULL}  /* Sentinel */
};
...
#pragma omp parallel for
        ...
        return PyArray_Return(result); 
		    ...
		    PyErr_SetString(CSphereModelError, 
                   "CSphereModel.evaluateTwoDimXY couldn't run.");
	        return NULL;
		}      	
}
/**
 ...
 *
 */ 
static PyObject * evalDistribution(CSphereModel *self, PyObject *args){
	PyObject *qx, *qy;
	PyArrayObject * pars;
	int npars ,mpars;
	...
	if ( !PyArg_ParseTuple(args,"O",&pars) ) {
	    PyErr_SetString(CSphereModelError, 
	    	"CSphereModel.evalDistribution expects a q value.");
		return NULL;
	}
    // Check params
    ...
    if(PyArray_Check(pars)==1) {
	    ...
	    if(npars==1) {
		    ...
		    PyErr_SetString(CSphereModelError, 
                   "CSphereModel.evalDistribution expect numpy array of one dimension.");
	        return NULL;
		}
	    ...
	    if(mpars!=2) {
	    	PyErr_SetString(CSphereModelError, 
	    		"CSphereModel.evalDistribution expects a list of dimension 2.");
	    	return NULL;
	    }
	     ...
	     if (PyArray_Check(qx) && PyArray_Check(qy)) {
		    ...
		    PyErr_SetString(CSphereModelError, 
                   "CSphereModel.evalDistribution expect 2 numpy arrays in list.");
	        return NULL;
	     }
	}
	PyErr_SetString(CSphereModelError, 
 ...
 */
static PyObject * calculate_ER(CSphereModel *self) {
 ...
 */
static PyObject * calculate_VR(CSphereModel *self) {
...
static PyObject * set_dispersion(CSphereModel *self, PyObject *args) {
	PyObject * disp;
	const char * par_name;

	if ( !PyArg_ParseTuple(args,"sO", &par_name, &disp) ) {
	    PyErr_SetString(CSphereModelError,
	    	"CSphereModel.set_dispersion expects a DispersionModel object.");
		return NULL;
	}
    ...
    if (!strcmp(par_name, "radius")) {
	    ...
	    PyErr_SetString(CSphereModelError,
	    	"CSphereModel.set_dispersion expects a valid parameter name.");
		return NULL;
	}
